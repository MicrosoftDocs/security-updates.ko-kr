---
TOCTitle: 'MS08-073'
Title: 'Microsoft 보안 공지 MS08-073 - 긴급'
ms:assetid: 'ms08-073'
ms:contentKeyID: 61230181
ms:mtpsurl: 'https://technet.microsoft.com/ko-KR/library/ms08-073(v=Security.10)'
---

Microsoft 보안 공지 MS08-073 - 긴급
===================================
...
이 보안 업데이트의 심각도는 Microsoft Windows 2000에서 실행되는 Internet Explorer 5.01 및 Internet Explorer 6 서비스 팩 1과 Windows XP에서 실행되는 Internet Explorer 6, Internet Explorer 7에 대해 긴급이며 Windows Server 2003에서 실행되는 Internet Explorer 6에 대해서는 보통입니다. 자세한 내용은 이 섹션에서 **영향을 받는 소프트웨어 및 영향을 받지 않는 소프트웨어**를 참조하십시오.

보안 업데이트는 Internet Explorer에서 매개 변수를 처리하고 악용 가능 조건을 발생시키는 오류를 처리하고 웹 페이지에 개체를 포함할 때 추가 데이터를 처리하는 방식을 수정하여 이러한 취약점을 해결합니다. 취약점에 대한 자세한 내용은 다음 섹션, **취약점 정보**에서 취약점 항목의 자주 제기되는 질문 사항(FAQ)을 참조하십시오.
...
<td style="border:1px solid black;">
[Windows Internet Explorer 7](https://www.microsoft.com/download/details.aspx?familyid=107cf54b-29d4-4c54-b091-2b5b3ffbf49d&displaylang=ko)
</td>
<td style="border:1px solid black;">
원격 코드 실행
</td>
...
파일 정보 세부 사항은 [Microsoft 기술 자료 문서 958215](https://support.microsoft.com/kb/958215)에 나와 있습니다.
...
<th style="border:1px solid black;" >
매개 변수 유효성 검사 메모리 손상 취약점(CVE-2008-4258)
</th>
<th style="border:1px solid black;" >
HTML 개체 메모리 손상 취약점(CVE-2008-4259)
</th>
<th style="border:1px solid black;" >
초기화되지 않은 메모리 손상 취약점(CVE-2008-4260)
</th>
<th style="border:1px solid black;" >
HTML 렌더링 메모리 손상 취약점(CVE-2008-4261)
</th>
<th style="border:1px solid black;" >
전체 심각도
</th>
...
<tr>
<td style="border:1px solid black;">
Windows Vista의 Internet Explorer 7 및 Windows Vista 서비스 팩 1의 Internet Explorer 7
</td>
<td style="border:1px solid black;">
해당 사항 없음
</td>
...
<tr class="alternateRow">
<td style="border:1px solid black;">
Windows Vista x64 Edition의 Internet Explorer 7 및 Windows Vista x64 Edition 서비스 팩 1의 Internet Explorer 7
</td>
<td style="border:1px solid black;">
해당 사항 없음
</td>
...
Internet Explorer에서 특정 탐색 메서드를 처리하는 방식에 원격 코드 실행 취약점이 존재합니다. 공격자는 특수하게 조작된 웹 페이지를 구성하여 이러한 취약점을 악용할 수 있습니다. 사용자가 해당 웹 페이지를 볼 경우 취약점으로 인해 원격 코드 실행이 허용될 수 있습니다. 취약점 악용에 성공한 공격자는 로그온한 사용자와 동일한 권한을 얻을 수 있습니다.

이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2008-4258 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2008-4258)을 참조하십시오.

#### 매개 변수 유효성 검사 메모리 손상 취약점(CVE-2008-4258)에 대한 완화 요소
...
#### 매개 변수 유효성 검사 메모리 손상 취약점(CVE-2008-4258)에 대한 대안
...
#### 매개 변수 유효성 검사 메모리 손상 취약점(CVE-2008-4258)에 대한 FAQ
...
**취약점의 원인은 무엇입니까?**  
Internet Explorer에서 메서드 호출 중 생성된 매개 변수의 유효성을 제대로 검사하지 않습니다. 이로 인해 메모리가 손상되고 공격자가 로그온한 사용자의 컨텍스트에서 임의 코드를 실행할 수 있습니다.
...
**업데이트는 어떤 기능을 수행합니까?**  
이 업데이트는 Internet Explorer에서 특정 메서드 호출 중 매개 변수의 유효성을 검사하는 방식을 수정합니다.
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2008-4259 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2008-4259)를 참조하십시오.

#### HTML 개체 메모리 손상 취약점(CVE-2008-4259)에 대한 완화 요소
...
-   Microsoft Internet Explorer 6은 이 취약점의 영향을 받지 않습니다.

#### HTML 개체 메모리 손상 취약점(CVE-2008-4259)에 대한 대안

대안이란 기본적인 취약점을 수정하지는 않지만 업데이트를 적용하기 전에 알려진 공격 경로를 차단하는 데 유용한 설정 또는 구성 변경을 지칭합니다. Microsoft는 다음과 같은 대안을 테스트했으며, 대안으로 인한 기능 저하 여부를 해당 항목에서 설명합니다.

-   **WebClientservice 사용 안 함**

    명령줄에서 WebClient 서비스를 사용하지 않도록 설정하려면 다음 단계를 수행하십시오.

    1.  **시작**, **실행**을 클릭하고 **Services.msc**를 입력한 다음 **확인**을 클릭합니다.
    2.  **WebClientservice**를 마우스 오른쪽 단추로 클릭한 다음 **속성**을 클릭합니다.
    ...
    **대안 실행 취소 방법.**
    ...
    명령줄에서 WebDAV 서비스를 다시 사용하도록 설정하려면 다음 단계를 수행하십시오.

    1.  **시작**, **실행**을 클릭하고 **Services.msc**를 입력한 다음 **확인**을 클릭합니다.
    2.  **WebClientservice**를 마우스 오른쪽 단추로 클릭한 다음 **속성**을 클릭합니다.
    3.  시작 유형을 **자동**으로 변경하고 **적용**을 클릭합니다.
    4.  **시작**, **확인**을 차례로 클릭하고 관리 응용 프로그램을 종료합니다.

#### HTML 개체 메모리 손상 취약점(CVE-2008-4259)에 대한 FAQ
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2008-4260 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2008-4260)을 참조하십시오.

#### 초기화되지 않은 메모리 손상 취약점(CVE-2008-4260)에 대한 완화 요소
...
#### 초기화되지 않은 메모리 손상 취약점(CVE-2008-4260)에 대한 대안

이 취약점에 대한 대안을 확인하지 못했습니다.

#### 초기화되지 않은 메모리 손상 취약점(CVE-2008-4260)에 대한 FAQ
...
**취약점의 원인은 무엇입니까?**  
Internet Explorer에서 삭제된 개체에 액세스하려고 시도합니다. 이로 인해 메모리가 손상되고 공격자가 로그온한 사용자의 컨텍스트에서 임의 코드를 실행할 수 있습니다.
...
Internet Explorer에서 웹 페이지에 개체를 포함하는 방식에 원격 코드 실행 취약점이 존재합니다. 공격자는 특수하게 조작된 웹 페이지를 구성하여 이러한 취약점을 악용할 수 있습니다. 사용자가 해당 웹 페이지를 볼 경우 취약점으로 인해 원격 코드 실행이 허용될 수 있습니다. 취약점 악용에 성공한 공격자는 로그온한 사용자와 동일한 권한을 얻을 수 있습니다.

이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2008-4261 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2008-4261)을 참조하십시오.

#### HTML 렌더링 메모리 손상 취약점(CVE-2008-4261)에 대한 완화 요소
...
#### HTML 렌더링 메모리 손상 취약점(CVE-2008-4261)에 대한 대안

이 취약점에 대한 대안을 확인하지 못했습니다.

#### HTML 렌더링 메모리 손상 취약점(CVE-2008-4261)에 대한 FAQ
...
**취약점의 원인은 무엇입니까?**  
페이지에 개체를 포함하는 동안 Internet Explorer에서 예상치 못한 데이터가 포함된 웹 페이지를 표시하면 시스템 메모리가 손상되어 공격자가 임의 코드를 실행할 수 있습니다.
...
**취약점으로 인해 주로 공격받을 위험이 있는 시스템은 무엇입니까?**  
악의적인 동작이 발생하려면 사용자가 로그온한 상태에서 해당 웹 사이트를 방문해야 합니다. 따라서 워크스테이션 또는 터미널 서버와 같이 Internet Explorer를 빈번하게 사용하는 시스템은 이 취약점을 이용하는 공격을 받을 위험이 가장 큽니다.

**업데이트는 어떤 기능을 수행합니까?**  
이 업데이트는 개체를 포함하는 동안 Internet Explorer에서 추가 데이터를 처리하는 방식을 수정합니다.
...
IE5.01sp4-KB958215-Windows2000sp4-x86-kor /quiet<br />
<br />
지원 대상인 모든 Windows 2000 에디션에 설치된 Internet Explorer 6 서비스 팩 1:<br />
IE6.0sp1-KB958215-Windows2000-x86-kor /quiet</td>
</tr>
<tr class="even">
<td style="border:1px solid black;">다시 시작하지 않고 설치</td>
<td style="border:1px solid black;">지원 대상인 모든 Windows 2000 에디션의 Internet Explorer 5.01 서비스 팩 4:<br />
IE5.01sp4-KB958215-Windows2000sp4-x86-kor /norestart<br />
<br />
지원 대상인 모든 Windows 2000 에디션에 설치된 Internet Explorer 6 서비스 팩 1:<br />
IE6.0sp1-KB958215-Windows2000-x86-kor /norestart</td>
</tr>
<tr class="odd">
...
<td style="border:1px solid black;">지원 대상인 모든 Windows 2000 에디션의 Internet Explorer 5.01 서비스 팩 4:<br />
제어판의 프로그램 추가/제거 도구 또는 %Windir%\\$NTUninstallKB958215-IE501SP4-20081016.120000$\Spuninst 폴더의 Spuninst.exe 유틸리티를 사용하십시오.<br />
<br />
지원 대상인 모든 Windows 2000 에디션에 설치된 Internet Explorer 6 서비스 팩 1:<br />
제어판의 프로그램 추가/제거 도구 또는 %Windir%\\$NTUninstallKB958215-IE6SP1-20081016.120000$\Spuninst 폴더의 Spuninst.exe 유틸리티를 사용하십시오.</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"><strong>파일 정보</strong></td>
<td style="border:1px solid black;"><a href="https://support.microsoft.com/kb/958215">Microsoft 기술 자료 문서 958215</a> 참조</td>
</tr>
<tr class="even">
...
Windowsxp-KB958215-x86-kor /quiet</td>
</tr>
<tr class="even">
<td style="border:1px solid black;"></td>
...
IE7-WindowsXP-KB958215-x86-kor /quiet</td>
</tr>
<tr class="even">
<td style="border:1px solid black;"></td>
...
IE7-WindowsServer2003.WindowsXP-KB958215-x64-kor /quiet</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;">다시 시작하지 않고 설치</td>
<td style="border:1px solid black;">Windows XP 서비스 팩 2 및 Windows XP 서비스 팩 3용 Internet Explorer 6:<br />
Windowsxp-KB958215-x86-kor /norestart</td>
</tr>
<tr class="even">
<td style="border:1px solid black;"></td>
<td style="border:1px solid black;">Windows XP Professional x64 Edition 및 Windows XP Professional x64 Edition 서비스 팩 2용 Internet Explorer 6:<br />
...
<td style="border:1px solid black;"></td>
<td style="border:1px solid black;">Windows XP 서비스 팩 2 및 Windows XP 서비스 팩 3용 Internet Explorer 7:<br />
IE7-WindowsXP-KB958215-x86-kor /norestart</td>
</tr>
<tr class="even">
<td style="border:1px solid black;"></td>
<td style="border:1px solid black;">Windows XP Professional x64 Edition 및 Windows XP Professional x64 Edition 서비스 팩 2용 Internet Explorer 7:<br />
IE7-WindowsServer2003.WindowsXP-KB958215-x64-kor /norestart</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;">로그 파일 업데이트</td>
...
제어판의 프로그램 추가/제거 도구 또는 %Windir%\$NTUninstal\KB958215$\Spuninst 폴더의 Spuninst.exe 유틸리티를 사용하십시오.</td>
</tr>
<tr class="even">
<td style="border:1px solid black;"></td>
...
Windowsserver2003-KB958215-x86-kor /quiet</td>
</tr>
<tr class="even">
<td style="border:1px solid black;"></td>
<td style="border:1px solid black;">지원 대상인 모든 Windows Server 2003 x64 기반 에디션용 Internet Explorer 6:<br />
WindowsServer2003.WindowsXP-KB958215-x64-kor /quiet</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"></td>
...
IE7-WindowsServer2003-KB958215-x86-kor /quiet</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"></td>
...
<td style="border:1px solid black;">다시 시작하지 않고 설치</td>
<td style="border:1px solid black;">지원 대상인 모든 Windows Server 2003 32비트 에디션용 Internet Explorer 6:<br />
Windowsserver2003-KB958215-x86-kor /norestart</td>
</tr>
<tr class="even">
<td style="border:1px solid black;"></td>
<td style="border:1px solid black;">지원 대상인 모든 Windows Server 2003 x64 기반 에디션용 Internet Explorer 6:<br />
WindowsServer2003.WindowsXP-KB958215-x64-kor /norestart</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"></td>
<td style="border:1px solid black;">지원 대상인 모든 Windows Server 2003 Itanium 기반 에디션용 Internet Explorer 6:<br />
...
<td style="border:1px solid black;"></td>
<td style="border:1px solid black;">지원 대상인 모든 Windows Server 2003 32비트 에디션용 Internet Explorer 7:<br />
IE7-WindowsServer2003-KB958215-x86-kor /norestart</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"></td>
<td style="border:1px solid black;">지원 대상인 모든 Windows Server 2003 x64 기반 에디션용 Internet Explorer 7:<br />
...
제어판의 프로그램 추가/제거 도구 또는 %Windir%\\$NTUninstallKB958215$\Spuninst 폴더의 Spuninst.exe 유틸리티를 사용하십시오.</td>
</tr>
<tr class="even">
<td style="border:1px solid black;"></td>
...
-   매개 변수 유효성 검사 메모리 손상 취약점(CVE-2008-4258)을 보고해 주신 Carlo Di Dato(shinnai)  
-   HTML 개체 메모리 손상 취약점(CVE-2008-4259)을 [TippingPoint (영문)](https://www.tippingpoint.com/) 및 [Zero Day Initiative (영문)](https://www.zerodayinitiative.com/)와 협력하여 보고해 주신 Brett Moore  
-   초기화되지 않은 메모리 손상 취약점(CVE-2008-4260)을 보고해 주신 [Casaba Security (영문)](https://www.casabasecurity.com/)의 Chris Weber  
-   HTML 렌더링 메모리 손상 취약점(CVE-2008-4261)을 보고해 주신 [Verisign iDefense Labs (영문)](https://labs.idefense.com/)의 Jun Mao
