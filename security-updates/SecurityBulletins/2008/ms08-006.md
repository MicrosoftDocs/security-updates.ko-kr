...
|--------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------|----------------|-------------|---------------------------------------------------------------------|
| Windows XP Professional 서비스 팩 2                                                              | [Microsoft IIS(인터넷 정보 서비스) 5.1](https://www.microsoft.com/download/details.aspx?familyid=2b498065-d682-4227-b23e-d234d7d6a3fe&displaylang=ko) | 원격 코드 실행 | 중요        | [MS06-034](https://technet.microsoft.com/security/bulletin/ms06-034) |
| Windows XP Professional x64 Edition 및 Windows XP Professional x64 Edition 서비스 팩 2           | [Microsoft IIS(인터넷 정보 서비스) 6.0](https://www.microsoft.com/download/details.aspx?familyid=df9875f7-04d6-486e-bdb5-35e9e305fa1d)                | 원격 코드 실행 | 중요        | [MS06-034](https://technet.microsoft.com/security/bulletin/ms06-034) |
| Windows Server 2003 서비스 팩 1 및 Windows Server 2003 서비스 팩 2                               | [Microsoft IIS(인터넷 정보 서비스) 6.0](https://www.microsoft.com/download/details.aspx?familyid=6583e798-d16d-419c-aee1-30c3e6c635b3&displaylang=ko) | 원격 코드 실행 | 중요        | [MS06-034](https://technet.microsoft.com/security/bulletin/ms06-034) |
| Windows Server 2003 x64 Edition 및 Windows Server 2003 x64 Edition 서비스 팩 2                   | [Microsoft IIS(인터넷 정보 서비스) 6.0](https://www.microsoft.com/download/details.aspx?familyid=e8286174-8209-409f-8805-e534715a741c&displaylang=ko) | 원격 코드 실행 | 중요        | [MS06-034](https://technet.microsoft.com/security/bulletin/ms06-034) |
| Windows Server 2003 SP1(Itanium 기반 시스템용) 및 Windows Server 2003 SP2(Itanium 기반 시스템용) | [Microsoft IIS(인터넷 정보 서비스) 6.0](https://www.microsoft.com/download/details.aspx?familyid=29faa70d-f1ac-4da4-b72a-faf1973cd845)                | 원격 코드 실행 | 중요        | [MS06-034](https://technet.microsoft.com/security/bulletin/ms06-034) |
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2008-0075 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2008-0075)를 참조하십시오.
...
이 취약점 악용에 성공한 공격자는 네트워크 서비스 계정 권한이 기본적으로 구성되어 있는 WPI(Worker Process Identity)와 동일한 권한으로 IIS 서버에서 작업을 수행할 수 있습니다. Network Service 계정 권한으로 구정된 서비스는 인증된 사용자 수준 액세스 권한만 가져올 수 있으며, 관리자 수준 액세스 권한은 가져올 수 없습니다. 자세한 내용은 [NetworkService 계정 (영문)](https://msdn2.microsoft.com/ko-kr/library/ms684272(vs.85).aspx)을 참조하십시오. 사용자 권한을 가진 계정을 사용하는 WPI로 응용 프로그램 풀이 구성되어 있는 IIS 서버는 기본 WPI 설정으로 응용 프로그램 풀이 구성되어 있는 IIS 서버보다 더 심각한 영향을 받습니다.
  
네트워크 서비스 계정은 원격 인증을 수행할 때 컴퓨터의 자격 증명을 사용하지만, 서버 자체에서의 권한 수준은 상당히 낮으며 로컬 관리자 권한이 없습니다. 자세한 내용은 [NetworkService Account(NetworkService 계정) (영문)](https://msdn2.microsoft.com/ko-kr/library/ms684272(vs.85).aspx), [IIS and Built-in Accounts (IIS 6.0)(IIS 및 내장 계정((IIS 6.0)) (영문)](https://www.microsoft.com/technet/prodtechnol/windowsserver2003/library/iis/3648346f-e4f5-474b-86c7-5a86e85fa1ff.mspx), [Configuring Worker Process Identities (IIS 6.0)(WPI 구성(IIS 6.0)) (영문)](https://www.microsoft.com/technet/prodtechnol/windowsserver2003/library/iis/76be66e2-1db1-432e-95be-14ff59c2ec75.mspx) 및 [Configuring Application Pools in IIS 6.0 (IIS 6.0)(IIS 6.0 응용 프로그램 풀 구성(IIS 6.0)) (영문)](https://www.microsoft.com/technet/prodtechnol/windowsserver2003/library/iis/2a231dcb-d786-4b6d-b3ba-bda04061a5e7.mspx)을 참조하십시오.
  
**공격자는 이러한 취약점을 어떻게 악용합니까?**  
공격자는 ASP 페이지에 특수하게 조작된 입력을 전달할 수 있습니다. 해당 ASP 페이지는 이미 존재하고 있거나 호스트된 환경에서 사용자가 업로드했을 수 있습니다.
  
**IIS용 작업자 프로세스란 무엇입니까?**  
작업자 프로세스는 웹 서버에 대한 서버측 처리 작업을 실행하는 프로세스입니다**.** 작업자 프로세스 및 IIS 6.0에서 구현 방법에 자세한 내용은 [Worker Processes (IIS 6.0)(작업자 프로세스(IIS 6.0)) (영문)](https://www.microsoft.com/technet/prodtechnol/windowsserver2003/library/iis/29f53968-0115-451f-b26d-5ad58d87b5d1.mspx)를 참조하십시오. 작업자 프로세스 및 격리 상태에서의 웹 기반 응용 프로그램 실행 방법에 대한 자세한 내용은 [Worker Process Isolation Mode (IIS 6.0)(작업자 프로세스 격리 모드(IIS 6.0)) (영문)](https://www.microsoft.com/technet/prodtechnol/windowsserver2003/library/iis/333f3410-b566-49bc-8399-57bcc404db8c.mspx)를 참조하십시오.
