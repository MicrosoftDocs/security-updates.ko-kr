---
TOCTitle: 'MS11-014'
Title: 'Microsoft 보안 공지 MS11-014 - 중요'
ms:assetid: 'ms11-014'
ms:contentKeyID: 61230381
ms:mtpsurl: 'https://technet.microsoft.com/ko-KR/library/ms11-014(v=Security.10)'
---
Microsoft 보안 공지 MS11-014 - 중요
===================================

로컬 보안 기관 하위 시스템 서비스의 취약점으로 인한 로컬 권한 상승 문제점(2478960)
----------------------------------------------------------------------------------

게시된 날짜: 2011년 2월 9일 수요일 | 업데이트된 날짜: 2011년 4월 29일 금요일
...
이 보안 업데이트는 Windows XP 및 Windows Server 2003의 LSASS(로컬 보안 기관 하위 시스템 서비스)에서 발견되어 비공개적으로 보고된 취약점 1건을 해결합니다. 이 보안 업데이트의 심각도는 지원 대상인 모든 운영 체제 에디션에 대해 중요입니다. 자세한 내용은 이 섹션에서 **영향을 받는 소프트웨어 및 영향을 받지 않는 소프트웨어**를 참조하십시오.
...
보안 업데이트는 LSASS가 인증 프로세스에 사용되는 특정 값을 처리하는 방식을 수정하여 취약점을 해결합니다. 취약점에 대한 자세한 내용은 다음 섹션, **취약점 정보**에서 각 취약점 항목의 자주 제기되는 질문 사항(FAQ)을 참조하십시오.
...
**문제점.** 이 보안 업데이트를 설치할 때 고객이 겪을 수 있는 현재까지 알려진 문제점은 [Microsoft 기술 자료 문서 2478960](http://support.microsoft.com/kb/2478960)에 나와 있습니다. 이 문서는 이러한 문제점에 대한 권장 해결 방법도 소개합니다.
...
|-------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------|-------------|---------------------------------------------------------------------|
| [Windows XP 서비스 팩 3](http://www.microsoft.com/downloads/details.aspx?familyid=541f228f-79b3-402a-8ff9-366c1e595227&displaylang=ko)                      | 권한 상승      | 중요        | 없음                                                                |
| [Windows XP Professional x64 Edition 서비스 팩 2](http://www.microsoft.com/downloads/details.aspx?familyid=82189bf2-3f34-4949-92da-eea98036d18e)            | 권한 상승      | 중요        | [MS08-002](http://technet.microsoft.com/security/bulletin/ms08-002) |
| [Windows Server 2003 서비스 팩 2](http://www.microsoft.com/downloads/details.aspx?familyid=4e31e6b1-577e-468e-9c94-67227d2273c2&displaylang=ko)             | 권한 상승      | 중요        | [MS08-002](http://technet.microsoft.com/security/bulletin/ms08-002) |
| [Windows Server 2003 x64 Edition 서비스 팩 2](http://www.microsoft.com/downloads/details.aspx?familyid=76e3c229-d812-433c-ad05-7cbd1f9c6a6d&displaylang=ko) | 권한 상승      | 중요        | [MS08-002](http://technet.microsoft.com/security/bulletin/ms08-002) |
| [Windows Server 2003 with SP2(Itanium 기반 시스템용)](http://www.microsoft.com/downloads/details.aspx?familyid=a0cde8d8-7c85-4fcb-bcf7-205064970b41)        | 권한 상승      | 중요        | [MS08-002](http://technet.microsoft.com/security/bulletin/ms08-002) |
...
<th style="border:1px solid black;" >LSASS 길이 유효성 검사 취약점(CVE-2011-0039)</th>
...
Microsoft Windows LSASS(로컬 보안 기관 하위 시스템 서비스)가 특수하게 조작된 인증 요청을 처리하는 방식에 권한 상승 취약점이 존재합니다. 취약점을 통해 공격자가 권한을 상승시키는 코드를 실행할 수 있습니다. 이 취약점을 악용한 공격자는 영향을 받는 시스템을 완전히 제어할 수 있습니다. 이렇게 되면 공격자가 프로그램을 설치할 수 있을 뿐 아니라 데이터를 보거나 변경하거나 삭제할 수 있고 전체 관리자 권한이 있는 새 계정을 만들 수도 있습니다.
  
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2011-0039 (영문)](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-0039)를 참조하십시오.
  
#### LSASS 길이 유효성 검사 취약점(CVE-2011-0039)에 대한 완화 요소
  
완화란 취약점의 악용 심각도를 낮출 수 있는 설정, 일반적인 구성 또는 최선책, 기본 상태를 지칭합니다. 다음과 같은 완화 요소가 현재 상황에서 유용할 수 있습니다.
  
-   이 취약점을 악용하려면 공격자가 유효한 로그온 자격 증명을 가지고 로컬로 로그온할 수 있어야 합니다. 익명의 사용자에 의해서나 원격으로는 이 취약점을 악용할 수 없습니다.
  
#### LSASS 길이 유효성 검사 취약점(CVE-2011-0039)에 대한 대안
  
이 취약점에 대한 대안을 확인하지 못했습니다.
  
#### LSASS 길이 유효성 검사 취약점(CVE-2011-0039)에 대한 FAQ
...
**취약점의 원인은 무엇입니까?**    
이 취약점은 Windows LSASS가 특수하게 조작된 인증 요청을 처리할 때 발생합니다.
...
**업데이트는 어떤 기능을 수행합니까?**    
업데이트는 LSASS가 인증 프로세스에 사용되는 특정 값을 처리하는 방식을 수정하여 이 취약점을 해결합니다.
...
<td style="border:1px solid black;">제어판의 <strong>프로그램 추가/제거</strong> 도구 또는 %Windir%\$NTUninstallKB2478960$\Spuninst 폴더의 Spuninst.exe 유틸리티를 사용하십시오.</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"><strong>파일 정보</strong></td>
<td style="border:1px solid black;"><a href="http://support.microsoft.com/kb/2478960">Microsoft 기술 자료 문서 2478960</a> 참조</td>
</tr>
<tr class="even">
...
-   LSASS 길이 유효성 검사 취약점(CVE-2011-0039)을 보고해 주신 Primavera BSS의 보안 테스터 Jorge Moura
...
-   V1.1(2011년 4월 29일): 요약의 알려진 문제점에 Microsoft 기술 자료 문서 2478960 링크가 추가되었습니다.
