---
TOCTitle: 'MS11-089'
Title: 'Microsoft 보안 공지 MS11-089 - 중요'
ms:assetid: 'ms11-089'
ms:contentKeyID: 61230456
ms:mtpsurl: 'https://technet.microsoft.com/ko-KR/library/ms11-089(v=Security.10)'
---


Microsoft 보안 공지 MS11-089 - 중요
===================================

Microsoft Office의 취약점으로 인한 원격 코드 실행 문제점 (2590602)
------------------------------------------------------------------
...
이 보안 업데이트의 심각도는 Microsoft Office 2007, Microsoft Office 2010 및 Microsoft Office for Mac 2011의 지원되는 모든 에디션에 대해 중요입니다. 자세한 내용은 이 섹션에서 영향을 받는 소프트웨어 및 영향을 받지 않는 소프트웨어를 참조하십시오.

보안 업데이트는 Microsoft Word가 특수하게 조작된 Word 파일을 구문 분석하는 방식을 수정하여 취약점을 해결합니다. 취약점에 대한 자세한 내용은 다음 섹션, 취약점 정보에서 각 취약점 항목의 자주 제기되는 질문 사항(FAQ)을 참조하십시오.
...
이 업데이트의 심각도가 영향을 받는 Office 버전에 대해서만 중요인 이유가 무엇입니까?  
...
취약점에 대한 공격 경로가 Microsoft Word를 통하는 것입니다. 이 공지가 Microsoft Office 업데이트인 이유는 무엇입니까?  
CVE-2011-1983의 공격은 영향을 받는 Microsoft Word 버전을 통해서만 진행되지만 취약한 코드가 Microsoft Office의 공유 구성 요소에 포함되어 있습니다. 지원 대상인 일부 Microsoft Visio([KB2553374(x86)](https://www.microsoft.com/download/details.aspx?familyid=20a9e33e-079d-4522-8cff-6a284d842979), [KB2553374 (x64)](https://www.microsoft.com/download/details.aspx?familyid=866f0a93-01a9-4e0a-8bf8-aa4dffce6c15) 참조), Microsoft Visio Viewer([KB2553353(x86)](https://www.microsoft.com/download/details.aspx?familyid=d8db143f-bc08-466e-81bb-2efef1d8a68d), [KB2553353 (x64)](https://www.microsoft.com/download/details.aspx?familyid=dee2c20c-0afd-4ebe-aabb-66593c76adf3) 참조), Microsoft Office WAC(Web Application Companions)([KB2553153](https://www.microsoft.com/download/details.aspx?familyid=f7e2fb4c-a883-4517-8c6d-740a69845fe8) 참조), Microsoft SharePoint Server 2010([KB2553132](https://www.microsoft.com/download/details.aspx?familyid=e03b4a55-810e-4503-8236-2e1a33ec3e7d) 참조)을 비롯한 기타 Microsoft Office 소프트웨어에 Microsoft Office의 취약한 공유 구성 요소가 포함되어 있지만 이 취약한 코드에 액세스하지 않으므로 이 취약점의 영향을 받지 않습니다. 그러나 취약한 코드가 있는 것이 사실이기 때문에 이 업데이트가 제공되는 것입니다.

Microsoft Office 소프트웨어를 설치했는데 이 업데이트가 제공되지 않는 이유는 무엇입니까?  
이 업데이트는 Microsoft Office의 취약한 공유 구성 요소가 들어 있는 Microsoft Office 소프트웨어가 실행되는 시스템에만 제공됩니다.

CVE-2011-1983의 공격은 영향을 받는 Microsoft Word 버전을 통해서만 진행됩니다. Microsoft Office의 취약한 공유 구성 요소가 있으므로 기타 Microsoft Office 소프트웨어에 이 업데이트가 제공될 수 있습니다. 하지만 취약한 코드에 액세스하지 않으므로 이 취약점의 영향을 받지는 않습니다.
...
다음의 심각도는 취약점의 잠재적인 최대 영향을 가정합니다. 이 보안 공지 발표 후 30일 이내의 심각도 및 보안 영향과 관련된 취약점 악용 가능성에 대해서는 [12월 공지 요약](https://technet.microsoft.com/ko-kr/security/bulletin/ms11-dec)의 악용 가능성 인덱스를 참조하십시오. 자세한 내용은 [Microsoft 악용 가능성 인덱스](https://technet.microsoft.com/security/cc998259)를 참조하십시오.
...
<th style="border:1px solid black;" >
Word 해제 후 사용 취약점(CVE-2011-1983)
</th>
<th style="border:1px solid black;" >
전체 심각도
</th>
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2011-1983(영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-1983)을 참조하십시오.

#### Word 해제 후 사용 취약점(CVE-2011-1983)에 대한 완화 요소

완화란 취약점의 악용 심각도를 낮출 수 있는 설정, 일반적인 구성 또는 최선책, 기본 상태를 지칭합니다. 다음과 같은 완화 요소가 현재 상황에서 유용할 수 있습니다.

-   웹을 통한 공격의 경우 공격자는 호스팅하는 웹 사이트에 이 취약점을 악용하는 Word 파일을 포함할 수 있습니다. 또한 사용자가 제공한 콘텐츠나 광고를 허용하거나 호스팅하는 공격 당한 웹 사이트에는 이 취약점을 악용할 수 있는 특수하게 조작된 콘텐츠가 포함되어 있을 수 있습니다. 그러나 어떠한 경우에도 공격자는 강제로 사용자가 이러한 웹 사이트를 방문하도록 만들 수 없습니다. 대신 공격자는 사용자가 전자 메일 메시지 또는 메신저 메시지의 링크를 클릭하여 공격자의 웹 사이트를 방문하고 특수하게 조작된 Word 파일을 열도록 유도하는 것이 일반적입니다.
...
#### Word 해제 후 사용 취약점(CVE-2011-1983)에 대한 대안
...
#### Word 해제 후 사용 취약점(CVE-2011-1983)에 대한 FAQ
...
취약점의 원인은 무엇입니까?  
이 취약점은 Microsoft Word가 메모리의 개체를 잘못 처리할 때 발생합니다.
...
이 취약점을 악용하려면 사용자가 특수하게 조작된 Word 파일을 영향을 받는 Microsoft Word 버전에서 열어야 합니다.
...
워크스테이션과 같이 Microsoft Word가 사용되는 시스템이 주로 위험합니다. 관리자가 사용자가 서버에 로그온하여 프로그램을 실행할 수 있도록 허용할 경우, 서버가 더 위험할 수 있습니다. 가장 좋은 방법은 이러한 기능을 허용하지 않는 것입니다.

업데이트는 어떤 기능을 수행합니까?  
보안 업데이트는 Microsoft Word가 특수하게 조작된 Word 파일을 구문 분석하는 방식을 수정하여 취약점을 해결합니다.
...
<td style="border:1px solid black;"><a href="https://support.microsoft.com/kb/2596785">Microsoft 기술 자료 문서 2596785</a> 참조</td>
</tr>
<tr class="even">
...
<tr class="odd">
<td style="border:1px solid black;">파일 정보</td>
<td style="border:1px solid black;"><a href="https://support.microsoft.com/kb/2589320">Microsoft 기술 자료 문서 2589320</a> 참조</td>
</tr>
<tr class="even">
<td style="border:1px solid black;">레지스트리 키 확인</td>
<td style="border:1px solid black;">해당 사항 없음</td>
...
Microsoft Office for Mac 2011 14.1.4 업데이트의 해당 언어 버전을 [Microsoft 다운로드 센터](https://www.microsoft.com/download/details.aspx?familyid=3c8017d6-232c-42a6-a133-96efe3ad3385)에서 다운로드하여 설치하십시오.
  
-   설치를 방해할 수 있으므로 바이러스 예방 응용 프로그램 및 모든 Microsoft Office 응용 프로그램을 포함하여 실행 중인 응용 프로그램을 모두 종료합니다.  
-   바탕 화면에서 Microsoft Office for Mac 2011 14.1.4 업데이트 볼륨을 엽니다. 이 단계가 자동으로 수행되었을 수도 있습니다.  
-   업데이트 과정을 시작하려면 Microsoft Office for Mac 2011 14.1.4 업데이트 볼륨 창에서 Microsoft Office for Mac 2011 14.1.4 업데이트 응용 프로그램을 두 번 클릭하고 화면의 지침을 따릅니다.  
-   설치가 성공적으로 완료되면 하드 디스크에서 업데이트 설치 프로그램을 제거할 수 있습니다. 설치가 성공적으로 완료되었는지 확인하려면 다음 "업데이트 설치 확인"을 참조하십시오. 업데이트 설치 프로그램을 제거하려면 먼저 Microsoft Office for Mac 2011 14.1.4 업데이트 볼륨을 휴지통에 끌어 놓은 다음 다운로드한 파일을 휴지통에 끌어 놓습니다.
...
설치된 최신 업데이트 버전 번호가 14.1.4이면 업데이트가 성공적으로 설치된 것입니다.
...
-   [TippingPoint's](https://www.tippingpoint.com/)[Zero Day Initiative](https://www.zerodayinitiative.com/)와 협력하여 Word 해제 후 사용 취약점(CVE-2011-1983)을 보고해 주신 Nikita Tarakanov(CISS Research Team) 및 Alexey Sintsov(Digital Security Research Group)
...
-   V1.1(2011년 12월 16일): 영향을 받지 않는 소프트웨어의 목록을 확장하고 업데이트 FAQ를 개정하여 Microsoft Office의 취약한 공유 구성 요소가 들어 있는 경우에 영향을 받지 않는 소프트웨어에도 업데이트가 제공될 수 있다는 것을 설명하도록 지원했습니다.  
-   V1.2(2012년 2월 22일): 이 업데이트가 적용되는 다음 영향을 받지 않는 소프트웨어에 대한 업데이트 패키지 KB 번호를 식별하기 위해 이 공지가 개정되었습니다. Microsoft Visio(KB2553374), Microsoft Visio Viewer(KB2553353), Microsoft Office Web Application Companions(WAC)(KB2553153), Microsoft SharePoint Server 2010(KB2553132) 자세한 내용은 업데이트 FAQ를 참조하십시오.
