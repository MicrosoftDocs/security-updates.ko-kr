...
Microsoft는 Windows 7 Embedded를 위한 업데이트를 알리기 위해 이 공지를 릴리스하였습니다. 이 플랫폼을 실행하는 고객은 이 업데이트를 다운로드하여 시스템에 적용하는 것이 좋습니다. 보안 업데이트는 [Microsoft Update](http://go.microsoft.com/fwlink/?linkid=40747) 및 [Windows Update](http://go.microsoft.com/fwlink/?linkid=21130)를 통해 제공됩니다. 다른 업데이트 패키지는 이 발표의 영향을 받지 않습니다.

**파일 정보 세부 사항은 어디에 있습니까?**  
파일 정보 세부 사항의 위치에 대해서는 보안 업데이트 배포 섹션에서 참조 표를 참조하십시오.

**이 업데이트는 Microsoft 보안 권고 2269637과 관련이 있습니까?**  
예, 이 업데이트에서 해결하는 Windows 구성 요소의 안전하지 않은 라이브러리 로드 취약점(CVE-2011-1991)은 응용 프로그램이 [Microsoft 보안 권고 2269637](http://technet.microsoft.com/security/advisory/2269637)에 설명된 외부 라이브러리를 로드하는 방식에 영향을 미치는 취약점 클래스와 관련이 있습니다. 이 보안 업데이트는 이러한 취약점 유형의 특정 인스턴스를 해결합니다.
...
다음의 심각도는 취약점의 잠재적인 최대 영향을 가정합니다. 이 보안 공지 발표 후 30일 이내의 심각도 및 보안 영향과 관련된 취약점 악용 가능성에 대해서는 [9월 공지 요약](http://technet.microsoft.com/security/bulletin/ms11-sep)의 악용 가능성 인덱스를 참조하십시오. 자세한 내용은 [Microsoft 악용 가능성 인덱스](http://technet.microsoft.com/en-us/security/cc998259.aspx)를 참조하십시오.
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2011-1991](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-1991)을 참조하십시오.
...
EAPHost는 802.1X 및 PPP(지점 간)와 같은 "써플리컨트" 프로토콜 구현의 인증을 위해 EAP(Extensible Authentication Protocol)를 제공하는 Microsoft Windows 네트워킹 구성 요소입니다. 또한 Microsoft NPS(네트워크 정책 서버)와 같은 "인증자" 기술을 통한 인증을 허용합니다. 자세한 내용은 MSDN 문서, [Extensible Authentication Protocol 호스트](http://msdn.microsoft.com/en-us/library/aa364249(v=vs.85).aspx)(영문)를 참조하십시오.
  
**MMC(Microsoft Management Console)란 무엇입니까?**  
MMC(Microsoft Management Console)는 관리 응용 프로그램에 대한 확장 가능 공통 프레젠테이션 서비스입니다. MMC는 Microsoft 및 기타 소프트웨어 공급업체에서 제공하는 스냅인을 위한 공통 호스트 환경을 제공합니다. 스냅인이 실제 관리 동작을 제공하고 MMC 자체는 관리 기능을 제공하지 않습니다. MMC 환경은 스냅인 간에 원활하게 통합될 수 있도록 합니다. 자세한 내용은 TechNet 문서 [Microsoft Management Console - 개요](http://technet.microsoft.com/en-us/library/bb742441.aspx)(영문)를 참조하십시오.
  
**폴더 리디렉션 구성 요소란 무엇입니까?**  
폴더 리디렉션은 사용자 및 관리자가 폴더의 경로를 새 위치로 리디렉션할 수 있도록 하는 IntelliMirror의 기능입니다. 새 위치는 로컬 컴퓨터의 폴더이거나 네트워크 공유의 디렉터리일 수 있습니다. 사용자는 마치 문서가 로컬 드라이브에 있는 것처럼 서버의 문서로 작업할 수 있게 됩니다. 예를 들어 주로 컴퓨터의 로컬 하드 디스크에 저장되어 있는 내 문서 폴더를 네트워크 위치로 리디렉션할 수 있습니다. 이 폴더의 문서는 네트워크에 연결된 어떤 컴퓨터에서도 사용할 수 있게 됩니다. 자세한 내용은 TechNet 문서 [Folder Redirection Overview(폴더 리디렉션 개요)](http://technet.microsoft.com/en-us/library/cc778976(ws.10).aspx)를 참조하십시오.
  
**IME(일본어) 구성 요소란 무엇입니까?**  
일본어 IME(입력기)를 사용하여 문서, 워크시트, 프레젠테이션, 메일 메시지, 발행물 및 웹 페이지에 일본어 텍스트를 입력할 수 있습니다. 이 IME 및 사용 가능한 다른 IME에 대한 자세한 내용은 MSDN 문서 [Installing and Using Input Method Editors(입력기 설치 및 사용)](http://msdn.microsoft.com/en-us/library/ee418266%28v=vs.85%29.aspx)를 참조하십시오.
...
예. 이 취약점은 공개되었습니다. 이 취약점에는 CVE(Common Vulnerability and Exposure) 번호 [CVE-2011-1991](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-1991)이 지정되었습니다.
...
<td style="border:1px solid black;"><a href="http://support.microsoft.com/kb/2570947">Microsoft 기술 자료 문서 2570947</a> 참조</td>
</tr>
<tr class="even">
