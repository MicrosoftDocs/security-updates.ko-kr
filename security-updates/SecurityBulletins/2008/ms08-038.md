---
TOCTitle: 'MS08-038'
Title: 'Microsoft 보안 공지 MS08-038 - 중요'
ms:assetid: 'ms08-038'
ms:contentKeyID: 61230146
ms:mtpsurl: 'https://technet.microsoft.com/ko-KR/library/ms08-038(v=Security.10)'
---

Microsoft 보안 공지 MS08-038 - 중요
===================================

Windows 탐색기의 취약점으로 인한 원격 코드 실행 문제점 (950582)
---------------------------------------------------------------
...
이 보안 업데이트는 공개적으로 보고된 취약점 1건을 해결합니다. 사용자가 특수하게 조작된 검색 조건 저장 파일을 열고 저장하면 이 Windows 탐색기 취약점을 통해 원격 코드 실행이 허용될 수 있습니다.사용자가 관리자 권한으로 로그온한 경우, 이 취약점을 악용한 공격자는 영향을 받는 시스템을 완전히 제어할 수 있습니다. 이렇게 되면 공격자가 프로그램을 설치할 수 있을 뿐 아니라 데이터를 보거나 변경하거나 삭제할 수 있고 모든 사용자 권한이 있는 새 계정을 만들 수도 있습니다. 시스템에 대한 사용자 권한이 적게 구성된 계정의 사용자는 관리자 권한으로 작업하는 사용자에 비해 영향을 적게 받습니다.

이 보안 업데이트의 심각도는 지원되는 모든 Windows Vista 및 Windows Server 2008 에디션에 대해 중요입니다. 자세한 내용은 이 섹션에서 **영향을 받는 소프트웨어 및 영향을 받지 않는 소프트웨어**를 참조하십시오.

이 보안 업데이트는 Windows 탐색기에서 검색 조건 저장을 구문 분석하는 방식을 수정하여 이러한 취약점을 해결합니다.취약점에 대한 자세한 내용은 **취약점 정보**에서 각 취약점 항목의 자주 제기되는 질문 사항(FAQ)을 참조하십시오.
...
|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------|-------------|---------------------------|
| [Windows Vista 및 Windows Vista 서비스 팩 1](https://www.microsoft.com/download/details.aspx?familyid=06739ca6-7368-4acb-bb67-7e8146071a29&displaylang=ko)                         | 원격 코드 실행 | 중요        | 없음                      |
| [Windows Vista x64 Edition 및 Windows Vista x64 Edition 서비스 팩 1](https://www.microsoft.com/download/details.aspx?familyid=74ea0893-7c2f-4fad-ad27-588ad953b046&displaylang=ko) | 원격 코드 실행 | 중요        | 없음                      |
| [Windows Server 2008 (32비트 시스템용)](https://www.microsoft.com/download/details.aspx?familyid=189a4170-b495-4904-9cbd-209e7494d303&displaylang=ko)\*                            | 원격 코드 실행 | 중요        | 없음                      |
| [Windows Server 2008 (x64 기반 시스템용)](https://www.microsoft.com/download/details.aspx?familyid=85d8701d-f8c7-4079-8a21-a3a9d5ba71ce&displaylang=ko)\*                          | 원격 코드 실행 | 중요        | 없음                      |
| [Windows Server 2008 (Itanium 기반 시스템용)](https://www.microsoft.com/download/details.aspx?familyid=b30ee4f0-850f-4ff3-86a4-663603a0a802)                                       | 원격 코드 실행 | 중요        | 없음                      |
...
파일 정보 세부 사항은 [Microsoft 기술 자료 문서 950582](https://support.microsoft.com/kb/950582)에 나와 있습니다.

**이 업데이트에 기능에 대한 보안 관련 변경 사항이 포함되어 있습니까?**  
예. 이 공지의 "취약점 세부 정보" 항목에 나열된 변경 사항 외에도 이 보안 업데이트는 Windows Vista 및 Windows Server 2008 시스템의 자동 실행 기능과 관련하여 공개적으로 알려진 문제점도 해결합니다. 이 업데이트는 NoDriveTypeAutorun 레지스트리 키에서 제어하는 마우스 오른쪽 단추 클릭 및 두 번 클릭 동작을 올바르게 비활성화합니다. [CVE-2008-0951(영문)](https://cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2008-0951)에서 확인된 문제점을 Windows Vista 및 Windows Server 2008에서 수정합니다. 이 레지스트리 키 사용에 대한 자세한 내용은 TechNet 문서, [Microsoft 기술 자료 문서 953252](https://support.microsoft.com/kb/953252)을 참조하십시오.
...
<th style="border:1px solid black;" >Windows 검색 조건 저장 취약점(CVE-2008-1435)</th>
...
Windows 검색 조건 저장 취약점(CVE-2008-1435)  
--------------------------------------------
  
특수하게 조작된 검색 조건 파일을 Windows 탐색기에서 저장할 때 원격 코드 실행 취약점이 존재합니다.이는 Windows 탐색기가 종료한 다음 악용 가능한 방식으로 다시 시작하도록 작동합니다.
  
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2008-1435(영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2008-1435)를 참조하십시오.
  
#### Windows 검색 조건 저장 취약점(CVE-2008-1435)에 대한 완화 요소
  
완화란 취약점의 악용 심각도를 낮출 수 있는 설정, 일반적인 구성 또는 최선책, 기본 상태를 지칭합니다. 다음과 같은 완화 요소가 현재 상황에서 유용할 수 있습니다.
  
-   메일을 통해 취약점을 자동으로 악용할 수 없습니다. 메일 메시지를 통한 공격에 성공하려면 메일로 보낸 첨부 파일을 사용자가 열어 저장해야 합니다.  
-   웹을 통한 공격의 경우 공격자는 호스팅하는 웹 사이트에 조작된 검색 조건 저장(.search-ms) 파일을 포함할 수 있습니다.또한 사용자가 제공한 콘텐츠나 광고를 허용하거나 호스팅하는 공격 당한 웹 사이트에는 이 취약점을 악용할 수 있는 특수하게 조작된 콘텐츠가 포함되어 있을 수 있습니다.그러나 어떠한 경우에도 공격자는 강제로 사용자가 이러한 웹 사이트를 방문하도록 만들 수 없습니다.이 취약점을 이용하려면 사용자 조작이 필요합니다.  
-   취약점 악용에 성공한 공격자는 로컬 사용자와 동일한 권한을 얻을 수 있습니다. 시스템에 대한 사용자 권한이 적게 구성된 계정의 사용자는 관리자 권한으로 작업하는 사용자에 비해 영향을 적게 받습니다.
  
#### Windows 검색 조건 저장 취약점(CVE-2008-1435)에 대한 대안
...
#### Windows 검색 조건 저장 취약점(CVE-2008-1435)에 대한 FAQ
...
**취약점의 원인은 무엇입니까?**    
검색 파일을 저장할 때 Windows 탐색기가 파일을 올바르게 구문 분석하지 않습니다.
  
**Windows 검색이란 무엇입니까?**  
Windows 검색은 Windows Vista와 Windows Server 2008의 표준 구성 요소이며, 기본적으로 사용하도록 설정되어 있습니다. Windows 검색을 사용하여 전자 메일, 연락처, 일정 약속, 문서, 사진, 멀티미디어, 기타 타사 형식 등의 일반적인 파일 및 데이터 형식을 즉시 검색할 수 있습니다. 이 기능을 통해 소비자와 정보 작업자는 가정 및 기업 환경에서 많은 양의 데이터를 효율적으로 찾고 관리하고 구성할 수 있습니다.자세한 내용은 [Windows 검색(영문)](https://www.microsoft.com/windows/products/winfamily/desktopsearch/default.mspx) 홈 페이지를 참조하십시오.Windows 검색이 Windows XP의 추가 기능이기는 하지만 Windows XP 시스템은 이 문제점의 영향을 받지 않습니다.
...
웹을 통한 공격의 경우 공격자는 호스팅하는 웹 사이트에 이 취약점 악용 시도에 사용되는 검색 조건 저장 파일을 두어야 합니다. 또한 사용자가 제공한 콘텐츠를 허용하거나 호스팅하는 공격 당한 웹 사이트에는 이 취약점을 악용할 수 있는 특수하게 조작된 콘텐츠가 포함되어 있을 수 있습니다. 공격자는 강제로 사용자가 특수하게 조작된 웹 사이트를 방문하도록 만들 수 없습니다. 대신 공격자는 사용자가 전자 메일 메시지 또는 인스턴트 메신저 메시지의 링크를 클릭하여 공격자의 웹 사이트를 방문하도록 유도하는 것이 일반적입니다.
...
**업데이트는 어떤 기능을 수행합니까?**    
이 보안 업데이트는 모든 검색 조건 저장 파일의 내용 구성에 대한 유효성 검사를 올바르게 수행하도록 Windows 탐색기의 검색 조건 저장 구문 분석 방식을 수정하여 취약점을 해결합니다.
  
**이 보안 공지를 게시했을 때 이 취약점이 공개되어 있었습니까?**    
예. 이 취약점은 공개되었습니다. 이 취약점에는 CVE(Common Vulnerability and Exposure) 번호 [CVE-2006-1435(영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2008-1435)이 지정되었습니다. 이 보안 공지에서는 공개적으로 보고된 취약점 및 내부 조사를 통해 발견된 추가적인 문제를 해결합니다.
...
<td style="border:1px solid black;"><a href="https://support.microsoft.com/kb/950582">Microsoft 기술 자료 문서 950582</a> 참조</td>
</tr>
<tr class="odd">
