---
TOCTitle: 'MS10-055'
Title: 'Microsoft 보안 공지 MS10-055 - 긴급'
ms:assetid: 'ms10-055'
ms:contentKeyID: 61230316
ms:mtpsurl: 'https://technet.microsoft.com/ko-KR/library/ms10-055(v=Security.10)'
---


Microsoft 보안 공지 MS10-055 - 긴급
===================================

Cinepak 코덱의 취약점으로 인한 원격 코드 실행 문제점(982665)
------------------------------------------------------------

게시된 날짜: 2010년 8월 11일 수요일 | 업데이트된 날짜: 2010년 8월 13일 금요일
...
이 보안 업데이트는 비공개적으로 보고된 Cinepak 코덱의 취약점을 해결합니다. 이 취약점으로 인해 사용자가 특수하게 조작된 미디어 파일을 열거나, 웹 사이트 또는 웹 콘텐츠를 제공하는 응용 프로그램으로부터 특수하게 조작된 스트리밍 콘텐츠를 받을 경우 원격 코드 실행이 허용될 수 있습니다. 취약점 악용에 성공한 공격자는 로컬 사용자와 동일한 권한을 얻을 수 있습니다. 시스템에 대한 사용자 권한이 적게 구성된 계정의 사용자는 관리자 권한으로 작업하는 사용자에 비해 영향을 적게 받습니다.

이 보안 업데이트의 심각도는 지원 대상인 모든 Windows XP, Windows Vista 및 Windows 7 에디션에 대해 긴급입니다. 자세한 내용은 이 섹션에서 **영향을 받는 소프트웨어 및 영향을 받지 않는 소프트웨어**를 참조하십시오.

이 보안 업데이트는 Cinepak 코덱이 미디어 파일을 압축 해제하는 방식을 수정하여 취약점을 해결합니다. 취약점에 대한 자세한 내용은 다음 섹션, **취약점 정보**에서 각 취약점 항목의 자주 제기되는 질문 사항(FAQ)을 참조하십시오.
...
**문제점.** 이 보안 업데이트를 설치할 때 고객이 겪을 수 있는 현재까지 알려진 문제점은 [Microsoft 기술 자료 문서 982665](https://support.microsoft.com/kb/982665)에 나와 있습니다. 이 문서는 이러한 문제점에 대한 권장 해결 방법도 소개합니다.
...
|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------|-------------|---------------------------|
| [Windows XP 서비스 팩 3](https://www.microsoft.com/download/details.aspx?familyid=5ddb5e34-f97a-47c6-96c8-ba2ed06ccb77&displaylang=ko)                                                         | 원격 코드 실행 | 긴급        | 없음                      |
| [Windows XP Professional x64 Edition 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=5cff5d6e-11a5-40ed-92ac-e12d287919e6)                                               | 원격 코드 실행 | 긴급        | 없음                      |
| [Windows Vista 서비스 팩 1 및 Windows Vista 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=60c81415-b61e-44a4-8dd9-cedec99eb70f&displaylang=ko)                         | 원격 코드 실행 | 긴급        | 없음                      |
| [Windows Vista x64 Edition 서비스 팩 1 및 Windows Vista x64 Edition 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=469b732d-ca62-4a48-bb55-99f2ae4ddcf5&displaylang=ko) | 원격 코드 실행 | 긴급        | 없음                      |
| [Windows 7(32비트 시스템용)](https://www.microsoft.com/download/details.aspx?familyid=2e782ac9-b5d5-490e-a01a-7d4481eab224&displaylang=ko)                                                     | 원격 코드 실행 | 긴급        | 없음                      |
| [Windows 7(x64 기반 시스템용)](https://www.microsoft.com/download/details.aspx?familyid=24751193-592f-4c44-a8d6-f4112d4f011b&displaylang=ko)                                                   | 원격 코드 실행 | 긴급        | 없음                      |
...
| Windows Server 2008(32 비트 시스템용) 및 Windows Server 2008(32 비트 시스템용) Service Pack 2        |
...
**Windows 7 서비스 팩 1 베타 버전은 어떻게 이 취약점의 영향을 받습니까?**  
Windows 7 서비스 팩 1 베타는 이 공지에 설명된 취약점의 영향을 받습니다. 이러한 베타 릴리스를 실행하는 고객은 이 업데이트를 다운로드하여 시스템에 적용하는 것이 좋습니다. 보안 업데이트는 [Microsoft Update](https://go.microsoft.com/fwlink/?linkid=40747) 및 [Windows Update](https://go.microsoft.com/fwlink/?linkid=21130)를 통해 제공됩니다. [Microsoft 다운로드 센터](https://go.microsoft.com/fwlink/?linkid=21129)에서 이 보안 업데이트를 다운로드할 수도 있습니다.
...
<th style="border:1px solid black;" >Cinepak 코덱 압축 해제 취약점(CVE-2010-2553)</th>
...
Cinepak 코덱에서 지원되는 형식의 파일을 처리하는 방식에 원격 코드 실행 취약점이 존재합니다. 이 취약점으로 인해 사용자가 특수하게 조작된 미디어 파일을 열 경우 코드 실행이 허용될 수 있습니다. 사용자가 관리자 권한으로 로그온한 경우, 이 취약점을 악용한 공격자는 영향을 받는 시스템을 완전히 제어할 수 있습니다. 이렇게 되면 공격자가 프로그램을 설치할 수 있을 뿐 아니라 데이터를 보거나 변경하거나 삭제할 수 있고 모든 사용자 권한이 있는 새 계정을 만들 수도 있습니다. 시스템에 대한 사용자 권한이 적게 구성된 계정의 사용자는 관리자 권한으로 작업하는 사용자에 비해 영향을 적게 받습니다.
  
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2010-2553 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-2553)을 참조하십시오.
  
#### Cinepak 코덱 압축 해제 취약점(CVE-2010-2553)에 대한 완화 요소
...
#### Cinepak 코덱 압축 해제 취약점(CVE-2010-2553)에 대한 대안
  
대안이란 기본적인 취약점을 수정하지는 않지만 업데이트를 적용하기 전에 알려진 공격 경로를 차단하는 데 유용한 설정 또는 구성 변경을 지칭합니다. Microsoft는 다음과 같은 대안을 테스트했으며, 대안으로 인한 기능 저하 여부를 해당 항목에서 설명합니다.
  
-   **iccvid.dll 액세스 제어 목록(ACL) 수정**
  
    iccvid.dll의 ACL을 좀 더 제한적으로 수정합니다.
    ...
    `32비트 에디션의` Windows Vista 및 Windows 7의 경우 관리자 권한 명령 프롬프트에서 다음 명령을 실행합니다.  
    ```
    ...
    **대안의 영향.** Cinepak 코덱을 사용하여 인코딩된 미디어 파일이 멀티미디어에서 재생되지 못합니다.
...
-   **DirectShow에서 CinepaK 인코딩 파일 구문 분석을 사용하지 않도록 설정**
  
    DirectShow 내에서 Cinepak 코덱으로 인코딩된 미디어 파일의 구문 분석을 사용하지 않도록 설정합니다.
    ...
    **대화형 방법 사용**
    ...
    4.  레지스트리 파일 내보내기 대화 상자에 **Drivers32\_Backup.reg**를 입력하고 **저장**을 클릭합니다.  
        **참고** 이렇게 하면 이 레지스트리 키의 백업이 기본적으로 내 문서 폴더에 만들어집니다.  
    5.  이름이 **vidc.cvid**이고 데이터가 **iccvid.dll**인 문자열을 찾아 선택합니다.  
    6.  키보드의 **Delete** 키를 눌러 레지스트리 키를 삭제합니다. 키 삭제 확인 대화 상자에 레지스트리 키를 삭제할지 묻는 메시지가 표시되면 **예**를 클릭합니다
    ...
    **관리되는 배포 스크립트 사용**
    ...
    2.  다음 내용을 .REG 확장명의 파일(예: Disable\_Cinepak\_Parser.reg)로 저장합니다.  
        **32비트 Windows 시스템**:  
    ...
    **대안의 영향.** DirectShow를 사용하여 MPEG Layer-3 콘텐츠를 재생하는 응용 프로그램에서 MPEG Layer-3 구문 분석을 사용할 수 없습니다.
    ...
    **대안 실행 취소 방법.**
    ...
    3.  레지스트리 파일 가져오기 대화 상자에서 **Drivers32\_Backup.reg**를 선택하고 **열기**를 클릭합니다.
...
#### Cinepak 코덱 압축 해제 취약점(CVE-2010-2553)에 대한 FAQ
...
**취약점의 원인은 무엇입니까?**    
Microsoft Windows에 포함되어 있는 Cinepak 코덱이 특수하게 조작된 미디어 파일을 제대로 처리하지 못합니다.
  
**Cinepak 코덱이란 무엇입니까?**    
Cinepak 코덱은 Windows Media Player에서 지원하는 미디어 인코더 및 디코더입니다. 이 코덱은 미디어 파일에 대한 비트스트림 인코딩 및 디코딩을 허용합니다. Cinepak은 벡터 양자화 기반 이미지 압축 및 프레임 차등을 사용합니다.
...
코덱은 인코더와 디코더의 두 가지 요소로 구성될 수 있습니다. 인코더는 압축(인코딩) 기능을 수행하고, 디코더는 압축 해제(디코딩) 기능을 수행합니다. 이러한 구성 요소가 모두 포함되어 있는 코덱도 있고 구성 요소 중 하나만 포함되어 있는 코덱도 있습니다.
  
예를 들어 오디오 CD에서 컴퓨터로 곡을 전송할 때 Windows Media Player는 기본적으로 Windows Media Audio 코덱을 사용하여 곡을 작은 WMA 파일로 압축합니다. 이 WMA 파일(또는 웹 사이트에서 스트리밍될 수 있는 모든 WMA 파일)을 재생할 때 Player는 스피커를 통해 곡이 재생될 수 있도록 Windows Media Audio 코덱을 사용하여 파일을 압축 해제합니다.
...
**업데이트는 어떤 기능을 수행합니까?**    
이 업데이트는 Cinepak 코덱이 미디어 파일을 압축 해제하는 방식을 수정하여 이 문제를 해결합니다.
...
<td style="border:1px solid black;">제어판의 <strong>프로그램 추가/제거</strong> 도구 또는 %Windir%\$NTUninstallKB982665$\Spuninst 폴더의 Spuninst.exe 유틸리티를 사용하십시오.</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"><strong>파일 정보</strong></td>
<td style="border:1px solid black;"><a href="https://support.microsoft.com/kb/982665">Microsoft 기술 자료 문서 982665</a> 참조</td>
</tr>
<tr class="even">
...
-   Cinepak 코덱 압축 해제 취약점(CVE-2010-2553)을 [TippingPoint (영문)](https://www.tippingpoint.com/)의 [Zero Day Initiative (영문)](https://www.zerodayinitiative.com/)와 협력하여 보고해 주신 익명 연구자
...
-   V1.1(2010년 8월 13일): Windows Server 2008(32 비트 시스템용) 및 Windows Server 2008(32 비트 시스템용) Service Pack 2이 영향을 받지 않는 소프트웨어에 추가 되었습니다.
