---
TOCTitle: 'MS12-032'
Title: 'Microsoft 보안 공지 MS12-032 - 중요'
ms:assetid: 'ms12-032'
ms:contentKeyID: 61230499
ms:mtpsurl: 'https://technet.microsoft.com/ko-KR/library/ms12-032(v=Security.10)'
---

Microsoft 보안 공지 MS12-032 - 중요
===================================
...
<td style="border:1px solid black;">
[MS11-083](https://technet.microsoft.com/ko-kr/security/bulletin/ms11-083)의 KB2588516은 KB2688338로 대체되었습니다.
</td>
</tr>
...
다음의 심각도는 취약점의 잠재적인 최대 영향을 가정합니다. 이 보안 공지 발표 후 30일 이내의 심각도 및 보안 영향과 관련된 취약점 악용 가능성에 대해서는 [5월 공지 요약](https://technet.microsoft.com/ko-kr/security/bulletin/ms12-may)의 악용 가능성 인덱스를 참조하십시오. 자세한 내용은 [Microsoft 악용 가능성 인덱스](https://technet.microsoft.com/en-us/security/cc998259.aspx)를 참조하십시오.
...
Windows 방화벽이 아웃바운드 브로드캐스트 패킷을 처리하는 방식으로 인해 Windows에 보안 기능 우회 취약점이 존재합니다. 이 취약점을 성공적으로 악용한 공격자는 Windows 방화벽 심층 방어 메커니즘을 우회하여 다른 취약점이 악용되도록 할 수 있습니다.

이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2012-0174(영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2012-0174)를 참조하십시오.

#### Windows 방화벽 우회 취약점(CVE-2012-0174)에 대한 완화 요소

이 취약점에 대해 확인된 완화 요소가 없습니다.

#### Windows 방화벽 우회 취약점(CVE-2012-0174)에 대한 대안

이 취약점에 대한 대안을 확인하지 못했습니다.

#### Windows 방화벽 우회 취약점(CVE-2012-0174)에 대한 FAQ

이 취약점을 악용하면 어느 작업까지 가능합니까?  
이 취약점은 보안 기능 우회 취약점입니다. 이 취약점을 성공적으로 악용한 공격자는 Windows 방화벽을 우회하여 다른 취약점이 악용되도록 할 수 있습니다.

취약점의 원인은 무엇입니까?  
이 취약점은 Windows 방화벽에서 아웃바운드 방화벽 규칙이 적용된 아웃바운드 브로드캐스트 패킷을 제대로 필터링하지 못할 때 발생합니다.

이 취약점의 영향을 받는 구성 요소는 무엇입니까?  
이 취약점의 영향을 받는 구성 요소는 TCP/IP 스택(tcpip.sys)입니다.
...
이 취약점을 성공적으로 악용한 공격자는 Windows 방화벽을 우회할 수 있습니다. 그런 다음 공격자는 다른 취약점을 이용하여 대상 시스템에 대한 정보를 얻거나 대상 시스템에서 코드를 실행할 수 있습니다.

공격자는 이러한 취약점을 어떻게 악용합니까?  
이 취약점을 이용하기 위해서는 공격자가 먼저 대상 컴퓨터의 로컬 서브넷에 대한 액세스 권한을 얻어야 합니다. 그런 다음 공격자는 다른 취약점을 이용하여 대상 시스템에 대한 정보를 얻거나 대상 시스템에서 코드를 실행할 수 있습니다.

취약점으로 인해 위험에 노출되는 시스템은 주로 무엇입니까?  
워크스테이션과 서버 모두 이 취약점으로 인한 위험에 노출되어 있습니다.

업데이트는 어떤 기능을 수행합니까?  
이 업데이트는 Windows 방화벽이 아웃바운드 브로드캐스트 패킷을 처리하는 방식을 수정하여 취약점을 해결합니다.

기본 아웃바운드 방화벽 정책이란 무엇입니까?  
기본 아웃바운드 방화벽 정책은 호스트에서 모든 아웃바운드 네트워크 트래픽을 허용하도록 구성됩니다. 원치 않는 아웃바운드 네트워크 트래픽을 차단하려면 고객이 트래픽 차단을 위한 방화벽 규칙을 만들어야 합니다. 아웃바운드 방화벽 규칙 추가에 대한 자세한 내용은 TechNet 문서 [원치 않는 아웃바운드 네트워크 트래픽을 차단하는 규칙 만들기](https://technet.microsoft.com/ko-kr/library/cc732306(v=ws.10).aspx)를 참조하십시오.
...
Windows 7 및 Windows Server 2008 R2에 영향을 미치는 Windows TCP/IP 스택에 권한 상승 취약점이 존재합니다. 모든 Microsoft Windows XP, Windows Server 2003, Windows Vista 및 Window Server 2008 에디션은 영향을 받지 않습니다. 이 취약점은 Windows TCP/IP 스택이 IPv6 주소 바인딩을 제대로 처리하지 못할 때 발생합니다.

이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2012-0179(영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2012-0179)를 참조하십시오.

#### TCP/IP Double Free 취약점(CVE-2012-0179)에 대한 완화 요소

완화란 취약점의 악용 심각도를 낮출 수 있는 설정, 일반적인 구성 또는 최선책, 기본 상태를 지칭합니다. 다음과 같은 완화 요소가 현재 상황에서 유용할 수 있습니다.

-   이 취약점을 악용하려면 공격자가 유효한 로그온 자격 증명을 가지고 로컬로 로그온할 수 있어야 합니다.

#### TCP/IP Double Free 취약점(CVE-2012-0179)에 대한 대안

이 취약점에 대한 대안을 확인하지 못했습니다.

#### TCP/IP Double Free 취약점(CVE-2012-0179)에 대한 FAQ

이 취약점을 악용하면 어느 작업까지 가능합니까?  
이 취약점은 권한 상승 취약점입니다.

취약점의 원인은 무엇입니까?  
이 취약점은 Windows TCP/IP 스택이 로컬 인터페이스로 IPv6 주소 바인딩을 제대로 처리하지 못할 때 발생합니다.
...
취약점으로 인해 위험에 노출되는 시스템은 주로 무엇입니까?  
워크스테이션 및 서버가 이 취약점의 영향을 받습니다.

업데이트는 어떤 기능을 수행합니까?  
이 업데이트는 Windows TCP/IP 스택이 로컬 인터페이스로 IPv6 주소 바인딩을 처리하는 방식을 수정하여 취약점을 해결합니다.

이 보안 공지를 게시했을 때 이 취약점이 공개되어 있었습니까?  
예. 이 취약점은 공개되었습니다. 이 취약점에는 CVE(Common Vulnerability and Exposure) 번호 [CVE-2012-0179(영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2012-0179)가 지정되었습니다.
...
<td style="border:1px solid black;"><a href="https://support.microsoft.com/kb/2688338">Microsoft 기술 자료 문서 2688338</a> 참조</td>
</tr>
<tr class="odd">
...
-   Windows 방화벽 우회 취약점(CVE-2012-0174)을 보고해 주신 [INFIGO IS(영문)](https://www.infigo.hr/)의 Bojan Zdrnja  
-   TCP/IP Double Free 취약점(CVE-2012-0179)에 대해 함께 작업해 주신 [Genesys Telecommunications(영문)](https://www.genesyslab.com/)의 Anatoliy Glagolev
...
-   V1.1(2012년 5월 10일): 취약점 정보 섹션의 CVE-2012-0174 및 CVE-2012-0179에 대한 완화 요소가 수정되었습니다.
