---
TOCTitle: 'MS09-011'
Title: 'Microsoft 보안 공지 MS09-011 - 긴급'
ms:assetid: 'ms09-011'
ms:contentKeyID: 61230197
ms:mtpsurl: 'https://technet.microsoft.com/ko-KR/library/ms09-011(v=Security.10)'
---


Microsoft 보안 공지 MS09-011 - 긴급
===================================
...
보안 업데이트는 DirectShow가 미디어 파일의 압축을 해제하는 방식을 수정하여 취약점을 해결합니다. 취약점에 대한 자세한 내용은 다음 섹션, **취약점 정보**에서 각 취약점 항목의 자주 제기되는 질문 사항(FAQ)을 참조하십시오.
...
파일 정보 세부 사항은 [Microsoft 기술 자료 문서 961373](https://support.microsoft.com/kb/961373)에 나와 있습니다.
...
<tr class="alternateRow">
<td style="border:1px solid black;">
DirectX 9.0\*이 설치된 Windows XP Professional x64 에디션 및 Windows XP Professional x64 에디션 서비스 팩 2
</td>
<td style="border:1px solid black;">
**긴급**   
원격 코드 실행
...
<tr>
<td style="border:1px solid black;">
DirectX 9.0\*이 설치된 Windows Server 2003 서비스 팩 1 및 Windows Server 2003 서비스 팩 2
</td>
<td style="border:1px solid black;">
**긴급**   
원격 코드 실행
...
<tr class="alternateRow">
<td style="border:1px solid black;">
DirectX 9.0\*이 설치된 Windows Server 2003 x64 Edition 및 Windows Server 2003 x64 Edition 서비스 팩 2
</td>
<td style="border:1px solid black;">
**긴급**   
원격 코드 실행
...
<tr>
<td style="border:1px solid black;">
DirectX 9.0\*이 설치된 Windows Server 2003 SP1(Itanium 기반 시스템용) 및 Windows Server 2003 SP2(Itanium 기반 시스템용)
</td>
<td style="border:1px solid black;">
**긴급**   
원격 코드 실행
...
Microsoft DirectShow에서 지원되는 형식의 파일을 처리하는 방식에 원격 코드 실행 취약점이 존재합니다. 이러한 취약점으로 인해 사용자가 특수하게 조작된 MJPEG 파일을 열 경우 코드 실행이 발생할 수 있습니다. 사용자가 관리자 권한으로 로그온한 경우, 이 취약점을 악용한 공격자는 영향을 받는 시스템을 완전히 제어할 수 있습니다. 이렇게 되면 공격자가 프로그램을 설치할 수 있을 뿐 아니라 데이터를 보거나 변경하거나 삭제할 수 있고 모든 사용자 권한이 있는 새 계정을 만들 수도 있습니다. 시스템에 대한 사용자 권한이 적게 구성된 계정의 사용자는 관리자 권한으로 작업하는 사용자에 비해 영향을 적게 받습니다.

이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2009-0084 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2009-0084)를 참조하십시오.

#### MJPEG 압축 해제 취약점(CVE-2009-0084)에 대한 완화 요소
...
#### MJPEG 압축 해제 취약점(CVE-2009-0084)에 대한 대안

대안이란 기본적인 취약점을 수정하지는 않지만 업데이트를 적용하기 전에 알려진 공격 경로를 차단하는 데 유용한 설정 또는 구성 변경을 지칭합니다. Microsoft는 다음과 같은 대안을 테스트했으며, 대안으로 인한 기능 저하 여부를 해당 항목에서 설명합니다.

-   **Quartz.dll에서 MJPEG 콘텐츠 디코딩 사용 안 함**
    ...
    **대화형 방법 사용**
    ...
    2.  다음 레지스트리 키를 찾아 클릭합니다. HKEY\_CLASSES\_ROOT\\CLSID\\{301056D0-6DFF-11D2-9EEB-006008039E37}
    3.  **파일** 메뉴를 클릭하고 **내보내기**를 선택합니다.
    4.  **레지스트리 파일 내보내기** 대화 상자에 **MJPEG\_Decoder\_Backup.reg**를 입력하고 저장을 클릭합니다.  
    ...
    **대안의 영향.**MJPEG 콘텐츠 재생이 사용하지 않도록 설정됩니다.
    ...
    **대안 실행 취소 방법.**
    ...
    3.  레지스트리 파일 가져오기 대화 상자에서 **MJPEG\_Decoder\_Backup.reg**를 선택하고 **열기**를 클릭합니다.
...
#### MJPEG 압축 해제 취약점(CVE-2009-0084)에 대한 FAQ
...
**취약점의 원인은 무엇입니까?**  
특수하게 조작된 압축 파일을 Microsoft DirectShow 구성 요소가 올바르게 처리하지 않습니다.
...
이 취약점을 이용하려면 사용자가 특수하게 조작된 MJPEG 파일을 열거나, 웹 사이트 또는 웹 콘텐츠를 제공하는 응용 프로그램으로부터 특수하게 조작된 스트리밍 콘텐츠를 받아야 합니다.
...
**업데이트는 어떤 기능을 수행합니까?**  
업데이트는 DirectShow가 미디어 파일의 압축을 해제하는 방식을 수정하여 문제점을 해결합니다.
...
<td style="border:1px solid black;">DirectX 8.1이 설치된 Microsoft Windows 2000 서비스 팩 4:<br />
Windows2000-DirectX8-KB961373-x86-kor /quiet<br />
<br />
DirectX 9.0, DirectX 9.0a, DirectX 9.0b 또는 DirectX 9.0c가 설치된 Microsoft Windows 2000 서비스 팩 4:<br />
Windows2000-DirectX9-KB961373-x86-kor /quiet</td>
</tr>
<tr class="even">
<td style="border:1px solid black;">다시 시작하지 않고 설치</td>
<td style="border:1px solid black;">DirectX 8.1이 설치된 Microsoft Windows 2000 서비스 팩 4:<br />
Windows2000-DirectX8-KB961373-x86-kor /norestart<br />
<br />
DirectX 9.0, DirectX 9.0a, DirectX 9.0b 또는 DirectX 9.0c가 설치된 Microsoft Windows 2000 서비스 팩 4:<br />
Windows2000-DirectX9-KB961373-x86-kor /norestart</td>
</tr>
<tr class="odd">
...
<br />
DirectX 8.1이 설치된 Microsoft Windows 2000 서비스 팩 4:<br />
...
<td style="border:1px solid black;"><a href="https://support.microsoft.com/kb/961373">Microsoft 기술 자료 문서 961373</a> 참조</td>
</tr>
<tr class="even">
...
WindowsXP- KB961373-x86-kor /quiet</td>
</tr>
<tr class="even">
<td style="border:1px solid black;"></td>
...
<tr class="odd">
<td style="border:1px solid black;">다시 시작하지 않고 설치</td>
<td style="border:1px solid black;">DirectX 9.0, DirectX 9.0a, DirectX 9.0b 또는 DirectX 9.0c가 설치된 지원 대상인 모든 Windows XP 서비스 팩 3 32비트 에디션:<br />
WindowsXP- KB961373-x86-kor /norestart</td>
</tr>
<tr class="even">
<td style="border:1px solid black;"></td>
<td style="border:1px solid black;">DirectX 9.0, DirectX 9.0a, DirectX 9.0b 또는 DirectX 9.0c가 설치된 지원 대상인 모든 Windows XP x64 기반 에디션:<br />
...
<tr class="even">
<td style="border:1px solid black;"><strong>제거 정보</strong></td>
<td style="border:1px solid black;">제어판의 <strong>프로그램 추가/제거</strong> 도구 또는 %Windir%\$NTUninstallKB961373$\Spuninst 폴더의 Spuninst.exe 유틸리티를 사용하십시오.</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"><strong>파일 정보</strong></td>
<td style="border:1px solid black;"><a href="https://support.microsoft.com/kb/961373">Microsoft 기술 자료 문서 961373</a> 참조</td>
...
WindowsServer2003-KB961373-x86-kor /quiet</td>
</tr>
<tr class="even">
<td style="border:1px solid black;"></td>
...
WindowsServer2003.WindowsXP-KB961373-x64-kor /quiet</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"></td>
...
<td style="border:1px solid black;">다시 시작하지 않고 설치</td>
<td style="border:1px solid black;">DirectX 9.0, DirectX 9.0a, DirectX 9.0b 또는 DirectX 9.0c가 설치된 지원 대상인 모든 Windows Server 2003 32비트 에디션:<br />
WindowsServer2003-KB961373-x86-kor /norestart</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"></td>
<td style="border:1px solid black;">DirectX 9.0, DirectX 9.0a, DirectX 9.0b 또는 DirectX 9.0c가 설치된 지원 대상인 모든 Windows Server 2003 x64 기반 에디션:<br />
WindowsServer2003.WindowsXP-KB961373-x64-kor /norestart</td>
</tr>
<tr class="even">
<td style="border:1px solid black;"></td>
<td style="border:1px solid black;">DirectX 9.0, DirectX 9.0a, DirectX 9.0b 또는 DirectX 9.0c가 설치된 지원 대상인 모든 Windows Server 2003 Itanium 기반 에디션:<br />
...
-   MJPEG 압축 해제 취약점(CVE-2009-0084)을 보고해 주신 [Kryptos Logic (영문)](https://www.kryptoslogic.com/)의 Piotr Bania
...
-   V1.1(2009년 4월 23일): DirectX 7.0이 설치된 Microsoft Windows 2000 서비스 팩 4를 영향을 받지 않는 소프트웨어 표에 추가했습니다. 이 변경 사항은 정보에만 해당됩니다.
