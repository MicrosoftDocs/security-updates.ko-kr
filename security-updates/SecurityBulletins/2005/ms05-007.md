...
-   Microsoft Windows XP 서비스 팩 1 및 Microsoft Windows XP 서비스 팩 2: [업데이트 다운로드](https://www.microsoft.com/download/details.aspx?familyid=b8c867c2-b7cd-4e2f-90e0-169b2c7125dc&displaylang=ko)
-   Microsoft Windows XP 64-Bit Edition 서비스 팩 1(Itanium): [업데이트 다운로드](https://www.microsoft.com/download/details.aspx?familyid=2f68945e-eeb8-42bc-a8ad-0d3991204889)
...
|--------------------------------------------------------------------------------------------------------------|----------------------|------------------------|------------------------|
| 명명된 파이프 취약점 - [CAN-2005-0051 (US)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=can-2005-0051) | 정보 공개            | 중요                   | 보통                   |
...
-   **컴퓨터 브라우저 서비스 비활성화**
    ...
    **대안의 영향:** 컴퓨터 브라우저 서비스 기능을 비활성화하면 명시적으로 컴퓨터 브라우저 서비스에 종속된 모든 서비스에 대해 시스템 이벤트 로그에 오류 메시지가 기록될 수 있습니다. 컴퓨터 브라우저 서비스에 대한 자세한 내용은 [Microsoft 기술 자료 문서 188001](https://support.microsoft.com/kb/188001)을 참고하십시오.

-   **그룹 정책 설정을 사용하여 이 기능이 필요하지 않은 모든 영향을 받는 시스템에서 컴퓨터 브라우저 서비스를 비활성화합니다.**  
    컴퓨터 브라우저 서비스가 공격 도구로 사용될 수 있으므로 그룹 정책 설정을 사용하여 비활성화합니다. Windows 2000 도메인 환경 또는 Server 2003 도메인 환경에서는 그룹 정책 개체 기능을 사용하여 로컬, 사이트, 도메인 또는 조직 구성 단위 수준에서 이 서비스가 시작되지 않도록 비활성화할 수 있습니다. 로그인 스크립트를 통해 이 서비스를 비활성화하는 방법에 대한 자세한 내용은 [Microsoft 기술 자료 문서 297789](https://support.microsoft.com/kb/297789)를 참조하십시오.
...
-   **방화벽에서 139 및 445 TCP 포트 차단:**
    ...
    이들 포트는 영향을 받는 프로토콜과 연결을 시작하는 데 사용되는 포트입니다. 방화벽에서 이 포트들을 차단하면 이 취약점을 악용한 방화벽 뒤의 시스템 공격을 막을 수 있습니다. 기타 포트를 악용할 수 있는 공격을 방지하려면 인터넷으로부터의 원치 않는 인바운드 통신을 모두 차단하는 것이 좋습니다. 이 포트에 대한 자세한 내용은 [여기](https://technet.microsoft.com/security/bulletin/ms04-045)를 참조하십시오.

-   **인터넷 연결 방화벽과 같은** [**개인 방화벽을 사용합니다.**](https://www.microsoft.com/korea/security/protect/windowsxp/firewall.asp) **인터넷 연결 방화벽은 Windows XP에 포함되어 있으며 “Microsoft 네트워크용 파일 및 프린터 공유”를 활성화하지 않습니다.**
...
-   **시스템에서 이 기능을 지원하는 고급 TCP/IP 필터링을 설정합니다.**
    ...
    고급 TCP/IP 필터링을 활성화하면 원치 않는 모든 인바운드 트래픽을 차단할 수 있습니다. TCP/IP 필터링을 구성하는 방법에 대한 자세한 내용은 [Microsoft 기술 자료 문서 309798](https://support.microsoft.com/kb/309798)을 참조하십시오. 이 문서의 내용은 Windows 2000을 참조하지만 Windows XP에도 적용됩니다.

-   **영향을 받는 시스템에서 IPSec을 사용하여 해당 포트를 차단합니다.**

    IPSec(인터넷 프로토콜 보안)을 사용하면 네트워크 통신을 보호할 수 있습니다. IPSec에 대한 자세한 내용 및 필터를 적용하는 방법은 [Microsoft 기술 자료 문서 313190](https://support.microsoft.com/kb/313190) 및 [Microsoft 기술 자료 문서 813878](https://support.microsoft.com/kb/813878)을 참조하십시오. 이 문서의 내용은 Windows 2000을 참조하지만 Windows XP에도 적용됩니다.
...
**명명된 파이프란 무엇입니까?**  
명명된 파이프는 동일 컴퓨터의 프로세스 사이 또는 네트워크를 통해 서로 다른 컴퓨터의 프로세스 사이에 통신을 제공하는 데 사용됩니다. 일반적인 명명된 파이프 리소스에는 파일 공유와 인쇄 공유가 포함됩니다. 명명된 파이프에 대한 자세한 내용은 다음 [MSDN Library 웹 사이트 (US)](https://msdn.microsoft.com/library/default.asp?url=/library/en-us/ipc/base/named_pipes.asp)를 참조하십시오.
...
-   명명된 파이프 취약점(CAN-2005-0051)을 보고해 주신 [Herve Schauer Consultants (US)](https://www.hsc.fr)의 Jean-Baptiste Marchand님
