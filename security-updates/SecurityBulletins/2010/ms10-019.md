...
**문제점.** 이 보안 업데이트를 설치할 때 고객이 겪을 수 있는 현재까지 알려진 문제점은 [Microsoft 기술 자료 문서 981210](https://support.microsoft.com/kb/981210)에 나와 있습니다. 이 문서는 이러한 문제점에 대한 권장 해결 방법도 소개합니다.
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2010-0486 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-0486)을 참조하십시오.
...
Windows Authenticode Signature Verification 함수 또는 WinVerifyTrust는 지정된 개체에서 신뢰할 수 있는 확인 작업을 수행합니다. 이 함수는 작업 식별자를 지원하는 신뢰할 수 있는 공급자(있는 경우)에게로 조회를 넘깁니다. WinVerifyTrust 함수는 다음 두 가지 작업을 수행합니다. 지정된 개체에서 서명 확인 및 신뢰할 수 있는 확인 작업 자세한 내용은 MSDN 문서, [WinVerifyTrust 함수 (영문)](https://msdn.microsoft.com/en-us/library/aa388208(vs.85).aspx)를 참조하십시오.

**Windows Authenticode 이식 가능한 실행 파일 서명 형식이란 무엇입니까?**  
Authenticode는 소프트웨어 바이너리의 출처 및 무결성 확인에 사용되는 디지털 서명 형식입니다. Authenticode는 PKCS(공개 키 암호화 표준) \#7 서명된 데이터 및 X.509 인증을 사용하여 Authenticode로 서명된 바이너리를 소프트웨어 게시자의 ID로 바인딩합니다. 용어 "Authenticode" 서명은 Authenticode Signature Verification 함수를 사용하여 생성 및 확인된 디지털 서명 형식을 나타냅니다. 이 형식에 대한 자세한 내용은 Windows 문서, [Windows Authenticode 이식 가능한 실행 파일 서명 형식(영문)](https://www.microsoft.com/whdc/winlogo/drvsign/authenticode_pe.mspx)을 참조하십시오.

**캐비닛 파일이란 무엇입니까?**  
캐비닛은 보통 뒤에 .CAB가 붙는 단일 파일이며 파일 라이브러리에 압출 파일을 저장합니다. 압축 파일은 여러 캐비닛 파일을 통해 유포될 수 있습니다. 설치 도중, 설치 응용 프로그램은 캐비닛에 저장된 파일의 압축을 해제하고 사용자 시스템으로 복사합니다. 자세한 내용은 MSDN 문서 [캐비닛 파일(영문)](https://msdn.microsoft.com/en-us/library/aa376906(v=vs.85).aspx)을 참조하십시오.
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2010-0487 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-0487)을 참조하십시오.
...
<td style="border:1px solid black;">Authenticode Signature Verification 5.1:<br />
<a href="https://support.microsoft.com/kb/978601">Microsoft 기술 자료 문서 978601</a> 참조<br />
<br />
Cabinet File Viewer Shell Extension 5.1:<br />
<a href="https://support.microsoft.com/kb/979309">Microsoft 기술 자료 문서 979309</a> 참조</td>
</tr>
<tr class="even">
