---
TOCTitle: 'MS11-047'
Title: 'Microsoft 보안 공지 MS11-047 - 중요'
ms:assetid: 'ms11-047'
ms:contentKeyID: 61230414
ms:mtpsurl: 'https://technet.microsoft.com/ko-KR/library/ms11-047(v=Security.10)'
---



Microsoft 보안 공지 MS11-047 - 중요
===================================
...
이 보안 업데이트는 Hyper-V Server가 가상 시스템에서 잘못된 형식의 요청을 처리하는 방식을 수정하여 이 취약점을 해결합니다. 취약점에 대한 자세한 내용은 다음 섹션, **취약점 정보**에서 각 취약점 항목의 자주 제기되는 질문 사항(FAQ)을 참조하십시오.
...
|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------|-------------|---------------------------------------------------------------------|
| [Windows Server 2008(x64 기반 시스템용) 및 Windows Server 2008(x64 기반 시스템용) 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=24789423-72b7-48d1-bdc1-f0e5174d99bb)\* | 서비스 거부    | 중요        | [MS10-102](https://technet.microsoft.com/security/bulletin/ms10-102) |
| [Windows Server 2008 R2(x64 기반 시스템용)](https://www.microsoft.com/download/details.aspx?familyid=c9c6c36d-a455-42f7-b7d4-9fb9824c07cb)\*                                                    | 서비스 거부    | 중요        | [MS10-102](https://technet.microsoft.com/security/bulletin/ms10-102) |
| [Windows Server 2008 R2(x64 기반 시스템용) 서비스 팩 1](https://www.microsoft.com/download/details.aspx?familyid=c9c6c36d-a455-42f7-b7d4-9fb9824c07cb)\*                                        | 서비스 거부    | 중요        | 없음                                                                |
...
<th style="border:1px solid black;" >VMBus 지속적 DoS 취약점(CVE-2011-1872)</th>
...
Windows Server 2008 및 Windows Server 2008 R2의 Hyper-V에 서비스 거부 취약점이 존재합니다. 이 취약점은 Hyper-V Server의 특정 컴퓨터 명령 시퀀스에 대한 불충분한 유효성 검사로 인해 발생합니다. 취약점 악용에 성공한 공격자는 영향을 받는 Hyper-V 시스템이 응답을 중지하도록 만듭니다. 그러면 해당 시스템에서 호스팅하는 모든 가상 컴퓨터가 영향을 받습니다.
  
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2011-1872 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-1872)를 참조하십시오.
  
#### VMBus 지속적 DoS 취약점(CVE-2011-1872)에 대한 완화 요소
  
완화란 취약점의 악용 심각도를 낮출 수 있는 설정, 일반적인 구성 또는 최선책, 기본 상태를 지칭합니다. 다음과 같은 완화 요소가 현재 상황에서 유용할 수 있습니다.
  
-   이 취약점을 악용하려면 공격자가 유효한 로그온 자격 증명을 가지고 로컬로 로그온할 수 있어야 합니다. 익명의 사용자에 의해서나 원격으로는 이 취약점을 악용할 수 없습니다.
  
#### VMBus 지속적 DoS 취약점(CVE-2011-1872)에 대한 대안
  
이 취약점에 대한 대안을 확인하지 못했습니다.
  
#### VMBus 지속적 DoS 취약점(CVE-2011-1872)에 대한 FAQ
  
**이 취약점을 악용하면 어느 작업까지 가능합니까?**      
이것은 서비스 거부 취약점입니다. 공격자는 이 취약점을 악용하여 영향을 받는 Hyper-V Server가 응답을 중지하고 다시 시작되도록 만들 수 있습니다. 서비스 거부 취약점으로 인해 공격자가 Hyper-V Server에서 코드를 실행하거나 사용자 권한이 상승되는 것은 아니지만 영향을 받는 시스템이 요청 수신을 중단하도록 만들 수 있습니다.
  
**취약점의 원인은 무엇입니까?**      
게스트에서 호스트로 보낸 특정 패킷의 불충분한 유효성 검사로 인해 취약점이 발생합니다. 이 불충분한 유효성 검사로 인한 이러한 명령 처리 때문에 Hyper-V Server 응용 프로그램의 과도한 리소스 소비가 발생할 수 있습니다.
  
**Hyper-V란 무엇입니까?**      
Hyper-V는 Windows Server 2008 및 Windows Server 2008 R2의 핵심 기능인 Hypervisor 기반 기술로 확장 가능하고, 신뢰할 수 있으며, 높은 가용성의 가상화 플랫폼을 제공합니다. 이러한 유형의 환경은 가상 컴퓨터를 만들고 관리하면서 하나의 물리적 컴퓨터에서 여러 운영 체제를 실행하고 운영 체제를 서로 격리시킬 수 있다는 점에서 유용합니다.
  
**공격자는 취약점을 악용하여 무엇을 할 수 있습니까?**      
이 취약점 악용에 성공한 공격자는 사용자의 시스템에서 응답을 중지하거나 다시 시작하도록 만들 수 있습니다. 이 취약점으로 인해 Hyper-V Server 및 이 서버에서 호스팅하는 모든 게스트 가상 컴퓨터의 리소스 소비가 100%가 될 수 있습니다.
  
**공격자는 이러한 취약점을 어떻게 악용합니까?**      
공격자는 Hyper-V Server에서 호스팅하는 게스트 가상 컴퓨터 중 하나에서 인증된 관리자여야 하며, 신뢰할 수 없는 실행 파일을 실행하여 인스턴스에 대해 해당 시스템에서 임의의 코드를 실행할 수 있어야 합니다. 그러면 이 실행 파일은 리소스 소비를 유발하는 잘못된 형식의 컴퓨터 명령 시퀀스를 호출함으로써 Hyper-V Server에서 무한 루프가 발생하여 다른 가상 컴퓨터의 요청을 처리하지 못하게 됩니다.
  
공격자가 이 서비스 거부 공격을 실행하기 위해 필요한 권한은 무엇입니까 **?**      
Hyper-V Server 및 Hyper-V Server에서 실행 중인 모든 가상 컴퓨터에 대해 이 서비스 거부 공격을 수행하려면 공격자가 Hyper-V Server에서 호스팅되는 가상 컴퓨터 중 하나에 대해 관리자 권한을 가지고 있어야 합니다. 공격자가 Hyper-V Server 자체에서 관리자 액세스 권한을 가질 필요는 없습니다.
  
**취약점으로 인해 주로 공격 받을 위험이 있는 시스템은 무엇입니까?**      
신뢰할 수 없는 사용자에 대한 액세스를 허용하는 게스트 가상 컴퓨터를 호스팅하는 Hyper-V 서버가 특히 이 취약점으로 인한 위험에 노출될 수 있습니다.
  
**업데이트는 어떤 기능을 수행합니까?**      
이 업데이트는 Hyper-V Server가 가상 시스템에서 잘못된 형식의 요청을 처리하는 방식을 수정합니다. 이를 통해 Hyper-V Server 응용 프로그램은 게스트에서 호스트로 보낸 VMBus 패킷의 유효성을 제대로 검사할 수 있습니다.
...
<td style="border:1px solid black;">지원 대상인 모든 Windows Server 2008 x64 기반 에디션에 설치된 Hyper-V Server:<br />
...
<td style="border:1px solid black;"><a href="https://support.microsoft.com/kb/2525835">Microsoft 기술 자료 문서 2525835</a> 참조</td>
</tr>
<tr class="odd">
...
<tr class="odd">
<td style="border:1px solid black;">사용자 개입 없이 설치</td>
<td style="border:1px solid black;">지원 대상인 모든 Windows Server 2008 R2 x64 기반 에디션에 설치된 Hyper-V Server:<br />
...
-   VMBus 지속적 DoS 취약점(CVE-2011-1872)을 보고해 주신 [Core Security Technologies (영문)](https://www.coresecurity.com/)의 Nicolas Economou
