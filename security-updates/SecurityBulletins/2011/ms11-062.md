...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2011-1974 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-1974)를 참조하십시오.
...
RAS(원격 액세스 서비스)를 사용하면 사용자가 전화선, 인터넷 또는 기타 네트워크 연결을 통해 원격 컴퓨터에 연결할 수 있으므로 해당 컴퓨터에서 실제로 원격 컴퓨터에 연결되어 있는 것처럼 작업할 수 있습니다. RAS를 사용하여 사용자는 전자 메일 주고받기, 팩스로 문서 보내기, 파일 검색, 사무실 프린터에서 문서 인쇄 등의 작업을 원격을 수행할 수 있습니다. 자세한 내용은 MSDN 문서 [RAS 아키텍처 개요(NDIS 5.1) (영문)](https://msdn.microsoft.com/library/ff563285(v=vs.85).aspx)를 참조하십시오.
...
NDISTAPI 드라이버는 RAS 아키텍처의 일부이며 NDISWAN을 TAPI 서비스에 연결합니다. 전화 통신 응용 프로그램이 TAPI 서비스를 구현하고 TAPI가 회선과 호출을 설정, 모니터링 및 해제하기 위한 요청을 할 수 있습니다. NDISTAPI 드라이버는 이러한 TAPI 요청을 NDISWAN에 전달합니다. 자세한 내용은 MSDN 문서 [NDISTAPI 개요(NDIS 5.1) (영문)](https://msdn.microsoft.com/library/ff554813(v=vs.85).aspx)를 참조하십시오.
  
**NDISWAN란 무엇입니까?**      
NDISWAN(Network Driver Interface Standard Wide Area Network)은 데이터 압축, 암호화, 루프백 및 WAN 미니포트 드라이버에서 사용되는 단순 PPP 프레이밍 같은 기능을 제공하는 시스템 제공 NDIS 중간 드라이버입니다. 따라서 매체에 특유한 이와 같은 기능만을 구현하기 위해서는 WAN 미니포트 드라이버가 있어야 합니다(예: ISDN의 경우 Q931 신호 필요). 자세한 내용은 MSDN 문서, [NDISWAN 개요 (영문)](https://msdn.microsoft.com/library/ff564653(v=vs.85).aspx)를 참조하십시오.
...
<td style="border:1px solid black;"><a href="https://support.microsoft.com/kb/2566454">Microsoft 기술 자료 문서 2566454</a> 참조</td>
</tr>
<tr class="even">
