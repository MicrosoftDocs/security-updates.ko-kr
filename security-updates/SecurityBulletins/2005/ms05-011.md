...
|-----------------------------------------------------------------------------------------------------------------|----------------------|--------------|------------|---------------------|
| 서버 메시지 블록 취약점 - [CAN-2005-0045 (US)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=can-2005-0045) | 원격 코드 실행       | 긴급         | 긴급       | 긴급                |
...
**참고** IPX/SPX 등 기타 프로토콜도 이 문제점에 취약할 수 있습니다. 이러한 프로토콜을 사용 중인 경우, 이 프로토콜에 대해서도 적절한 포트를 차단해야 합니다. IPX/SPX에 대한 자세한 내용은 [여기 (US)](https://www.microsoft.com/resources/documentation/windows/xp/all/reskit/en-us/prch_cnn_goue.asp)를 참조하십시오.

-   **방화벽에서 139 및 445 TCP 포트 차단:**

    이들 포트는 영향을 받는 프로토콜과 연결을 시작하는 데 사용되는 포트입니다. 방화벽의 인바운드와 아웃바운드 모두에서 이들 포트를 차단하면 이 취약점을 악용한 방화벽 뒤의 시스템 공격을 예방할 수 있습니다. 기타 포트를 악용할 수 있는 공격을 방지하려면 인터넷으로부터의 원치 않는 인바운드 통신을 모두 차단하는 것이 좋습니다. 이 포트에 대한 자세한 내용은 [여기](https://technet.microsoft.com/security/bulletin/ms04-045)를 참조하십시오.
...
-   서버 메시지 블록 취약점(CAN-2005-0045)을 보고해 주신 [eEYE (US)](https://www.eeye.com)
