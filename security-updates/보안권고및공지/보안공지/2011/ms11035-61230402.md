...
**문제점.** 이 보안 업데이트를 설치할 때 고객이 겪을 수 있는 현재까지 알려진 문제점은 [Microsoft 기술 자료 문서 2524426](http://support.microsoft.com/kb/2524426)에 나와 있습니다. 이 문서는 이러한 문제점에 대한 권장 해결 방법도 소개합니다.
...
|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------|-------------|---------------------------------------------------------------------|
| [Windows Server 2003 서비스 팩 2](http://www.microsoft.com/downloads/details.aspx?familyid=0d289146-8470-45dd-a886-d1b35a2c6d4b&displaylang=ko)                                                                      | 원격 코드 실행 | 긴급        | [MS09-039](http://technet.microsoft.com/security/bulletin/ms09-039) |
| [Windows Server 2003 x64 Edition 서비스 팩 2](http://www.microsoft.com/downloads/details.aspx?familyid=0d3e85a9-42ff-447b-ab8c-095a1c0b224c&displaylang=ko)                                                          | 원격 코드 실행 | 긴급        | [MS09-039](http://technet.microsoft.com/security/bulletin/ms09-039) |
| [Windows Server 2003 with SP2(Itanium 기반 시스템용)](http://www.microsoft.com/downloads/details.aspx?familyid=4c388fe8-883a-4118-a7ff-50f1e9d0a071)                                                                 | 원격 코드 실행 | 긴급        | [MS09-039](http://technet.microsoft.com/security/bulletin/ms09-039) |
| [Windows Server 2008(32비트 시스템용) 및 Windows Server 2008(32비트 시스템용) 서비스 팩 2](http://www.microsoft.com/downloads/details.aspx?familyid=9019a3ff-d953-4b0b-8c7a-05aa9cf65c92&displaylang=ko)\*           | 원격 코드 실행 | 긴급        | 없음                                                                |
| [Windows Server 2008(x64 기반 시스템용) 및 Windows Server 2008(x64 기반 시스템용) 서비스 팩 2](http://www.microsoft.com/downloads/details.aspx?familyid=841130fa-4522-4079-b7eb-f8466ac3ef12&displaylang=ko)\*       | 원격 코드 실행 | 긴급        | 없음                                                                |
| [Windows Server 2008 R2(x64 기반 시스템용) 및 Windows Server 2008 R2(x64 기반 시스템용) 서비스 팩 1](http://www.microsoft.com/downloads/details.aspx?familyid=8732b41e-2f25-4ee1-a8b7-a8b8b9e22d08&displaylang=ko)\* | 원격 코드 실행 | 긴급        | 없음                                                                |
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2011-1248 (영문)](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-1248)을 참조하십시오.
...
-   **방화벽에서 TCP 포트 42 및 UDP 포트 42를 차단합니다.**
    ...
    이 포트들은 영향을 받는 구성 요소와 연결을 시작하는 데 사용됩니다. 기업 방화벽에서 이러한 인바운드 및 아웃바운드 포트를 모두 차단하면 이 취약점을 악용한 방화벽 뒤의 시스템 침입으로부터 보호할 수 있습니다. 다른 포트를 사용하는 공격을 방지하려면 인바운드 원치 않는 인터넷 통신을 모두 차단하는 것이 좋습니다. 포트에 대한 자세한 내용은 TechNet 문서, [TCP 및 UDP 포트 할당 (영문)](http://go.microsoft.com/fwlink/?linkid=21312)을 참조하십시오. Windows 방화벽에 대한 자세한 내용은 TechNet 문서, [단일 컴퓨터에서 Windows 방화벽을 구성하는 방법 (영문)](http://www.microsoft.com/technet/security/smallbusiness/prodtech/windowsxp/cfgfwall.mspx)을 참조하십시오.
...
**WINS란 무엇입니까?**    
WINS(Windows Internet Name Service)는 NetBT(NetBIOS over TCP/IP)를 지원하도록 특수하게 고안되었습니다. 사용자가 리소스에 액세스하는 데 NetBIOS 이름이 필요한 모든 환경에서는 WINS가 필수적입니다. 그러한 네트워크에서 WINS를 사용하지 않으면, 원격 네트워크 리소스에 연결하거나 파일 및 인쇄 공유 연결을 구성하기 위해 LMHOSTS 파일을 사용해야 합니다. 자세한 내용은 TechNet 문서 [WINS란 무엇입니까?(영문)](http://technet.microsoft.com/library/cc784180.aspx)를 참조하십시오.
...
<td style="border:1px solid black;"><a href="http://support.microsoft.com/kb/2524426">Microsoft 기술 자료 문서 2524426</a> 참조</td>
</tr>
<tr class="even">
...
-   WINS 서비스 응답 실패 취약점(CVE-2011-1248)을 [TippingPoint's (영문)](http://www.tippingpoint.com/)[Zero Day Initiative (영문)](http://www.zerodayinitiative.com/)와 협력하여 보고해 주신 Luigi Auriemma
