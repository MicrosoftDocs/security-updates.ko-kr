---
TOCTitle: 'MS12-052'
Title: 'Microsoft 보안 공지 MS12-052 - 긴급'
ms:assetid: 'ms12-052'
ms:contentKeyID: 61230519
ms:mtpsurl: 'https://technet.microsoft.com/ko-KR/library/ms12-052(v=Security.10)'
---

Microsoft 보안 공지 MS12-052 - 긴급
===================================

Internet Explorer 누적 보안 업데이트 (2722913)
----------------------------------------------

게시된 날짜: 2012년 8월 15일 수요일 | 업데이트된 날짜: 2013년 7월 18일 목요일
...
이 보안 업데이트는 Internet Explorer가 메모리에서 개체를 처리하는 방식을 수정하여 취약점을 해결합니다. 취약점에 대한 자세한 내용은 다음 섹션, 취약점 정보에서 각 취약점 항목의 자주 제기되는 질문 사항(FAQ)을 참조하십시오.
...
알려진 문제점. 이 보안 업데이트를 설치할 때 고객이 겪을 수 있는 현재까지 알려진 문제점은 [Microsoft 기술 자료 문서 2722913](https://support.microsoft.com/kb/2722913)에 나와 있습니다. 이 문서는 이러한 문제점에 대한 권장 해결 방법도 소개합니다.
...
<td style="border:1px solid black;">
[MS12-037](https://go.microsoft.com/fwlink/?linkid=249045)의 KB2699988은 KB2722913으로 대체되었습니다.
</td>
</tr>
...
<td style="border:1px solid black;">
[MS12-044](https://go.microsoft.com/fwlink/?linkid=254377)의 KB2719177은 KB2722913으로 대체되었습니다.
</td>
</tr>
<tr>
<td style="border:1px solid black;">
...
이 보안 업데이트의 SHA1 및 SHA2 해시는 다운로드한 보안 업데이트 패키지의 진위 여부를 검증하는 데 사용할 수 있습니다. 이 업데이트와 관련된 해시 정보는 [Microsoft 기술 자료 문서 2722913](https://support.microsoft.com/kb/2722913)을 참조하십시오.
...
이 보안 업데이트는 ***MS12-056***과 어떤 관련이 있습니까 ?  
이 공지에서 설명하는 JavaScript 정수 오버플로 원격 코드 실행 취약점(CVE-2012-2523)은 [MS12-056](https://go.microsoft.com/fwlink/?linkid=256487)을 통해서도 해결됩니다.

시스템에 Internet Explorer 8을 설치한 고객은 CVE-2012-2523에 설명된 취약점으로부터 보호하기 위해 [MS12-056](https://go.microsoft.com/fwlink/?linkid=256487)에서 설명하는 KB2706045 업데이트를 설치해야 합니다.

이 공지에서 설명하는 업데이트(KB2722913)는 Internet Explorer 9를 실행하는 시스템에 대한 CVE-2012-2523을 해결하므로 해당 시스템에 Internet Explorer 9를 설치한 고객은 KB2706045 업데이트를 설치할 필요가 없습니다.
...
다음의 심각도는 취약점의 잠재적인 최대 영향을 가정합니다. 이 보안 공지 발표 후 30일 이내의 심각도 및 보안 영향과 관련된 취약점 악용 가능성에 대해서는 [8월 공지 요약](https://technet.microsoft.com/security/bulletin/ms12-aug)의 악용 가능성 인덱스를 참조하십시오. 자세한 내용은 [Microsoft 악용 가능성 인덱스](https://technet.microsoft.com/security/cc998259.aspx)를 참조하십시오.
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2012-1526](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2012-1526)(영문)을 참조하십시오.

#### 레이아웃 메모리 손상 취약점(CVE-2012-1526)에 대한 완화 요소
...
-   기본적으로 Windows Server 2003, Windows Server 2008 및 Windows Server 2008 R2의 Internet Explorer는 [보안 강화 구성](https://technet.microsoft.com/library/dd883248.aspx)으로 알려진 제한된 모드로 실행됩니다. 이 모드에서는 이 취약점이 완화됩니다. Internet Explorer 보안 강화 구성에 대한 자세한 내용은 이 취약점에 대한 FAQ 항목을 참조하십시오.

#### 레이아웃 메모리 손상 취약점(CVE-2012-1526)에 대한 대안
...
#### 레이아웃 메모리 손상 취약점(CVE-2012-1526)에 대한 FAQ
...
이 취약점을 이용하여 악의적인 작업을 수행하려면 사용자가 시스템에 로그온하여 해당 웹 사이트를 탐색해야 합니다. 따라서 워크스테이션 또는 터미널 서버와 같이 Internet Explorer 6 또는 Internet Explorer 7을 빈번하게 사용하는 시스템은 이 취약점을 이용하는 공격을 받을 위험이 가장 큽니다.

Windows Server 2003, Windows Server 2008 또는 Windows Server 2008 R2에서 Internet Explorer를 실행하고 있습니다. 이 취약점이 완화됩니까?  
예. 기본적으로 Windows Server 2003, Windows Server 2008 및 Windows Server 2008 R2의 Internet Explorer는 [보안 강화 구성](https://technet.microsoft.com/library/dd883248.aspx)으로 알려진 제한된 모드로 실행됩니다. 보안 강화 구성은 미리 구성된 Internet Explorer 설정 그룹으로, 사용자나 관리자가 서버에서 특수하게 조작된 웹 콘텐츠를 다운로드하고 실행할 가능성을 줄여 줍니다. Internet Explorer의 신뢰할 수 있는 사이트 영역에 추가되지 않는 웹 사이트에 대한 완화 요소입니다.
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2012-2521](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2012-2521)(영문)을 참조하십시오.

#### 비동기 NULL 개체 액세스 원격 코드 실행 취약점(CVE-2012-2521)에 대한 완화 요소
...
#### 비동기 NULL 개체 액세스 원격 코드 실행 취약점(CVE-2012-2521)에 대한 대안
...
#### 비동기 NULL 개체 액세스 원격 코드 실행 취약점(CVE-2012-2521)에 대한 FAQ
...
Internet Explorer가 손상되어 삭제된 가상 함수 테이블에 액세스하는 방식에 원격 코드 실행 취약점이 존재합니다. 취약점으로 인해 메모리가 손상되고 공격자가 현재 사용자의 컨텍스트에서 임의 코드를 실행할 수 있습니다.

이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2012-2522](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2012-2522)(영문)를 참조하십시오.

#### 가상 함수 테이블 손상 원격 코드 실행 취약점(CVE-2012-2522)에 대한 완화 요소
...
#### 가상 함수 테이블 손상 원격 코드 실행 취약점(CVE-2012-2522)에 대한 대안
...
#### 가상 함수 테이블 손상 원격 코드 실행 취약점(CVE-2012-2522)에 대한 FAQ

이 취약점을 악용하면 어느 작업까지 가능합니까?  
이 취약점은 현재 사용자 컨텍스트에 적용되는 원격 코드 실행 취약점입니다.

취약점의 원인은 무엇입니까?  
Internet Explorer가 손상된 가상 함수 테이블에 액세스하려고 할 때 메모리가 손상되어 공격자가 현재 사용자의 컨텍스트에서 임의 코드를 실행할 수 있습니다.
...
Internet Explorer가 복사 작업 도중 메모리에서 개체의 크기를 계산하는 방식에 원격 코드 실행 취약점이 존재합니다. 취약점으로 인해 메모리가 손상되고 공격자가 현재 사용자의 컨텍스트에서 임의 코드를 실행할 수 있습니다.

이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2012-2523](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2012-2523)(영문)을 참조하십시오.

#### JavaScript 정수 오버플로 원격 코드 실행 취약점(CVE-2012-2523)에 대한 완화 요소
...
-   기본적으로 지원 대상인 모든 Microsoft Outlook, Microsoft Outlook Express 및 Windows Mail 버전은 제한된 사이트 영역에서 HTML 메일 메시지를 엽니다. 스크립트 및 ActiveX 컨트롤을 비활성화하는 제한된 사이트 영역은 공격자가 이 취약점을 악용하여 악성 코드를 실행하는 위험을 줄이는 데 도움이 됩니다. 사용자가 메일에 포함된 링크를 클릭하면 웹 기반 공격 시나리오를 통해 이 취약점 악용에 취약해질 수 있습니다.
-   기본적으로 Windows Server 2003, Windows Server 2008 및 Windows Server 2008 R2의 Internet Explorer는 [보안 강화 구성](https://technet.microsoft.com/library/dd883248.aspx)으로 알려진 제한된 모드로 실행됩니다. 이 모드에서는 이 취약점이 완화됩니다. Internet Explorer 보안 강화 구성에 대한 자세한 내용은 이 취약점에 대한 FAQ 항목을 참조하십시오.
-   64비트 버전의 Internet Explorer를 사용하도록 추가로 구성된 64비트 버전의 Microsoft Windows만 이 취약점의 영향을 받습니다.

#### JavaScript 정수 오버플로 원격 코드 실행 취약점(CVE-2012-2523)에 대한 대안
...
-   32비트 버전 ***Internet Explorer*** ***사용***

    32비트 버전 Internet Explorer의 바로 가기를 만들려면 다음을 수행하십시오.

    1.  Windows 탐색기에서 "C:\\Program Files (x86)\\Internet Explorer\\"로 이동합니다.
    2.  "iexplore.exe"를 마우스 오른쪽 단추로 클릭하고 바로 가기 만들기를 클릭합니다.
    3.  바탕 화면에 바로 가기를 만듭니다. 빠른 실행 도구 모음에 바로 가기를 복사할 수도 있습니다.

    직접 만든 이 바로 가기를 사용하여 32비트 버전 Internet Explorer를 실행합니다.

    또한 다음 단계를 수행하여 32비트 버전 Internet Explorer를 기본 브라우저로 설정할 수도 있습니다.

    1.  바탕 화면에 만든 바로 가기를 클릭하여 32비트 버전 Internet Explorer를 엽니다.
    2.  Internet Explorer를 기본 브라우저로 설정할 것인지 묻는 메시지가 표시되면 예를 클릭합니다. 이렇게만 하면 모든 설정이 완료됩니다. 메시지가 나타나지 않으면 다음 단계로 이동합니다.
    3.  도구 단추를 클릭한 다음 인터넷 옵션을 클릭합니다.
    4.  프로그램 탭을 클릭한 후 기본 웹 브라우저로 설정을 클릭합니다.
    5.  확인을 클릭한 다음 Internet Explorer를 닫습니다.

    이제 32비트 버전 Internet Explorer가 기본 웹 브라우저로 설정되었습니다.

#### JavaScript 정수 오버플로 원격 코드 실행 취약점(CVE-2012-2523)에 대한 FAQ

이 취약점을 악용하면 어느 작업까지 가능합니까?  
이 취약점은 현재 사용자 컨텍스트에 적용되는 원격 코드 실행 취약점입니다.

취약점의 원인은 무엇입니까?  
이 취약점은 Internet Explorer가 복사 작업 도중 메모리에서 개체의 크기를 부적절하게 계산할 때 발생합니다.
...
취약점으로 인해 주로 공격 받을 위험이 있는 시스템은 무엇입니까?  
워크스테이션 또는 터미널 서버와 같이 Internet Explorer 8 또는 Internet Explorer 9를 빈번하게 사용하는 시스템은 이 취약점을 이용하는 공격을 받을 위험이 가장 큽니다.

Windows Server 2003, Windows Server 2008 또는 Windows Server 2008 R2에서 Internet Explorer를 실행하고 있습니다. 이 취약점이 완화됩니까?  
예. 기본적으로 Windows Server 2003, Windows Server 2008 및 Windows Server 2008 R2의 Internet Explorer는 [보안 강화 구성](https://technet.microsoft.com/library/dd883248.aspx)으로 알려진 제한된 모드로 실행됩니다. 보안 강화 구성은 미리 구성된 Internet Explorer 설정 그룹으로, 사용자나 관리자가 서버에서 특수하게 조작된 웹 콘텐츠를 다운로드하고 실행할 가능성을 줄여 줍니다. Internet Explorer의 신뢰할 수 있는 사이트 영역에 추가되지 않는 웹 사이트에 대한 완화 요소입니다.

업데이트는 어떤 기능을 수행합니까?  
이 업데이트는 Internet Explorer가 메모리에서 개체를 처리하는 방식을 수정하여 Internet Explorer 9의 취약점을 해결합니다.

참고 이 업데이트는 Internet Explorer 8에 대해서는 이 취약점을 해결하지 않습니다. Internet Explorer 8의 경우 [MS12-056](https://go.microsoft.com/fwlink/?linkid=256487)에 설명된 KB2706045 업데이트를 통해 이 업데이트를 사용할 수 있습니다.

내 시스템에 Internet Explorer 8이 설치되어 있습니다. CVE-2012-2523을 해결하는 업데이트는 어디에서 구할 수 있습니까?  
시스템에 Internet Explorer 8을 설치한 고객은 [MS12-056](https://go.microsoft.com/fwlink/?linkid=256487)에 설명된 KB2706045 업데이트를 설치하여 CVE-2012-2523에 설명된 취약점을 해결할 수 있습니다.

이 업데이트(KB2722913)는 Internet Explorer 9를 실행하는 시스템에 대한 CVE-2012-2523을 해결하므로 해당 시스템에 Internet Explorer 9를 설치한 고객은 KB2706045 업데이트를 설치할 필요가 없습니다.
...
<td style="border:1px solid black;"></td>
<td style="border:1px solid black;">지원 대상인 모든 Windows XP 에디션용 Internet Explorer 7:<br />
제어판의 프로그램 추가/제거 항목 또는 %Windir%\ie7updates\KB2722913-IE7\spuninst 폴더의 Spuninst.exe 유틸리티를 사용하십시오.</td>
</tr>
<tr class="even">
<td style="border:1px solid black;"></td>
<td style="border:1px solid black;">지원 대상인 모든 Windows XP 에디션용 Internet Explorer 8:<br />
제어판의 프로그램 추가/제거 항목 또는 %Windir%\ie8updates\KB2722913-IE8\spuninst 폴더의 Spuninst.exe 유틸리티를 사용하십시오.</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;">파일 정보</td>
<td style="border:1px solid black;"><a href="https://support.microsoft.com/kb/2722913">Microsoft 기술 자료 문서 2722913</a> 참조</td>
</tr>
<tr class="even">
...
<td style="border:1px solid black;">제거 정보</td>
<td style="border:1px solid black;">지원 대상인 모든 Windows Server 2003 32비트 에디션, 64비트 기반 에디션 및 Itanium 기반 에디션용 Internet Explorer 6:<br />
제어판의 프로그램 추가/제거 항목 또는 %Windir%\$NTUninstallKB2722913$\Spuninst 폴더의 Spuninst.exe 유틸리티를 사용하십시오.</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"></td>
<td style="border:1px solid black;">지원 대상인 모든 Windows Server 2003 32비트 에디션, x64 기반 에디션 및 Itanium 기반 에디션용 Internet Explorer 7:<br />
...
-   [VeriSign iDefense Labs](https://labs.idefense.com/)(영문)와 협력하여 레이아웃 메모리 손상 취약점(CVE-2012-1526)을 보고해 주신 GWSlabs  
-   [Beyond Security's SecuriTeam Secure Disclosure](https://www.beyondsecurity.com/ssd.html) (영문)프로그램과 협력하여 비동기 NULL 개체 액세스 원격 코드 실행 취약점(CVE-2012-2521)을 보고해 주신 Derek Soeder  
-   가상 함수 테이블 손상 원격 코드 실행 취약점(CVE-2012-2522)을 보고해 주신 [Trend Micro](https://www.trendmicro.com/)의 Sung-ting Tsai 및 Ming-Chieh Pan  
-   JavaScript 정수 오버플로 원격 코드 실행 취약점(CVE-2012-2523)을 보고해 주신 [Google Chrome Security Team](https://chrome.google.com/)의 Cris Neckar
...
-   V1.0(2012년 8월 15일): 공지가 게시되었습니다.  
-   V1.1(2012년 8월 16일): Windows 8 Release Preview 및 Windows Server 2012 출시 후보 버전에 대해 잘못된 FAQ를 제거했습니다. Windows 8 Release Preview 및 Windows Server 2012 출시 후보 버전은 이 공지에 설명된 취약점의 영향을 받지 않습니다.  
-   V1.2(2013년 7월 18일): 요약의 알려진 문제점에 Microsoft 기술 자료 문서 2722913 링크가 추가되었습니다.
