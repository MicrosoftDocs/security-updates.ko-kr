---
TOCTitle: 'MS10-014'
Title: 'Microsoft 보안 공지 MS10-014 - 중요'
ms:assetid: 'ms10-014'
ms:contentKeyID: 61230274
ms:mtpsurl: 'https://technet.microsoft.com/ko-KR/library/ms10-014(v=Security.10)'
---

Microsoft 보안 공지 MS10-014 - 중요
===================================
...
이 보안 업데이트는 비공개적으로 보고된 Microsoft Windows의 취약점을 해결합니다. 신뢰할 수 있는 비 Windows Kerberos 영역의 인증된 사용자로부터 특수하게 조작된 티켓 갱신 요청이 Windows Kerberos 도메인으로 보내질 경우 이 취약점으로 인해 서비스 거부가 발생할 수 있습니다. 서비스 거부는 도메인 컨트롤러가 다시 시작될 때까지 지속될 수 있습니다.
...
이 업데이트는 Kerberos 서버에서 티켓 갱신 요청을 처리하는 방식을 수정하여 취약점을 해결합니다. 취약점에 대한 자세한 내용은 다음 섹션, **취약점 정보**에서 각 취약점 항목의 자주 제기되는 질문 사항(FAQ)을 참조하십시오.
...
|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------|-------------|---------------------------|
| [Microsoft Windows 2000 Server 서비스 팩 4](https://www.microsoft.com/download/details.aspx?familyid=56a9afba-a6ee-4fb9-ba85-e51d9f94de27&displaylang=ko)                                                      | 서비스 거부    | 중요        | 없음                      |
| [Windows Server 2003 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=738e2fda-96fc-413d-a5c7-74b1fac1d6b3&displaylang=ko)                                                                | 서비스 거부    | 중요        | 없음                      |
| [Windows Server 2003 x64 Edition 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=f8ad539d-8191-4864-977b-ad4e31c04ba0&displaylang=ko)                                                    | 서비스 거부    | 중요        | 없음                      |
| [Windows Server 2003 SP2(Itanium 기반 시스템용)](https://www.microsoft.com/download/details.aspx?familyid=d549c927-add7-4d83-bfc7-15dc35663dfb)                                                                | 서비스 거부    | 중요        | 없음                      |
| [Windows Server 2008(32비트 시스템용) 및 Windows Server 2008(32비트 시스템용) 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=75327470-0f14-4cdd-bcb7-64684c61c267&displaylang=ko)\*     | 서비스 거부    | 중요        | 없음                      |
| [Windows Server 2008(x64 기반 시스템용) 및 Windows Server 2008(x64 기반 시스템용) 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=7d0f8f81-fc10-450a-a653-06f89acba9fa&displaylang=ko)\* | 서비스 거부    | 중요        | 없음                      |
...
업데이트는 영향을 받는 구성 요소가 설치되어 있고 또한 사용하도록 설정되어 있는 시스템에만 제공됩니다. 도메인 컨트롤러 역할을 하도록 구성되지 않은 시스템에는 취약한 구성 요소가 포함되어 있지만 영향을 받지 않으며 자동 업데이트를 통해 이 업데이트가 제공되지 않습니다.
...
<th style="border:1px solid black;" >Kerberos Null 포인터 역참조 취약점(CVE-2010-0035)</th>
...
Kerberos 구현에 서비스 거부 취약점이 존재합니다. 이 취약점은 혼합 모드 Kerberos 구현에서 원격 비 Windows 영역의 클라이언트에 의한 TGT(Ticket-Granting-Ticket) 갱신 요청을 적절하게 처리하지 못해 발생합니다. 취약점 악용에 성공한 공격자는 영향을 받는 Windows 도메인 컨트롤러가 응답을 중지하도록 만들 수 있습니다.
  
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2010-0035 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-0035)를 참조하십시오.
  
#### Kerberos Null 포인터 역참조 취약점(CVE-2010-0035)에 대한 완화 요소
  
완화란 취약점의 악용 심각도를 낮출 수 있는 설정, 일반적인 구성 또는 최선책, 기본 상태를 지칭합니다. 다음과 같은 완화 요소가 현재 상황에서 유용할 수 있습니다.
  
-   공격자가 이 취약점을 악용하려면 Windows 도메인 컨트롤러에서 신뢰하는 영역에 대한 유효한 로그온 자격 증명이 필요합니다. 익명 사용자는 이 취약점을 악용할 수 없습니다.  
-   이 취약점은 도메인 컨트롤러에만 영향을 줍니다. 도메인 컨트롤러의 역할을 수행하지 않는 서버는 영향을 받지 않습니다.
  
#### Kerberos Null 포인터 역참조 취약점(CVE-2010-0035)에 대한 대안
  
이 취약점에 대한 대안을 확인하지 못했습니다.
  
#### Kerberos Null 포인터 역참조 취약점(CVE-2010-0035)에 대한 FAQ
  
**이 취약점을 악용하면 어느 작업까지 가능합니까?**    
이것은 서비스 거부 취약점입니다. 원격의 인증된 공격자는 기존 TGT(Ticket-Granting-Ticket)에 대한 특수하게 조작된 갱신 요청을 보내 영향을 받는 시스템이 응답을 중지하도록 만들 수 있으며, 결국 서비스 거부가 발생할 수 있습니다. 서비스 거부는 도메인 컨트롤러가 다시 시작될 때까지 지속될 수 있습니다.
  
**취약점의 원인은 무엇입니까?**    
이 취약점은 비 Windows Kerberos 도메인에서 오는 티켓 갱신 요청을 잘못 처리하여 발생합니다.
...
**Ticket-Granting-Ticket 또는 TGT란 무엇입니까?**    
Kerberos에서 클라이언트에는 KDC에 의해 임시 키가 지정됩니다. 이 임시 키는 한 번의 특정 로그온 세션에만 유효합니다. 사용자가 로그온하면 클라이언트는 기타 모든 서비스에 대한 티켓을 요청하듯이 KDC에 대한 티켓을 요청합니다. KDC에서는 KDC의 전체 티켓 부여 서비스인 특수 서버에 대한 로그온 세션 키 및 티켓을 생성하여 응답합니다. 하나의 로그온 세션 키 사본이 티켓에 포함되며, 티켓은 KDC의 마스터 키로 암호화됩니다. 또 하나의 로그온 세션 키 사본은 사용자의 로그온 암호에서 파생된 사용자의 마스터 키로 암호화됩니다. 이 티켓과 암호화된 세션 키가 모두 클라이언트로 보내집니다.
  
클라이언트가 KDC의 응답을 받으면 사용자의 암호에서 파생된 사용자의 마스터 키로 로그온 세션 키를 해독합니다. 클라이언트는 이제 로그온 세션 키를 사용하여 KDC에서 받는 모든 서버 세션 키 사본을 해독할 수 있으므로 사용자의 암호에서 파생된 키가 더 이상 필요하지 않습니다. 클라이언트는 KDC의 전체 티켓 부여 서비스에 대한 티켓과 함께 로그온 세션 키를 티켓 캐시에 저장합니다. 전체 티켓 부여 서비스에 대한 티켓을 TGT(ticket-granting ticket)라고 합니다.
  
**공격자는 취약점을 악용하여 무엇을 할 수 있습니까?**    
이 취약점을 악용한 공격자는 영향을 받는 시스템이 응답을 중지하도록 만듭니다. 따라서 기능을 복원하려면 관리자가 시스템을 수동으로 다시 시작해야 합니다. 도메인 컨트롤러를 완전히 사용할 수 없게 하더라도, 이미 Kerberos 티켓을 가진 사용자가 사용하지 못하도록 할 수는 없습니다. 이미 티켓을 부여 받은 경우 모든 리소스에 계속 액세스할 수 있습니다. 그러나 서비스 거부로 인해 도메인 컨트롤러가 다른 리소스에 대한 액세스를 허용하는 어떠한 새 티켓도 발급하지 못하게 됩니다.
  
**공격자는 이러한 취약점을 어떻게 악용합니까?**    
Windows Kerberos 도메인에서 신뢰하는 비 Windows Kerberos 영역에서 인증된 공격자는 특수하게 조작된 티켓 갱신 요청을 Kerberos KDC에 보내어 이 취약점을 악용할 수 있습니다.
  
**취약점으로 인해 주로 공격받을 위험이 있는 시스템은 무엇입니까?**    
혼합 모드 Kerberos 영역이 정의된 도메인 컨트롤러가 이 취약점의 영향을 받습니다. 혼합 모드 Kerberos 영역은 Windows Kerberos 영역에서 Microsoft 제품이 아닌 타사의 Kerberos 구현을 신뢰하는 배포 영역입니다.
  
**업데이트는 어떤 기능을 수행합니까?**    
이 업데이트는 Kerberos 서버에서 티켓 갱신 요청을 처리하는 방식을 수정하여 취약점을 해결합니다.
...
| **배포**                  |                                                                                                                                  |  
| 사용자 개입 없이 설치     | Windows2000-KB977290-x86-kor /quiet                                                                                              |  
| 다시 시작하지 않고 설치   | Windows2000-KB977290-x86-kor /norestart                                                                                          |  
| 로그 파일 업데이트        | kb977290.log                                                                                                                     |  
| 추가 정보                 | **검색, 배포 도구 및 지침**을 참조하십시오.                                                                                      |  
| **다시 시작 요구 사항**   |                                                                                                                                  |  
| 다시 시작 필요 여부       | 예, 이 보안 업데이트를 적용한 후에는 컴퓨터를 다시 시작해야 합니다.                                                              |  
| 핫패칭                    | 해당 사항 없음                                                                                                                   |  
| **제거 정보**             | 제어판의 **프로그램 추가/제거** 도구 또는 %Windir%\\$NTUninstallKB977290$\\Spuninst 폴더의 Spuninst.exe 유틸리티를 사용하십시오. |  
| **파일 정보**             | [Microsoft 기술 자료 문서 977290](https://support.microsoft.com/kb/977290) 참조                                                   |  
| **레지스트리 키 확인**    | HKEY\_LOCAL\_MACHINE\\SOFTWARE\\Microsoft\\Updates\\Windows 2000\\SP5\\KB977290\\Filelist                                        |
...
WindowsServer2003-KB977290-x86-kor /quiet</td>
</tr>
<tr class="even">
<td style="border:1px solid black;"></td>
<td style="border:1px solid black;">지원 대상인 모든 Windows Server 2003 x64 기반 에디션:<br />
WindowsServer2003.WindowsXP-KB977290-x64-kor /quiet</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"></td>
...
<td style="border:1px solid black;">지원 대상인 모든 Windows Server 2003 32비트 에디션:<br />
WindowsServer2003-KB977290-x86-kor /norestart</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"></td>
<td style="border:1px solid black;">지원 대상인 모든 Windows Server 2003 x64 기반 에디션:<br />
WindowsServer2003.WindowsXP-KB977290-x64-kor /norestart</td>
</tr>
<tr class="even">
<td style="border:1px solid black;"></td>
<td style="border:1px solid black;">지원 대상인 모든 Windows Server 2003 Itanium 기반 에디션:<br />
...
<td style="border:1px solid black;">제어판의 <strong>프로그램 추가/제거</strong> 도구 또는 %Windir%\$NTUninstallKB977290$\Spuninst 폴더의 Spuninst.exe 유틸리티를 사용하십시오.</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"><strong>파일 정보</strong></td>
<td style="border:1px solid black;"><a href="https://support.microsoft.com/kb/977290">Microsoft 기술 자료 문서 977290</a> 참조</td>
</tr>
<tr class="even">
