---
TOCTitle: 'MS12-028'
Title: 'Microsoft 보안 공지 MS12-028 - 중요'
ms:assetid: 'ms12-028'
ms:contentKeyID: 61230495
ms:mtpsurl: 'https://technet.microsoft.com/ko-KR/library/ms12-028(v=Security.10)'
---

Microsoft 보안 공지 MS12-028 - 중요
===================================

Microsoft Office의 취약점으로 인한 원격 코드 실행 문제점 (2639185)
------------------------------------------------------------------

게시된 날짜: 2012년 4월 10일 화요일 | 업데이트된 날짜: 2012년 4월 26일 목요일
...
이 보안 업데이트의 심각도는 Microsoft Office 2007 서비스 팩 2, Microsoft Works 9 및 Microsoft Works 6–9 파일 변환기 에디션에 대해 중요입니다. 자세한 내용은 이 섹션에서 영향을 받는 소프트웨어 및 영향을 받지 않는 소프트웨어를 참조하십시오.

이 보안 업데이트는 취약한 Microsoft Works 변환기를 제거하여 취약점을 해결합니다. 고객은 최신 버전의 이 취약점의 영향을 받지 않는 Microsoft Works 변환기를 사용해야 합니다. 업데이트를 설치한 후 Microsoft Works 변환기가 제거되었지만 최신 버전의 Microsoft Works 변환기를 다운로드하지 않은 고객이 Works 파일을 열려고 하면 최신 버전의 Microsoft Works 변환기를 다운로드하는 방법을 지침으로 제공하는 메시지가 나타납니다. 취약점에 대한 자세한 내용은 취약점 정보에서 각 취약점 항목의 자주 제기되는 질문 사항(FAQ)을 참조하십시오.
...
<td style="border:1px solid black;">
[MS09-024](https://technet.microsoft.com/ko-kr/security/bulletin/ms09-024)의 KB969559는 KB2596871로 대체되었습니다.
</td>
</tr>
<tr>
<th style="border:1px solid black;" colspan="4">
...
<td style="border:1px solid black;">
[MS10-105](https://technet.microsoft.com/ko-kr/security/bulletin/ms10-105)의 KB2431831은 KB2680317로 대체되었습니다.
</td>
</tr>
<tr>
<td style="border:1px solid black;">
...
<td style="border:1px solid black;">
[MS09-024](https://technet.microsoft.com/ko-kr/security/bulletin/ms09-024)의 KB968326은 KB2680326으로 대체되었습니다.
</td>
</tr>
</table>
...
Microsoft Office 2007 서비스 팩 3를 설치했는데 영향을 받지 않는데 이 업데이트가 제공되는 이유는 무엇입니까?  
CVE-2012-0177에 설명된 취약점이 Microsoft Office 2007 서비스 팩 3에 영향을 주는 것은 아니지만 업데이트 메커니즘이 업데이트 패키지보다 낮은 버전의 파일을 시스템에서 검색하여 업데이트를 제공합니다.

파일 정보 세부 사항은 어디에 있습니까?  
파일 정보 세부 사항의 위치에 대해서는 보안 업데이트 배포 섹션에서 참조 표를 참조하십시오.

Microsoft Works 6-9 파일 변환기란 무엇입니까?  
Microsoft Works 6-9 파일 변환기는 Microsoft Office 응용 프로그램에서 Works 6, 7, 8 및 9 버전에서 지원되는 Works 워드 프로세서 파일 형식으로 파일 열기, 편집 및 저장이 가능하도록 해주는 선택적 다운로드입니다.
...
Microsoft Office Works 파일 변환기에 원격 코드 실행 취약점이 존재합니다. 이 취약점을 악용한 공격자는 영향을 받는 시스템을 완전히 제어할 수 있습니다. 이렇게 되면 공격자가 프로그램을 설치할 수 있을 뿐 아니라 데이터를 보거나 변경하거나 삭제할 수 있고 새 계정을 만들 수도 있습니다.

이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2012-0177(영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2012-0177)을 참조하십시오.

#### Office WPS 변환기 힙 오버플로 취약점(CVE-2012-0177)에 대한 완화 요소

완화란 취약점의 악용 심각도를 낮출 수 있는 설정, 일반적인 구성 또는 최선책, 기본 상태를 지칭합니다. 다음과 같은 완화 요소가 현재 상황에서 유용할 수 있습니다.

-   웹을 통한 공격의 경우 공격자는 호스팅하는 웹 사이트에 이 취약점을 악용하는 특수하게 조작된 파일을 포함할 수 있습니다. 또한 사용자가 제공한 콘텐츠나 광고를 허용하거나 호스팅하는 공격 당한 웹 사이트에는 이 취약점을 악용할 수 있는 특수하게 조작된 콘텐츠가 포함되어 있을 수 있습니다. 그러나 어떠한 경우에도 공격자는 강제로 사용자가 이러한 웹 사이트를 방문하도록 만들 수 없습니다. 대신 공격자는 사용자가 웹 사이트를 방문하도록 유도해야 하는데, 일반적으로 사용자를 공격자의 웹 사이트로 유인하는 전자 메일 메시지 또는 인스턴트 메신저 메시지에 포함된 링크를 클릭하도록 하는 방법을 사용합니다. 그 후 공격자는 사용자가 특수하게 조작된 파일을 열도록 유도합니다.
...
#### Office WPS 변환기 힙 오버플로 취약점(CVE-2012-0177)에 대한 대안

대안이란 기본적인 취약점을 수정하지는 않지만 업데이트를 적용하기 전에 알려진 공격 경로를 차단하는 데 유용한 설정 또는 구성 변경을 지칭합니다. Microsoft는 다음과 같은 대안을 테스트했으며, 대안으로 인한 기능 저하 여부를 해당 항목에서 설명합니다.

-   신뢰할 수 없는 출처에서 받았거나, 출처는 신뢰할 수 있지만 예상치 못한 Works 파일을 열지 않음

    신뢰할 수 없는 출처에서 받았거나, 출처는 신뢰할 수 있지만 예상치 못한 Works(.wps) 파일을 열지 않습니다. 사용자가 특수하게 조작된 파일을 열 경우 이 취약점이 악용될 수 있습니다.

#### Office WPS 변환기 힙 오버플로 취약점(CVE-2012-0177)에 대한 FAQ

이 취약점을 악용하면 어느 작업까지 가능합니까?  
이 취약점은 원격 코드 실행 취약점입니다.

취약점의 원인은 무엇입니까?  
이 취약점은 Microsoft Works 변환기가 특수하게 조작된 파일의 구문을 분석할 경우 .wps 파일 형식을 적절히 확인하지 못할 때 발생합니다.
...
이 취약점을 악용하려면 사용자가 특수하게 조작된 .wps 파일을 영향을 받는 Microsoft Office 버전으로 열어야 합니다.

전자 메일을 통한 공격의 경우 공격자는 특수하게 조작된 파일을 사용자에게 보내고 파일을 열도록 유도하는 방식으로 이 취약점을 악용할 수 있습니다.

웹을 통한 공격의 경우 공격자는 이 취약점 악용 시도에 사용되는 특수하게 조작된 Works 파일이 포함된 웹 사이트를 호스팅해야 합니다. 또한 사용자가 제공한 콘텐츠를 허용하거나 호스팅하는 공격 당한 웹 사이트에는 이 취약점을 악용할 수 있는 특수하게 조작된 콘텐츠가 포함되어 있을 수 있습니다. 공격자는 강제로 사용자가 이러한 웹 사이트를 방문하도록 만들 수 없습니다. 대신 공격자는 사용자가 공격자의 사이트로 연결되는 링크를 눌러 공격자의 웹 사이트를 방문하도록 유도해야 합니다.

취약점으로 인해 위험에 노출되는 시스템은 주로 무엇입니까?  
영향을 받는 Microsoft Office 또는 Microsoft Works 버전이 설치된 워크스테이션과 터미널 서버가 주로 위험합니다. 관리자가 사용자가 서버에 로그온하여 프로그램을 실행할 수 있도록 허용할 경우, 서버가 더 위험할 수 있습니다. 가장 좋은 방법은 이러한 기능을 허용하지 않는 것입니다.

업데이트는 어떤 기능을 수행합니까?  
이 업데이트는 취약한 Microsoft Works 변환기를 제거하여 취약점을 해결합니다. 고객은 최신 버전의 이 취약점의 영향을 받지 않는 Microsoft Works 변환기를 사용해야 합니다. 업데이트를 설치한 후 Microsoft Works 변환기가 제거되었지만 최신 버전의 Microsoft Works 변환기를 다운로드하지 않은 고객이 Works 파일을 열려고 하면 최신 버전의 Microsoft Works 변환기를 다운로드하는 방법을 지침으로 제공하는 메시지가 나타납니다.
...
| Microsoft Works 6–9 파일 변환기   | 예                  | 예                         |
...
<td style="border:1px solid black;"><a href="https://support.microsoft.com/kb/2596871">Microsoft 기술 자료 문서 2596871</a> 참조</td>
</tr>
<tr class="even">
...
<tr class="even">
<td style="border:1px solid black;"></td>
<td style="border:1px solid black;">Microsoft Works 6–9 파일 변환기:<br />
...
<tr class="odd">
<td style="border:1px solid black;">파일 정보</td>
<td style="border:1px solid black;">Microsoft Works 9의 경우 <a href="https://support.microsoft.com/kb/2680317">Microsoft 기술 자료 문서 2680317</a> 참조</td>
</tr>
<tr class="even">
<td style="border:1px solid black;"></td>
<td style="border:1px solid black;">Microsoft Works 6–9 파일 변환기의 경우 <a href="https://support.microsoft.com/kb/2680326">Microsoft Knowledge Base Article 2680326</a> 참조</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;">레지스트리 키 확인</td>
<td style="border:1px solid black;">해당 사항 없음</td>
...
-   Office WPS 변환기 힙 오버플로 취약점(CVE-2012-0177)을 보고해 주신 [IOActive, Ltd, (영문)](https://ioactive.co.uk/)의 Shaun Colley
...
-   V1.1(2012년 4월 26일): Microsoft Office 2007 서비스 팩 3를 실행하는 고객에게 이 업데이트를 제공하는 이유를 설명하기 위해 업데이트 FAQ에 항목이 추가되었습니다.
