...
|----------------------------------------------------------------------------------------------------------------|----------------------|------------------------|------------------------|
| PGM 코드 실행 취약점 - [CVE-2006-3442 (영문)](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2006-3442) | 원격 코드 실행       | 중요                   | 중요                   |
...
PGM은 안정적이고 확장 가능한 멀티캐스트 프로토콜로서 수신기가 손실을 탐지하여 손실된 데이터의 재전송을 요청하거나 응용 프로그램에 복구 불가능한 손실을 통보하도록 지원합니다. PGM은 안정적인 수신을 위한 프로토콜입니다. 즉, 수신기가 모든 데이터를 수신하도록 보장함으로써 보낸 사람의 수신 책임을 면해줍니다. PGM은 여러 출처에서 여러 수신기로 중복이 없는 멀티캐스트 데이터를 전달해야 하는 응용 프로그램에 적합합니다. PGM는 배달 확인을 지원하지 않으며, 여러 보낸 사람으로부터의 패킷 주문을 보장하지도 않습니다. PGM에 대한 자세한 내용은 [MSDN 문서 (영문)](http://msdn.microsoft.com/library/default.asp?url=/library/en-us/winsock/winsock/pgm_senders_and_receivers.asp)를 참조하십시오.

**MSMQ와 MSMQ 서비스란 무엇입니까?**  
MSMQ(Microsoft Message Queuing) 서비스는 다른 시간대에 실행 중인 응용 프로그램이 일시적으로 오프라인 상태일 수 있는 이기종 네트워크 및 시스템과 통신할 수 있는 기능입니다. PGM에 대한 자세한 내용은 [MSDN 문서 (영문)](http://msdn.microsoft.com/library/default.asp?url=/library/en-us/msmq/html/ff917e87-05d5-478f-9430-0f560675ece1.asp)를 참조하십시오.
