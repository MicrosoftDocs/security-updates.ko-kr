---
TOCTitle: 'MS10-007'
Title: 'Microsoft 보안 공지 MS10-007 - 긴급'
ms:assetid: 'ms10-007'
ms:contentKeyID: 61230267
ms:mtpsurl: 'https://technet.microsoft.com/ko-KR/library/ms10-007(v=Security.10)'
---

Microsoft 보안 공지 MS10-007 - 긴급
===================================
...
이 보안 업데이트는 ShellExecute API가 입력 매개 변수를 검사하는 방식을 수정하여 취약점을 해결합니다. 취약점에 대한 자세한 내용은 다음 섹션, **취약점 정보**에서 각 취약점 항목의 자주 제기되는 질문 사항(FAQ)을 참조하십시오.
...
|------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------|-------------|---------------------------|
| [Microsoft Windows 2000 서비스 팩 4](https://www.microsoft.com/download/details.aspx?familyid=92234237-a8eb-4ce4-bc5e-cd86feb7dbd3&displaylang=ko)               | 원격 코드 실행 | 긴급        | 없음                      |
| [Windows XP 서비스 팩 2 및 Windows XP 서비스 팩 3](https://www.microsoft.com/download/details.aspx?familyid=b8e7bf17-a037-4200-9ae2-2280b19766a4&displaylang=ko) | 원격 코드 실행 | 긴급        | 없음                      |
| [Windows XP Professional x64 Edition 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=b8d83f30-9cd7-4d6b-b2b9-65d0a483cb9c)                 | 원격 코드 실행 | 긴급        | 없음                      |
| [Windows Server 2003 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=5cb2e203-18fb-4887-a1c9-289d86b8ba11&displaylang=ko)                  | 원격 코드 실행 | 긴급        | 없음                      |
| [Windows Server 2003 x64 Edition 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=90360537-9311-45e2-8047-9a971f90c3c3&displaylang=ko)      | 원격 코드 실행 | 긴급        | 없음                      |
| [Windows Server 2003 SP2(Itanium 기반 시스템용)](https://www.microsoft.com/download/details.aspx?familyid=d695ca9f-a1db-4c0f-94b6-7112861c28da)                  | 원격 코드 실행 | 긴급        | 없음                      |
...
**이 보안 업데이트는 MS10-002에서 설명한 CVE-2010-0027과 어떤 관련이 있습니까?**  
URL 유효성 검사 취약점(CVE-2010-0027)은 사용자 시스템에 설치된 Microsoft Windows 버전 및 Internet Explorer 버전에 따라 이 업데이트(MS10-007)를 적용하거나 [Internet Explorer 누적 보안 업데이트(MS10-002)](https://technet.microsoft.com/security/bulletin/ms10-002)를 적용하여 해결합니다.
...
<th style="border:1px solid black;" >URL 유효성 검사 취약점(CVE-2010-0027)</th>
...
영향을 받는 Microsoft Windows 버전에 원격 코드 실행 취약점이 있습니다. 이 취약점은 ShellExecute API 기능으로 전달된 입력의 부적절한 유효성 검사로 인해 발생합니다. 이 취약점을 악용한 공격자는 영향을 받는 시스템을 완전히 제어할 수 있습니다.
...
이 취약점은 원격 코드 실행 취약점입니다. 이 취약점 악용에 성공한 공격자는 현재 로그인한 사용자와 동일한 권한으로 작업을 수행할 수 있습니다.
  
**취약점의 원인은 무엇입니까?**    
웹 브라우저 같은 응용 프로그램에서 ShellExecute API 기능을 사용하여 특수하게 조작된 데이터를 처리하는 경우, ShellExecute가 해당 데이터 스트림의 유효성을 잘못 검사할 수 있으며 이로 인해 로컬 클라이언트 시스템에서 바이너리가 실행될 수 있습니다.
...
**ShellExecute란 무엇입니까?**    
ShellExecute는 Windows Shell API(응용 프로그래밍 인터페이스) 기능의 일부로 특정 파일에서 작업을 수행합니다. 예를 들어, 이를 통해 특정 파일 유형에 대한 정확한 처리기가 실행될 수 있습니다. ShellExecute 기능에 대한 자세한 내용은 MSDN 문서 [ShellExecute 기능 (영문)](https://msdn.microsoft.com/en-us/library/bb762153(vs.85).aspx)을 참조하십시오.
  
**API(응용 프로그래밍 인터페이스)란 무엇입니까?**    
API는 구성 요소, 응용 프로그램 또는 운영 체제와 함께 사용할 수 있는 기능의 집합입니다. 일반적으로 API는 몇몇 특정 기능을 제공하는 하나 이상의 DLL로 구성되어 있습니다.
  
**공격자는 취약점을 악용하여 무엇을 할 수 있습니까?**    
이 취약점을 악용에 성공한 공격자는 로그온한 사용자 자격으로 임의 코드를 실행할 수 있습니다. 사용자가 관리자 권한의 사용자 권한으로 로그온한 경우 공격자가 영향을 받는 시스템을 완전히 제어할 수 있습니다. 이렇게 되면 공격자가 프로그램을 설치할 수 있을 뿐 아니라 데이터를 보거나 변경하거나 삭제할 수 있고 모든 사용자 권한이 있는 새 계정을 만들 수도 있습니다. 시스템에 대한 사용자 권한이 적게 구성된 계정의 사용자는 관리자 권한으로 작업하는 사용자에 비해 영향을 적게 받습니다.
  
**공격자는 이러한 취약점을 어떻게 악용합니까?**    
공격자는 응용 프로그램이 조작된 문자열을 구문 분석하도록 특수하게 조작된 웹 페이지 등의 콘텐츠를 제공할 수 있습니다. 이 콘텐츠가 하이퍼링크와 같은 ShellExecute API 호출로 전달될 경우 공격자는 로그온한 사용자가 모르게 클라이언트 시스템에서 바이너리를 실행할 수 있습니다.
  
**취약점으로 인해 주로 공격받을 위험이 있는 시스템은 무엇입니까?**    
주로 신뢰할 수 없는 인터넷 콘텐츠를 탐색하는 클라이언트 시스템이 취약점으로 인한 위험에 노출되어 로그온한 사용자의 컨텍스트에서 악용될 수 있습니다.
  
**업데이트는 어떤 기능을 수행합니까?**    
이 업데이트는 지원 대상인 Microsoft Windows 2000, Windows XP 및 Windows Server 2003 에디션에서 ShellExecute API가 입력 매개 변수를 검사하는 방식을 수정하여 취약점을 해결합니다.
...
| **배포**                  |                                                                                                                                  |  
| 사용자 개입 없이 설치     | Windows2000-KB975713-x86-kor /quiet                                                                                              |  
| 다시 시작하지 않고 설치   | Windows2000-KB975713-x86-kor /norestart                                                                                          |  
| 로그 파일 업데이트        | kb975713.log                                                                                                                     |  
| 추가 정보                 | **검색, 배포 도구 및 지침**을 참조하십시오.                                                                                      |  
| **다시 시작 요구 사항**   |                                                                                                                                  |  
| 다시 시작 필요 여부       | 예, 이 보안 업데이트를 적용한 후에는 컴퓨터를 다시 시작해야 합니다.                                                              |  
| 핫패칭                    | 해당 사항 없음                                                                                                                   |  
| **제거 정보**             | 제어판의 **프로그램 추가/제거** 도구 또는 %Windir%\\$NTUninstallKB975713$\\Spuninst 폴더의 Spuninst.exe 유틸리티를 사용하십시오. |  
| **파일 정보**             | [Microsoft 기술 자료 문서 975713](https://support.microsoft.com/kb/975713) 참조                                                   |  
| **레지스트리 키 확인**    | HKEY\_LOCAL\_MACHINE\\SOFTWARE\\Microsoft\\Updates\\Windows 2000\\SP5\\KB975713\\Filelist                                        |
...
WindowsXP-KB975713-x86-kor /quiet</td>
</tr>
<tr class="even">
<td style="border:1px solid black;"></td>
...
<td style="border:1px solid black;">Windows XP 서비스 팩 2 및 Windows XP 서비스 팩 3:<br />
WindowsXP-KB975713-x86-kor /norestart</td>
</tr>
<tr class="even">
<td style="border:1px solid black;"></td>
<td style="border:1px solid black;">Windows XP Professional x64 Edition 서비스 팩 2:<br />
...
<td style="border:1px solid black;">제어판의 <strong>프로그램 추가/제거</strong> 도구 또는 %Windir%\$NTUninstallKB975713$\Spuninst 폴더의 Spuninst.exe 유틸리티를 사용하십시오.</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"><strong>파일 정보</strong></td>
<td style="border:1px solid black;"><a href="https://support.microsoft.com/kb/975713">Microsoft 기술 자료 문서 975713</a> 참조</td>
</tr>
<tr class="even">
...
WindowsServer2003-KB975713-x86-kor /quiet</td>
</tr>
<tr class="even">
<td style="border:1px solid black;"></td>
<td style="border:1px solid black;">지원 대상인 모든 Windows Server 2003 x64 기반 에디션:<br />
WindowsServer2003.WindowsXP-KB975713-x64-kor /quiet</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"></td>
...
<td style="border:1px solid black;">다시 시작하지 않고 설치</td>
<td style="border:1px solid black;">지원 대상인 모든 Windows Server 2003 32비트 에디션:<br />
WindowsServer2003-KB975713-x86-kor /norestart</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"></td>
<td style="border:1px solid black;">지원 대상인 모든 Windows Server 2003 x64 기반 에디션:<br />
WindowsServer2003.WindowsXP-KB975713-x64-kor /norestart</td>
</tr>
<tr class="even">
<td style="border:1px solid black;"></td>
<td style="border:1px solid black;">지원 대상인 모든 Windows Server 2003 Itanium 기반 에디션:<br />
...
-   [TippingPoint (영문)](https://www.tippingpoint.com/) 및 [Zero Day Initiative (영문)](https://www.zerodayinitiative.com/)와 협력하여 URL 유효성 검사 취약점(CVE-2010-0027)을 보고해 주신 Brett Moore
-   URL 유효성 검사 취약점(CVE-2010-0027)을 보고해 주신 [Lostmon Lords (영문)](https://lostmon.blogspot.com/)
