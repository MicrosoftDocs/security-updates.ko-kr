...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2011-1868 (영문)](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-1868)을 참조하십시오.
...
분산 파일 시스템(DFS)을 통해 관리자는 서로 다른 서버에 있는 공유 폴더를 하나 이상의 DFS 네임스페이스로 투명하게 연결하여 그룹화할 수 있습니다. DFS 네임스페이스는 조직의 공유 폴더에 대한 가상 보기입니다. DFS 도구를 사용하여 관리자는 네임스페이스에 표시할 공유 폴더를 선택하고, 이러한 폴더가 나타나는 계층을 설계하며, 공유 폴더가 네임스페이스에 나타나는 이름을 결정할 수 있습니다. 사용자가 네임스페이스를 볼 때 폴더는 단일 대용량 하드 디스크에 있는 것으로 나타납니다. 사용자는 데이터를 호스팅하는 서버 이름이나 공유 폴더를 알 필요 없이 네임스페이스에서 폴더를 탐색할 수 있습니다. 자세한 내용은 [DFS 기술 참조 (영문)](http://technet.microsoft.com/en-us/library/cc757042(ws.10).aspx)를 참조하십시오.
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2011-1869 (영문)](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-1869)를 참조하십시오.
...
<td style="border:1px solid black;"><a href="http://support.microsoft.com/kb/2535512">Microsoft 기술 자료 문서 2535512</a> 참조</td>
</tr>
<tr class="even">
...
-   DFS 참조 응답 취약점(CVE-2011-1869)을 보고해 주신 [NGS Software (영문)](http://www.ngssoftware.com/)의 Laurent Gaffié
