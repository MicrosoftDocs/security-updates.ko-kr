---
TOCTitle: 'MS09-009'
Title: 'Microsoft 보안 공지 MS09-009 - 긴급'
ms:assetid: 'ms09-009'
ms:contentKeyID: 61230195
ms:mtpsurl: 'https://technet.microsoft.com/ko-KR/library/ms09-009(v=Security.10)'
---


Microsoft 보안 공지 MS09-009 - 긴급
===================================

Microsoft Office Excel의 취약점으로 인한 원격 코드 실행 문제점(968557)
----------------------------------------------------------------------

게시된 날짜: 2009년 4월 15일 수요일 | 업데이트된 날짜: 2009년 4월 23일 목요일
...
이 보안 업데이트는 [Microsoft 보안 권고 968272](https://technet.microsoft.com/security/advisory/968272)에서 처음 설명한 취약점도 해결합니다.
...
<tr>
<td style="border:1px solid black;">
[Word, Excel, PowerPoint 2007 파일 형식 서비스 팩 1용 Microsoft Office 호환 기능 팩](https://www.microsoft.com/download/details.aspx?familyid=05f7c517-e551-4dcd-b24a-5d548f2d09cf&displaylang=ko)  
(KB960003)
</td>
<td style="border:1px solid black;">
해당 사항 없음
...
\*Microsoft Office Excel 2007 서비스 팩 1을 사용하는 고객의 경우 이 보안 공지에서 설명한 취약점으로부터 보호하기 위해 보안 업데이트 패키지 KB959997 이외에도 [Word, Excel, PowerPoint 2007 파일 형식용 Microsoft Office 호환 기능 팩 서비스 팩 1용 보안 업데이트(KB958439)](https://www.microsoft.com/download/details.aspx?familyid=05f7c517-e551-4dcd-b24a-5d548f2d09cf)를 설치해야 합니다.
...
파일 정보 세부 사항은 [Microsoft 기술 자료 문서 968557](https://support.microsoft.com/kb/968557)에 나와 있습니다.
...
**이 업데이트의 심각도가 Excel 2000에 대해서만 긴급이고, 영향을 받는 다른 모든 버전의 Excel에 대해서는 중요인 이유가 무엇입니까?**  
...
<th style="border:1px solid black;" >
메모리 손상 취약점(CVE-2009-0100)
</th>
<th style="border:1px solid black;" >
메모리 손상 취약점(CVE-2009-0238)
</th>
<th style="border:1px solid black;" >
전체 심각도
</th>
...
사용자가 조작된 개체가 포함된 특수하게 조작된 Excel 파일을 열 때 Microsoft Office Excel의 취약점으로 인해 원격 코드 실행을 허용할 수 있습니다. 이 취약점을 악용한 공격자는 영향을 받는 시스템을 완전히 제어할 수 있습니다. 이렇게 되면 공격자가 프로그램을 설치할 수 있을 뿐 아니라 데이터를 보거나 변경하거나 삭제할 수 있고 모든 사용자 권한이 있는 새 계정을 만들 수도 있습니다.

이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2009-0100 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2009-0100)을 참조하십시오.

#### 메모리 손상 취약점(CVE-2009-0100)에 대한 완화 요소
...
#### 메모리 손상 취약점(CVE-2009-0100)에 대한 대안
...
#### 메모리 손상 취약점(CVE-2009-0100)에 대한 FAQ
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2009-0238 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2009-0238)을 참조하십시오.

#### 메모리 손상 취약점(CVE-2009-0238)에 대한 완화 요소
...
#### 메모리 손상 취약점(CVE-2009-0238)에 대한 대안
...
#### 메모리 손상 취약점(CVE-2009-0238)에 대한 FAQ

**이 취약점을 악용하면 어느 작업까지 가능합니까?**  
이 취약점은 원격 코드 실행 취약점입니다. 이 취약점을 악용한 공격자는 영향을 받는 시스템을 원격으로 완전히 제어할 수 있습니다. 이렇게 되면 공격자가 프로그램을 설치할 수 있을 뿐 아니라 데이터를 보거나 변경하거나 삭제할 수 있고 모든 사용자 권한이 있는 새 계정을 만들 수도 있습니다.

**취약점의 원인은 무엇입니까?**  
특수하게 조작된 Excel 스프레드시트를 열 때 Microsoft Office Excel의 취약점으로 인해 원격 코드 실행을 허용할 수 있습니다. Microsoft Office Excel이 Excel 스프레드시트 형식을 구문 분석하는 방식에 취약점이 존재합니다.
...
**업데이트는 어떤 기능을 수행합니까?**  
이 업데이트는 특수하게 조작된 Excel 파일을 열 때 Microsoft Excel이 처리하는 방식을 수정하여 취약점을 제거합니다.

**이 보안 공지를 게시했을 때 이 취약점이 공개되어 있었습니까?**  
예. 이 취약점은 공개되었습니다. 이 취약점에는 CVE(Common Vulnerability and Exposure) 번호 [CVE-2009-0238 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2009-0238)이 지정되었습니다. 이 취약점은 [Microsoft 보안 권고 968272](https://technet.microsoft.com/security/advisory/968272)에서 처음 설명되었습니다.
...
<td style="border:1px solid black;">office2000-kb959964-fullfile-kor /q:a</td>
</tr>
<tr class="even">
<td style="border:1px solid black;">다시 시작하지 않고 설치</td>
<td style="border:1px solid black;">office2000-kb959964-fullfile-kor /r:n</td>
</tr>
<tr class="odd">
...
<td style="border:1px solid black;"><a href="https://support.microsoft.com/kb/968557">Microsoft 기술 자료 문서 968557</a> 참조</td>
</tr>
<tr class="even">
...
<tr class="odd">
<td style="border:1px solid black;">사용자 개입 없이 설치</td>
<td style="border:1px solid black;">officeXP-kb959988-fullfile-kor /q:a</td>
</tr>
<tr class="even">
<td style="border:1px solid black;">다시 시작하지 않고 설치</td>
<td style="border:1px solid black;">officeXP-kb959988-fullfile-kor /r:n</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;">로그 파일 업데이트</td>
<td style="border:1px solid black;">해당 사항 없음</td>
...
이 보안 업데이트를 제거하려면 제어판의 **프로그램 추가/제거** 도구를 사용하십시오. **참고** 이 업데이트를 제거할 때 Microsoft Office XP CD를 CD 드라이브에 넣으라는 메시지가 표시될 수 있습니다. 제어판의 **프로그램 추가/제거**에서 업데이트를 제거하는 옵션이 표시되지 않을 수 있습니다. 이 문제의 가능한 원인은 여러 가지입니다. 제거에 대한 자세한 내용은 [Microsoft 기술 자료 문서 903771](https://support.microsoft.com/kb/903771)을 참조하십시오.
...
office2003-kb959995-fullfile-kor /q:a<br />
<br />
Excel Viewer 2003:<br />
office2003-kb959993-fullfile-kor /q:a</td>
</tr>
<tr class="even">
<td style="border:1px solid black;">다시 시작하지 않고 설치</td>
<td style="border:1px solid black;">Excel 2003:<br />
office2003-kb959995-fullfile-kor /r:n<br />
<br />
Excel Viewer 2003:<br />
office2003-kb959993-fullfile-kor /r:n</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;">로그 파일 업데이트</td>
<td style="border:1px solid black;">해당 사항 없음</td>
...
<td style="border:1px solid black;"><strong>제거 정보</strong></td>
<td style="border:1px solid black;">제어판의 <strong>프로그램 추가/제거 도구</strong>를 사용하십시오.
<strong>참고</strong> 이 업데이트를 제거할 때 Microsoft Office 2003 CD를 CD 드라이브에 넣으라는 메시지가 표시될 수 있습니다. 제어판의 <strong>프로그램 추가/제거</strong>에서 업데이트를 제거하는 옵션이 표시되지 않을 수 있습니다. 이 문제의 가능한 원인은 여러 가지입니다. 제거에 대한 자세한 내용은 <a href="https://support.microsoft.com/kb/903771">Microsoft 기술 자료 문서 903771</a>을 참조하십시오.</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"><strong>파일 정보</strong></td>
<td style="border:1px solid black;"><a href="https://support.microsoft.com/kb/968557">Microsoft 기술 자료 문서 968557</a> 참조</td>
...
**참고** 이 업데이트를 제거할 때 Microsoft Office 2003 CD를 CD 드라이브에 넣으라는 메시지가 표시될 수 있습니다. 제어판의 **프로그램 추가/제거**에서 업데이트를 제거하는 옵션이 표시되지 않을 수 있습니다. 이 문제의 가능한 원인은 여러 가지입니다. 제거에 대한 자세한 내용은 [Microsoft 기술 자료 문서 903771](https://support.microsoft.com/kb/903771)을 참조하십시오.
...
<br />
Word, Excel, PowerPoint 2007 파일 형식용 Microsoft Office 호환 기능 팩 서비스 팩 1:<br />
...
Microsoft Office 2004 for Mac 11.5.4 업데이트의 해당 언어 버전을 [Microsoft 다운로드 센터](https://www.microsoft.com/download/details.aspx?familyid=52271140-89be-4b9c-baa2-cea09097d703)에서 다운로드하여 설치하십시오.
...
-   바탕 화면에서 Microsoft Office 2004 for Mac 11.5.4 업데이트 볼륨을 엽니다. 이 단계가 자동으로 수행되었을 수도 있습니다.  
-   업데이트 프로세스를 시작하려면 Microsoft Office 2004 for Mac 11.5.4 업데이트 볼륨 창에서 Microsoft Office 2004 for Mac 11.5.4 업데이트 응용 프로그램을 두 번 클릭하고 화면에 나타난 지침을 따릅니다.  
-   설치가 성공적으로 완료되면 하드 디스크에서 업데이트 설치 프로그램을 제거할 수 있습니다. 설치가 성공적으로 완료되었는지 확인하려면 다음 "업데이트 설치 확인"을 참조하십시오. 업데이트 설치 프로그램을 제거하려면 먼저 Microsoft Office 2004 for Mac 11.5.4 업데이트 볼륨을 휴지통에 끌어 놓은 다음 다운로드한 파일을 휴지통에 끌어 놓습니다.
...
버전 번호가 **11.5.4**면 업데이트가 설치된 것입니다.
...
Microsoft Office 2004 for Mac 12.1.7 업데이트의 해당 언어 버전을 [Microsoft 다운로드 센터](https://www.microsoft.com/download/details.aspx?familyid=f6e407eb-11a5-433f-8006-4b822953ca98)에서 다운로드하여 설치하십시오.
  
-   설치를 방해할 수 있으므로 바이러스 예방 응용 프로그램, 모든 Microsoft Office 응용 프로그램, Microsoft Messenger for Mac, Office 알림을 포함하여 실행 중인 응용 프로그램을 모두 종료합니다.  
-   바탕 화면에서 Microsoft Office 2008 for Mac 12.1.7 업데이트 볼륨을 엽니다. 이 단계가 자동으로 수행되었을 수도 있습니다.  
-   업데이트 과정을 시작하려면 Microsoft Office 2008 for Mac 12.1.7 업데이트 볼륨 창에서 Microsoft Office 2008 for Mac 12.1.7 업데이트 응용 프로그램을 두 번 클릭하고 화면의 지침을 따릅니다.  
-   설치가 성공적으로 완료되면 하드 디스크에서 업데이트 설치 프로그램을 제거할 수 있습니다. 설치가 성공적으로 완료되었는지 확인하려면 다음 "업데이트 설치 확인"을 참조하십시오. 업데이트 설치 프로그램을 제거하려면 먼저 Microsoft Office 2008 for Mac 12.1.7 업데이트 볼륨을 휴지통에 끌어 놓은 다음 다운로드한 파일을 휴지통에 끌어 놓습니다.
...
버전 번호가 **12.1.7**이면 업데이트가 설치된 것입니다.
...
-   메모리 손상 취약점(CVE-2009-0100)을 보고해 주신 Fortinet, [FortiGuard Global Security Research Team (영문)](https://www.fortiguardcenter.com/)의 Haifei Li
...
-   V1.1(2009년 4월 23일): 검색, 배포 도구 및 지침 섹션의 MBSA 및 SMS 표에 Excel Viewer 2003 서비스 팩 3을 추가했습니다. 이 변경 사항은 정보에만 해당됩니다. 보안 공지 바이너리나 검색 로직은 변경되지 않았습니다.
