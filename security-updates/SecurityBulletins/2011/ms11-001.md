---
TOCTitle: 'MS11-001'
Title: 'Microsoft 보안 공지 MS11-001 - 중요'
ms:assetid: 'ms11-001'
ms:contentKeyID: 61230367
ms:mtpsurl: 'https://technet.microsoft.com/ko-KR/library/ms11-001(v=Security.10)'
---

Microsoft 보안 공지 MS11-001 - 중요
===================================

Windows 백업 관리자의 취약점으로 인한 원격 코드 실행 문제점(2478935)
--------------------------------------------------------------------

게시된 날짜: 2011년 1월 12일 수요일
...
이 보안 업데이트의 심각도는 지원 대상인 모든 Windows Vista 에디션에 대해 중요입니다. 자세한 내용은 이 섹션에서 **영향을 받는 소프트웨어 및 영향을 받지 않는 소프트웨어**를 참조하십시오.

보안 업데이트는 Windows 백업 관리자에서 외부 라이브러리를 로드하는 방식을 수정하여 취약점을 해결합니다. 취약점에 대한 자세한 내용은 다음 섹션, **취약점 정보**에서 각 취약점 항목의 자주 제기되는 질문 사항(FAQ)을 참조하십시오.
...
|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------|-------------|---------------------------|
| [Windows Vista 서비스 팩 1 및 Windows Vista 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=4eeaad20-729c-4594-8853-e4ae55e9d491&displaylang=ko)                         | 원격 코드 실행 | 중요        | 없음                      |
| [Windows Vista x64 Edition 서비스 팩 1 및 Windows Vista x64 Edition 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=b2839cd0-1958-4a22-9aac-0e0aa8f2b52c&displaylang=ko) | 원격 코드 실행 | 중요        | 없음                      |
...
<th style="border:1px solid black;" >백업 관리자의 안전하지 않은 라이브러리 로드 취약점(CVE-2010-3145)</th>
...
백업 관리자의 안전하지 않은 라이브러리 로드 취약점(CVE-2010-3145)  
-----------------------------------------------------------------
  

Microsoft Windows 백업 관리자에서 DLL 파일 로드를 처리하는 방식에 원격 코드 실행 취약점이 존재합니다. 이 취약점을 악용한 공격자는 영향을 받는 시스템을 완전히 제어할 수 있습니다. 이렇게 되면 공격자가 프로그램을 설치할 수 있을 뿐 아니라 데이터를 보거나 변경하거나 삭제할 수 있고 모든 사용자 권한이 있는 새 계정을 만들 수도 있습니다. 시스템에 대한 사용자 권한이 적게 구성된 계정의 사용자는 관리자 권한으로 작업하는 사용자에 비해 영향을 적게 받습니다.
  
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2010-3145 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-3145)를 참조하십시오.
  
#### 백업 관리자의 안전하지 않은 라이브러리 로드 취약점(CVE-2010-3145)에 대한 완화 요소
  
완화란 취약점의 악용 심각도를 낮출 수 있는 설정, 일반적인 구성 또는 최선책, 기본 상태를 지칭합니다. 다음과 같은 완화 요소가 현재 상황에서 유용할 수 있습니다.
  
-   공격이 성공하려면 사용자가 신뢰할 수 없는 원격 파일 시스템 위치 또는 WebDAV 공유를 방문한 후 Windows 백업 카탈로그(.wbcat) 파일을 열어야 합니다.  
-   파일 공유 프로토콜인 SMB(Server Message Block)는 경계 방화벽에서 자주 비활성화됩니다. 이것이 이 취약점의 가능한 공격 경로를 제한합니다.
  
#### 백업 관리자의 안전하지 않은 라이브러리 로드 취약점(CVE-2010-3145)에 대한 대안
...
#### 백업 관리자의 안전하지 않은 라이브러리 로드 취약점(CVE-2010-3145)에 대한 FAQ
...
**취약점의 원인은 무엇입니까?**  
이 취약점은 Windows 백업 관리자가 외부 라이브러리 로드에 사용되는 경로를 잘못 제한하기 때문에 발생합니다.
  
**Windows 백업 관리자란 무엇입니까?**  
Windows 백업 관리자(sdclt.exe)를 통해 사용자는 컴퓨터 시스템 파일을 이전 시점으로 복원할 수 있습니다. 시스템 복원은 시스템 보호라는 기능을 사용하여 정기적으로 컴퓨터에서 복원 지점을 만들고 저장합니다. 이러한 복원 지점에는 레지스트리 설정에 대한 정보 및 Windows에서 사용하는 기타 시스템 정보가 포함됩니다. 이러한 복원 지점은 수동으로 만들 수도 있습니다.
...
이 취약점을 악용하려면 사용자가 특수하게 조작된 DLL(동적 연결 라이브러리) 파일과 동일한 디렉터리에 있는 합법적인 .wbcat 파일을 열어야 합니다. .wbcat 파일이 열리면 Windows 백업 관리자 응용 프로그램이 특수하게 조작된 DLL 파일 로드 및 포함된 코드 실행을 시도할 수 있습니다.
  
네트워크 공격 시나리오에서 공격자는 합법적인 .wbcat 파일 및 특수하게 조작된 DLL을 UNC 또는 WebDAV 위치와 같은 네트워크 공유에 두고 사용자가 .wbcat 파일을 열도록 유도할 수 있습니다. 사용자가 .wbcat 파일을 열면 .wbcat 파일과 같은 디렉터리에 포함되어 있는 특수하게 조작된 DLL 파일이 로드될 수 있습니다. 공격자가 강제로 사용자가 네트워크 공유에서 .wbcat 파일을 열도록 할 수는 없습니다. 대신 공격자는 사용자가 파일을 열도록 유도해야 하는데, 일반적으로 사용자를 네트워크 공유로 유인하는 전자 메일 메시지 또는 인스턴트 메신저 메시지에 포함된 링크를 클릭하도록 하는 방법을 사용합니다. 그 후 공격자는 사용자가 .wbcat 파일을 열도록 유도합니다.
  
전자 메일을 통한 공격의 경우 공격자는 합법적인 .wbcat 첨부 파일을 사용자에게 전송하고 사용자가 특수하게 조작된 DLL 파일이 들어 있는 디렉터리에 해당 첨부 파일을 추가한 후 .wbcat 파일을 열도록 유도하여 이 취약점을 악용할 수 있습니다. 사용자가 .wbcat 파일을 열면 .wbcat 파일과 같은 디렉터리에 포함되어 있는 특수하게 조작된 DLL 파일이 로드될 수 있습니다. 사용자가 메일 메시지에 포함된 링크를 클릭하면 네트워크 공격 시나리오를 통한 이 취약점 악용에 취약해질 수 있습니다.
...
**업데이트는 어떤 기능을 수행합니까?**  
이 업데이트는 Windows 백업 관리자가 외부 라이브러리를 로드하는 방식을 수정하여 이 취약점을 해결합니다.
  
**이 보안 공지를 게시했을 때 이 취약점이 공개되어 있었습니까?**  
예. 이 취약점은 공개되었습니다. 이 취약점에는 CVE(Common Vulnerability and Exposure) 번호 [CVE-2010-3145 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-3145)가 지정되었습니다.
...
<td style="border:1px solid black;"><a href="https://support.microsoft.com/kb/2478935">Microsoft 기술 자료 문서 2478935</a> 참조</td>
</tr>
<tr class="odd">
