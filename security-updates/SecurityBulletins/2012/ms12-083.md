---
TOCTitle: 'MS12-083'
Title: 'Microsoft 보안 공지 MS12-083 - 중요'
ms:assetid: 'ms12-083'
ms:contentKeyID: 61230550
ms:mtpsurl: 'https://technet.microsoft.com/ko-KR/library/ms12-083(v=Security.10)'
---

Microsoft 보안 공지 MS12-083 - 중요
===================================
...
보안 업데이트는 Microsoft Windows에서 인증서의 유효성을 확인하는 방식을 수정하여 취약점을 해결합니다. 취약점에 대한 자세한 내용은 취약점 정보에서 각 취약점 항목의 자주 제기되는 질문 사항(FAQ)을 참조하십시오.
...
[Windows Server 2008 R2(x64 기반 시스템용)](https://www.microsoft.com/download/details.aspx?familyid=d3fb096b-87b5-4715-a093-9ec9b021a40f&displaylang=ko)  
(KB2765809)
</td>
<td style="border:1px solid black;">
보안 기능 우회
...
<td style="border:1px solid black;">
[Windows Server 2008 R2(x64 기반 시스템용) 서비스 팩 1](https://www.microsoft.com/download/details.aspx?familyid=d3fb096b-87b5-4715-a093-9ec9b021a40f&displaylang=ko)  
(KB2765809)
</td>
<td style="border:1px solid black;">
보안 기능 우회
...
<tr>
<td style="border:1px solid black;">
[Windows Server 2012](https://www.microsoft.com/download/details.aspx?familyid=d6654bf1-1ab9-4691-8729-793eb6d0e563&displaylang=ko)  
(KB2765809)
</td>
<td style="border:1px solid black;">
보안 기능 우회
...
<tr class="alternateRow">
<td style="border:1px solid black;">
[Windows Server 2008 R2(x64 기반 시스템용)](https://www.microsoft.com/download/details.aspx?familyid=d3fb096b-87b5-4715-a093-9ec9b021a40f&displaylang=ko)(Server Core 설치)  
(KB2765809)
</td>
<td style="border:1px solid black;">
보안 기능 우회
...
<tr>
<td style="border:1px solid black;">
[Windows Server 2008 R2(x64 기반 시스템용) 서비스 팩 1](https://www.microsoft.com/download/details.aspx?familyid=d3fb096b-87b5-4715-a093-9ec9b021a40f&displaylang=ko)(Server Core 설치)  
(KB2765809)
</td>
<td style="border:1px solid black;">
보안 기능 우회
...
<tr class="alternateRow">
<td style="border:1px solid black;">
[Windows Server 2012](https://www.microsoft.com/download/details.aspx?familyid=d6654bf1-1ab9-4691-8729-793eb6d0e563&displaylang=ko)(Server Core 설치)  
(KB2765809)
</td>
<td style="border:1px solid black;">
보안 기능 우회
...
이 보안 업데이트의 SHA1 및 SHA2 해시는 다운로드한 보안 업데이트 패키지의 진위 여부를 검증하는 데 사용할 수 있습니다. 이 업데이트와 관련된 해시 정보는 [Microsoft 기술 자료 문서 2765809](https://support.microsoft.com/kb/2765809)를 참조하십시오.
...
IP-HTTPS 구성 요소가 인증서를 처리하는 방식으로 인해 Windows에 보안 기능 우회 취약점이 존재합니다. 이 취약점을 성공적으로 악용한 공격자는 인증서 유효성 확인을 우회할 수 있습니다.

이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2012-2549](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2012-2549)(영문)를 참조하십시오.

#### 해지된 인증서 우회 취약점(CVE-2012-2549)에 대한 완화 요소

완화란 취약점의 악용 심각도를 낮출 수 있는 설정, 일반적인 구성 또는 최선책, 기본 상태를 지칭합니다. 다음과 같은 완화 요소가 현재 상황에서 유용할 수 있습니다.

-   취약점을 악용하려면 공격자는 도메인에서 발급된 인증서를 처리해야 합니다.
-   조직 내부의 시스템에 로그온하려면 시스템 또는 도메인 자격 증명이 필요합니다.

#### 해지된 인증서 우회 취약점(CVE-2012-2549)에 대한 대안

대안이란 기본적인 취약점을 수정하지는 않지만 업데이트를 적용하기 전에 알려진 공격 경로를 차단하는 데 유용한 설정 또는 구성 변경을 지칭합니다. Microsoft는 다음과 같은 대안을 테스트했으며, 대안으로 인한 기능 저하 여부를 해당 항목에서 설명합니다.

-   해지된 클라이언트 인증서와 연결된 도메인 컴퓨터 계정을 사용하지 않습니다.

    Windows Server 2008 R2 및 Windows Server 2012에서 컴퓨터 계정을 사용하지 않는 방법은 [컴퓨터 계정 사용 안 함 또는 사용](https://technet.microsoft.com/ko-kr/library/cc732965.aspx)을 참조하십시오.

    스크립트를 사용하여 여러 컴퓨터 계정을 사용하지 않는 방법은 [컴퓨터 계정 사용 안 함](https://gallery.technet.microsoft.com/scriptcenter/ac6c44dd-db2f-487a-a51f-3ecb69b9386c)을 참조하십시오.

#### 해지된 인증서 우회 취약점(CVE-2012-2549x)에 대한 FAQ

이 취약점을 악용하면 어느 작업까지 가능합니까?  
이 취약점은 보안 기능 우회 취약점입니다.

취약점의 원인은 무엇입니까?  
이 취약점은 Windows에서 인증서의 유효성을 제대로 확인하지 못할 때 발생합니다.

이 취약점의 영향을 받는 구성 요소는 무엇입니까?  
이 취약점의 영향을 받는 Windows 구성 요소는 IP-HTTPS입니다.

IP-HTTPS란 무엇입니까?   
IP-HTTPS(IP over HTTPS)는 보안 HTTP 연결을 사용하여 보안 IP 터널을 설정할 수 있는 프로토콜입니다. 일반적으로 Microsoft DirectAccess 배포에서 사용됩니다. 자세한 내용은 [IP-HTTPS(IP over HTTPS) 터널링 프로토콜](https://msdn.microsoft.com/en-us/library/dd358571(v=prot.20).aspx)을 참조하십시오.

DirectAccess란 무엇입니까?   
Windows 7 및 Windows Server 2008 R2 운영 체제에서 도입된 DirectAccess는 원격 사용자가 IP-HTTPS와 같은 보안 프로토콜을 사용하여 엔터프라이즈 공유, 웹 사이트 및 응용 프로그램에 안전하게 액세스할 수 있도록 합니다. 자세한 내용은 [DirectAccess](https://technet.microsoft.com/en-us/network/dd420463.aspx)를 참조하십시오.

공격자는 취약점을 악용하여 무엇을 할 수 있습니까?  
이 취약점 악용에 성공한 공격자는 인증서의 유효성에 의존하는 보안 기능을 우회할 수 있습니다.

공격자는 이러한 취약점을 어떻게 악용합니까?  
공격자가 IP-HTTPS 구성 요소를 사용하는 응용 프로그램이나 서비스에 해지된 인증서를 제출할 경우 취약점으로 인해 보안 기능 우회가 허용될 수 있습니다.

취약점으로 인해 주로 공격 받을 위험이 있는 시스템은 무엇입니까?  
서버가 이 취약점으로 인한 위험에 노출됩니다.

업데이트는 어떤 기능을 수행합니까?  
업데이트는 Microsoft Windows에서 인증서의 유효성을 확인하는 방식을 수정하여 취약점을 해결합니다.
...
<td style="border:1px solid black;"><a href="https://support.microsoft.com/kb/2765809">Microsoft 기술 자료 문서 2765809</a> 참조</td>
</tr>
<tr class="odd">
