...
-   **Aurigma Image Uploader**. Aurigma는 취약점을 해결하는 [권고 (영문)](https://go.microsoft.com/fwlink/?linkid=122004)와 업데이트를 발표하였습니다. 자세한 내용은 Aurigma의 권고를 참조하십시오. 이 킬(Kill) 비트는 해당 ActiveX 컨트롤 소유자의 요청에 의해 설정됩니다. 지원이 필요한 고객은 Aurigma에 문의하십시오. 이 ActiveX 컨트롤의 클래시 식별자(CLSID)는 이 권고의 **자주 제기되는 질문 사항**에 나열되어 있습니다.
-   **HP Instant Support**. [HP](https://www.hp.com/)는 [(c01422264) (영문)](https://go.microsoft.com/fwlink/?linkid=122005) 및 [(c01439758) (영문)](https://go.microsoft.com/fwlink/?linkid=125347) 등 2개의 권고와 이러한 취약점을 해결하기 위한 업데이트를 발표했습니다. 자세한 내용과 다운로드 위치는 HP의 권고를 참조하십시오. 이 킬(Kill) 비트는 해당 ActiveX 컨트롤 소유자의 요청에 의해 설정됩니다. 지원이 필요한 고객은 [HP](https://www.hp.com/)에 문의하십시오. 이 ActiveX 컨트롤의 클래스 식별자(CLSID)는 이 권고의 **자주 제기되는 질문 사항**에 나열되어 있습니다.

이 업데이트 설치에 대한 자세한 내용은 [Microsoft 기술 자료 문서 953839 (영문)](https://support.microsoft.com/kb/953839)를 참조하십시오.
...
| 참고 자료                    | 번호                                                    |
|------------------------------|---------------------------------------------------------|
| **Microsoft 기술 자료 문서** | [953839 (영문)](https://support.microsoft.com/kb/953839) |
...
아니요, 자동 업데이트를 위해 이 업데이트는 Microsoft 보안 공지 [MS08-032](https://technet.microsoft.com/security/bulletin/ms08-032)에 설명된 ActiveX 킬(Kill) 비트 누적 보안 업데이트(950760)를 대체하지 않습니다. 자동 업데이트는 이 업데이트(953839) 설치 여부에 관계 없이 MS08-032 업데이트를 고객에게 계속 제공합니다. 그러나 이 업데이트(953839)를 설치한 고객은 MS08-032에 제공된 킬(kill) 비트로 보호받기 위해 MS08-032 업데이트를 설치할 필요가 없습니다.

**Windows Server 2008 Server Core 설치를 사용하는 사용자가 이 업데이트를 설치해야 합니까?**  
Windows Server 2008 Server Core 설치를 사용하는 사용자에게 이 업데이트가 제공되지만 설치할 필요는 없습니다. Server Core 설치 옵션에 대한 자세한 내용은 [Server Core (영문)](https://msdn.microsoft.com/en-us/library/ms723891(vs.85).aspx)를 참조하십시오. 참고: Server Core 설치 옵션은 Windows Server 2008의 특정 에디션에 적용되지 않습니다. [Server Core 설치 옵션 비교(영문)](https://www.microsoft.com/windowsserver2008/en/us/compare-core-installation.aspx)를 참조하십시오.
...
그렇습니다. 이 업데이트에는 이전에 Microsoft 보안 공지 [MS08-032](https://technet.microsoft.com/security/bulletin/ms08-032)에 발표된 킬 비트도 포함되어 있습니다.
...
다음 클래스 식별자는 [Aurigma (영문)](https://www.aurigma.com/)가 취약한 클래스 식별자 목록에 대해 킬 비트를 설정하도록 요청한 것입니다. 자세한 내용은 Aurigma가 발표한 [권고 (영문)](https://go.microsoft.com/fwlink/?linkid=122004)를 참조하십시오.
...
다음 클래스 식별자는 [HP](https://www.hp.co.kr/)가 취약한 클래스 식별자 목록에 대해 킬 비트를 설정하도록 요청한 것입니다. 자세한 내용은 HP가 발표한 [권고 (영문)](https://go.microsoft.com/fwlink/?linkid=122005)를 참조하십시오.
...
Microsoft는 이 업데이트를 설치하도록 고객에게 권장합니다. 이 업데이트에 대해 더 자세히 알고 싶은 고객은 [Microsoft 기술 자료 문서 953839 (영문)](https://support.microsoft.com/kb/953839)를 검토하십시오.
