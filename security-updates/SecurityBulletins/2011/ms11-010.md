---
TOCTitle: 'MS11-010'
Title: 'Microsoft 보안 공지 MS11-010 - 중요'
ms:assetid: 'ms11-010'
ms:contentKeyID: 61230377
ms:mtpsurl: 'https://technet.microsoft.com/ko-KR/library/ms11-010(v=Security.10)'
---

Microsoft 보안 공지 MS11-010 - 중요
===================================
...
이 보안 업데이트는 Windows XP 및 Windows Server 2003의 CSRSS(Microsoft Windows Client/Server Run-time Subsystem)에서 발견되어 비공개적으로 보고된 취약점 1건을 해결합니다. 이 보안 업데이트의 심각도는 지원 대상인 모든 운영 체제 에디션에 대해 중요입니다. 자세한 내용은 이 섹션에서 **영향을 받는 소프트웨어 및 영향을 받지 않는 소프트웨어**를 참조하십시오.
...
이 보안 업데이트는 사용자의 프로세스가 로그오프 시 종료되는 방식을 수정하여 취약점을 해결합니다. 취약점에 대한 자세한 내용은 다음 섹션, **취약점 정보**에서 각 취약점 항목의 자주 제기되는 질문 사항(FAQ)을 참조하십시오.
...
|-------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------|-------------|---------------------------------------------------------------------|
| [Windows XP 서비스 팩 3](http://www.microsoft.com/downloads/details.aspx?familyid=cfa10178-9859-4e03-bedc-e3f5297a0251&displaylang=ko)                      | 권한 상승      | 중요        | [MS10-011](http://technet.microsoft.com/security/bulletin/ms10-011) |
| [Windows XP Professional x64 Edition 서비스 팩 2](http://www.microsoft.com/downloads/details.aspx?familyid=9f0b7b77-5b90-4a4b-97a4-0c1ce6a70126)            | 권한 상승      | 중요        | [MS10-011](http://technet.microsoft.com/security/bulletin/ms10-011) |
| [Windows Server 2003 서비스 팩 2](http://www.microsoft.com/downloads/details.aspx?familyid=aed08b96-24cc-4e23-8fd5-c7e52f8ef41a&displaylang=ko)             | 권한 상승      | 중요        | [MS10-011](http://technet.microsoft.com/security/bulletin/ms10-011) |
| [Windows Server 2003 x64 Edition 서비스 팩 2](http://www.microsoft.com/downloads/details.aspx?familyid=ca7879e1-e295-445d-a658-0a31be1928cc&displaylang=ko) | 권한 상승      | 중요        | [MS10-011](http://technet.microsoft.com/security/bulletin/ms10-011) |
| [Windows Server 2003 SP2(Itanium 기반 시스템용)](http://www.microsoft.com/downloads/details.aspx?familyid=50855101-a15c-4c81-ad81-a7fe3f1d2026)             | 권한 상승      | 중요        | [MS10-011](http://technet.microsoft.com/security/bulletin/ms10-011) |
...
<th style="border:1px solid black;" >CSRSS 권한 상승 취약점(CVE-2011-0030)</th>
...
사용자가 로그오프할 때 CSRSS(Windows Client/Server Run-time Subsystem)가 프로세스를 종료하는 방식에 권한 상승 취약점이 존재합니다. 이 취약점을 성공적으로 악용한 공격자는 다음에 시스템에 로그온한 사용자의 작업을 모니터링하도록 설계된 코드를 실행할 수 있습니다. 그러면 로그온한 사용자가 액세스할 수 있는 영향을 받는 시스템의 민감한 정보가 노출되거나 데이터가 액세스될 수 있습니다. 이 민감한 정보에는 공격자가 나중에 권한 상승에 사용하거나 시스템의 다른 사용자로 코드를 실행할 수 있는 이후 사용자의 로그온 자격 증명이 포함될 수 있습니다. 공격자가 이 취약점을 악용하더라도 코드를 실행하거나 사용자 권한을 승격시킬 수는 없습니다. 하지만 영향을 받는 시스템 또는 네트워크를 추가로 손상시키는 데 필요한 정보를 수집하는 데 이 취약점을 사용할 수 있습니다. 관리자 권한이 있는 사용자가 다음에 시스템에 로그온하는 경우 공격자는 커널 모드에서 임의의 코드를 실행할 수 있습니다. 이렇게 되면 공격자가 프로그램을 설치할 수 있을 뿐 아니라 데이터를 보거나 변경하거나 삭제할 수 있고 모든 시스템 권한이 있는 새 계정을 만들 수도 있습니다.
  
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2011-0030 (영문)](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-0030)을 참조하십시오.
  
#### CSRSS 권한 상승 취약점(CVE-2011-0030)에 대한 완화 요소
  
완화란 취약점의 악용 심각도를 낮출 수 있는 설정, 일반적인 구성 또는 최선책, 기본 상태를 지칭합니다. 다음과 같은 완화 요소가 현재 상황에서 유용할 수 있습니다.
  
-   이 취약점을 악용하려면 공격자가 유효한 로그온 자격 증명을 가지고 로컬로 로그온할 수 있어야 합니다. 익명의 사용자에 의해서나 원격으로는 이 취약점을 악용할 수 없습니다.
  
#### CSRSS 권한 상승 취약점(CVE-2011-0030)에 대한 대안
  
이 취약점에 대한 대안을 확인하지 못했습니다.
  
#### CSRSS 권한 상승 취약점(CVE-2011-0030)에 대한 FAQ
  
**이 취약점을 악용하면 어느 작업까지 가능합니까?**  
이 취약점은 권한 상승 취약점입니다. 이 취약점을 악용한 공격자는 영향을 받는 시스템을 완전히 제어할 수 있습니다. 이렇게 되면 공격자가 프로그램을 설치할 수 있을 뿐 아니라 데이터를 보거나 변경하거나 삭제할 수 있고 모든 시스템 권한이 있는 새 계정을 만들 수도 있습니다. 취약점을 악용하려면 공격자가 로컬로 시스템에 로그온하고 프로그램을 실행할 수 있어야 합니다.
  
**취약점의 원인은 무엇입니까?**  
이 취약점은 사용자가 시스템에서 로그오프할 때 CSRSS(Windows Client/Server Run-time Subsystem)가 사용자 프로세스를 올바르게 종료하지 못해 발생합니다.
...
이 취약점을 성공적으로 악용한 공격자는 다음에 시스템에 로그온한 사용자의 작업을 모니터링하도록 설계된 코드를 실행할 수 있습니다. 그러면 로그온한 사용자가 액세스할 수 있는 영향을 받는 시스템의 민감한 정보가 노출되거나 데이터가 액세스될 수 있습니다. 이 민감한 정보에는 공격자가 나중에 권한 상승에 사용하거나 시스템의 다른 사용자로 코드를 실행할 수 있는 자격 증명이 포함됩니다. 이 민감한 정보가 노출되면, 이 취약점 악용에 성공한 공격자는 다음에 시스템에 로그온한 사용자의 권한 수준으로 코드를 실행할 수 있습니다. 관리자가 시스템에 로그온할 경우 공격자의 코드는 상승된 권한으로 실행됩니다.
  
**공격자는 이러한 취약점을 어떻게 악용합니까?**  
이 취약점을 악용하려면 공격자가 먼저 시스템에 로그온해야 합니다. 그런 다음 공격자는 공격자가 로그오프한 후에도 계속 실행되도록 특수하게 조작된 응용 프로그램을 시작할 수 있습니다. 새로운 사용자가 자신의 자격 증명으로 로그온하면 공격자의 프로세스는 새로 로그온한 사용자가 수행하는 모든 작업을 모니터링할 수 있습니다. 새로 로그온한 사용자가 관리자인 경우 노출된 정보는 영향을 받는 시스템을 추가로 손상시키는 데 사용될 수 있습니다.
  
**취약점으로 인해 주로 공격받을 위험이 있는 시스템은 무엇입니까?**  
로컬로 로그온하는 권한을 가진 다수의 사용자가 신뢰할 수 없는 응용 프로그램을 실행하는 시스템이 이 취약점으로 인한 위험에 주로 노출될 수 있습니다.
  
**업데이트는 어떤 기능을 수행합니까?**  
업데이트는 사용자의 프로세스가 로그오프 시 종료되는 방식을 수정하여 취약점을 해결합니다.
...
<td style="border:1px solid black;">제어판의 <strong>프로그램 추가/제거</strong> 도구 또는 %Windir%\$NTUninstallKB2476687$\Spuninst 폴더의 Spuninst.exe 유틸리티를 사용하십시오.</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"><strong>파일 정보</strong></td>
<td style="border:1px solid black;"><a href="http://support.microsoft.com/kb/2476687">Microsoft 기술 자료 문서 2476687</a> 참조</td>
</tr>
<tr class="even">
...
-   CSRSS 권한 상승 취약점(CVE-2011-0030)을 보고해 주신 [Department of Information Security, Beijing University (영문)](http://www.ss.pku.edu.cn/en/)의 Sihan Qing(교수), Weiping Wen(부교수), Liang Yin 및 Husheng Zhou(대학원생)
