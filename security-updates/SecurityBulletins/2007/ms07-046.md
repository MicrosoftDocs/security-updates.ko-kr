---
TOCTitle: 'MS07-046'
Title: 'Microsoft 보안 공지 MS07-046 - 긴급'
ms:assetid: 'ms07-046'
ms:contentKeyID: 61230085
ms:mtpsurl: 'https://technet.microsoft.com/ko-KR/library/ms07-046(v=Security.10)'
---


Microsoft 보안 공지 MS07-046 - 긴급
===================================

GDI의 취약점으로 인한 원격 코드 실행 문제점 (931906)
----------------------------------------------------
...
이 업데이트는 Windows 2003 Server 서비스 팩 2 및 Windows Vista를 제외한 지원되는 모든 Windows 에디션에 대해 긴급 보안 업데이트입니다. 자세한 내용은 이 섹션에서 **영향을 받는 소프트웨어 및 영향을 받지 않는 소프트웨어**를 참조하십시오.

이 보안 업데이트는 그래픽 렌더링 엔진의 이미지 처리 방식을 수정하여 이러한 취약점을 제거합니다. 취약점에 대한 자세한 내용은 다음 섹션, **취약점 정보**에서 각 취약점 항목의 자주 제기되는 질문 사항(FAQ)을 참조하십시오.

**권장 사항:** 고객은 보안 업데이트를 즉시 적용하는 것이 좋습니다.
...
|----------------------------------------------------------------------------------------------------------------------------------------------------|----------------|-------------|---------------------------------------------------------------------|
| [Microsoft Windows 2000 서비스 팩 4](https://www.microsoft.com/download/details.aspx?familyid=8fc8340b-c2b3-4559-835c-caa00cf086b9&displaylang=ko) | 원격 코드 실행 | 긴급        | [MS06-001](https://technet.microsoft.com/security/bulletin/ms06-001) |
| [Windows XP 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=dc29475d-c0bb-4d35-8dd6-4ca1cac32315&displaylang=ko)             | 원격 코드 실행 | 긴급        | [MS06-001](https://technet.microsoft.com/security/bulletin/ms06-001) |
| [Windows XP Professional x64 Edition](https://www.microsoft.com/download/details.aspx?familyid=3c81730a-981a-4649-b2d9-45144230d512)               | 원격 코드 실행 | 긴급        | [MS06-001](https://technet.microsoft.com/security/bulletin/ms06-001) |
| [Windows Server 2003 서비스 팩 1](https://www.microsoft.com/download/details.aspx?familyid=5374583d-de68-4d65-bca8-598d6b98b8b3&displaylang=ko)    | 원격 코드 실행 | 긴급        | [MS06-001](https://technet.microsoft.com/security/bulletin/ms06-001) |
| [Windows Server 2003 x64 Edition](https://www.microsoft.com/download/details.aspx?familyid=c3359f27-e03e-4a4f-b896-3bda39f69f7e)                   | 원격 코드 실행 | 긴급        | [MS06-001](https://technet.microsoft.com/security/bulletin/ms06-001) |
| [Windows Server 2003 SP1(Itanium 기반 시스템용)](https://www.microsoft.com/download/details.aspx?familyid=92822479-2060-4357-a340-ed096f180b2b)    | 원격 코드 실행 | 긴급        | [MS06-001](https://technet.microsoft.com/security/bulletin/ms06-001) |
...
<th style="border:1px solid black;" >GDI의 원격 코드 실행 취약점 – CVE-2007-3034</th>
...
GDI의 원격 코드 실행 취약점– CVE-2007-3034  
------------------------------------------
  
그래픽 렌더링 엔진에서 특수하게 조작된 이미지를 처리하는 방식에 원격 코드 실행 취약점이 존재합니다. 공격자는 사용자가 특수하게 조작된 전자 메일의 첨부 파일을 열 때 원격 코드가 실행되도록 이미지를 조작하여 이러한 취약점을 악용할 수 있습니다.
  
취약점 악용에 성공한 공격자는 로컬 사용자와 동일한 권한을 얻을 수 있습니다. 시스템에 대한 사용자 권한이 적게 구성된 계정의 사용자는 관리자 권한으로 작업하는 사용자에 비해 영향을 적게 받습니다.
  
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2007-3034 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2007-3034)를 참조하십시오.
  
#### GDI에 있는 원격 코드 실행 취약점 – CVE-2007-3034에 대한 완화 요소
  
완화란 취약점의 악용 심각도를 낮출 수 있는 설정, 일반적인 구성 또는 최선책, 기본 상태를 지칭합니다. 다음과 같은 완화 요소가 현재 상황에서 유용할 수 있습니다.
  
-   취약점 악용에 성공한 공격자는 로컬 사용자와 동일한 권한을 얻을 수 있습니다. 시스템에 대한 사용자 권한이 적게 구성된 계정의 사용자는 관리자 권한으로 작업하는 사용자에 비해 영향을 적게 받습니다.  
-   Microsoft Windows Vista 및 Windows Server 2003 서비스 팩 2는 이 문제의 영향을 받지 않습니다.
  
#### GDI에 있는 원격 코드 실행 취약점 – CVE-2007-3034에 대한 대안
  
이 취약점에 대한 대안을 확인하지 못했습니다.
  
#### GDI에 있는 원격 코드 실행 취약점 – CVE-2007-3034에 대한 FAQ
...
**취약점의 원인은 무엇입니까?**  
그래픽 렌더링 엔진에서 특수하게 조작된 이미지를 처리하는 방식에 임의 코드 실행 취약점이 존재합니다.
  
**공격자는 취약점을 악용하여 무엇을 할 수 있습니까?**  
이 취약점을 성공적으로 악용한 공격자는 영향을 받는 시스템에서 코드를 실행할 수 있습니다.
  
**공격자는 이러한 취약점을 어떻게 악용합니까?**  
공격자는 악의적인 웹 페이지 또는 특수하게 조작된 전자 메일 첨부 파일을 만든 다음 사용자가 해당 첨부 파일을 열도록 유도하여 이 취약점을 악용할 수 있습니다. 사용자가 첨부 파일을 열면 공격자는 로컬로 로그온한 사용자의 보안 컨텍스트에서 임의 코드를 실행할 수 있습니다.
  
**GDI란 무엇입니까?**  
Microsoft Windows GDI(그래픽 장치 인터페이스)를 통해 응용 프로그램이 비디오 디스플레이와 프린터 모두에서 그래픽과 형식 있는 텍스트를 사용할 수 있습니다. Windows 기반 응용 프로그램은 그래픽 하드웨어에 직접 액세스하지 않습니다. GDI가 응용 프로그램 대신 장치 드라이버와 상호 작용을 수행합니다. GDI에 대한 자세한 내용은 Windows GDI 시작 페이지를 참조하십시오.
  
**취약점으로 인해 주로 공격받을 위험이 있는 시스템은 무엇입니까?**  
워크스테이션과 터미널 서버가 주로 위험합니다.
  
**업데이트는 어떤 기능을 수행합니까?**  
업데이트는 이미지 처리에 오버플로 유효성 검사를 추가하여 취약점을 제거합니다.
...
| Windows XP Professional x64 Edition            | 아니오     | 예         |  
| Windows Server 2003 서비스 팩 1                | 예         | 예         |  
| Windows Server 2003 x64 Edition                | 아니오     | 예         |  
| Windows Server 2003 SP1(Itanium 기반 시스템용) | 아니오     | 예         |
...
Windows2000-kb938829-x86-kor /quiet</td>
</tr>
<tr class="even">
<td style="border:1px solid black;">다시 시작하지 않고 설치</td>
<td style="border:1px solid black;">Windows 2000 서비스 팩 4:<br />
Windows2000-kb938829-Windows2000sp4-x86-kor /norestart</td>
</tr>
<tr class="odd">
...
지원 대상인 모든 Windows 2000 서비스 팩 4 에디션:
...
Windowsxp-kb938829-x86-kor /quiet</td>
</tr>
<tr class="even">
<td style="border:1px solid black;"></td>
<td style="border:1px solid black;">Windows XP Professional x64 Edition:<br />
Windowsserver2003-kb938829-x86-kor /quiet</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;">다시 시작하지 않고 설치</td>
<td style="border:1px solid black;">Windows XP 서비스 팩 2:<br />
Windowsxp-kb938829-x86-kor /norestart</td>
</tr>
<tr class="even">
<td style="border:1px solid black;"></td>
<td style="border:1px solid black;">Windows XP Professional x64 Edition:<br />
Windowsserver2003-kb938829-x86-kor /norestart</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;">로그 파일 업데이트</td>
...
<tr class="even">
<td style="border:1px solid black;"><strong>제거 정보</strong></td>
<td style="border:1px solid black;">제어판의 프로그램 추가/제거 도구 또는 %Windir%\$NTUninstallKB938829$\Spuninst 폴더의 Spuninst.exe 유틸리티를 사용하십시오.</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"><strong>파일 정보</strong></td>
<td style="border:1px solid black;">전체 파일 목록은 아래에 있는 <strong>파일 정보</strong> 참조</td>
...
<tr class="odd">
<td style="border:1px solid black;">사용자 개입 없이 설치</td>
<td style="border:1px solid black;">지원 대상인 모든 Windows Server 2003 서비스 팩 1, Windows Server 2003 x64 Edition 및 Windows Server 2003 SP1(Itanium 기반 시스템용) 에디션:<br />
Windowsserver2003-kb938829-x86-kor /quiet</td>
</tr>
<tr class="even">
<td style="border:1px solid black;">다시 시작하지 않고 설치</td>
...
<td style="border:1px solid black;"><strong>제거 정보</strong></td>
<td style="border:1px solid black;">지원 대상인 모든 Windows Server 2003 서비스 팩 1, Windows Server 2003 x64 Edition 및 Windows Server 2003 SP1(Itanium 기반 시스템용) 에디션:<br />
제어판의 프로그램 추가/제거 도구 또는 %Windir%\$NTUninstallKB938829$\Spuninst 폴더의 Spuninst.exe 유틸리티를 사용하십시오.</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"><strong>파일 정보</strong></td>
<td style="border:1px solid black;">전체 파일 목록은 아래에 있는 <strong>파일 정보</strong> 참조</td>
...
지원되는 모든 Windows Server 2003 서비스 팩 1 에디션:
...
지원 대상인 모든 Windows Server 2003 x64 Edition:
...
지원되는 모든 Windows Server 2003 SP1(Itanium 기반 시스템용) 에디션:
...
-   원격 코드 실행 취약점(CVE-2007-3034)을 보고해 주신 [eEye Digital Security (영문)](https://www.eeye.com/html/index.html)
