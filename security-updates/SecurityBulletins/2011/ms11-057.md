...
심층 보안 업데이트 중 하나는 사용자가 Internet Explorer에서 도메인 간 끌어서 놓기 작업을 차단하도록 시스템을 구성할 수 있는 기능을 추가합니다. 이 추가된 기능에 대한 자세한 내용은 [Microsoft 기술 자료 문서 2581921](https://support.microsoft.com/kb/2581921)을 참조하십시오.
...
예, 이 업데이트는 [CVE-2011-1347 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-1347)이라고 공개적으로 참조된 보호 모드 우회 문제점을 해결합니다.
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2011-1257 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-1257)을 참조하십시오.
...
-   기본적으로 Windows Server 2003, Windows Server 2008 및 Windows Server 2008 R2의 Internet Explorer는 [보안 강화 구성](https://technet.microsoft.com/library/dd883248(ws.10).aspx)으로 알려진 제한된 모드로 실행됩니다. 이 모드에서는 이 취약점이 완화됩니다. Internet Explorer 보안 강화 구성에 대한 자세한 내용은 이 취약점에 대한 FAQ 항목을 참조하십시오.
...
예. 기본적으로 Windows Server 2003, Windows Server 2008 및 Windows Server 2008 R2의 Internet Explorer는 [보안 강화 구성](https://technet.microsoft.com/library/dd883248(ws.10).aspx)으로 알려진 제한된 모드로 실행됩니다. 보안 강화 구성은 미리 구성된 Internet Explorer 설정 그룹으로, 사용자나 관리자가 서버에서 특수하게 조작된 웹 콘텐츠를 다운로드하고 실행할 가능성을 줄여 줍니다. Internet Explorer의 신뢰할 수 있는 사이트 영역에 추가되지 않는 웹 사이트에 대한 완화 요소입니다.
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2011-1960 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-1960)을 참조하십시오.
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2011-1961 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-1961)을 참조하십시오.
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2011-1962 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-1962)을 참조하십시오.
...
**이 보안 공지를 게시했을 때 이 취약점이 공개되어 있었습니까?**    
예. 이 취약점은 공개되었습니다. 이 취약점에는 CVE(Common Vulnerability and Exposure) 번호 [CVE-2011-1962 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-1962)가 지정되었습니다.

**이 보안 공지를 게시했을 때 Microsoft는 이 취약점이 악용되었다는 보고를 받았습니까?**    
아니요. Microsoft는 이 보안 공지를 처음에 게시했을 때 이 취약점이 고객을 공격하는 데 공개적으로 사용되었다는 정보를 확인하지 못했습니다.

**이 보안 업데이트를 적용하면 취약점을 악용하려는 일반에 공개된 코드로부터 고객을 보호할 수 있습니까?**    
예. 이 보안 업데이트는 공개된 개념 증명 코드를 사용하여 악용될 수 있는 취약점을 해결합니다. 해결된 취약점에 CVE(Common Vulnerability and Exposure) 번호 [CVE-2011-1962 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-1962)가 지정되었습니다.
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2011-1963 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-1963)을 참조하십시오.
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2011-1964 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-1964)를 참조하십시오.
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2011-2383 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-2383)을 참조하십시오.
...
**이 보안 공지를 게시했을 때 이 취약점이 공개되어 있었습니까?**    
예. 이 취약점은 공개되었습니다. 이 취약점에는 CVE(Common Vulnerability and Exposure) 번호 [CVE-2011-2383 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-2383)이 지정되었습니다.

**이 보안 공지를 게시했을 때 Microsoft는 이 취약점이 악용되었다는 보고를 받았습니까?**    
아니요. Microsoft는 이 보안 공지를 처음에 게시했을 때 이 취약점이 고객을 공격하는 데 공개적으로 사용되었다는 정보를 확인하지 못했습니다.

**이 보안 업데이트를 적용하면 취약점을 악용하려는 일반에 공개된 코드로부터 고객을 보호할 수 있습니까?**    
예. 이 보안 업데이트는 공개된 개념 증명 코드를 사용하여 악용될 수 있는 취약점을 해결합니다. 해결된 취약점에 CVE(Common Vulnerability and Exposure) 번호 [CVE-2011-2383 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-2383)이 지정되었습니다.
...
<td style="border:1px solid black;"><a href="https://support.microsoft.com/kb/2559049">Microsoft 기술 자료 문서 2559049</a> 참조</td>
</tr>
<tr class="even">
...
-   이벤트 처리기 정보 유출 취약점(CVE-2011-1960)을 보고해 주신 [Opera Software ASA (영문)](https://www.opera.com/)의 Yngve N. Pettersen  
-   창 열기 경쟁 조건 취약점(CVE-2011-1257)을 보고해 주신 [Lostmon Lords (영문)](https://lostmon.blogspot.com)  
-   텔넷 처리기 원격 코드 실행 취약점(CVE-2011-1961)을 보고해 주신 Security Professionals Network Inc.의 Makoto Shiotsuki  
-   XSLT 메모리 손상 취약점(CVE-2011-1963)을 [TippingPoint's (영문)](https://www.tippingpoint.com/)[Zero Day Initiative (영문)](https://www.zerodayinitiative.com/)와 협력하여 보고해 주신 익명 연구자  
-   Style 개체 메모리 손상 취약점(CVE-2011-1964)을 [TippingPoint's (영문)](https://www.tippingpoint.com/)[Zero Day Initiative (영문)](https://www.zerodayinitiative.com/)와 협력하여 보고해 주신 [Harmony Security (영문)](https://www.harmonysecurity.com/)의 Stephen Fewer  
-   이 공지에 포함된 심층 보안 변경 사항에 대해 협력해 주신 [Google Inc.](https://www.google.com/)의 Michal Zalewski  
-   Internet Explorer의 보호 모드 개선을 위해 [TippingPoint's (영문)](https://www.tippingpoint.com/) [Zero Day Initiative (영문)](https://www.zerodayinitiative.com/)와 협력해 주신 [Harmony Security (영문)](https://www.harmonysecurity.com/)의 Stephen Fewer
