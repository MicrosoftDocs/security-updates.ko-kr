...
문제점. 이 보안 업데이트를 설치할 때 고객이 겪을 수 있는 현재까지 알려진 문제점은 [Microsoft 기술 자료 문서 2586448](https://support.microsoft.com/kb/2586448)에 나와 있습니다. 이 문서는 이러한 문제점에 대한 권장 해결 방법도 소개합니다. 현재 알려진 문제점 및 권장 해결 방법은 이 소프트웨어의 특정 릴리스에만 해당되나, 이 문서는 다른 문서에 대한 링크도 제공합니다.
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2011-1993(영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-1993)을 참조하십시오.
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2011-1995(영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-1995)를 참조하십시오.
...
#### OLEAuto32.dll 원격 코드 실행 취약점(CVE-2011-1995)에 대한 FAQ

이 취약점을 악용하면 어느 작업까지 가능합니까?  
이 취약점은 원격 코드 실행 취약점입니다. 취약점 악용에 성공한 공격자는 로그온한 사용자와 동일한 권한을 얻을 수 있습니다. 시스템에 대한 사용자 권한이 적게 구성된 계정의 사용자는 관리자 권한으로 작업하는 사용자에 비해 영향을 적게 받습니다.

취약점의 원인은 무엇입니까?  
Internet Explorer가 초기화되지 않은 개체에 액세스하려고 할 때 메모리가 손상되어 공격자가 로그온한 사용자의 컨텍스트에서 임의 코드를 실행할 수 있습니다.
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2011-1996(영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-1996)을 참조하십시오.

#### Option 요소 원격 코드 실행 취약점(CVE-2011-1996)에 대한 완화 요소
...
#### Option 요소 원격 코드 실행 취약점(CVE-2011-1996)에 대한 대안
...
#### Option 요소 원격 코드 실행 취약점(CVE-2011-1996)에 대한 FAQ
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2011-1997(영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-1997)을 참조하십시오.

#### OnLoad 이벤트 원격 코드 실행 취약점(CVE-2011-1997)에 대한 완화 요소
...
-   기본적으로 지원 대상인 모든 Microsoft Outlook, Microsoft Outlook Express 및 Windows Mail 버전은 스크립트와 ActiveX 컨트롤을 비활성화하는 제한된 사이트 영역에서 HTML 메일 메시지를 열어 공격자가 이 취약성을 사용하여 악의적인 코드를 실행할 수 없도록 위험 수준을 줄여 줍니다. 사용자가 메일에 포함된 링크를 클릭하면 웹 기반 공격 시나리오를 통해 이 취약점 악용에 취약해질 수 있습니다.
-   기본적으로 Windows Server 2003, Windows Server 2008 및 Windows Server 2008 R2의 Internet Explorer는 [보안 강화 구성](https://technet.microsoft.com/ko-kr/library/dd883248(ws.10).aspx)으로 알려진 제한된 모드로 실행됩니다. 이 모드에서는 이 취약점이 완화됩니다. Internet Explorer 보안 강화 구성에 대한 자세한 내용은 이 취약점에 대한 FAQ 항목을 참조하십시오.
-   Internet Explorer 7, Internet Explorer 8 및 Internet Explorer 9은 이 취약점의 영향을 받지 않습니다.

#### OnLoad 이벤트 원격 코드 실행 취약점(CVE-2011-1997)에 대한 대안
...
#### OnLoad 이벤트 원격 코드 실행 취약점(CVE-2011-1997)에 대한 FAQ
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2011-1998(영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-1998)을 참조하십시오.

#### Jscript9.dll 원격 코드 실행 취약점(CVE-2011-1998)에 대한 완화 요소
...
#### Jscript9.dll 원격 코드 실행 취약점(CVE-2011-1998)에 대한 대안
...
#### Jscript9.dll 원격 코드 실행 취약점(CVE-2011-1998)에 대한 FAQ
...
공격자는 이러한 취약점을 어떻게 악용합니까?  
공격자는 Internet Explorer를 통해 이 취약점을 악용하도록 설계하여 특수하게 조작된 웹 사이트를 호스팅한 다음 사용자가 이 웹 사이트를 보고 작업을 수행하도록 유도할 수 있습니다. 공격자는 사용자가 제공한 콘텐츠가 광고를 허용하거나 호스팅하는 웹 사이트와 공격에 노출된 웹 사이트를 이용할 수도 있습니다. 이러한 웹 사이트에 이 취약점을 악용하는 특수하게 조작된 콘텐츠가 포함될 수 있습니다. 그러나 어떠한 경우에도 공격자는 강제로 사용자가 공격자 제어 콘텐츠를 보도록 만들 수는 없습니다. 대신 공격자는 사용자가 공격자의 웹 사이트에 연결되는 전자 메일 메시지나 메신저 메시지에서 링크를 클릭하거나 전자 메일을 통해 보낸 첨부 파일을 여는 등의 조치를 취하도록 유도해야 합니다.
...
Internet Explorer에서 역참조된 메모리 주소에 액세스하는 방식에 원격 코드 실행 취약점이 존재합니다. 취약점으로 인해 메모리가 손상되고 공격자가 로그온한 사용자의 컨텍스트에서 임의 코드를 실행할 수 있습니다.

이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2011-1999(영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-1999)를 참조하십시오.

#### Select 요소 원격 코드 실행 취약점(CVE-2011-1999)에 대한 완화 요소
...
#### Select 요소 원격 코드 실행 취약점(CVE-2011-1999)에 대한 대안
...
#### Select 요소 원격 코드 실행 취약점(CVE-2011-1999)에 대한 FAQ

이 취약점을 악용하면 어느 작업까지 가능합니까?  
이 취약점은 원격 코드 실행 취약점입니다. 취약점 악용에 성공한 공격자는 로그온한 사용자와 동일한 권한을 얻을 수 있습니다. 시스템에 대한 사용자 권한이 적게 구성된 계정의 사용자는 관리자 권한으로 작업하는 사용자에 비해 영향을 적게 받습니다.

취약점의 원인은 무엇입니까?  
Internet Explorer가 역참조된 메모리 주소에 액세스하려고 할 때 메모리가 손상되어 공격자가 로그온한 사용자의 컨텍스트에서 임의 코드를 실행할 수 있습니다.
...
업데이트는 어떤 기능을 수행합니까?  
이 업데이트는 Internet Explorer가 메모리를 할당 및 액세스하는 방식을 수정하여 취약점을 해결합니다.
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2011-2000(영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-2000)을 참조하십시오.

#### Body 요소 원격 코드 실행 취약점(CVE-2011-2000)에 대한 완화 요소
...
#### Body 요소 원격 코드 실행 취약점(CVE-2011-2000)에 대한 대안
...
#### Body 요소 원격 코드 실행 취약점(CVE-2011-2000)에 대한 FAQ
...
Internet Explorer가 손상된 가상 함수 테이블에 액세스하는 방식에 원격 코드 실행 취약점이 존재합니다. 취약점으로 인해 메모리가 손상되고 공격자가 로그온한 사용자의 컨텍스트에서 임의 코드를 실행할 수 있습니다.

이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2011-2001(영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-2001)을 참조하십시오.

#### 가상 함수 테이블 손상 원격 코드 실행 취약점(CVE-2011-2001)에 대한 완화 요소
...
#### 가상 함수 테이블 손상 원격 코드 실행 취약점(CVE-2011-2001)에 대한 대안
...
#### 가상 함수 테이블 손상 원격 코드 실행 취약점(CVE-2011-2001)에 대한 FAQ

이 취약점을 악용하면 어느 작업까지 가능합니까?  
이 취약점은 원격 코드 실행 취약점입니다. 취약점 악용에 성공한 공격자는 로그온한 사용자와 동일한 권한을 얻을 수 있습니다. 시스템에 대한 사용자 권한이 적게 구성된 계정의 사용자는 관리자 권한으로 작업하는 사용자에 비해 영향을 적게 받습니다.

취약점의 원인은 무엇입니까?  
Internet Explorer가 손상된 가상 함수 테이블에 액세스하려고 할 때 공격자가 로그온한 사용자의 컨텍스트에서 임의 코드를 실행하도록 할 수 있습니다.
...
<td style="border:1px solid black;"></td>
<td style="border:1px solid black;">지원 대상인 모든 Windows XP 에디션용 Internet Explorer 7:<br />
제어판의 프로그램 추가/제거 도구 또는 %Windir%\ie7updates\KB2586448-IE7\spuninst 폴더의 Spuninst.exe 유틸리티를 사용하십시오.</td>
</tr>
<tr class="even">
<td style="border:1px solid black;"></td>
<td style="border:1px solid black;">지원 대상인 모든 Windows XP 에디션용 Internet Explorer 8:<br />
제어판의 프로그램 추가/제거 도구 또는 %Windir%\ie8updates\KB2586448-IE8\spuninst 폴더의 Spuninst.exe 유틸리티를 사용하십시오.</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;">파일 정보</td>
<td style="border:1px solid black;"><a href="https://support.microsoft.com/kb/2586448">Microsoft 기술 자료 문서 2586448</a> 참조</td>
</tr>
<tr class="even">
...
<td style="border:1px solid black;">제거 정보</td>
<td style="border:1px solid black;">지원 대상인 모든 Windows Server 2003 32비트 에디션, 64비트 기반 에디션 및 Itanium 기반 에디션용 Internet Explorer 6:<br />
제어판의 프로그램 추가/제거 도구 또는 %Windir%\$NTUninstallKB2586448$\Spuninst 폴더의 Spuninst.exe 유틸리티를 사용하십시오.</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"></td>
<td style="border:1px solid black;">지원 대상인 모든 Windows Server 2003 32비트 에디션, x64 기반 에디션 및 Itanium 기반 에디션용 Internet Explorer 7:<br />
...
-   Scroll 이벤트 원격 코드 실행 취약점(CVE-2011-1993)을 보고해 주신 McAfee Labs의 Vishwas Sharma  
-   OLEAuto32.dll 원격 코드 실행 취약점(CVE-2011-1995)을 보고해 주신 [Greplin(영문)](https://www.greplin.com)의 David Bloom  
-   Option 요소 원격 코드 실행 취약점(CVE-2011-1996)을 [TippingPoint(영문)](https://www.tippingpoint.com)의 [Zero Day Initiative(영문)](https://www.zerodayinitiative.com)와 협력하여 보고해 주신 Ivan Fratric  
-   OnLoad 이벤트 원격 코드 실행 취약점(CVE-2011-1997)을 [VeriSign iDefense Labs(영문)](https://labs.idefense.com)와 협력하여 보고해 주신 [GWSlabs(영문)](https://www.gwslabs.com)  
-   Jscript9.dll 원격 코드 실행 취약점(CVE-2011-1998)을 보고해 주신 [Greplin(영문)](https://www.greplin.com)의 David Bloom  
-   Select 요소 원격 코드 실행 취약점(CVE-2011-1999)을 [TippingPoint(영문)](https://www.tippingpoint.com)의 [Zero Day Initiative(영문)](https://www.zerodayinitiative.com)와 협력하여 보고해 주신 Ivan Fratric  
-   Body 요소 원격 코드 실행 취약점(CVE-2011-2000)을 [TippingPoint(영문)](https://www.tippingpoint.com)의 [Zero Day Initiative(영문)](https://www.zerodayinitiative.com)와 협력하여 보고해 주신 Sebastian Apelt  
-   가상 함수 테이블 손상 원격 코드 실행 취약점(CVE-2011-2001)을 [TippingPoint(영문)](https://www.tippingpoint.com)의 [Zero Day Initiative(영문)](https://www.zerodayinitiative.com)와 협력하여 보고해 주신 익명 연구자  
-   이 공지에 포함된 심층 보안 변경 사항에 대해 협력해 주신 [Google Inc.](https://www.google.com/korea)의 Eduardo Vela Nava와 [Greplin(영문)](https://www.greplin.com)의 David Bloom  
-   이 공지에 포함된 심층 보안 변경 사항에 대해 협력해 주신 [Soroush Dalili(영문)](https://www.secproject.com)  
-   이 공지에 포함된 심층 보안 변경 사항에 대해 협력해 주신 [Google Inc.](https://www.google.com/korea)의 Billy Rios
...
-   V1.1(2011년 10월 13일): 요약의 알려진 문제점에 Microsoft 기술 자료 문서 2586448 링크가 추가되었습니다.
