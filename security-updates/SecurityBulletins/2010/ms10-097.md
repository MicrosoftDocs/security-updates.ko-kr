---
TOCTitle: 'MS10-097'
Title: 'Microsoft 보안 공지 MS10-097 - 중요'
ms:assetid: 'ms10-097'
ms:contentKeyID: 61230357
ms:mtpsurl: 'https://technet.microsoft.com/ko-KR/library/ms10-097(v=Security.10)'
---


Microsoft 보안 공지 MS10-097 - 중요
===================================
...
이 보안 업데이트는 Microsoft Windows 인터넷 연결 등록 마법사의 공개된 취약점을 해결합니다. 이 보안 업데이트의 심각도는 지원 대상인 모든 Windows XP 및 Windows Server 2003 에디션에 대해 중요입니다. 지원 대상인 모든 Windows Vista, Windows Server 2008, Windows 7 및 Windows Server 2008 R2 에디션은 이 취약점의 영향을 받지 않습니다. 자세한 내용은 이 섹션에서 **영향을 받는 소프트웨어 및 영향을 받지 않는 소프트웨어**를 참조하십시오.
...
이 보안 업데이트는 인터넷 연결 등록 마법사가 외부 라이브러리를 로드하는 방식을 수정하여 취약점을 해결합니다. 취약점에 대한 자세한 내용은 다음 섹션, **취약점 정보**에서 각 취약점 항목의 자주 제기되는 질문 사항(FAQ)을 참조하십시오.
...
|------------------------------------------------------------------------------------------------------------------------------------------------------|----------------|-------------|---------------------------|
| [Windows XP 서비스 팩 3](https://www.microsoft.com/download/details.aspx?familyid=fa9a1aac-b9c5-4d4e-9083-a080ad4ccc6f)                              | 원격 코드 실행 | 중요        | 없음                      |
| [Windows XP Professional x64 Edition 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=6dabc306-c858-46b1-815c-cd8d011ff62e)     | 원격 코드 실행 | 중요        | 없음                      |
| [Windows Server 2003 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=8fa2cfa4-a01d-4910-b69f-736aeb585bab)                     | 원격 코드 실행 | 중요        | 없음                      |
| [Windows Server 2003 x64 Edition 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=17b0b340-73b2-42a7-9d86-1297c63dcc2b)         | 원격 코드 실행 | 중요        | 없음                      |
| [Windows Server 2003 with SP2(Itanium 기반 시스템용)](https://www.microsoft.com/download/details.aspx?familyid=04a178cc-1afd-4e47-8cab-05e402e5a568) | 원격 코드 실행 | 중요        | 없음                      |
...
<th style="border:1px solid black;" >인터넷 연결 등록 마법사의 안전하지 않은 라이브러리 로드 취약점(CVE-2010-3144)</th>
...
Microsoft Windows의 구성 요소인 인터넷 연결 등록 마법사가 DLL 파일 로드를 처리하는 방식에 원격 코드 실행 취약점이 존재합니다. 이 취약점을 악용한 공격자는 영향을 받는 시스템을 완전히 제어할 수 있습니다. 이렇게 되면 공격자가 프로그램을 설치할 수 있을 뿐 아니라 데이터를 보거나 변경하거나 삭제할 수 있고 모든 사용자 권한이 있는 새 계정을 만들 수도 있습니다. 시스템에 대한 사용자 권한이 적게 구성된 계정의 사용자는 관리자 권한으로 작업하는 사용자에 비해 영향을 적게 받습니다.
  
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2010-3144 (영문)](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-3144)를 참조하십시오.
  
#### 인터넷 연결 등록 마법사의 안전하지 않은 라이브러리 로드 취약점(CVE-2010-3144)에 대한 완화 요소
...
#### 인터넷 연결 등록 마법사의 안전하지 않은 라이브러리 로드 취약점(CVE-2010-3144)에 대한 대안
...
#### 인터넷 연결 등록 마법사의 안전하지 않은 라이브러리 로드 취약점(CVE-2010-3144)에 대한 FAQ
...
**취약점의 원인은 무엇입니까?**    
이 취약점은 인터넷 연결 등록 마법사가 외부 라이브러리 로드에 사용되는 경로를 잘못 제한하기 때문에 발생합니다.
  
**인터넷 연결 등록 마법사란 무엇입니까?**    
인터넷 연결 등록 마법사를 사용하면 사용자가 새 컴퓨터에 대한 연결을 구성하거나 이전 컴퓨터에서 갖고 있던 기존 인터넷 서비스 계정을 사용하도록 할 수 있습니다. 또한 이 마법사는 내부 설정이 손상된 사용자에게 도움을 줍니다. 자동 구성 기능은 설정을 복원하기 때문에 사용자는 사용자 ID, 암호 및 서버의 POP(상호 접속 위치)만 제공하면 됩니다.
...
공격자는 사용자가 특수하게 조작된 DLL(동적 연결 라이브러리) 파일과 동일한 네트워크 디렉터리에 있는 합법적인 .ins 또는 .isp 파일을 열도록 유도할 수 있습니다. 이렇게 하면 파일을 열 때 인터넷 연결 등록 마법사가 DLL 파일 로드 및 포함된 코드 실행을 시도할 수 있습니다.
  
전자 메일 공격 시나리오에서 공격자는 사용자에게 합법적인 .ins 또는 .isp 첨부 파일을 전송하고 사용자가 특수하게 조작된 DLL 파일이 들어 있는 디렉터리에 해당 첨부 파일을 추가하고 .ins 또는 .isp 파일을 열도록 유도하여 이 취약점을 악용할 수 있습니다. 이렇게 하면 .ins 또는 .isp 파일을 열 때 인터넷 연결 등록 마법사가 DLL 파일 로드 및 포함된 코드 실행을 시도할 수 있습니다.
  
네트워크 시나리오에서 공격자는 합법적인 .ins 또는 .isp 파일 및 특수하게 조작된 DLL을 UNC 또는 WebDAV 위치에 두고 사용자가 합법적인 파일을 열도록 유도할 수 있습니다. 공격자는 웹 사이트를 호스팅하고 이 웹 사이트에서 합법적인 파일을 참조하여 웹 브라우저를 통해 취약점이 유발되도록 할 수도 있습니다.
...
**업데이트는 어떤 기능을 수행합니까?**    
이 업데이트는 인터넷 연결 등록 마법사가 외부 라이브러리를 로드하는 방식을 수정하여 이 취약점을 해결합니다.
  
**이 보안 공지를 게시했을 때 이 취약점이 공개되어 있었습니까?**    
예. 이 취약점은 공개되었습니다. 이 취약점에는 CVE(Common Vulnerability and Exposure) 번호 [CVE-2010-3144 (영문)](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-3144)가 지정되었습니다.
...
<td style="border:1px solid black;">제어판의 <strong>프로그램 추가/제거</strong> 도구 또는 %Windir%\$NTUninstallKB2443105$\Spuninst 폴더의 Spuninst.exe 유틸리티를 사용하십시오.</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"><strong>파일 정보</strong></td>
<td style="border:1px solid black;"><a href="http://support.microsoft.com/kb/2443105">Microsoft 기술 자료 문서 2443105</a> 참조</td>
</tr>
<tr class="even">
...
-   인터넷 연결 등록 마법사의 안전하지 않은 라이브러리 로드 취약점(CVE-2010-3144)을 보고해 주신 [NGS Software (영문)](http://www.ngssoftware.com/)의 Muhaimin Dzulfakar
