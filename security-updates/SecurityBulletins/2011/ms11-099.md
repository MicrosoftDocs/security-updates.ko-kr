---
TOCTitle: 'MS11-099'
Title: 'Microsoft 보안 공지 MS11-099 - 중요'
ms:assetid: 'ms11-099'
ms:contentKeyID: 61230466
ms:mtpsurl: 'https://technet.microsoft.com/ko-KR/library/ms11-099(v=Security.10)'
---


Microsoft 보안 공지 MS11-099 - 중요
===================================

Internet Explorer 누적 보안 업데이트 (2618444)
----------------------------------------------

게시된 날짜: 2011년 12월 13일 화요일 | 업데이트된 날짜: 2012년 1월 11일 수요일
...
이 보안 업데이트의 심각도는 Windows 클라이언트의 Internet Explorer 7, Internet Explorer 8 및 Internet Explorer 9과 Windows Server 2008 R2의 Internet Explorer 9에 대해 중요입니다. 또한 이 보안 업데이트의 등급은 지원되는 모든 Windows XP 에디션의 Internet Explorer 6에 대해 보통입니다. 또한 Windows 서버(Windows Server 2008 R2 제외)의 Internet Explorer에 대해 낮음입니다. 자세한 내용은 이 섹션에서 영향을 받는 소프트웨어 및 영향을 받지 않는 소프트웨어를 참조하십시오.

업데이트는 Internet Explorer XSS 필터 동작을 수정하고, Explorer가 외부 라이브러리를 로드하는 방식, Internet Explorer가 웹 서버에서 제공한 콘텐츠 설정을 적용하는 방식을 수정하여 이러한 취약점을 해결합니다. 취약점에 대한 자세한 내용은 다음 섹션, 취약점 정보에서 각 취약점 항목의 자주 제기되는 질문 사항(FAQ)을 참조하십시오.
...
알려진 문제점. 이 보안 업데이트를 설치할 때 고객이 겪을 수 있는 현재까지 알려진 문제점은 [Microsoft 기술 자료 문서 2618444](https://support.microsoft.com/kb/2618444)에 나와 있습니다. 이 문서는 이러한 문제점에 대한 권장 해결 방법도 소개합니다.
...
이 공지가 2012년 1월 10일에 개정된 이유는 무엇입니까?  
Microsoft는 업데이트 MS11-099가 Internet Explorer에 대해 [SSL/TLS의 취약점으로 인한 정보 유출 문제점](https://go.microsoft.com/fwlink/?linkid=232510) 업데이트인 MS12-006에서 제공된 보호를 허용했다는 점을 알리기 위해 이 공지를 개정했습니다. 자세한 내용은 [MS12-006](https://technet.microsoft.com/ko-kr/security/bulletin/ms12-006)을 참조하십시오.

이 보안 업데이트는 MS12-006및 CVE-2011-3389과 어떤 관련이 있습니까?  
[SSL/TLS의 취약점으로 인한 정보 유출 문제점](https://go.microsoft.com/fwlink/?linkid=232510) 업데이트인 MS12-006은 WinHTTP에 영향을 미치는 SSL 및 TLS 프로토콜 취약점([CVE-2011-3389(영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-3389))을 해결하고 이러한 보호 효과가 시스템 전체에 적용될 수 있도록 합니다. 업데이트 MS11-099는 Internet Explorer에 대한 이러한 보호 효과를 허용합니다. 자세한 내용은[MS12-006](https://technet.microsoft.com/ko-kr/security/bulletin/ms12-006)을 참조하십시오.
...
다음의 심각도는 취약점의 잠재적인 최대 영향을 가정합니다. 이 보안 공지 발표 후 30일 이내의 심각도 및 보안 영향과 관련된 취약점 악용 가능성에 대해서는 [12월 공지 요약](https://technet.microsoft.com/security/bulletin/ms11-dec)의 악용 가능성 인덱스를 참조하십시오. 자세한 내용은 [Microsoft 악용 가능성 인덱스](https://technet.microsoft.com/ko-kr/security/cc998259.aspx)를 참조하십시오.
...
<th style="border:1px solid black;" >
XSS 필터 정보 유출 취약점(CVE-2011-1992)
</th>
<th style="border:1px solid black;" >
Internet Explorer의 안전하지 않은 라이브러리 로드 취약점(CVE-2011-2019)
</th>
<th style="border:1px solid black;" >
Content-Disposition 정보 유출 취약점(CVE-2011-3404)
</th>
<th style="border:1px solid black;" >
전체 심각도
</th>
...
Internet Explorer에 정보 유출 취약점이 존재합니다. 공격자는 악의적인 JavaScript 코드가 들어 있는 특수하게 조작된 웹 페이지를 생성하여 이 취약점을 악용할 수 있습니다. 이 취약점 악용에 성공한 공격자는 다른 도메인이나 Internet Explorer 영역의 콘텐츠를 볼 수 있습니다.

이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2011-1992(영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-1992)를 참조하십시오.

#### XSS 필터 정보 유출 취약점(CVE-2011-1992)에 대한 완화 요소
...
#### XSS 필터 정보 유출 취약점(CVE-2011-1992)에 대한 대안
...
-   XSS 필터 사용 안 함
    ...
    1.  다음 내용을 .REG 확장명의 파일(예: Disable\_XSS\_Filter.reg)로 저장합니다.
    2.  `Windows 레지스트리편집기버전 5.00[HKEY_LOCAL_MACHINE\SOFTWARE\Wow6432Node\Microsoft\Internet Explorer\MAIN\FeatureControl\FEATURE_XSSFILTER]"iexplorer.exe"=dword:00000000[HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Internet Explorer\MAIN\FeatureControl\FEATURE_XSSFILTER]"iexplorer.exe"=dword:00000000`
	3.  상승된 권한의 명령 프롬프트에서 다음 명령으로 Disable_XSS_Filter.reg 를 실행합니다.
	4.  `Regedit.exe /s Disable_XSS_Filter.reg`

    대안의 영향. Internet Explorer 8 및 Internet Explorer 9 XSS(사이트 간 스크립팅) 필터가 제공하는 보호를 사용할 수 없게 됩니다.

    대안 실행 취소 방법.

    상승된 권한의 명령 프롬프트에서 다음 명령을 실행하여 원래 상태로 복원합니다.

    1.  다음 내용을 .REG 확장명의 파일(예: Enable\_XSS\_Filter.reg)로 저장합니다.
    2.  `Windows 레지스트리편집기버전 5.00[HKEY_LOCAL_MACHINE\SOFTWARE\Wow6432Node\Microsoft\Internet Explorer\MAIN\FeatureControl\FEATURE_XSSFILTER]"iexplorer.exe"=dword:00000001[HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Internet Explorer\MAIN\FeatureControl\FEATURE_XSSFILTER]"iexplorer.exe"=dword:00000001`
	3.  상승된 권한의 명령 프롬프트에서 다음 명령으로 Enable_XSS_Filter.reg 를 실행합니다.
	4.  `Regedit.exe /s Enable_XSS_Filter.reg`

#### XSS 필터 정보 유출 취약점(CVE-2011-1992)에 대한 FAQ
...
취약점의 원인은 무엇입니까?  
Internet Explore XSS 필터는 공격자가 시행 착오 공격을 통해 여러 다른 도메인의 콘텐츠를 읽도록 허용할 수 있습니다.

Internet Explorer 8 및 Internet Explorer 9 사이트 간 스크립팅(XSS) 필터란 무엇입니까?  
사이트 간 스크립팅 공격은 사용 중인 웹 사이트에서 취약점 악용을 시도합니다. 사이트 간 스크립팅 공격은 새로운 주요 온라인 위협입니다. Internet Explorer 8 및 Internet Explorer 9에는 이러한 유형의 공격을 감지하고 유해한 스크립트를 사용하지 않도록 하는 사이트 간 스크립팅 필터가 포함되어 있습니다. 기본적으로 Internet Explorer 8 이상 버전은 인터넷 영역에서 사이트 간 스크립팅 필터를 사용하도록 설정되어 있습니다.

공격자는 취약점을 악용하여 무엇을 할 수 있습니까?  
이 취약점 악용에 성공한 공격자는 다른 도메인이나 Internet Explorer 영역의 콘텐츠를 볼 수 있습니다.

공격자는 이러한 취약점을 어떻게 악용합니까?  
공격자는 Internet Explorer를 통해 이 취약점을 악용하도록 설계하여 특수하게 조작된 웹 사이트를 호스팅한 다음 사용자가 이 웹 사이트를 보도록 유도할 수 있습니다. 공격자는 사용자가 제공한 콘텐츠가 광고를 허용하거나 호스팅하는 웹 사이트와 공격에 노출된 웹 사이트를 이용할 수도 있습니다. 이러한 웹 사이트에 이 취약점을 악용하는 특수하게 조작된 콘텐츠가 포함될 수 있습니다. 그러나 어떠한 경우에도 공격자는 강제로 사용자가 이러한 웹 사이트를 방문하도록 만들 수 없습니다. 대신 공격자는 사용자가 전자 메일 메시지 또는 메신저 메시지의 링크를 클릭하여 공격자의 웹 사이트를 방문하도록 유도하는 것이 일반적입니다.

배너 광고에서 이 취약점을 악용하도록 설계된 특수하게 조작된 웹 콘텐츠를 표시하거나 웹 콘텐츠를 전달하는 다른 방법을 사용하여 영향을 받는 시스템에 대한 공격을 시도할 수도 있습니다.

취약점으로 인해 위험에 노출되는 시스템은 주로 무엇입니까?  
이 취약점을 이용하여 악의적인 작업을 수행하려면 사용자가 해당 웹 사이트를 방문해야 합니다. 따라서 워크스테이션 또는 터미널 서버와 같이 Internet Explorer를 빈번하게 사용하는 시스템은 이 취약점을 이용하는 공격을 받을 위험이 가장 큽니다.
...
업데이트는 어떤 기능을 수행합니까?  
업데이트는 Internet Explorer XSS 필터 동작을 수정하여 이 취약점을 해결합니다.
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2011-2019(영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-2019)를 참조하십시오.

#### Internet Explorer의 안전하지 않은 라이브러리 로드 취약점(CVE-2011-2019)에 대한 완화 요소
...
#### Internet Explorer의 안전하지 않은 라이브러리 로드 취약점(CVE-2011-2019)에 대한 대안
...
#### Internet Explorer의 안전하지 않은 라이브러리 로드 취약점(CVE-2011-2019)에 대한 FAQ
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2011-3404(영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-3404)를 참조하십시오.

#### Content-Disposition 정보 유출 취약점(CVE-2011-3404)에 대한 완화 요소
...
#### Content-Disposition 정보 유출 취약점(CVE-2011-3404)에 대한 대안
...
-   인터넷 및 로컬 인트라넷 보안 영역 설정을 "높음"으로 설정하여 해당 영역에서 ActiveX 컨트롤과 Active 스크립팅 차단
    ...
    8.  에서 슬라이더를
    9.  높음
    10. 으로 이동합니다. 그러면 사용자가 방문하는 모든 웹 사이트에 대한 보안 수준을 높음으로 설정합니다.
...
#### Content-Disposition 정보 유출 취약점(CVE-2011-3404)에 대한 FAQ

이 취약점을 악용하면 어느 작업까지 가능합니까?  
이는 정보 유출 취약점입니다. 이 취약점을 악용한 공격자는 사용자가 웹 사이트를 방문할 때 브라우저를 통해 예상치 못한 작업을 수행함으로써 공격자의 웹 페이지 도메인이나 영역이 아닌 다른 도메인이나 Internet Explorer 영역의 콘텐츠를 볼 수 있습니다.
...
공격자는 이러한 취약점을 어떻게 악용합니까?  
공격자는 정보를 훔치려는 도메인에서 Internet Explorer를 통해 이 취약점을 악용하도록 디자인된 특수하게 조작된 웹 콘텐츠를 업로드하고 사용자가 웹 사이트를 방문하도록 유도할 수 있습니다. 공격자는 사용자가 제공한 콘텐츠가 광고를 허용하거나 호스팅하는 웹 사이트와 공격에 노출된 웹 사이트를 이용할 수도 있습니다. 이러한 웹 사이트에 이 취약점을 악용하는 특수하게 조작된 콘텐츠가 포함될 수 있습니다. 그러나 어떠한 경우에도 공격자는 강제로 사용자가 이러한 웹 사이트를 방문하고 콘텐츠를 다운로드하도록 만들 수 없습니다. 대신 공격자는 사용자가 전자 메일 메시지 또는 메신저 메시지의 링크를 클릭하여 공격자의 웹 사이트를 방문하고 콘텐츠를 다운로드하도록 유도하는 것이 일반적입니다.
...
<td style="border:1px solid black;"></td>
<td style="border:1px solid black;">지원 대상인 모든 Windows XP 에디션용 Internet Explorer 7:<br />
제어판의 프로그램 추가/제거 도구 또는 %Windir%\ie7updates\KB2618444-IE7\spuninst 폴더의 Spuninst.exe 유틸리티를 사용하십시오.</td>
</tr>
<tr class="even">
<td style="border:1px solid black;"></td>
<td style="border:1px solid black;">지원 대상인 모든 Windows XP 에디션용 Internet Explorer 8:<br />
제어판의 프로그램 추가/제거 도구 또는 %Windir%\ie8updates\KB2618444-IE8\spuninst 폴더의 Spuninst.exe 유틸리티를 사용하십시오.</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;">파일 정보</td>
<td style="border:1px solid black;"><a href="https://support.microsoft.com/kb/2618444">Microsoft 기술 자료 문서 2618444</a> 참조</td>
</tr>
<tr class="even">
...
<td style="border:1px solid black;">제거 정보</td>
<td style="border:1px solid black;">지원 대상인 모든 Windows Server 2003 32비트 에디션, 64비트 기반 에디션 및 Itanium 기반 에디션용 Internet Explorer 6:<br />
제어판의 프로그램 추가/제거 도구 또는 %Windir%\$NTUninstallKB2618444$\Spuninst 폴더의 Spuninst.exe 유틸리티를 사용하십시오.</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"></td>
<td style="border:1px solid black;">지원 대상인 모든 Windows Server 2003 32비트 에디션, x64 기반 에디션 및 Itanium 기반 에디션용 Internet Explorer 7:<br />
...
-   XSS 필터 정보 유출 취약점(CVE-2011-1992)을 보고해 주신 Thomas Stehle  
-   Internet Explorer의 안전하지 않은 라이브러리 로드 취약점(CVE-2011-2019)을 보고해 주신 [Citrix Security Team(영문)](https://www.citrix.com)의 Andy Cooper  
-   Content-Disposition 정보 유출 취약점(CVE-2011-3404)을 보고해 주신 [Google Inc.](https://www.google.com/)의 [Robert Swiecki(영문)](https://www.swiecki.net)  
-   Content-Disposition 정보 유출 취약점 취약점(CVE-2011-3404)을 해결하기 위해 협력해 주신 [Yosuke Hasegawa(영문)](https://utf-8.jp/)  
-   이 공지에 포함된 심층 보안 변경 사항에 대해 협력해 주신 [Jan Schejbal(영문)](https://janschejbal.wordpress.com/)
...
-   V1.1(2011년 12월 13일): CVE-2011-1992에 대한 완화 요소가 수정되었습니다. CVE-2011-3404의 심각도가 수정되었습니다. 이 변경 사항은 정보에만 해당됩니다. 보안 업데이트 파일이나 검색 로직에 대한 변경 사항은 없습니다.  
-   V1.2(2012년 1월 11일): 업데이트 MS11-099가 Internet Explorer에 대해 SSL/TLS의 취약점으로 인한 정보 유출 문제점 업데이트인 MS12-006에서 제공된 보호를 허용했다는 점을 알렸습니다. 자세한 내용은 Update FAQ를 참조하십시오.
