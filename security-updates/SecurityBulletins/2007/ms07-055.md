---
TOCTitle: 'MS07-055'
Title: 'Microsoft 보안 공지 MS07-055 - 긴급'
ms:assetid: 'ms07-055'
ms:contentKeyID: 61230094
ms:mtpsurl: 'https://technet.microsoft.com/ko-KR/library/ms07-055(v=Security.10)'
---


Microsoft 보안 공지 MS07-055 - 긴급
===================================
...
이 취약점은 Windows 2000을 실행하는 시스템에만 존재합니다. 그러나 Windows 2000에서 업그레이드한 지원되는 Windows XP 및 Windows Server 2003 32비트 에디션도 영향을 받을 수 있습니다. 따라서 Windows 2000 서비스 팩 4, Windows XP 서비스 팩 2 32비트 에디션 및 지원되는 32비트 Windows Server 2003 에디션에 대해서 긴급 보안 업데이트입니다. 자세한 내용은 **영향을 받는 소프트웨어 및 영향을 받지 않는 소프트웨어**를 참조하십시오.

이 보안 업데이트는 더 이상 지원되지 않는 파일 유형을 제거하고 Kodak 이미지 뷰어에서 특수하게 조작된 파일 유형을 처리하는 방식을 향상시켜 취약점을 해결합니다. 취약점에 대한 자세한 내용은 다음 섹션, **취약점 정보**에서 각 취약점 항목의 자주 제기되는 질문 사항(FAQ)를 참조하십시오.
...
|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------|-------------|---------------------------|
| [Microsoft Windows 2000 서비스 팩 4](https://www.microsoft.com/download/details.aspx?familyid=29763117-c2dc-4746-b31e-0b27350118e6&displaylang=ko)                                 | 원격 코드 실행 | 긴급        | 없음                      |
| [Windows XP 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=be52f740-e9c9-4228-95c0-00995213bbd0&displaylang=ko)                                             | 원격 코드 실행 | 긴급        | 없음                      |
| [Windows Server 2003 서비스 팩 1 및 Windows Server 2003 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=9a5c9e5d-4908-48bf-9346-745b4c6f6d4e&displaylang=ko) | 원격 코드 실행 | 긴급        | 없음                      |

**참고** Windows 2000에서 업그레이드하지 않았으며 지원되는 Windows XP 및 Windows Server 2003 에디션은 취약하지 않습니다.

**영향을 받지 않는 소프트웨어**

| 운영 체제                                                                                        |
|--------------------------------------------------------------------------------------------------|
| Windows XP x64 Edition 및 Windows XP x64 Edition 서비스 팩 2                                     |
...
**Windows XP 또는 Windows Server 2003를 실행하지만 Kodak 이미지 뷰어가 설치되어 있지 않습니다. 현재 취약한 상태입니까?**  
아닙니다. 처음으로 또는 전체 설치한 Windows XP 및 Windows 2003은 취약하지 않습니다. Windows 2000에서 업그레이드한 Windows XP 및 Windows Server 2003을 실행하는 시스템만 취약합니다.
...
<th style="border:1px solid black;" >Kodak 이미지 뷰어 원격 코드 실행 취약점 – CVE-2007-2217</th>
...
Kodak 이미지 뷰어 원격 코드 실행 취약점 – CVE-2007-2217  
-------------------------------------------------------
  
Windows용 Kodak 이미지 뷰어에서 특수하게 조작된 이미지 파일을 처리하는 방식에 원격 코드 실행 취약점이 존재합니다. 공격자는 사용자가 웹 사이트를 방문하거나 특수하게 조작된 전자 메일 메시지를 보거나 전자 메일 첨부 파일을 열어 볼 경우 원격 코드 실행을 허용할 수 있는 특수하게 조작된 이미지를 만들어 취약점을 악용할 수 있습니다. 이 취약점을 악용한 공격자는 영향을 받는 시스템을 완전히 제어할 수 있습니다.
  
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2007-2217 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2007-2217)을 참조하십시오.
  
#### Kodak 이미지 뷰어 원격 코드 실행 취약점 – CVE-2007-2217에 대한 완화 요소
  
완화란 취약점의 악용 심각도를 낮출 수 있는 설정, 일반적인 구성 또는 최선책, 기본 상태를 지칭합니다. 다음과 같은 완화 요소가 현재 상황에서 유용할 수 있습니다.
  
-   Windows 2000에서 업그레이드한 Windows XP 및 Windows Server 2003을 실행하는 시스템만 취약합니다.  
-   파일을 볼 때 ‘Windows 기본 폴더 사용’을 사용한 경우 사용자는 셸 기반 공격으로부터 보호됩니다.  
-   Office 2003은 파일 연결을 덮어쓰는 이미지 뷰어 응용 프로그램을 설치하므로 이 취약점으로 인해 취약해지지 않습니다.  
...
#### Kodak 이미지 뷰어 원격 코드 실행 취약점 – CVE-2007-2217에 대한 대안
...
-   **oieng400.dll 액세스 제어 목록 수정**
  
    oieng400.dll의 액세스 제어 목록(ACL)을 더 엄격하게 수정하려면 다음 절차를 따르십시오.
    ...
    **대안의 영향:** Kodak 이미지 뷰어가 열리지 않습니다.  
    **대안 실행 취소 방법:** 이 대안의 3단계에서 설명한 ACL을 복원합니다.
  
#### Kodak 이미지 뷰어 원격 코드 실행 취약점 – CVE-2007-2217에 대한 FAQ
...
**취약점의 원인은 무엇입니까?**  
Windows용 Kodak 이미지 뷰어에서 특수하게 조작된 이미지 파일의 구문을 제대로 분석하지 못합니다. 이로 인해 메모리가 손상되고 공격자가 로그온한 사용자의 컨텍스트에서 임의 코드를 실행할 수 있습니다.
...
이 취약점은 사용자가 특수하게 조작된 이미지 파일을 보거나 열어야 합니다. 영향을 받는 Kodak 이미지 뷰어 라이브러리를 사용하여 파일을 보거나 여는 응용 프로그램이 설치되어 있는 시스템은 위험합니다.
  
**업데이트는 어떤 기능을 수행합니까?**  
업데이트는 Kodak 이미지 뷰어의 특수하게 조작된 파일 유형 처리 방식을 향상시켜 취약점을 제거합니다.
...
| **배포**                  |                                                                                                                      |  
| 사용자 개입 없이 설치     | Windows2000-KB923810-x86-kor /quiet                                                                                  |  
| 다시 시작하지 않고 설치   | Windows2000-KB923810-x86-kor /norestart                                                                              |  
| 로그 파일 업데이트        | KB923810.log                                                                                                         |  
| 추가 정보                 | **검색, 배포 도구 및 지침**을 참조하십시오.                                                                          |  
| **다시 시작 요구 사항**   |                                                                                                                      |  
| 다시 시작 필요            | 예, 이 보안 업데이트를 적용한 후에는 컴퓨터를 다시 시작해야 합니다.                                                  |  
| 핫패칭                    | 해당 사항 없음                                                                                                       |  
| **제거 정보**             | 제어판의 프로그램 추가/제거 또는 %Windir%\\$NTUninstallKB9238109$\\Spuninst에 있는 SP 설치 제거 도구를 사용하십시오. |  
| **파일 정보**             | 전체 파일 목록은 다음 섹션에 있는 **파일 정보** 참조                                                                 |  
| **레지스트리 키 확인**    | HKEY\_LOCAL\_MACHINE\\SOFTWARE\\Microsoft\\Updates\\Windows 2000\\SP5\\KB923810\\Filelist                            |
...
| **배포**                  |                                                                                                                      |
| 사용자 개입 없이 설치     | WindowsXP-KB923810-x86-kor /quiet                                                                                    |
| 다시 시작하지 않고 설치   | WindowsXP-KB923810-x86-kor /norestart                                                                                |
| 로그 파일 업데이트        | KB923810.log                                                                                                         |
| 추가 정보                 | **검색, 배포 도구 및 지침**을 참조하십시오.                                                                          |
| **다시 시작 요구 사항**   |                                                                                                                      |
| 다시 시작 필요            | 예, 이 보안 업데이트를 적용한 후에는 컴퓨터를 다시 시작해야 합니다.                                                  |
...
| **제거 정보**             | 제어판의 프로그램 추가/제거 또는 %Windir%\\$NTUninstallKB9238109$\\Spuninst에 있는 SP 설치 제거 도구를 사용하십시오. |
| **파일 정보**             | 전체 파일 목록은 다음 섹션에 있는 **파일 정보** 참조                                                                 |
| **레지스트리 키 확인**    | HKEY\_LOCAL\_MACHINE\\SOFTWARE\\Microsoft\\Updates\\Windows XP\\SP3\\KB923810\\Filelist                              |

#### 파일 정보

이 보안 업데이트의 한국어 버전은 다음 표의 파일 특성을 가집니다. 이러한 파일의 날짜와 시간은 UTC(Coordinated Universal Time) 형식으로 나열되며 파일 정보를 볼 때 로컬 시간으로 변환됩니다. UTC와 로컬 시간의 차이를 확인하려면 제어판의 날짜 및 시간 도구에서 **표준 시간대** 탭을 사용하십시오.

지원 대상인 모든 Windows XP 에디션:
...
이 업데이트는 Microsoft Windows 2000에서 운영 체제를 업그레이드한 컴퓨터에만 설치됩니다. 컴퓨터에 Windows 2000이 설치된 적이 없으면 이 취약점으로 인해 취약해지지 않습니다.
...
| **향후 서비스 팩에 포함** | 이 문제점에 대한 업데이트는 향후 서비스 팩 또는 업데이트 롤업에 포함될 예정입니다.                                   |
| **배포**                  |                                                                                                                      |
| 사용자 개입 없이 설치     | WindowsServer2003-KB923810-x86-kor /quiet                                                                            |
| 다시 시작하지 않고 설치   | WindowsServer2003-KB923810-x86-kor /norestart                                                                        |
| 로그 파일 업데이트        | KB92810.log                                                                                                          |
| 추가 정보                 | **검색, 배포 도구 및 지침**을 참조하십시오.                                                                          |
| **다시 시작 요구 사항**   |                                                                                                                      |
| 다시 시작 필요            | 예, 이 보안 업데이트를 적용한 후에는 컴퓨터를 다시 시작해야 합니다.                                                  |
| 핫패칭                    | 해당 사항 없음                                                                                                       |
| **제거 정보**             | 제어판의 프로그램 추가/제거 또는 %Windir%\\$NTUninstallKB9238109$\\Spuninst에 있는 SP 설치 제거 도구를 사용하십시오. |
| **파일 정보**             | 전체 파일 목록은 다음 섹션에 있는 **파일 정보** 참조                                                                 |
| **레지스트리 키 확인**    | HKEY\_LOCAL\_MACHINE\\SOFTWARE\\Microsoft\\Updates\\Windows Server 2003\\SP3\\KB923810\\Filelist                     |

#### 파일 정보

이 보안 업데이트의 한국어 버전은 다음 표의 파일 특성을 가집니다. 이러한 파일의 날짜와 시간은 UTC(Coordinated Universal Time) 형식으로 나열되며 파일 정보를 볼 때 로컬 시간으로 변환됩니다. UTC와 로컬 시간의 차이를 확인하려면 제어판의 날짜 및 시간 도구에서 **표준 시간대** 탭을 사용하십시오.

지원 대상인 모든 Windows Server 2003 에디션:
...
-   Kodak 이미지 뷰어 원격 코드 실행 취약점(CVE-2007-2217)을 보고해 주신 Cu Fang
-   Kodak 이미지 뷰어 원격 코드 실행 취약점(CVE-2007-2217)을 보고해 주신 [Global 360 (영문)](https://www.global360.com/products/g360_imaging/default.asp)의 Rita Schappler
...
-   V1.1 (2007년 10월 17일): 영향을 받지 않는 소프트웨어에 Windows XP x64 Edition이 추가되어 공지가 업데이트되었습니다.
