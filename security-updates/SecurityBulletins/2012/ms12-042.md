---
TOCTitle: 'MS12-042'
Title: 'Microsoft 보안 공지 MS12-042 - 중요'
ms:assetid: 'ms12-042'
ms:contentKeyID: 61230509
ms:mtpsurl: 'https://technet.microsoft.com/ko-KR/library/ms12-042(v=Security.10)'
---

Microsoft 보안 공지 MS12-042 - 중요
===================================
...
이 보안 업데이트의 심각도는 지원 대상인 모든 Windows XP 및 Windows Server 2003 32비트 에디션, Windows 7(x64 기반 시스템용), Windows Server 2008 R2(x64 기반 시스템용)에 대해 중요입니다. 자세한 내용은 이 섹션에서 영향을 받는 소프트웨어 및 영향을 받지 않는 소프트웨어를 참조하십시오.

이 보안 업데이트는 Windows User Mode Scheduler가 특정 시스템 요청을 처리하는 방식과 Windows가 BIOS ROM을 관리하는 방식을 수정하여 취약점을 해결합니다. 취약점에 대한 자세한 내용은 다음 섹션, 취약점 정보에서 각 취약점 항목의 자주 제기되는 질문 사항(FAQ)을 참조하십시오.
...
<td style="border:1px solid black;">
[MS11-098](https://go.microsoft.com/fwlink/?linkid=232424)의 KB2633171은 KB2707511로 대체되었습니다.
</td>
</tr>
...
<td style="border:1px solid black;">
[MS11-068](https://go.microsoft.com/fwlink/?linkid=223578)의 KB2556532는 KB2709715로 대체되었습니다.
</td>
</tr>
<tr class="alternateRow">
<td style="border:1px solid black;">
...
Windows User Mode Scheduler가 시스템 요청을 처리하는 방식에 권한 상승 취약점이 존재합니다. 이 취약점 악용에 성공한 공격자는 커널 모드에서 임의 코드를 실행할 수 있습니다. 이렇게 되면 공격자가 프로그램을 설치할 수 있을 뿐 아니라 데이터를 보거나 변경하거나 삭제할 수 있고 전체 관리자 권한이 있는 새 계정을 만들 수도 있습니다.

이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2012-0217](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2012-0217)(영문)을 참조하십시오.

#### User Mode Scheduler 메모리 손상 취약점(CVE-2012-0217)에 대한 완화 요소

완화란 취약점의 악용 심각도를 낮출 수 있는 설정, 일반적인 구성 또는 최선책, 기본 상태를 지칭합니다. 다음과 같은 완화 요소가 현재 상황에서 유용할 수 있습니다.

-   이 취약점을 악용하려면 공격자가 유효한 로그온 자격 증명을 가지고 로컬로 로그온할 수 있어야 합니다. 익명의 사용자에 의해서나 원격으로는 이 취약점을 악용할 수 없습니다.
-   이 취약점은 Intel x64 기반 버전의 Windows 7 및 Windows Server 2008 R2에만 영향을 줍니다.
-   AMD 또는 ARM 기반 CPU가 탑재된 시스템은 이 취약점의 영향을 받지 않습니다.

#### User Mode Scheduler 메모리 손상 취약점(CVE-2012-0217)에 대한 대안

이 취약점에 대한 대안을 확인하지 못했습니다.

#### User Mode Scheduler 메모리 손상 취약점(CVE-2012-0217)에 대한 FAQ

이 취약점을 악용하면 어느 작업까지 가능합니까?  
이 취약점은 권한 상승 취약점입니다.

취약점의 원인은 무엇입니까?  
이 취약점은 Windows User Mode Scheduler가 특정 시스템 요청을 잘못 처리할 때 발생합니다.

User Mode Scheduler(UMS) 란 무엇입니까?  
User Mode Scheduler(UMS)는 시스템 API에 포함된 경량 메커니즘으로, 응용 프로그램이 자체 스레드를 스케줄링할 때 사용됩니다. 커널에서 UMS 커널이 차단된 경우 시스템 스케줄러가 관여하거나 프로세서 제어 권한을 되찾지 않고도 애플리케이션이 UMS 스레드 간 전환할 수 있습니다.
...
업데이트는 어떤 기능을 수행합니까?  
이 업데이트는 Windows User Mode Scheduler가 특정 시스템 요청을 처리하는 방식을 수정하여 취약점을 해결합니다.
...
Windows가 BIOS 메모리를 처리하는 방식에 권한 상승 취약점이 존재합니다. 이 취약점 악용에 성공한 공격자는 커널 모드에서 임의 코드를 실행할 수 있습니다. 이렇게 되면 공격자가 프로그램을 설치할 수 있을 뿐 아니라 데이터를 보거나 변경하거나 삭제할 수 있고 전체 관리자 권한이 있는 새 계정을 만들 수도 있습니다.

이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2012-1515](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2012-1515)(영문)를 참조하십시오.

#### BIOS ROM 손상 취약점(CVE-2012-1515)에 대한 완화 요소

완화란 취약점의 악용 심각도를 낮출 수 있는 설정, 일반적인 구성 또는 최선책, 기본 상태를 지칭합니다. 다음과 같은 완화 요소가 현재 상황에서 유용할 수 있습니다.

-   이 취약점을 악용하려면 공격자가 유효한 로그온 자격 증명을 가지고 로컬로 로그온할 수 있어야 합니다. 익명의 사용자에 의해서나 원격으로는 이 취약점을 악용할 수 없습니다.

#### BIOS ROM 손상 취약점(CVE-2012-1515)에 대한 대안

이 취약점에 대한 대안을 확인하지 못했습니다.

#### BIOS ROM 손상 취약점(CVE-2012-1515)에 대한 FAQ

이 취약점을 악용하면 어느 작업까지 가능합니까?  
이 취약점은 권한 상승 취약점입니다.

취약점의 원인은 무엇입니까?  
이 취약점은 Windows가 BIOS ROM을 잘못 보호할 때 발생합니다.
...
업데이트는 어떤 기능을 수행합니까?  
이 업데이트는 Windows가 BIOS ROM을 관리하는 방식을 수정하여 취약점을 해결합니다.

이 보안 공지를 게시했을 때 이 취약점이 공개되어 있었습니까?  
예. 이 취약점은 공개되었습니다. 이 취약점에는 CVE(Common Vulnerability and Exposure) 번호 [CVE-2012-1515](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2012-1515)(영문)가 지정되었습니다.
...
<td style="border:1px solid black;">제어판의 프로그램 추가/제거 항목 또는 %Windir%\$NTUninstallKB2707511$\Spuninst 폴더의 Spuninst.exe 유틸리티를 사용하십시오.</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;">파일 정보</td>
<td style="border:1px solid black;"><a href="https://support.microsoft.com/kb/2707511">Microsoft 기술 자료 문서 2707511</a> 참조</td>
</tr>
<tr class="even">
...
<tr class="even">
<td style="border:1px solid black;">파일 정보</td>
<td style="border:1px solid black;"><a href="https://support.microsoft.com/kb/2709715">Microsoft 기술 자료 문서 2709715</a> 참조</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;">레지스트리 키 확인</td>
...
-   User Mode Scheduler 메모리 손상 취약점(CVE-2012-0217)을 보고해 주신 [Bromium](https://www.bromium.com/)의 Rafal Wojtczuk 및 [SUSE](https://www.suse.com/)의 Jan Beulich
