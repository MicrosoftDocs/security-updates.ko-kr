...
<td style="border:1px solid black;">
[MS11-083](http://technet.microsoft.com/ko-kr/security/bulletin/ms11-083)의 KB2588516은 KB2688338로 대체되었습니다.
</td>
</tr>
...
<span></span>
다음의 심각도는 취약점의 잠재적인 최대 영향을 가정합니다. 이 보안 공지 발표 후 30일 이내의 심각도 및 보안 영향과 관련된 취약점 악용 가능성에 대해서는 [5월 공지 요약](http://technet.microsoft.com/ko-kr/security/bulletin/ms12-may)의 악용 가능성 인덱스를 참조하십시오. 자세한 내용은 [Microsoft 악용 가능성 인덱스](http://technet.microsoft.com/en-us/security/cc998259.aspx)를 참조하십시오.
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2012-0174(영문)](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2012-0174)를 참조하십시오.
...
기본 아웃바운드 방화벽 정책은 호스트에서 모든 아웃바운드 네트워크 트래픽을 허용하도록 구성됩니다. 원치 않는 아웃바운드 네트워크 트래픽을 차단하려면 고객이 트래픽 차단을 위한 방화벽 규칙을 만들어야 합니다. 아웃바운드 방화벽 규칙 추가에 대한 자세한 내용은 TechNet 문서 [원치 않는 아웃바운드 네트워크 트래픽을 차단하는 규칙 만들기](http://technet.microsoft.com/ko-kr/library/cc732306(v=ws.10).aspx)를 참조하십시오.
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2012-0179(영문)](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2012-0179)를 참조하십시오.
...
이 보안 공지를 게시했을 때 이 취약점이 공개되어 있었습니까? 
예. 이 취약점은 공개되었습니다. 이 취약점에는 CVE(Common Vulnerability and Exposure) 번호 [CVE-2012-0179(영문)](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2012-0179)가 지정되었습니다.
...
<td style="border:1px solid black;"><a href="http://support.microsoft.com/kb/2688338">Microsoft 기술 자료 문서 2688338</a> 참조</td>
</tr>
<tr class="odd">
...
-   Windows 방화벽 우회 취약점(CVE-2012-0174)을 보고해 주신 [INFIGO IS(영문)](http://www.infigo.hr/)의 Bojan Zdrnja  
-   TCP/IP Double Free 취약점(CVE-2012-0179)에 대해 함께 작업해 주신 [Genesys Telecommunications(영문)](http://www.genesyslab.com/)의 Anatoliy Glagolev
