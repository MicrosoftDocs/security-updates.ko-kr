...
-   Microsoft Windows 2000 서비스 팩 4 – [업데이트 다운로드](https://www.microsoft.com/download/details.aspx?familyid=3832ff23-6b04-4ca2-80b9-d344b4cc98ea&displaylang=ko)
...
|---------------------------------------------------------------------------------------------------------------|----------------------|--------------|
| Windows 커널 취약점 - [CAN-2005-2827 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=can-2005-2827) | 권한 상승            | 중요         |
...
Windows NT Workstation 4.0 서비스 팩 6a, Windows NT Server 4.0 서비스 팩 6a, Windows 2000 서비스 팩 2 및 Windows 2000 서비스 팩 3에 대한 지원 주기가 끝났습니다. 이러한 운영 체제 버전을 사용 중인 고객은 이후의 취약점에 노출되지 않도록 지원되는 버전으로 마이그레이션하는 것을 우선적으로 고려하십시오. Windows 제품 지원 주기에 대한 자세한 내용은 [Microsoft 지원 주기 정책 웹 사이트](https://go.microsoft.com/fwlink/?linkid=21742)를 참조하십시오. 이러한 운영 체제 버전의 확장 보안 업데이트 지원 기간에 대한 자세한 내용은 [Microsoft 기술 지원 서비스 웹 사이트](https://go.microsoft.com/fwlink/?linkid=33328)를 참조하십시오.
...
커널 내에서 비동기 프로시저 호출이 처리되는 방식에 [권한 상승](https://www.microsoft.com/korea/security/glossary.mspx) 취약점이 존재합니다. 이 취약점을 통해 로그온한 사용자가 시스템에 대한 전체 제어 권한을 얻을 수 있습니다.
...
이것은 [권한 상승](https://www.microsoft.com/korea/security/glossary.mspx) 취약점입니다. 이 취약점을 악용한 공격자는 영향을 받는 시스템을 완전히 제어할 수 있습니다. 이렇게 되면 공격자가 프로그램을 설치할 수 있을 뿐 아니라 데이터를 보거나 변경하거나 삭제할 수 있고 모든 사용자 권한이 있는 새 계정을 만들 수도 있습니다. 취약점을 악용하기 위해서는 공격자가 로컬로 시스템에 로그온하고 프로그램을 실행할 수 있어야 합니다.

**취약점의 원인은 무엇입니까?**  
APC(asynchronous procedure call) 큐 목록에서 항목을 처리하기 위해 사용되는 메소드가 원인입니다.

**APC(asynchronous procedure call)란 무엇입니까?**  
APC(Asynchronous Procedure Call)는 특정 스레드의 컨텍스트에서 비동기적으로 실행되는 함수입니다. 자세한 내용은 다음 [Microsoft 웹 사이트 (영문)](https://msdn.microsoft.com/library/default.asp?url=/library/en-us/dllproc/base/asynchronous_procedure_calls.asp)를 참조하십시오.
...
-   Windows 커널 취약점(CAN-2005-2827)을 보고해 주신 [eEye Digital Security (영문)](https://www.eeye.com/)에 감사드립니다.
