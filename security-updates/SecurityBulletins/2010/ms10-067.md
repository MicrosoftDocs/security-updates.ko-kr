---
TOCTitle: 'MS10-067'
Title: 'Microsoft 보안 공지 MS10-067 - 중요'
ms:assetid: 'ms10-067'
ms:contentKeyID: 61230328
ms:mtpsurl: 'https://technet.microsoft.com/ko-KR/library/ms10-067(v=Security.10)'
---


Microsoft 보안 공지 MS10-067 - 중요
===================================
...
|-------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------|-------------|---------------------------|
| [Windows XP 서비스 팩 3](https://www.microsoft.com/download/details.aspx?familyid=fc4369ec-864a-42ae-a850-b006872241fe&displaylang=ko)                      | 원격 코드 실행 | 중요        | 없음                      |
| [Windows XP Professional x64 Edition 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=7567986a-261d-4def-9fa5-c667994c7844)            | 원격 코드 실행 | 중요        | 없음                      |
| [Windows Server 2003 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=3290e7ee-1e4a-464c-abfd-17fc4108601e&displaylang=ko)             | 원격 코드 실행 | 중요        | 없음                      |
| [Windows Server 2003 x64 Edition 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=b21570cc-4f90-4ed8-b901-a34584d44a63&displaylang=ko) | 원격 코드 실행 | 중요        | 없음                      |
| [Windows Server 2003 SP2(Itanium 기반 시스템용)](https://www.microsoft.com/download/details.aspx?familyid=c9c4427d-54c8-4f3c-9a94-818196cd5180)             | 원격 코드 실행 | 중요        | 없음                      |
...
<th style="border:1px solid black;" >WordPad Word 97 텍스트 변환기 메모리 손상 취약점(CVE-2010-2563)</th>
...
특수하게 조작된 Word 97 문서를 구문 분석할 때 Microsoft WordPad에서 메모리를 처리하는 방식에 원격 코드 실행 취약점이 존재합니다. 이 취약점으로 인해 사용자가 잘못된 형식의 구조를 포함하는 특수하게 조작된 Word 파일을 열면 원격 코드 실행이 허용될 수 있습니다.
  
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2010-2563 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-2563)을 참조하십시오.
  
#### WordPad Word 97 텍스트 변환기 메모리 손상 취약점(CVE-2010-2563)에 대한 완화 요소
...
#### WordPad Word 97 텍스트 변환기 메모리 손상 취약점(CVE-2010-2563)에 대한 대안
...
-   변환기 파일에 대한 액세스를 제한하여 WordPad Word 97 텍스트 변환기를 사용하지 않도록 설정합니다.
    ...
    **참고** 이러한 명령 중 일부를 통해 오류 메시지가 발생할 수 있습니다. 사용 권한을 변경하려는 파일이 존재하지 않을 경우 이러한 오류 메시지가 발생할 수 있습니다.
...
#### WordPad Word 97 텍스트 변환기 메모리 손상 취약점(CVE-2010-2563)에 대한 FAQ
...
**취약점의 원인은 무엇입니까?**    
이 취약점은 WordPad 텍스트 변환기가 Word 97 문서의 특정 필드를 잘못 구문 분석할 때 발생합니다.
...
웹을 통한 공격의 경우 공격자는 이 취약점 악용 시도에 사용되는 Word 97 문서를 포함한 웹 사이트를 호스팅해야 합니다. 또한 사용자가 제공한 콘텐츠를 허용하거나 호스팅하는 공격 당한 웹 사이트에는 이 취약점을 악용할 수 있는 특수하게 조작된 콘텐츠가 포함되어 있을 수 있습니다. 공격자는 강제로 사용자가 특수하게 조작된 웹 사이트를 방문하도록 만들 수 없습니다. 대신, 공격자는 사용자가 공격자의 사이트로 연결되는 링크를 눌러 공격자의 웹 사이트를 방문하도록 유인해야 합니다. 확인 메시지가 표시되며 사용자는 문서를 수동으로 열도록 수락해야 합니다.
  
**취약점으로 인해 주로 공격받을 위험이 있는 시스템은 무엇입니까?**    
WordPad를 사용하여 신뢰할 수 없는 출처에서 Word 문서를 연 워크스테이션, 터미널 서버와 같은 영향을 받는 시스템이 주로 위험합니다. 관리자가 사용자가 서버에 로그온하여 프로그램을 실행할 수 있도록 허용할 경우, 서버가 더 위험할 수 있습니다. 가장 좋은 방법은 이러한 기능을 허용하지 않는 것입니다. Microsoft Word가 설치된 시스템은 일반적으로 WordPad 대신 Microsoft Office로 Word 문서를 엽니다.
...
<td style="border:1px solid black;">제어판의 <strong>프로그램 추가/제거</strong> 도구 또는 %Windir%\$NTUninstallKB2259922$\Spuninst 폴더의 Spuninst.exe 유틸리티를 사용하십시오.</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"><strong>파일 정보</strong></td>
<td style="border:1px solid black;"><a href="https://support.microsoft.com/kb/2259922">Microsoft 기술 자료 문서 2259922</a> 참조</td>
</tr>
<tr class="even">
...
-   WordPad Word 97 텍스트 변환기 메모리 손상 취약점(CVE-2010-2563)을 보고해 주신 [iDefense Labs (영문)](https://labs.idefense.com/)의 S0lute
