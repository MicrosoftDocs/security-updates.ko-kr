---
TOCTitle: 'MS12-047'
Title: 'Microsoft 보안 공지 MS12-047 - 중요'
ms:assetid: 'ms12-047'
ms:contentKeyID: 61230514
ms:mtpsurl: 'https://technet.microsoft.com/ko-KR/library/ms12-047(v=Security.10)'
---

Microsoft 보안 공지 MS12-047 - 중요
===================================
...
이 보안 업데이트는 후크 프로시저를 만들 때 Windows 커널 모드 드라이버가 키보드 레이아웃 파일을 처리하고 콜백 매개 변수의 유효성을 검사하는 방식을 수정함으로써 상기 취약점을 해결합니다. 취약점에 대한 자세한 내용은 다음 섹션, 취약점 정보에서 각 취약점 항목의 자주 제기되는 질문 사항(FAQ)을 참조하십시오.
...
<td style="border:1px solid black;">
[MS12-041](https://go.microsoft.com/fwlink/?linkid=251707)의 KB2709162는 KB2718523으로 대체되었습니다.
</td>
</tr>
...
<tr>
<td style="border:1px solid black;">
[Windows Server 2008(32비트 시스템용) 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=ed201422-7c65-4c20-a825-8cecab1aeebc)(Server Core 설치)  
(KB2718523)
</td>
<td style="border:1px solid black;">
권한 상승
...
<tr class="alternateRow">
<td style="border:1px solid black;">
[Windows Server 2008(x64 기반 시스템용) 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=8e1a27c9-6495-4030-8a46-264afd78a5a1)(Server Core 설치)  
(KB2718523)
</td>
<td style="border:1px solid black;">
권한 상승
...
<tr>
<td style="border:1px solid black;">
[Windows Server 2008 R2(x64 기반 시스템용)](https://www.microsoft.com/download/details.aspx?familyid=73a9ff32-4009-4fd4-a82b-1e22c09d3087)(Server Core 설치)   
(KB2718523)
</td>
<td style="border:1px solid black;">
권한 상승
...
<tr class="alternateRow">
<td style="border:1px solid black;">
[Windows Server 2008 R2(x64 기반 시스템용) 서비스 팩 1](https://www.microsoft.com/download/details.aspx?familyid=73a9ff32-4009-4fd4-a82b-1e22c09d3087)(Server Core 설치)  
(KB2718523)
</td>
<td style="border:1px solid black;">
권한 상승
...
이 보안 업데이트의 SHA1 및 SHA2 해시는 다운로드한 보안 업데이트 패키지의 진위 여부를 검증하는 데 사용할 수 있습니다. 이 업데이트와 관련된 해시 정보는 [Microsoft 기술 자료 문서 2718523](https://support.microsoft.com/kb/2718523)을 참조하십시오.
...
Windows 커널 모드 드라이버가 특정 키보드 레이아웃을 관리하는 방식에 권한 상승 취약점이 존재합니다. 이 취약점 악용에 성공한 공격자는 커널 모드에서 임의 코드를 실행할 수 있습니다. 이렇게 되면 공격자가 프로그램을 설치할 수 있을 뿐 아니라 데이터를 보거나 변경하거나 삭제할 수 있고 전체 관리자 권한이 있는 새 계정을 만들 수도 있습니다.

이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2012-1890](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2012-1890)(영문)을 참조하십시오.

#### 키보드 레이아웃 취약점(CVE-2012-1890)에 대한 완화 요소

완화란 취약점의 악용 심각도를 낮출 수 있는 설정, 일반적인 구성 또는 최선책, 기본 상태를 지칭합니다. 다음과 같은 완화 요소가 현재 상황에서 유용할 수 있습니다.

-   이 취약점을 악용하려면 공격자가 유효한 로그온 자격 증명을 가지고 로컬로 로그온할 수 있어야 합니다.

#### 키보드 레이아웃 취약점(CVE-2012-1890)에 대한 대안

이 취약점에 대한 대안을 확인하지 못했습니다.

#### 키보드 레이아웃 취약점(CVE-2012-1890)에 대한 FAQ
...
예. 이 취약점은 공개되었습니다. 이 취약점에는 CVE(Common Vulnerability and Exposure) 번호 [CVE-2012-1890](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2012-1890)(영문)이 지정되었습니다.
...
후크 프로시저를 생성할 때 Windows 커널 모드 드라이버가 매개 변수의 유효성 검사를 부적절하게 처리하는 경우 권한 상승 취약점이 존재합니다. 이 취약점 악용에 성공한 공격자는 커널 모드에서 임의 코드를 실행할 수 있습니다. 이렇게 되면 공격자가 프로그램을 설치할 수 있을 뿐 아니라 데이터를 보거나 변경하거나 삭제할 수 있고 모든 사용자 권한이 있는 새 계정을 만들 수도 있습니다.

이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2012-1893](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2012-1893)(영문)을 참조하십시오.

#### Win32k 부정확한 유형 처리 취약점(CVE-2012-1893)에 대한 완화 요소

완화란 취약점의 악용 심각도를 낮출 수 있는 설정, 일반적인 구성 또는 최선책, 기본 상태를 지칭합니다. 다음과 같은 완화 요소가 현재 상황에서 유용할 수 있습니다.

-   이 취약점을 악용하려면 공격자가 유효한 로그온 자격 증명을 가지고 로컬로 로그온할 수 있어야 합니다.

#### Win32k 부정확한 유형 처리 취약점(CVE-2012-1893)에 대한 대안

이 취약점에 대한 대안을 확인하지 못했습니다.

#### Win32k 부정확한 유형 처리 취약점(CVE-2012-1893)에 대한 FAQ

이 취약점을 악용하면 어느 작업까지 가능합니까?  
이 취약점은 권한 상승 취약점입니다.

취약점의 원인은 무엇입니까?  
후크 프로시저를 생성할 때 Windows 커널 모드 드라이버가 매개 변수 유효성 검사를 잘못 처리하는 경우에 이 취약점이 발생합니다.
...
업데이트는 어떤 기능을 수행합니까?  
이 업데이트는 후크 프로시저를 생성할 때 Windows 커널 모드 드라이버가 콜백 매개 변수 유효성 검사를 처리하는 방식을 수정함으로써 취약점을 해결합니다.
...
<td style="border:1px solid black;">제어판의 프로그램 추가/제거 항목 또는 %Windir%\$NTUninstallKB2718523$\Spuninst 폴더의 Spuninst.exe 유틸리티를 사용하십시오.</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;">파일 정보</td>
<td style="border:1px solid black;"><a href="https://support.microsoft.com/kb/2718523">Microsoft 기술 자료 문서 2718523</a> 참조</td>
</tr>
<tr class="even">
...
<tbody>
<tr class="odd">
<td style="border:1px solid black;">향후서비스 팩<strong><em>에</em><em></em><em>포함</em><em></em></strong></td>
<td style="border:1px solid black;">이 문제점에 대한 업데이트는 향후 서비스 팩 또는 업데이트 롤업에 포함될 예정입니다.</td>
</tr>
<tr class="even">
<td style="border:1px solid black;">배포</td>
...
-   키보드 레이아웃 취약점(CVE-2012-1890)을 보고해 주신 [Core Security Technologies](https://www.coresecurity.com/)의 Nicolas Economou  
-   키보드 레이아웃 취약점(CVE-2012-1890)을 보고해 주신 [Qihoo 360 Security Center](https://www.360.cn/)  
-   Win32k 부정확한 유형 처리 취약점(CVE-2012-1893)을 [Secunia Research](https://secunia.com/)와 협력하여 보고해 주신 Neusoft Corporation의 Lufeng Li
