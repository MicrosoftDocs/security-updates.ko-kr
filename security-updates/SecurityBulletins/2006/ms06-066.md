---
TOCTitle: 'MS06-066'
Title: 'Microsoft 보안 공지 MS06-066 - 중요'
ms:assetid: 'ms06-066'
ms:contentKeyID: 61230027
ms:mtpsurl: 'https://technet.microsoft.com/ko-KR/library/ms06-066(v=Security.10)'
---

Microsoft 보안 공지 MS06-066 - 중요
===================================

Client Service for NetWare의 취약점으로 인한 원격 코드 실행 문제점(923980)
--------------------------------------------------------------------------
...
-   Microsoft Windows 2000 서비스 팩 4 - [업데이트 다운로드](https://www.microsoft.com/download/details.aspx?familyid=3cf0b0d1-ff07-40ac-a6ac-44dc4a54f91e&displaylang=ko)
-   Microsoft Windows XP 서비스 팩 2 - [업데이트 다운로드](https://www.microsoft.com/download/details.aspx?familyid=2f54258f-1071-467b-80a2-e4dbfc050667&displaylang=ko)
-   Microsoft Windows Server 2003 및 Microsoft Windows Server 2003 서비스 팩 1 - [업데이트 다운로드](https://www.microsoft.com/download/details.aspx?familyid=f23574f7-4033-45ac-8ad8-6cced2ee9285&displaylang=ko)
...
이 업데이트는 새로 발견되어 비공개적으로 보고된 취약점을 해결합니다. 각 취약점은 이 공지의 "취약점 세부 사항" 섹션의 해당 항목에 설명되어 있습니다.

Client Service for NetWare는 Windows 2000 Server에서 Gateway Service for NetWare라고도 부릅니다.

취약한 Microsoft Windows 버전에서 이 취약점 악용에 성공한 공격자는 클라이언트 워크스테이션을 원격으로 완전히 제어할 수 있습니다. 이렇게 되면 공격자가 프로그램을 설치할 수 있을 뿐 아니라 데이터를 보거나 변경하거나 삭제할 수 있고 모든 사용자 권한이 있는 새 계정을 만들 수도 있습니다.

가능한 빨리 이 업데이트를 설치할 것을 권장합니다.

**심각도 및 취약점 확인:**

| 취약점                                                                                                                                            | 취약점으로 인한 영향 | Windows 2000 서비스 팩 4 | Windows XP Professional 서비스 팩 2 | Windows Server 2003 및 Windows Server 2003 서비스 팩 1 |
|---------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|--------------------------|-------------------------------------|--------------------------------------------------------|
| Microsoft Client Service for NetWare 메모리 손상 취약점 - [CVE-2006-4688 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2006-4688) | 원격 코드 실행       | 중요                     | 중요                                | 보통                                                   |
| NetWare 드라이버 서비스 거부 취약점 - [CVE-2006-4689 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2006-4689)                     | 서비스 거부          | 보통                     | 보통                                | 낮음                                                   |
| **모든 취약점의 전체 심각도**                                                                                                                     | ** **                | **중요**                 | **중요**                            | **보통**                                               |
...
| 공지 번호    | Windows 2000 서비스 팩 4 | Windows XP Professional 서비스 팩 2 | Windows Server 2003 및 Windows Server 2003 서비스 팩 1 |
|--------------|--------------------------|-------------------------------------|--------------------------------------------------------|
| **MS05-046** | 대체되지 않음            | 대체됨                              | 대체되지 않음                                          |
...
#### Client Service for NetWare 메모리 손상 취약점 - CVE-2006-4688:

CSNW(Client Service for NetWare)에 원격 코드 실행 취약점이 존재하며, 이 취약점을 악용한 공격자는 영향을 받는 시스템을 완전히 제어할 수 있습니다.

#### Client Service for NetWare 메모리 손상 취약점 - CVE-2006-4688에 대한 완화 요소:

-   Windows XP Home Edition은 이 문제점에 취약하지 않습니다. Windows XP Home Edition에는 이 취약한 구성 요소가 포함되어 있지 않습니다.
-   영향을 받는 시스템에 대한 공격을 수행하려면 공격자는 Windows Server 2003 및 Windows Server 2003 서비스 팩 1에서 유효한 로그온 자격 증명을 지닌 인증된 사용자여야 합니다.
...
-   기본적으로 Client Service for NetWare는 영향을 받는 운영 체제 버전에 설치되어 있지 않습니다. 이 서비스를 설치한 고객만 이 문제점에 취약할 가능성이 있습니다.

#### Client Service for NetWare 메모리 손상 취약점 - CVE-2006-4688에 대한 대안:
...
**참고** 일반적으로 CSNW는 IPX(Internetwork Packet Exchange) 및 SPX(Sequenced Packet Exchange) 프로토콜과 연결되어 있습니다. 그러나 이러한 설치된 프로토콜을 사용하여 CSNW를 악용할 수는 없습니다. 다음의 대안은 가장 일반적으로 사용하는 프로토콜인 TCP/IP를 기준으로 합니다. IPX, SPX 등의 프로토콜을 사용하는 경우, 이러한 프로토콜이 사용하는 포트도 필요한 경우 차단해야 합니다. IPX 및 SPX에 대한 자세한 내용은 이 사이트를 방문하십시오.

-   **필요하지 않은 경우, Client Service for NetWare를 제거합니다.**

    Client Service for NetWare가 필요하지 않은 경우, 제거합니다. 이를 수행하려면 다음 절차를 따르십시오.
    ...
    **Windows 2000**
    ...
    1.  관리자로 로그인합니다.
    2.  **내 네트워크 환경**을 마우스 오른쪽 단추로 클릭합니다.
    3.  **등록 정보**를 클릭합니다.
    4.  **로컬 영역 연결**을 클릭합니다.
    5.  **CSNW**를 강조 표시하고 **설치 제거**를 클릭합니다.
    6.  팝업 대화 상자가 표시되면 **예**를 클릭합니다.
...
-   **방화벽에서 TCP 포트 139 및 445 차단:**
    ...
    이 포트들은 영향을 받는 구성 요소와 연결을 시작하는 데 사용됩니다. 방화벽에서 TCP 포트 139 및 445를 차단하면 이 취약점을 악용한 방화벽 뒤의 시스템 공격을 막을 수 있습니다. 기타 포트를 악용할 수 있는 공격을 방지하려면 들어오는 방향의 원치 않는 인터넷 통신을 모두 차단하는 것이 좋습니다. 이 포트에 대한 자세한 내용은 다음 [웹 사이트](https://go.microsoft.com/fwlink/?linkid=21312)를 참조하십시오.
...
-   **네트워크를 통해 취약점을 악용하려는 시도로부터 보호하려면 Windows XP에 포함되어 있는** [**Windows 방화벽**](https://www.microsoft.com/korea/athome/security/default.mspx)**과 같은 개인 방화벽을 사용합니다.**
...
#### Client Service for NetWare 메모리 손상 취약점 - CVE-2006-4688에 대한 FAQ:
...
**이 취약점은 MS05-046으로 해결된 취약점과 어떤 관계가 있습니까?**  
두 취약점 모두 NetWare Client service에 존재합니다. 그러나 이 업데이트는 MS05-046으로 해결되지 않은 새로운 취약점을 해결합니다. MS05-046은 해당 공지에서 설명한 취약점으로부터 보호하지만 이 새로운 취약점은 해결하지 못합니다. 이 업데이트는 Windows XP 서비스 팩 2에서만 MS005-046을 대체합니다. 영향을 받는 다른 시스템을 두 취약점으로부터 모두 보호하려면 이 업데이트와 MS05-046을 설치해야 합니다.

#### NetWare 드라이버 서비스 거부 취약점 - CVE-2006-4689:

CSNW(Client Services for NetWare)에 서비스 거부 취약점이 존재하여 Client Services for NetWare 서비스를 실행하는 영향을 받는 시스템에 공격자가 특수하게 조작된 네트워크 메시지를 보낼 수 있습니다. 공격자는 시스템이 응답을 중지하도록 만들 수 있습니다.

#### NetWare 드라이버 서비스 거부 취약점 - CVE-2006-4689에 대한 완화 요소:

-   Windows XP Home Edition Service Pack 2 및 Windows XP Media Center Edition 2005는 취약한 구성 요소가 포함되어 있지 않으며, 이 문제점에 취약하지 않습니다.
-   영향을 받는 시스템에 대한 공격을 수행하려면 공격자는 Windows Server 2003 및 Windows Server 2003 서비스 팩 1에서 유효한 로그온 자격 증명을 지닌 인증된 사용자여야 합니다.
-   영향을 받은 구성 요소가 필요한 고객의 경우, 최상의 방화벽 구성 방법과 표준 기본값 방화벽 구성을 이용하면 기업 경계 외부에서 들어오는 공격으로부터 네트워크를 보호할 수 있습니다. 인터넷과 연결되는 시스템의 경우, 필요한 포트만 최소한으로 열어 두는 것이 안전합니다.
-   기본적으로 Client Service for NetWare는 영향을 받는 운영 체제 버전에 설치되어 있지 않습니다. 이 서비스를 수동으로 설치한 고객만 이 문제점에 취약할 가능성이 있습니다.

#### NetWare 드라이버 서비스 거부 취약점 - CVE-2006-4689에 대한 대안:

Microsoft는 다음과 같은 대안을 테스트했습니다. 이러한 대안이 취약점 자체를 해결하지는 않지만 알려진 공격 경로를 차단하는 데 도움이 됩니다. 대안으로 인해 기능이 저하되는 경우는 아래 섹션에 별도로 표시하였습니다.

**참고** 일반적으로 CSNW는 IPX(Internetwork Packet Exchange) 및 SPX(Sequenced Packet Exchange) 프로토콜과 연결되어 있습니다. 그러나 이러한 설치된 프로토콜을 사용하여 CSNW를 악용할 수는 없습니다. 다음의 대안은 가장 일반적으로 사용하는 프로토콜인 TCP/IP를 기준으로 합니다. IPX, SPX 등의 프로토콜을 사용하는 경우, 이러한 프로토콜이 사용하는 포트도 필요한 경우 차단해야 합니다. IPX 및 SPX에 대한 자세한 내용은 다음 Microsoft 웹 사이트를 참조하십시오.
...
-   **네트워크를 통해 취약점을 악용하려는 시도로부터 보호하려면 Windows XP에 포함되어 있는** [**Windows 방화벽**](https://www.microsoft.com/korea/athome/security/default.mspx)**과 같은 개인 방화벽을 사용합니다.**
    ...
    기본적으로 Windows XP 및 Windows Server 2003의 [Windows 방화벽 기능](https://www.microsoft.com/korea/athome/security/default.mspx)은 들어오는 방향의 원치 않는 트래픽을 차단하여 인터넷 연결을 보호합니다. 들어오는 방향의 원치 않는 인터넷 통신을 모두 차단하는 것이 좋습니다.
...
#### NetWare 드라이버 서비스 거부 취약점 - CVE-2006-4689에 대한 FAQ:

**이 취약점을 악용하면 어느 작업까지 가능합니까?**  
이것이 바로 서비스 거부 취약점입니다. 공격자는 이 취약점을 악용하여 영향을 받는 시스템이 응답을 중지하게 하고, 서버가 자동으로 다시 시작되도록 만듭니다. 이 시간 동안 서버는 요청에 응답할 수 없습니다. 서비스 거부 취약점으로 인해 공격자가 코드를 실행하거나 사용자 권한이 상승되는 것은 아니지만 영향을 받는 시스템이 요청 수신을 중단하도록 만들 수 있습니다.
...
**누가 취약점을 악용할 수 있습니까?**  
Windows 2000 및 Windows XP 서비스 팩 2에서 특수하게 조작한 메시지를 영향을 받는 시스템에 전달할 수 있는 모든 익명의 사용자가 이 취약점을 악용할 수 있습니다. Windows Server 2003에서 공격자가 이 취약점 악용에 성공하려면 유효한 로그온 자격 증명이 필요합니다.

**공격자는 이러한 취약점을 어떻게 악용합니까?**  
공격자는 특수하게 조작한 일련의 네트워크 메시지를 만들어 영향을 받는 시스템에 보낸 다음, 직접 네트워크를 통해 취약점을 악용할 수 있습니다. 메시지로 인해 영향을 받는 시스템이 응답하지 않게 됩니다.
...
**Windows2000-KB923980-x86-KOR /quiet**

**참고** **/quiet** 스위치를 사용하면 모든 메시지가 표시되지 않습니다. 마찬가지로 실패 메시지도 표시되지 않습니다. 관리자는 지원되는 방법 중 하나를 사용하여 **/quiet** 스위치를 사용할 때 설치가 제대로 되었는지 확인해야 합니다. 또한 관리자는 이 스위치를 사용할 때 KB923980.log 파일에서 모든 실패 메시지를 검토해야 합니다.

컴퓨터를 다시 시작하지 않고 보안 업데이트를 설치하려면 Windows 2000 서비스 팩 4 명령 프롬프트에서 다음 명령을 사용하십시오.

**Windows2000-KB923980-x86-KOR /norestart**
...
시스템 관리자는 Spuninst.exe 유틸리티를 사용하여 이 보안 업데이트를 제거할 수도 있습니다. Spuninst.exe 유틸리티는 %Windir%\\$NTUninstallKB923980 $\\Spuninst 폴더에 있습니다.
...
**Windowsxp-KB923980-x86-KOR /quiet**

**참고** **/quiet** 스위치를 사용하면 모든 메시지가 표시되지 않습니다. 마찬가지로 실패 메시지도 표시되지 않습니다. 관리자는 지원되는 방법 중 하나를 사용하여 **/quiet** 스위치를 사용할 때 설치가 제대로 되었는지 확인해야 합니다. 또한 관리자는 이 스위치를 사용할 때 KB923980.log 파일에서 모든 실패 메시지를 검토해야 합니다.

컴퓨터를 다시 시작하지 않고 보안 업데이트를 설치하려면 Windows XP 명령 프롬프트에서 다음 명령을 사용하십시오.

**Windowsxp-KB923980-x86-KOR /norestart**
...
Windows XP Professional 서비스 팩 2 및 Windows XP Tablet PC Edition 2005:
...
**Windowsserver2003-KB923980-x86-KOR /quiet**

**참고** **/quiet** 스위치를 사용하면 모든 메시지가 표시되지 않습니다. 마찬가지로 실패 메시지도 표시되지 않습니다. 관리자는 지원되는 방법 중 하나를 사용하여 **/quiet** 스위치를 사용할 때 설치가 제대로 되었는지 확인해야 합니다. 또한 관리자는 이 스위치를 사용할 때 KB923980.log 파일에서 모든 실패 메시지를 검토해야 합니다.

컴퓨터를 다시 시작하지 않고 보안 업데이트를 설치하려면 Windows Server 2003 명령 프롬프트에서 다음 명령을 사용하십시오.

**Windowsserver2003-KB923980-x86-KOR /norestart**
...
-   Client Service for NetWare 메모리 손상 취약점(CVE-2006-4688)을 보고해 주신 [NGS Software (영문)](https://www.ngssoftware.com/)의 Peter Winter-Smith
-   Client Service for NetWare 메모리 손상 취약점(CVE-2006-4688) 및 Client Service for NetWare 드라이버 서비스 거부 취약점(CVE-2006-4689)을 보고해 주신 [McAfee (영문)](https://www.mcafee.com/us)의 Sam Arun Raj
