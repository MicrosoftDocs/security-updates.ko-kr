---
TOCTitle: 'MS13-077'
Title: 'Microsoft 보안 공지 MS13-077 - 중요'
ms:assetid: 'ms13-077'
ms:contentKeyID: 61230627
ms:mtpsurl: 'https://technet.microsoft.com/ko-KR/library/ms13-077(v=Security.10)'
---

Microsoft 보안 공지 MS13-077 - 중요
===================================

Windows 서비스 제어 관리자의 취약점으로 인한 권한 상승 문제점 (2872339)
-----------------------------------------------------------------------

게시된 날짜: 2013년 9월 11일 수요일 | 업데이트된 날짜: 2013년 9월 12일 목요일
...
이 보안 업데이트는 손상된 서비스 설명이 제공될 때 SCM(서비스 제어 관리자)이 메모리의 개체를 처리하는 방식을 수정하여 취약점을 해결합니다. 취약점에 대한 자세한 내용은 취약점 정보에서 각 취약점 항목의 자주 제기되는 질문 사항(FAQ)을 참조하십시오.
...
| 기술 자료 문서 | [2872339](https://support.microsoft.com/kb/2872339) |
|----------------|-----------------------------------------------------|
| 파일 정보      | 예                                                  |
| SHA1/SHA2 해시 | 예                                                  |
| 알려진 문제점  | 예                                                  |
...
[Windows 7(32비트 시스템용) 서비스 팩 1](https://www.microsoft.com/download/details.aspx?familyid=5ed61bc5-9467-4b25-9efd-93f99ff59f51&displaylang=ko)  
(2872339)
</td>
<td style="border:1px solid black;">
권한 상승
...
<td style="border:1px solid black;">
[Windows 7(x64 기반 시스템용) 서비스 팩 1](https://www.microsoft.com/download/details.aspx?familyid=ad6c821f-eef9-4cff-b1cd-b1c0beb3c41b&displaylang=ko)  
(2872339)
</td>
<td style="border:1px solid black;">
권한 상승
...
<td style="border:1px solid black;">
[MS13-063](https://go.microsoft.com/fwlink/?linkid=309338)의 2859537
</td>
</tr>
<tr>
<th colspan="4">
...
<tr>
<td style="border:1px solid black;">
[Windows Server 2008 R2(x64 기반 시스템용) 서비스 팩 1](https://www.microsoft.com/download/details.aspx?familyid=b5b99216-409e-451c-b21a-3a0e2c2f417f&displaylang=ko)  
(2872339)
</td>
<td style="border:1px solid black;">
권한 상승
...
<tr class="alternateRow">
<td style="border:1px solid black;">
[Windows Server 2008 R2(Itanium 기반 시스템용) 서비스 팩 1](https://www.microsoft.com/download/details.aspx?familyid=6ebcf7a2-486b-425e-a068-948adf1860de)  
(2872339)
</td>
<td style="border:1px solid black;">
권한 상승
...
<tr>
<td style="border:1px solid black;">
[Windows Server 2008 R2(x64 기반 시스템용) 서비스 팩 1](https://www.microsoft.com/download/details.aspx?familyid=b5b99216-409e-451c-b21a-3a0e2c2f417f&displaylang=ko)(Server Core 설치)  
(2872339)
</td>
<td style="border:1px solid black;">
권한 상승
...
<th style="border:1px solid black;" >
서비스 제어 관리자 Double Free 취약점(CVE-2013-3862)
</th>
<th style="border:1px solid black;" >
전체 심각도
</th>
...
<span></span>
Windows SCM(서비스 제어 관리자)이 메모리의 개체를 처리하는 방식에 권한 상승 취약점이 존재합니다. 이 취약점 악용에 성공한 공격자는 임의 코드를 실행하여 영향을 받는 시스템을 완전히 제어할 수 있습니다. 이렇게 되면 공격자가 프로그램을 설치할 수 있을 뿐 아니라 데이터를 보거나 변경하거나 삭제할 수 있고 모든 사용자 권한이 있는 새 계정을 만들 수도 있습니다.

이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2013-3862](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2013-3862)(영문)를 참조하십시오.

#### 완화 요소

완화란 취약점의 악용 심각도를 낮출 수 있는 설정, 일반적인 구성 또는 최선책, 기본 상태를 지칭합니다. 다음과 같은 완화 요소가 현재 상황에서 유용할 수 있습니다.

-   이 취약점을 악용하려면 공격자가 유효한 로그온 자격 증명을 갖고 있고 로컬로 로그온할 수 있거나, 공격자가 특수하게 조작한 응용 프로그램을 실행하도록 사용자를 유도해야 합니다.
...
취약점의 원인은 무엇입니까?  
이 취약점은 SCM(서비스 제어 관리자)이 Windows 레지스트리에서 손상된 서비스 설명을 검색할 때 "double free" 조건이 되기 때문에 발생합니다.

"double free" 조건이란 무엇입니까?   
double free 조건은 프로그램이 할당된 메모리를 두 번 이상 해제하도록 하는 조건입니다. 이미 해제된 메모리를 해제하는 것은 메모리 손상을 일으킬 수 있습니다. 공격자는 손상이 일어난 경우 실행된 메모리에 임의 코드를 추가할 수 있습니다.

일반적으로 이 취약점은 서비스 거부 발생을 유발합니다. 그러나 일부 상황에서는 코드가 실행될 수 있습니다. 영향 받는 각 시스템의 고유한 메모리 레이아웃 때문에 이 취약점을 대규모로 악용하는 것은 어렵습니다.

서비스 제어 관리자란 무엇입니까?   
SCM(서비스 제어 관리자)은 운영 체제의 성공적인 시작을 위해 설치된 서비스 및 드라이버 서비스의 데이터베이스를 관리하며 이러한 서비스를 통합하고 안전하게 관리하기 위한 수단을 제공합니다. 데이터베이스는 Windows 시스템 레지스트리에 저장되며 각 서비스 또는 드라이버 서비스에 대한 구성 및 보안 정보를 담고 있습니다.

공격자는 취약점을 악용하여 무엇을 할 수 있습니까?  
이 취약점을 성공적으로 악용한 공격자는 서비스 제어 관리자의 컨텍스트 내에서 임의 코드를 실행할 수 있습니다(services.exe).

공격자는 이러한 취약점을 어떻게 악용합니까?  
공격 시나리오에서 공격자는 인증된 사용자가 특수하게 조작된 응용 프로그램을 실행하도록 유도할 수 있습니다. 이 응용 프로그램에 의해 Double Free 조건이 발생하여 SCM(서비스 제어 관리자)의 컨텍스트에서 임의 코드가 실행될 수 있습니다.

또한 인증된 공격자가 대상 시스템에 로그온하여 악성 응용 프로그램을 실행할 수 있습니다. 이 응용 프로그램에 의해 Double Free 조건이 발생하여 SCM(서비스 제어 관리자)의 컨텍스트에서 임의 코드가 실행될 수 있습니다.
...
업데이트는 어떤 기능을 수행합니까?  
이 업데이트는 손상된 서비스 설명이 제공될 때 SCM(서비스 제어 관리자)이 메모리의 개체를 처리하는 방식을 수정하여 취약점을 해결합니다.
...
<td style="border:1px solid black;"><a href="https://support.microsoft.com/kb/2872339">Microsoft 기술 자료 문서 2872339</a> 참조</td>
</tr>
<tr class="odd">
...
-   V1.1(2013년 9월 12일): 기술 자료 문서의 알려진 문제점 항목이 "없음"에서 "예"로 업데이트되었습니다.
