...
-   Windows Server 2003 x64 Edition용 Microsoft Internet Explorer 6 - [업데이트 다운로드](https://www.microsoft.com/download/details.aspx?familyid=c18db204-0f2c-4dd4-b29c-0938ff1bfd7b)
...
#### COM 개체 인스턴스화 메모리 손상 취약점 - CVE-2007-0219에 대한 대안:
...
Internet Explorer에서 FTP 서버의 특정 응답을 해석하는 방식에 원격 코드 실행 취약점이 존재합니다. 공격자는 FTP 세션에서 특수하게 조작된 FTP 응답을 Internet Explorer에 포함된 FTP 클라이언트에 보내 취약점을 악용할 수 있습니다. 취약점 악용에 성공한 공격자는 로컬 사용자와 동일한 권한을 얻을 수 있습니다. 시스템에 대한 사용자 권한이 적게 구성된 계정의 사용자는 관리자 권한으로 작업하는 사용자에 비해 영향을 적게 받습니다.

#### FTP 서버 응답 구분 문석 메모리 손상 취약점 - CVE-2007-0217에 대한 완화 요소:
...
#### FTP 서버 응답 구분 문석 메모리 손상 취약점 - CVE-2007-0217에 대한 대안:
...
#### FTP 서버 응답 구분 문석 메모리 손상 취약점 - CVE-2007-0217에 대한 FAQ:

**이 취약점을 악용하면 어느 작업까지 가능합니까?**  
이것은 원격 코드 실행 취약점입니다. 이 취약점을 악용한 공격자는 영향을 받는 시스템을 원격으로 완전히 제어할 수 있습니다. 이렇게 되면 공격자가 프로그램을 설치할 수 있을 뿐 아니라 데이터를 보거나 변경하거나 삭제할 수 있고 모든 사용자 권한이 있는 새 계정을 만들 수도 있습니다. 시스템에 대한 사용자 권한이 적게 구성된 계정의 사용자는 관리자 권한으로 작업하는 사용자에 비해 영향을 적게 받습니다.

**취약점의 원인은 무엇입니까?**  
Internet Explorer에서 특수하게 조작된 FTP 서버 응답을 처리할 때 시스템 메모리가 손상되어 공격자가 임의 코드를 실행할 수 있습니다.

**공격자는 취약점을 악용하여 무엇을 할 수 있습니까?**  
취약점 악용에 성공한 공격자는 로컬 사용자와 동일한 권한을 얻을 수 있습니다. 시스템에 대한 사용자 권한이 적게 구성된 계정의 사용자는 관리자 권한으로 작업하는 사용자에 비해 영향을 적게 받습니다.

**공격자는 이러한 취약점을 어떻게 악용합니까?**  
웹 기반 공격 시나리오에서 공격자는 이 취약점을 악용하려고 시도하는 FTP 서버를 호스팅해야 합니다. 공격자는 사용자가 FTP 서버를 방문하도록 만들 수 없습니다. 대신 공격자는 사용자가 웹 페이지, 전자 메일 메시지 또는 인스턴트 메신저 요청의 링크를 클릭하여 공격자의 FTP 사이트를 방문하도록 유도하는 것이 일반적입니다.

**취약점으로 인해 노출되는 시스템은 주로 무엇입니까?**  
이 취약점이 악용되어 악의적인 동작이 실행되려면 사용자가 로그인하여 Internet Explorer에 포함된 FTP 클라이언트를 사용하여 FTP 서버를 방문해야 합니다. 따라서 워크스테이션 또는 터미널 서버와 같이 Internet Explorer를 빈번하게 사용하는 시스템은 이 취약점을 이용하는 공격을 받을 위험이 가장 큽니다.
...
**업데이트는 어떤 기능을 수행합니까?**  
업데이트는 Internet Explorer에서 FTP 서버 응답을 처리하는 방식을 수정하여 취약점을 제거합니다.
...
**IE5.01sp4-KB928090-Windows2000sp4-x86-kor /quiet**

Internet Explorer 6.0 서비스 팩 1:

**IE6.0sp1-KB928090-Windows2000-x86-kor /quiet**

**참고** **/quiet** 스위치를 사용하면 모든 메시지가 표시되지 않습니다. 마찬가지로 실패 메시지도 표시되지 않습니다. 관리자는 지원되는 방법 중 하나를 사용하여 **/quiet** 스위치를 사용할 때 설치가 제대로 되었는지 확인해야 합니다. 관리자는 이 스위치를 사용할 때 KB928090-IE501SP4-20070125.120000.log 또는 KB928090-IE6SP1-20070125.120000.log 파일에서 모든 실패 메시지를 검토해야 합니다.

시스템을 다시 시작하지 않고 보안 업데이트를 설치하려면 Windows 2000 서비스 팩 4 명령 프롬프트에서 다음 명령 중 하나를 사용하십시오.

Internet Explorer 5.01 서비스 팩 4:

**IE5.01sp4-KB928090-Windows2000sp4-x86-kor /norestart**

Internet Explorer 6.0 서비스 팩 1:

**IE6.0sp1-KB928090-Windows2000-x86-kor /norestart**
...
시스템 관리자는 Spuninst.exe 유틸리티를 사용하여 이 보안 업데이트를 제거할 수도 있습니다. Spuninst.exe 유틸리티는 각각 %Windir%\\$NTUninstallKB928090-IE501SP4-20070125.120000$\\Spuninst 및 %Windir%\\$NTUninstallKB928090-IE6SP1-20070125.120000$\\Spuninst 폴더에 있습니다.
...
다음 레지스트리 키 중 하나를 검토하여 보안 업데이트로 설치된 파일을 확인할 수도 있습니다.
...
**참고** 이 레지스트리 키에 설치된 파일의 전체 목록 중 일부가 들어 있지 않을 수 있습니다. 또한 관리자나 OEM이 928090 보안 업데이트를 Windows 설치 원본 파일에 통합하거나 포함시키는 경우 이러한 레지스트리 키가 제대로 만들어지지 않을 수 있습니다.
...
사용자 개입 없이 보안 업데이트를 설치하려면 Microsoft Windows XP 명령 프롬프트에서 다음 명령 중 하나를 사용하십시오.

Windows XP 서비스 팩 2용 Internet Explorer 6:

**Windowsxp-kb928090-x86-kor /quiet**

Windows XP 서비스 팩 2용 Internet Explorer 7:

**IE7-KB928090-WindowsXP-x86-kor /quiet**

**참고** **/quiet** 스위치를 사용하면 모든 메시지가 표시되지 않습니다. 마찬가지로 실패 메시지도 표시되지 않습니다. 관리자는 지원되는 방법 중 하나를 사용하여 **/quiet** 스위치를 사용할 때 설치가 제대로 되었는지 확인해야 합니다. 또한 관리자는 KB928090.log 또는 KB928090-IE7.log 파일에서 모든 실패 메시지를 검토해야 합니다.

시스템을 다시 시작하지 않고 보안 업데이트를 설치하려면 Windows XP 명령 프롬프트에서 다음 명령 중 하나를 사용하십시오.

Windows XP 서비스 팩 2용 Internet Explorer 6:

**Windowsxp-kb928090-x86-kor /norestart**

Windows XP 서비스 팩 2용 Internet Explorer 7:

**IE7-KB928090-WindowsXP-x86-kor /norestart**
...
시스템 관리자는 Spuninst.exe 유틸리티를 사용하여 이 보안 업데이트를 제거할 수도 있습니다. Spuninst.exe 유틸리티는 %Windir%\\$NTUninstallKB928090$\\Spuninst 및 %Windir%\\ie7updates\\KB928090-IE7\\spuninst 폴더에 있습니다.
...
Windows XP Professional x64용 Internet Explorer 6:
...
Windows XP Professional x64용 Internet Explorer 7:
...
Windows XP Professional x64 Edition용 Internet Explorer 6:
...
Windows XP Professional x64 Edition용 Internet Explorer 7:

HKEY\_LOCAL\_MACHINE\\SOFTWARE\\Microsoft\\Updates\\Windows Server 2003\\SP0\\KB928090-IE7\\Filelist

**참고** 이러한 레지스트리 키에는 설치된 파일의 전체 목록이 포함되어 있지 않을 수도 있습니다. 또한 관리자나 OEM이 928090 보안 업데이트를 Windows 설치 원본 파일에 통합하거나 포함시키는 경우 이러한 레지스트리 키가 제대로 만들어지지 않을 수 있습니다.
...
Windows Server 2003 서비스 팩 1용 Windows Internet Explorer 7 보안 업데이트를 적용하려면 Windows Server 2003 서비스 팩 1이 필요합니다.
...
사용자 개입 없이 보안 업데이트를 설치하려면 Windows Server 2003 명령 프롬프트에서 다음 명령 중 하나를 사용하십시오.

Windows Server 2003 및 Windows Server 2003 서비스 팩 1용 Internet Explorer 6:

**Windowsserver2003-kb928090-x86-kor /quiet**

Windows Server 2003 서비스 팩 1용 Internet Explorer 7:

**IE7-KB928090-WindowsServer2003-x86-kor / quiet**

**참고** **/quiet** 스위치를 사용하면 모든 메시지가 표시되지 않습니다. 마찬가지로 실패 메시지도 표시되지 않습니다. 관리자는 지원되는 방법 중 하나를 사용하여 **/quiet** 스위치를 사용할 때 설치가 제대로 되었는지 확인해야 합니다. 또한 관리자는 이 스위치를 사용할 때 KB928090.log 파일에서 모든 실패 메시지를 검토해야 합니다.

시스템을 다시 시작하지 않고 보안 업데이트를 설치하려면 Windows Server 2003 명령 프롬프트에서 다음 명령 중 하나를 사용하십시오.

Windows Server 2003 및 Windows Server 2003 서비스 팩 1용 Internet Explorer 6:

**Windowsserver2003-kb928090-x86-kor /norestart**

Windows Server 2003 서비스 팩 1용 Internet Explorer 7:

**IE7-KB928090-WindowsServer2003-x86-kor / norestart**
...
Windows Server 2003 Enterprise Edition(Itanium 기반 시스템용), Windows Server 2003 Datacenter Edition(Itanium 기반 시스템용), Windows Server 2003 Enterprise Edition SP1(Itanium 기반 시스템용), Windows Server 2003 Datacenter Edition SP1(Itanium 기반 시스템용)용 Internet Explorer 6:
...
Windows Server 2003 Enterprise Edition SP1(Itanium 기반 시스템용), Windows Server 2003 Datacenter Edition SP1(Itanium 기반 시스템용)용 Internet Explorer 7:
...
Windows Server 2003 Standard x64 Edition, Windows Server 2003 Enterprise x64 Edition, Windows Server 2003 Datacenter x64 Edition, Windows Server 2003 R2 Standard x64 Edition, Windows Server 2003 R2 Enterprise x64 Edition, Windows Server 2003 R2 Datacenter x64 Edition용 Internet Explorer 6:
...
Windows Server 2003 Standard x64 Edition, Windows Server 2003 Enterprise x64 Edition, Windows Server 2003 Datacenter x64 Edition, Windows Server 2003 R2 Standard x64 Edition, Windows Server 2003 R2 Enterprise x64 Edition, Windows Server 2003 R2 Datacenter x64 Edition용 Internet Explorer 7:
...
-   COM 개체 인스턴스화 메모리 손상 취약점(CVE-2007-0219)에 설명된 5개 클래스 식별자를 보고해 주신 [BreakingPoint Systems (영문)](https://www.bpointsys.com/)의 H D Moore
-   FTP 서버 응답 구문 분석 메모리 손상 취약점(CVE-2007-0217)을 보고해 주신[iDefense (영문)](https://idefense.com/)
...
-   V1.1 (2007년 2월 21일): Windows Internet Explorer 7에 대한 설치 확인 키를 수정하여 공지가 개정되었습니다. Windows Server 2003의 제거 정보도 정확한 폴더로 업데이트되었습니다.
