---
TOCTitle: 'MS10-075'
Title: 'Microsoft 보안 공지 MS10-075 - 긴급'
ms:assetid: 'ms10-075'
ms:contentKeyID: 61230336
ms:mtpsurl: 'https://technet.microsoft.com/ko-KR/library/ms10-075(v=Security.10)'
---
...
이 보안 업데이트는 Media Player 네트워크 공유 서비스에서 발견되어 비공개적으로 보고된 취약점 1건을 해결합니다. 공격자가 영향을 받는 시스템에 특수하게 조작된 RTSP 패킷을 보낼 경우 이 취약점으로 인해 원격 코드 실행이 발생할 수 있습니다. 그러나 홈 미디어에 대한 인터넷 액세스는 기본적으로 사용되지 않습니다. 이 기본 구성에서 공격자는 동일한 서브넷 내에서만 이 취약점을 악용할 수 있습니다.

이 보안 업데이트의 심각도는 지원 대상인 Windows 7 에디션에 대해서는 긴급, 지원 대상인 모든 Windows Vista 에디션에 대해서는 중요입니다. 자세한 내용은 이 섹션에서 **영향을 받는 소프트웨어 및 영향을 받지 않는 소프트웨어**를 참조하십시오.

보안 업데이트는 Windows Media Player 네트워크 공유 서비스에서 특수하게 조작된 RTSP 패킷을 처리하는 방식을 수정하여 취약점을 해결합니다. 취약점에 대한 자세한 내용은 다음 섹션, **취약점 정보**에서 각 취약점 항목의 자주 제기되는 질문 사항(FAQ)을 참조하십시오.
...
|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------|-------------|---------------------------|
| [Windows Vista 서비스 팩 1 및 Windows Vista 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=4a481825-d9ad-4a7c-aa89-f40fb9651961&displaylang=ko)                         | 원격 코드 실행 | 중요        | 없음                      |
| [Windows Vista x64 Edition 서비스 팩 1 및 Windows Vista x64 Edition 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=15d8f81b-97b0-43d9-b218-1cdd759cb2ec&displaylang=ko) | 원격 코드 실행 | 중요        | 없음                      |
| [Windows 7(32비트 시스템용)](https://www.microsoft.com/download/details.aspx?familyid=1a3953fe-ba48-4980-a65d-74e3b756d53c&displaylang=ko)                                                     | 원격 코드 실행 | 긴급        | 없음                      |
| [Windows 7(x64 기반 시스템용)](https://www.microsoft.com/download/details.aspx?familyid=5759d2a3-7f35-4fa1-8ab4-17145839fa26&displaylang=ko)                                                   | 원격 코드 실행 | 긴급        | 없음                      |
...
**영향을 받는 Microsoft Windows 버전은 실행 중이지만 Windows Media Player 네트워크 공유 서비스는 시작되지 않았습니다. 이 업데이트가 제공됩니까?**  
예, 이 업데이트는 Windows Media Player 네트워크 공유 서비스 시작 여부에 관계없이 영향을 받는 Windows 운영 체제를 실행 중인 모든 시스템에 제공됩니다.

**심각도가 Windows** **Vista에 대해서는 중요이고, Windows 7에 대해서는 긴급인 이유는 무엇입니까?**  
Windows 7 Home Basic과 Windows 7 Home Premium 에디션에서는 Media Player 네트워크 공유 서비스가 기본적으로 사용되므로 취약점의 심각도가 긴급입니다. Windows 7 Professional, Windows 7 Enterprise 및 Windows 7 Ultimate 에디션의 경우 서비스가 기본적으로 사용되지 않습니다.

영향을 받는 Windows Vista 버전의 경우 서비스가 기본적으로 사용되지 않습니다. 이 완화 요소는 지원 대상인 Windows Vista 에디션의 심각도 등급을 중요로 낮춥니다.
...
<th style="border:1px solid black;" >RTSP 해제 후 사용 취약점(CVE-2010-3225)</th>
...
Microsoft Windows Media Player 네트워크 공유 서비스에는 원격 사용자가 특수하게 조작된 네트워크 패킷을 응용 프로그램의 네트워크 스트리밍 서비스 인스턴스에 보내고 현재 응용 프로그램의 컨텍스트에서 원격 코드를 실행할 수 있는 취약점이 존재합니다.
  
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2010-3225 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-3225)를 참조하십시오.
  
#### RTSP 해제 후 사용 취약점(CVE-2010-3225)에 대한 완화 요소
  
완화란 취약점의 악용 심각도를 낮출 수 있는 설정, 일반적인 구성 또는 최선책, 기본 상태를 지칭합니다. 다음과 같은 완화 요소가 현재 상황에서 유용할 수 있습니다.
  
-   기본적으로 홈 미디어에 대한 인터넷 액세스는 사용되지 않습니다. 이 기본 구성에서 공격자는 동일한 서브넷 내의 컴퓨터에서만 이 취약점을 악용할 수 있습니다.  
-   지원 대상인 Windows Vista 에디션과 Windows 7 Professional, Windows 7 Enterprise 및 Windows 7 Ultimate 에디션에서는 Windows Media Player 네트워크 공유 서비스가 기본적으로 사용되지 않습니다.
  
#### RTSP 해제 후 사용 취약점(CVE-2010-3225)에 대한 대안
  
대안이란 기본적인 취약점을 수정하지는 않지만 업데이트를 적용하기 전에 알려진 공격 경로를 차단하는 데 유용한 설정 또는 구성 변경을 지칭합니다. Microsoft는 다음과 같은 대안을 테스트했으며, 대안으로 인한 기능 저하 여부를 해당 항목에서 설명합니다.
  
-   **영향을 받는 시스템에서 Windows Media Player 네트워크 공유 서비스 사용 안 함**
    ...
    **참고** 레지스트리 편집기를 잘못 사용하면 시스템 전체에 심각한 문제가 발생할 수 있으며 운영 체제를 다시 설치해야 할 수도 있습니다. Microsoft는 레지스트리 편집기를 잘못 사용하여 발생하는 문제에 대한 해결을 보증하지 않습니다. 레지스트리 편집기의 사용에 따른 모든 책임은 사용자에게 있습니다. 레지스트리를 편집하는 방법에 대한 자세한 내용은 레지스트리 편집기(Regedit.exe)의 "키와 값 변경" 도움말 항목 또는 Regedt32.exe의 "레지스트리의 정보 추가와 삭제" 및 "레지스트리 정보 편집" 도움말 항목을 참조하십시오.
    ...
    4.  **레지스트리 파일 내보내기** 대화 상자에 **WMPNetworkSvc\_Backup.reg**를 입력하고 **저장**을 클릭합니다.  
        **참고** 이렇게 하면 이 레지스트리 키의 백업이 기본적으로 내 문서 폴더에 만들어집니다.  
    5.  레지스트리 편집기의 오른쪽 창에서 **Start** 값을 선택합니다. **Enter** 키를 눌러 값을 편집합니다. **값** 데이터 필드를 **4**로 변경하고 **Enter** 키를 누릅니다.  
    6.  .상승된 권한의 명령 프롬프트에서 다음 명령을 실행하여 서비스를 중지합니다.  
    ...
    **관리되는 배포 스크립트**
    ...
    2.  다음 내용을 .REG 확장명의 파일(예: Disable\_WMPNetworkSvc.reg)로 저장합니다.  
    ...
    4.  상승된 권한의 명령 프롬프트에서 다음 명령을 실행하여 서비스를 중지합니다.  
        
        ```
        sc stop WMPNetworkSvc
        ```
  
    **대안의 영향.** Windows 미디어 공유가 사용하지 않도록 설정됩니다.
    ...
    **대안 실행 취소 방법.**
    ...
    3.  **레지스트리 파일 가져오기** 대화 상자에서 위의 "대화형 방법"에서 만든 **WMPNetworkSvc\_Backup.reg** 파일을 선택하고 **열기**를 클릭합니다.  
    4.  레지스트리 편집기를 끝낸 다음 컴퓨터를 다시 시작합니다.
  
    관리되는 배포 스크립트 사용:
  
    1.  2단계에서 만든 레지스트리 스크립트를 대상 컴퓨터에서 다음 명령으로 실행합니다.  
...
#### RTSP 해제 후 사용 취약점(CVE-2010-3225)에 대한 FAQ
  
**이 취약점을 악용하면 어느 작업까지 가능합니까?**    
이 취약점은 원격 코드 실행 취약점입니다. 이 취약점 악용에 성공한 공격자는 네트워크 서비스 계정의 보안 컨텍스트에서 코드를 실행할 수 있습니다.
  
**취약점의 원인은 무엇입니까?**    
이 취약점은 Microsoft Windows Media Player 네트워크 공유 서비스에서 RTSP(Real Time Streaming Protocol) 패킷을 처리하는 방식으로 인해 발생합니다.
  
**공격자는 이러한 취약점을 어떻게 악용합니까?**    
공격자는 특수하게 조작된 RTSP 패킷을 만들어 영향을 받는 시스템에 보내 이러한 취약점을 악용할 수 있습니다.
  
**RTSP란 무엇입니까?**    
RTSP(Real Time Streaming Protocol)는 실시간 속성이 있는 데이터의 전달을 제어하기 위한 응용 프로그램 수준 프로토콜입니다. RTSP가 제공하는 확장할 수 있는 프레임워크를 통해 오디오, 비디오 등의 실시간 데이터를 요청 시 제어된 방식으로 전달할 수 있습니다. 데이터 원본에는 라이브 데이터 피드와 저장된 클립이 모두 포함될 수 있습니다. 이 프로토콜은 여러 데이터 전달 세션을 제어하는 데 사용되며 이 프로토콜을 통해 UDP, 멀티캐스트 UDP, TCP 등의 전달 채널과 RTP(RFC 1889)에 기반한 전달 메커니즘을 선택할 수 있습니다. RTSP는 [RFC 2326 (영문)](https://www.ietf.org/rfc/rfc2326.txt)에서 정의합니다.
  
RTSP는 인덱싱된 Windows Media 파일 중지, 일시 중지, 되감기, 빨리 감기 등의 플레이어 제어 작업을 지원합니다. RTSP를 사용하여 Windows Media Player 9 시리즈 이상이나 Windows Media Services 9 시리즈 이상을 실행 중인 컴퓨터로 콘텐츠를 스트리밍할 수 있습니다.
  
**네트워크 서비스 계정이란 무엇입니까?**    
네트워크 서비스 계정은 기본 제공되는 계정으로, 리소스와 개체에 대해 Users 그룹의 구성원보다 많은 액세스 권한을 가집니다. 네트워크 서비스 계정으로 실행되는 서비스는 컴퓨터 계정의 자격 증명을 사용하여 네트워크 리소스에 액세스합니다. 계정의 실제 이름은 NT AUTHORITY\\NETWORK SERVICE입니다. 자세한 내용은 MSDN 문서, [NetworkService 계정 (영문)](https://msdn.microsoft.com/en-us/library/ms684272(vs.85).aspx)을 참조하십시오.
  
**Windows Media Player 네트워크 공유 서비스란 무엇입니까?**    
Windows Media Player에서는 라이브러리의 파일을 홈 네트워크에 있는 다른 사용자와 공유하고 네트워크에 있는 다른 사용자가 공유하고 있는 라이브러리의 파일을 탐색할 수 있습니다. Windows 미디어 공유는 Windows Media Player 네트워크 공유 서비스를 사용합니다.
  
**Windows Media Player 네트워크 공유 서비스가 내 시스템에서 실행되고 있는지 여부를 어떻게 확인할 수 있습니까?**    
**시작**, **실행**을 클릭하고 **Services.msc**를 입력한 다음 **확인**을 클릭합니다. 서비스 제어판에서 Windows Media Player 네트워크 공유 서비스의 상태가 "시작됨"인지 여부를 확인합니다.
  
**공격자는 취약점을 악용하여 무엇을 할 수 있습니까?**    
이 취약점 악용에 성공한 공격자는 네트워크 서비스 계정의 보안 컨텍스트에서 영향을 받는 시스템을 제어할 수 있습니다.
  
**취약점으로 인해 주로 공격 받을 위험이 있는 시스템은 무엇입니까?**    
Windows Media Player 네트워크 공유 서비스(wmpnetwk.exe)를 실행하여 미디어를 스트리밍하고 있는 시스템이 주로 위험합니다.
...
<td style="border:1px solid black;"><a href="https://support.microsoft.com/kb/2281679">Microsoft 기술 자료 문서 2281679</a> 참조</td>
</tr>
<tr class="odd">
...
-   RTSP 해제 후 사용 취약점(CVE-2010-3225)을 [TippingPoint (영문)](https://www.tippingpoint.com/)의 [Zero Day Initiative (영문)](https://www.zerodayinitiative.com/)와 협력하여 보고해 주신 Oleksandr Mirosh
