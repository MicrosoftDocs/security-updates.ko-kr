...
**문제점.** 이 보안 업데이트를 설치할 때 고객이 겪을 수 있는 현재까지 알려진 문제점은 [Microsoft 기술 자료 문서 2489256](https://support.microsoft.com/kb/2489256)에 나와 있습니다. 이 문서는 이러한 문제점에 대한 권장 해결 방법도 소개합니다.
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2010-3972 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-3972)를 참조하십시오.
...
-   **경계면 방화벽에서 FTP 인바운드 트래픽 차단**
    ...
    이 포트는 영향을 받는 구성 요소와 연결을 시작하는 데 사용됩니다. 경계면 방화벽에서 이 포트를 차단하면 이 취약점을 악용한 방화벽 뒤의 시스템에 대한 공격을 막을 수 있습니다. 기타 포트를 악용할 수 있는 공격을 방지하려면 인터넷으로부터의 원치 않는 인바운드 통신을 모두 차단하는 것이 좋습니다. 포트에 대한 자세한 내용은 TechNet 문서, [TCP 및 UDP 포트 할당 (영문)](https://go.microsoft.com/fwlink/?linkid=21312)을 참조하십시오.
  
    **대안의 영향.** 응용 프로그램 및 서비스가 영향을 받는 포트를 사용할 수 있습니다. 이러한 포트에 대한 연결을 차단하면 여러 응용 프로그램 또는 서비스가 작동하지 않을 수 있습니다.
  
    **대안 실행 취소 방법.**
  
    방화벽에서 TCP 포트 21의 차단을 해제합니다. 포트에 대한 자세한 내용은 TechNet 문서, [TCP 및 UDP 포트 할당 (영문)](https://go.microsoft.com/fwlink/?linkid=21312)을 참조하십시오.
...
예. 이 취약점은 공개되었습니다. 이 취약점에는 CVE(Common Vulnerability and Exposure) 번호 [CVE-2010-3972 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-3972)가 지정되었습니다.
...
<td style="border:1px solid black;"><a href="https://support.microsoft.com/kb/2489256">Microsoft 기술 자료 문서 2489256</a> 참조</td>
</tr>
<tr class="odd">
