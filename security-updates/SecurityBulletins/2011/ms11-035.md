---
TOCTitle: 'MS11-035'
Title: 'Microsoft 보안 공지 MS11-035 - 긴급'
ms:assetid: 'ms11-035'
ms:contentKeyID: 61230402
ms:mtpsurl: 'https://technet.microsoft.com/ko-KR/library/ms11-035(v=Security.10)'
---



Microsoft 보안 공지 MS11-035 - 긴급
===================================

WINS의 취약점으로 인한 원격 코드 실행 문제점(2524426)
-----------------------------------------------------

게시된 날짜: 2011년 5월 11일 수요일
...
이 보안 업데이트의 심각도는 WINS가 설치된 지원 대상인 Windows Server 2003, Windows Server 2008(Itanium 제외) 및 Windows Server 2008 R2(Itanium 제외)를 실행하는 서버에 대해 긴급입니다. 자세한 내용은 이 섹션에서 **영향을 받는 소프트웨어 및 영향을 받지 않는 소프트웨어**를 참조하십시오.

보안 업데이트는 버퍼를 매개 변수로 전달할 때 발생하는 논리 오류를 수정하여 취약점을 해결합니다. 취약점에 대한 자세한 내용은 다음 섹션, **취약점 정보**에서 각 취약점 항목의 자주 제기되는 질문 사항(FAQ)을 참조하십시오.
...
**문제점.** 이 보안 업데이트를 설치할 때 고객이 겪을 수 있는 현재까지 알려진 문제점은 [Microsoft 기술 자료 문서 2524426](https://support.microsoft.com/kb/2524426)에 나와 있습니다. 이 문서는 이러한 문제점에 대한 권장 해결 방법도 소개합니다.
...
|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------|-------------|---------------------------------------------------------------------|
| [Windows Server 2003 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=0d289146-8470-45dd-a886-d1b35a2c6d4b&displaylang=ko)                                                                      | 원격 코드 실행 | 긴급        | [MS09-039](https://technet.microsoft.com/security/bulletin/ms09-039) |
| [Windows Server 2003 x64 Edition 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=0d3e85a9-42ff-447b-ab8c-095a1c0b224c&displaylang=ko)                                                          | 원격 코드 실행 | 긴급        | [MS09-039](https://technet.microsoft.com/security/bulletin/ms09-039) |
| [Windows Server 2003 with SP2(Itanium 기반 시스템용)](https://www.microsoft.com/download/details.aspx?familyid=4c388fe8-883a-4118-a7ff-50f1e9d0a071)                                                                 | 원격 코드 실행 | 긴급        | [MS09-039](https://technet.microsoft.com/security/bulletin/ms09-039) |
| [Windows Server 2008(32비트 시스템용) 및 Windows Server 2008(32비트 시스템용) 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=9019a3ff-d953-4b0b-8c7a-05aa9cf65c92&displaylang=ko)\*           | 원격 코드 실행 | 긴급        | 없음                                                                |
| [Windows Server 2008(x64 기반 시스템용) 및 Windows Server 2008(x64 기반 시스템용) 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=841130fa-4522-4079-b7eb-f8466ac3ef12&displaylang=ko)\*       | 원격 코드 실행 | 긴급        | 없음                                                                |
| [Windows Server 2008 R2(x64 기반 시스템용) 및 Windows Server 2008 R2(x64 기반 시스템용) 서비스 팩 1](https://www.microsoft.com/download/details.aspx?familyid=8732b41e-2f25-4ee1-a8b7-a8b8b9e22d08&displaylang=ko)\* | 원격 코드 실행 | 긴급        | 없음                                                                |
...
<th style="border:1px solid black;" >WINS 서비스 응답 실패 취약점(CVE-2011-1248)</th>
...
WINS(Windows Internet Name Service)에서는 WINS 서비스로 보낸 특수하게 조작된 WINS 네트워크 패킷 내의 데이터 구조에 대한 유효성 검사가 충분하게 수행되지 못할 경우 원격 코드 실행 취약점이 발생합니다.
  
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2011-1248 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-1248)을 참조하십시오.
  
#### WINS 서비스 응답 실패 취약점(CVE-2011-1248)에 대한 완화 요소
  
완화란 취약점의 악용 심각도를 낮출 수 있는 설정, 일반적인 구성 또는 최선책, 기본 상태를 지칭합니다. 다음과 같은 완화 요소가 현재 상황에서 유용할 수 있습니다.
  
-   기본적으로 WINS는 영향을 받는 운영 체제에 설치되지 않으며, 이 구성 요소를 수동으로 설치한 고객만 이 문제의 영향을 받습니다.
  
#### WINS 서비스 응답 실패 취약점(CVE-2011-1248)에 대한 대안
  
대안이란 기본적인 취약점을 수정하지는 않지만 업데이트를 적용하기 전에 알려진 공격 경로를 차단하는 데 유용한 설정 또는 구성 변경을 지칭합니다. Microsoft는 다음과 같은 대안을 테스트했으며, 대안으로 인한 기능 저하 여부를 해당 항목에서 설명합니다.
  
-   **방화벽에서 TCP 포트 42 및 UDP 포트 42를 차단합니다.**
  
    이 포트들은 영향을 받는 구성 요소와 연결을 시작하는 데 사용됩니다. 기업 방화벽에서 이러한 인바운드 및 아웃바운드 포트를 모두 차단하면 이 취약점을 악용한 방화벽 뒤의 시스템 침입으로부터 보호할 수 있습니다. 다른 포트를 사용하는 공격을 방지하려면 인바운드 원치 않는 인터넷 통신을 모두 차단하는 것이 좋습니다. 포트에 대한 자세한 내용은 TechNet 문서, [TCP 및 UDP 포트 할당 (영문)](https://go.microsoft.com/fwlink/?linkid=21312)을 참조하십시오. Windows 방화벽에 대한 자세한 내용은 TechNet 문서, [단일 컴퓨터에서 Windows 방화벽을 구성하는 방법 (영문)](https://www.microsoft.com/technet/security/smallbusiness/prodtech/windowsxp/cfgfwall.mspx)을 참조하십시오.
  
#### WINS 서비스 응답 실패 취약점(CVE-2011-1248)에 대한 FAQ
...
**취약점의 원인은 무엇입니까?**      
WINS에서 소켓 전송 예외를 처리할 때 논리 오류로 인해 일부 사용자가 제공한 값이 스택 프레임 내에 남고 다른 컨텍스트에서 다시 사용됩니다. 이 취약점 악용에 성공한 원격 공격자는 원격 코드 실행으로 이어질 수 있는 방법으로 메모리를 손상시킬 수 있습니다. 그런 다음 공격자는 Windows Server 2003의 경우 시스템 컨텍스트에서 또는 Windows Server 2008 및 Windows Server 2008 R2의 경우 로컬 서비스 컨텍스트에서 시스템을 제어할 수 있게 됩니다.
  
**WINS란 무엇입니까?**      
WINS(Windows Internet Name Service)는 NetBT(NetBIOS over TCP/IP)를 지원하도록 특수하게 고안되었습니다. 사용자가 리소스에 액세스하는 데 NetBIOS 이름이 필요한 모든 환경에서는 WINS가 필수적입니다. 그러한 네트워크에서 WINS를 사용하지 않으면, 원격 네트워크 리소스에 연결하거나 파일 및 인쇄 공유 연결을 구성하기 위해 LMHOSTS 파일을 사용해야 합니다. 자세한 내용은 TechNet 문서 [WINS란 무엇입니까?(영문)](https://technet.microsoft.com/library/cc784180.aspx)를 참조하십시오.
  
**공격자는 취약점을 악용하여 무엇을 할 수 있습니까?**      
이 취약점 악용에 성공한 공격자는 임의 코드를 실행할 수 있습니다. 이렇게 되면 공격자가 프로그램을 설치할 수 있을 뿐 아니라 날짜를 보거나 변경하거나 삭제할 수 있고 WINS 서비스의 권한이 있는 새 계정을 만들 수도 있습니다.
  
**공격자는 이러한 취약점을 어떻게 악용합니까?**      
공격자는 특수하게 조작한 네트워크 패킷을 만들어 영향을 받는 WINS 시스템에 보내는 방법으로 이 취약점을 악용할 수 있습니다.
  
**취약점으로 인해 주로 공격 받을 위험이 있는 시스템은 무엇입니까?**      
Windows Server 2003, Windows Server 2008 또는 Windows Server 2008 R2에서 실행되는 WINS 서버가 공격 받을 위험이 큽니다.
  
**업데이트는 어떤 기능을 수행합니까?**      
업데이트는 버퍼를 매개 변수로 전달할 때 함수 사이의 계약을 위반하는 논리 오류를 수정합니다.
...
<td style="border:1px solid black;">제어판의 <strong>프로그램 추가/제거</strong> 도구 또는 %Windir%\$NTUninstallKB2524426$\Spuninst 폴더의 Spuninst.exe 유틸리티를 사용하십시오.</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"><strong>파일 정보</strong></td>
<td style="border:1px solid black;"><a href="https://support.microsoft.com/kb/2524426">Microsoft 기술 자료 문서 2524426</a> 참조</td>
</tr>
<tr class="even">
...
-   WINS 서비스 응답 실패 취약점(CVE-2011-1248)을 [TippingPoint's (영문)](https://www.tippingpoint.com/)[Zero Day Initiative (영문)](https://www.zerodayinitiative.com/)와 협력하여 보고해 주신 Luigi Auriemma
