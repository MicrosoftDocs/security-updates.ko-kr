---
TOCTitle: 'MS10-078'
Title: 'Microsoft 보안 공지 MS10-078 - 중요'
ms:assetid: 'ms10-078'
ms:contentKeyID: 61230339
ms:mtpsurl: 'https://technet.microsoft.com/ko-KR/library/ms10-078(v=Security.10)'
---
...
이 보안 업데이트는 Windows OTF(OpenType 글꼴) 서식 드라이버에서 발견되어 비공개적으로 보고된 취약점 2건을 해결합니다. 이 보안 업데이트의 심각도는 지원 대상인 모든 Windows XP 및 Windows Server 2003 에디션에 대해 중요입니다. 지원 대상인 모든 Windows Vista, Windows Server 2008, Windows 7 및 Windows Server 2008 R2 에디션은 이 취약점의 영향을 받지 않습니다. 자세한 내용은 이 섹션에서 **영향을 받는 소프트웨어 및 영향을 받지 않는 소프트웨어**를 참조하십시오.
...
보안 업데이트는 OTF(OpenType 글꼴) 서식 드라이버에서 OpenType 글꼴을 처리할 때 메모리를 할당하고 정수 계산을 수행하는 방식을 수정하여 취약점을 해결합니다. 취약점에 대한 자세한 내용은 다음 섹션, **취약점 정보**에서 각 취약점 항목의 자주 제기되는 질문 사항(FAQ)를 참조하십시오.
...
|-------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------|-------------|---------------------------------------------------------------------|
| [Windows XP 서비스 팩 3](https://www.microsoft.com/download/details.aspx?familyid=0553fc7c-deed-4594-a133-d621551310dc&displaylang=ko)                      | 권한 상승      | 중요        | [MS10-037](https://technet.microsoft.com/security/bulletin/ms10-037) |
| [Windows XP Professional x64 Edition 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=1ad30596-bac6-4d48-8b15-0245960c443b)            | 권한 상승      | 중요        | [MS10-037](https://technet.microsoft.com/security/bulletin/ms10-037) |
| [Windows Server 2003 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=f94763e7-b1db-4043-aa79-d5be1a42307d&displaylang=ko)             | 권한 상승      | 중요        | [MS10-037](https://technet.microsoft.com/security/bulletin/ms10-037) |
| [Windows Server 2003 x64 Edition 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=6fca5cab-7e11-4911-a6a8-f73f113b2963&displaylang=ko) | 권한 상승      | 중요        | [MS10-037](https://technet.microsoft.com/security/bulletin/ms10-037) |
| [Windows Server 2003 SP2(Itanium 기반 시스템용)](https://www.microsoft.com/download/details.aspx?familyid=e8f297e2-0dfd-421a-b598-a78199ad6baa)             | 권한 상승      | 중요        | [MS10-037](https://technet.microsoft.com/security/bulletin/ms10-037) |
...
<th style="border:1px solid black;" >OpenType 글꼴 구문 분석 취약점(CVE-2010-2740)</th>
<th style="border:1px solid black;" >OpenType 글꼴 유효성 검사 취약점(CVE-2010-2741)</th>
...
Windows OTF(OpenType 글꼴) 서식 드라이버에서 특수하게 조작된 OpenType 글꼴을 부적절하게 구문 분석하는 방식에 권한 상승 취약점이 존재합니다. 이 취약점 악용에 성공한 공격자는 커널 모드에서 임의 코드를 실행할 수 있습니다. 이렇게 되면 공격자가 프로그램을 설치할 수 있을 뿐 아니라 데이터를 보거나 변경하거나 삭제할 수 있고 모든 사용자 권한이 있는 새 계정을 만들 수도 있습니다.
  
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2010-2740 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-2740)을 참조하십시오.
  
#### OpenType 글꼴 구문 분석 취약점(CVE-2010-2740)에 대한 완화 요소
  
완화란 취약점의 악용 심각도를 낮출 수 있는 설정, 일반적인 구성 또는 최선책, 기본 상태를 지칭합니다. 다음과 같은 완화 요소가 현재 상황에서 유용할 수 있습니다.
  
-   이 취약점을 악용하려면 공격자가 유효한 로그온 자격 증명을 가지고 로컬로 로그온할 수 있어야 합니다. 익명의 사용자에 의해서나 원격으로는 이 취약점을 악용할 수 없습니다.
  
#### OpenType 글꼴 구문 분석 취약점(CVE-2010-2740)에 대한 대안
  
이 취약점에 대한 대안을 확인하지 못했습니다.
  
#### OpenType 글꼴 구문 분석 취약점(CVE-2010-2740)에 대한 FAQ
...
**취약점의 원인은 무엇입니까?**    
이 취약점은 OTF(OpenType 글꼴) 서식 드라이버에서 특수하게 조작된 글꼴을 구문 분석할 때 메모리를 제대로 할당하지 못해 발생합니다.
  
**OpenType 글꼴 서식이란 무엇입니까?**    
OpenType은 Microsoft와 Adobe가 Apple의 TrueType 글꼴 서식을 확장하여 공동으로 개발한 글꼴 서식입니다. OpenType 글꼴 파일에는 TrueType이나 PostScript 윤곽선 글꼴을 구성하는 데이터가 테이블 형식으로 포함됩니다. 래스터라이저는 글꼴에 포함된 테이블의 데이터 조합을 사용하여 TrueType이나 PostScript 문자 윤곽선을 렌더링합니다. 이 지원 데이터 중에는 사용하는 윤곽선 서식에 관계없이 사용되는 것도 있고 TrueType이나 PostScript별로 사용되는 것도 있습니다. 자세한 내용은 [Microsoft 입력 체계 OpenType FAQ (영문)](https://www.microsoft.com/typography/opentypefaq.mspx)를 참조하십시오.
...
예. 기본적으로 OTF(OpenType 글꼴)를 렌더링하는 경우에는 타사 웹 브라우저도 이 취약점의 영향을 받습니다. 이러한 응용 프로그램을 사용하여 특수하게 조작된 포함 글꼴로 웹 사이트를 볼 경우 이 취약점으로 인해 로그온한 사용자의 컨텍스트에서 원격 코드 실행이 발생할 수 있습니다. 기본적으로 Internet Explorer는 이 문제점의 영향을 받지 않습니다.
  
**업데이트는 어떤 기능을 수행합니까?**    
업데이트는 OTF(OpenType 글꼴) 서식 드라이버에서 글꼴을 구문 분석할 때 메모리를 할당하는 방식을 수정하여 취약점을 해결합니다.
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2010-2741 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-2741)을 참조하십시오.
  
#### OpenType 글꼴 유효성 검사 취약점(CVE-2010-2741)에 대한 완화 요소
  
완화란 취약점의 악용 심각도를 낮출 수 있는 설정, 일반적인 구성 또는 최선책, 기본 상태를 지칭합니다. 다음과 같은 완화 요소가 현재 상황에서 유용할 수 있습니다.
  
-   이 취약점을 악용하려면 공격자가 유효한 로그온 자격 증명을 가지고 로컬로 로그온할 수 있어야 합니다. 익명의 사용자에 의해서나 원격으로는 이 취약점을 악용할 수 없습니다.
  
#### OpenType 글꼴 유효성 검사 취약점(CVE-2010-2741)에 대한 대안
  
이 취약점에 대한 대안을 확인하지 못했습니다.
  
#### OpenType 글꼴 유효성 검사 취약점(CVE-2010-2741)에 대한 FAQ
  
**이 취약점을 악용하면 어느 작업까지 가능합니까?**    
이 취약점은 권한 상승 취약점입니다. 이 취약점 악용에 성공한 공격자는 임의 코드를 실행하여 영향을 받는 시스템을 완전히 제어할 수 있습니다. 이렇게 되면 공격자가 프로그램을 설치할 수 있을 뿐 아니라 데이터를 보거나 변경하거나 삭제할 수 있고 모든 사용자 권한이 있는 새 계정을 만들 수도 있습니다.
  
**취약점의 원인은 무엇입니까?**    
이 취약점은 OTF(OpenType 글꼴) 서식 드라이버에서 특수하게 조작된 OpenType 글꼴을 처리할 때 정수 계산을 제대로 수행하지 못해 발생합니다.
...
**타사 웹 브라우저가 이 취약점의 영향을 받습니까?**    
예. 기본적으로 OTF(OpenType 글꼴)를 렌더링하는 경우에는 타사 웹 브라우저도 이 취약점의 영향을 받습니다. 이러한 응용 프로그램을 사용하여 특수하게 조작된 포함 글꼴로 웹 사이트를 볼 경우 이 취약점으로 인해 로그온한 사용자의 컨텍스트에서 원격 코드 실행이 발생할 수 있습니다.
  
**업데이트는 어떤 기능을 수행합니까?**    
업데이트는 OTF(OpenType 글꼴) 서식 드라이버에서 OpenType 글꼴을 처리할 때 정수 계산을 수행하는 방식을 수정하여 이 취약점을 해결합니다.
...
<td style="border:1px solid black;">제어판의 <strong>프로그램 추가/제거</strong> 도구 또는 %Windir%\$NTUninstallKB2279986$\Spuninst 폴더의 Spuninst.exe 유틸리티를 사용하십시오.</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"><strong>파일 정보</strong></td>
<td style="border:1px solid black;"><a href="https://support.microsoft.com/kb/2279986">Microsoft 기술 자료 문서 2279986</a> 참조</td>
</tr>
<tr class="even">
...
-   OpenType 글꼴 구문 분석 취약점(CVE-2010-2740)을 보고해 주신 [Siberas](https://www.siberas.de/)의 Sebastian Apelt
-   OpenType 글꼴 유효성 검사 취약점(CVE-2010-2741)을 보고해 주신 [Core Security Technologies (영문)](https://www.coresecurity.com)의 Diego Juarez
