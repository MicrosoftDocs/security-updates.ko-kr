---
TOCTitle: 'MS10-101'
Title: 'Microsoft 보안 공지 MS10-101 - 중요'
ms:assetid: 'ms10-101'
ms:contentKeyID: 61230361
ms:mtpsurl: 'https://technet.microsoft.com/ko-KR/library/ms10-101(v=Security.10)'
---


Microsoft 보안 공지 MS10-101 - 중요
===================================
...
이 보안 업데이트의 심각도는 지원 대상인 모든 Windows Server 2003, Windows XP, Windows Server 2008(Itanium 기반 에디션 제외) 및 Windows Server 2008 R2(Itanium 기반 에디션 제외) 에디션에 대해 중요입니다. 자세한 내용은 이 섹션에서 **영향을 받는 소프트웨어 및 영향을 받지 않는 소프트웨어**를 참조하십시오.

이 보안 업데이트는 Netlogon RPC 서비스 인터페이스에서 사용자 제공 데이터의 유효성 검사를 수정하여 취약점을 해결합니다. 취약점에 대한 자세한 내용은 다음 섹션, **취약점 정보**에서 각 취약점 항목의 자주 제기되는 질문 사항(FAQ)을 참조하십시오.
...
|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------|-------------|---------------------------|
| [Windows Server 2003 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=ad843b97-2f6e-4406-a17a-627b7db8a926&displaylang=ko)                                                                | 서비스 거부    | 중요        | 없음                      |
| [Windows Server 2003 x64 Edition 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=0b0a06e7-0ae5-41f4-9ff5-d524fc0afbfa&displaylang=ko)                                                    | 서비스 거부    | 중요        | 없음                      |
| [Windows Server 2003 with SP2(Itanium 기반 시스템용)](https://www.microsoft.com/download/details.aspx?familyid=7c0c2850-e81d-4347-aeb3-47036caa7c1b)                                                           | 서비스 거부    | 중요        | 없음                      |
| [Windows Server 2008(32비트 시스템용) 및 Windows Server 2008(32비트 시스템용) 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=6793f75b-cdf4-42ef-a53e-a1acb5b662d1&displaylang=ko)\*     | 서비스 거부    | 중요        | 없음                      |
| [Windows Server 2008(x64 기반 시스템용) 및 Windows Server 2008(x64 기반 시스템용) 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=85add876-ca5a-4a92-984e-188a72e349fc&displaylang=ko)\* | 서비스 거부    | 중요        | 없음                      |
| [Windows Server 2008 R2(x64 기반 시스템용)](https://www.microsoft.com/download/details.aspx?familyid=d7307afd-84a0-434e-9658-bf9f8ae4b938&displaylang=ko)\*                                                    | 서비스 거부    | 중요        | 없음                      |
...
<th style="border:1px solid black;" >Netlogon RPC Null 역참조 DOS 취약점(CVE-2010-2742)</th>
...
영향 받는 Windows Server 버전의 Netlogon RPC 서비스 구현에 인증된 원격 서비스 거부 공격 취약점이 존재합니다. 이 취약점을 성공적으로 악용한 공격자는 영향 받는 Windows Server 버전이 다시 시작되도록 할 수 있습니다.
  
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2010-2742 (영문)](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-2742)를 참조하십시오.
  
#### Netlogon RPC Null 역참조 DOS 취약점(CVE-2010-2742)에 대한 완화 요소
  
완화란 취약점의 악용 심각도를 낮출 수 있는 설정, 일반적인 구성 또는 최선책, 기본 상태를 지칭합니다. 다음과 같은 완화 요소가 현재 상황에서 유용할 수 있습니다.
  
-   이 취약점은 도메인 컨트롤러로 구성된 Windows 서버에만 영향을 미칩니다.  
-   공격자는 이러한 취약점을 이용하기 위해 영향 받는 도메인 컨트롤러와 동일한 도메인에 가입된 시스템에 대한 관리자 권한이 있어야 합니다.
  
#### Netlogon RPC Null 역참조 DOS 취약점(CVE-2010-2742)에 대한 대안
  
이 취약점에 대한 대안을 확인하지 못했습니다.
  
#### Netlogon RPC Null 역참조 DOS 취약점(CVE-2010-2742)에 대한 FAQ
  
**이 취약점을 악용하면 어느 작업까지 가능합니까?**    
이것은 서비스 거부 취약점입니다. 이 취약점을 악용한 공격자는 영향 받는 시스템이 다시 시작되도록 할 수 있습니다. 서비스 거부 취약점으로 인해 공격자가 코드를 실행하거나 사용자 권한이 상승되는 것은 아니지만 영향을 받는 시스템이 다시 시작되도록 만들 수 있습니다.
  
**취약점의 원인은 무엇입니까?**    
이 취약점은 Netlogon RPC Service가 사용자 제공 데이터의 유효성을 잘못 검사할 때 발생합니다.
  
**Netlogon RPC이란 무엇입니까?**    
Netlogon Remote Protocol은 도메인 기반 네트워크에서 사용자 및 시스템 인증에 사용되는 RPC(원격 프로시저 호출) 인터페이스입니다. Netlogon RPC에 대한 자세한 내용은 MSDN 문서 [Netlogon Remote Protocol 사양 (영문)](http://msdn.microsoft.com/en-us/library/cc237009(v=prot.10).aspx)을 참조하십시오.
  
**공격자는 취약점을 악용하여 무엇을 할 수 있습니까?**    
이 취약점을 성공적으로 악용한 공격자는 도메인 컨트롤러가 다시 시작되게 할 수 있습니다.
  
**공격자는 이러한 취약점을 어떻게 악용합니까?**    
공격자는 특수하게 조작된 RPC 패킷을 Netlogon RPC Service 인터페이스로 보내 이 취약점을 악용할 수 있습니다. 영향 받는 모든 플랫폼에서 공격자가 이 취약점을 악용하려면 관리자 권한이 있는 유효한 인증 자격 증명이 있어야 합니다.
  
**취약점으로 인해 주로 공격 받을 위험이 있는 시스템은 무엇입니까?**    
도메인 컨트롤러로 구성되고 Netlogon 서비스를 호스팅하는 Windows Server가 이 취약점의 영향을 받습니다. Windows 클라이언트 운영 체제는 이 취약점의 영향을 받지 않습니다.
  
**내 Windows Server가 도메인 컨트롤러로 구성되었는지 여부는 어떻게 알 수 있습니까?**    
레지스트리 키 HKLM\\SYSTEM\\CurrentControlSet\\Control\\ProductOptions\\ProductType이 LanmanNT로 설정되어 있으면 시스템이 도메인 컨트롤러로 구성된 것입니다.
  
**업데이트는 어떤 기능을 수행합니까?**    
이 업데이트는 Netlogon RPC Service 인터페이스에서 사용자 제공 데이터의 유효성을 적절히 검사하여 이 취약점을 해결합니다.
...
<td style="border:1px solid black;">제어판의 <strong>프로그램 추가/제거</strong> 도구 또는 %Windir%\$NTUninstallKB2207559$\Spuninst 폴더의 Spuninst.exe 유틸리티를 사용하십시오.</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"><strong>파일 정보</strong></td>
<td style="border:1px solid black;"><a href="http://support.microsoft.com/kb/2207559">Microsoft 기술 자료 문서 2207559</a> 참조</td>
</tr>
<tr class="even">
...
-   Netlogon RPC Null 역참조 DOS 취약점(CVE-2010-2742)을 보고해 주신 The Samba Team의 Matthias Dieter Wallnöfer 및 Andrew Bartlett
