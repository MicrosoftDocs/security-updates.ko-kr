---
TOCTitle: 'MS10-053'
Title: 'Microsoft 보안 공지 MS10-053 - 긴급'
ms:assetid: 'ms10-053'
ms:contentKeyID: 61230314
ms:mtpsurl: 'https://technet.microsoft.com/ko-KR/library/ms10-053(v=Security.10)'
---



Microsoft 보안 공지 MS10-053 - 긴급
===================================

Internet Explorer 누적 보안 업데이트(2183461)
---------------------------------------------
...
이 보안 업데이트는 Internet Explorer에서 발견되어 비공개적으로 보고된 취약점 6건을 해결합니다. 가장 위험한 취약점으로 인해 사용자가 Internet Explorer를 사용하여 특수하게 조작된 웹 페이지를 볼 경우 원격 코드 실행이 허용될 수 있습니다. 시스템에 대한 사용자 권한이 적게 구성된 계정의 사용자는 관리자 권한으로 작업하는 사용자에 비해 영향을 적게 받습니다.

이 보안 업데이트의 등급은 Internet Explorer 6, Internet Explorer 7 및 Internet Explorer 8에 대해 긴급입니다. 자세한 내용은 이 섹션의 **영향을 받는 소프트웨어 및 영향을 받지 않는 소프트웨어**를 참조하십시오.

보안 업데이트는 보안 검사를 강제로 수행하고 메모리에서 개체를 처리하는 방식을 수정하여 취약점을 해결합니다. 취약점에 대한 자세한 내용은 다음 섹션, **취약점 정보**에서 각 취약점 항목의 자주 제기되는 질문 사항(FAQ)를 참조하십시오.
...
이 업데이트에는 Internet Explorer 8 XSS 필터에 대한 심층 보안 업데이트가 포함되어 있습니다.
...
Internet Explorer에는 스크립트가 다른 도메인이나 Internet Explorer 영역에서 브라우저 창에 액세스하는 것을 허용할 수 있는 정보 유출 취약점이 존재합니다. 공격자는 사용자가 웹 페이지를 보고 마우스를 사용하여 브라우저 창과 상호 작용하는 경우 정보 유출을 허용할 수 있도록 특수하게 조작된 웹 페이지를 구성하여 이러한 취약점을 악용할 수 있습니다.

이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2010-1258 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-1258)을 참조하십시오.

#### 이벤트 처리기 도메인 간 취약점(CVE-2010-1258)에 대한 완화 요소
...
#### 이벤트 처리기 도메인 간 취약점(CVE-2010-1258)에 대한 대안
...
#### 이벤트 처리기 도메인 간 취약점(CVE-2010-1258)에 대한 FAQ
...
이 취약점 악용에 성공한 공격자는 웹 페이지를 보는 사용자 컨텍스트에서 스크립트를 실행할 수 있으므로 공격자에게 사용자 정보가 유출될 수 있습니다.
...
**업데이트는 어떤 기능을 수행합니까?**  
업데이트는 Internet Explorer가 스크립트 원본을 확인할 때 보안 검사를 강제를 수행하도록 수정합니다.
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2010-2556 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-2556)을 참조하십시오.

#### 초기화되지 않은 메모리 손상 취약점(CVE-2010-2556)에 대한 완화 요소
...
#### 초기화되지 않은 메모리 손상 취약점(CVE-2010-2556)에 대한 대안
...
#### 초기화되지 않은 메모리 손상 취약점(CVE-2010-2556)에 대한 FAQ
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2010-2557 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-2557)을 참조하십시오.

#### 초기화되지 않은 메모리 손상 취약점(CVE-2010-2557)에 대한 완화 요소
...
#### 초기화되지 않은 메모리 손상 취약점(CVE-2010-2557)에 대한 대안
...
#### 초기화되지 않은 메모리 손상 취약점(CVE-2010-2557)에 대한 FAQ
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2010-2558 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-2558)을 참조하십시오.

#### 경쟁 조건 메모리 손상 취약점(CVE-2010-2558)에 대한 완화 요소

완화란 취약점의 악용 심각도를 낮출 수 있는 설정, 일반적인 구성 또는 최선책, 기본 상태를 지칭합니다. 다음과 같은 완화 요소가 현재 상황에서 유용할 수 있습니다.

-   웹을 통한 공격의 경우 공격자는 호스팅하는 웹 사이트에 이 취약점을 악용하는 웹 페이지를 포함할 수 있습니다. 또한 사용자가 제공한 콘텐츠나 광고를 허용하거나 호스팅하는 공격 당한 웹 사이트에는 이 취약점을 악용할 수 있는 특수하게 조작된 콘텐츠가 포함되어 있을 수 있습니다. 그러나 어떠한 경우에도 공격자는 강제로 사용자가 이러한 웹 사이트를 방문하도록 만들 수 없습니다. 대신 공격자는 사용자가 전자 메일 메시지 또는 메신저 메시지의 링크를 클릭하여 공격자의 웹 사이트를 방문하도록 유도하는 것이 일반적입니다.

#### 경쟁 조건 메모리 손상 취약점(CVE-2010-2558)에 대한 대안

이 취약점에 대한 대안을 확인하지 못했습니다.

#### 경쟁 조건 메모리 손상 취약점(CVE-2010-2558)에 대한 FAQ
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2010-2559 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-2559)를 참조하십시오.

#### 초기화되지 않은 메모리 손상 취약점(CVE-2010-2559)에 대한 완화 요소
...
#### 초기화되지 않은 메모리 손상 취약점(CVE-2010-2559)에 대한 대안
...
#### 초기화되지 않은 메모리 손상 취약점(CVE-2010-2559)에 대한 FAQ
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2010-2560 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-2560)을 참조하십시오.

#### HTML 레이아웃 메모리 손상 취약점(VE-2010-2560)에 대한 완화 요소
...
#### HTML 레이아웃 메모리 손상 취약점(VE-2010-2560)에 대한 대안
...
#### HTML 레이아웃 메모리 손상 취약점(VE-2010-2560)에 대한 FAQ
...
제어판의 <strong>프로그램 추가/제거</strong> 도구 또는 %Windir%\$NTUninstallKB2183461$\Spuninst 폴더의 Spuninst.exe 유틸리티를 사용하십시오.</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"></td>
<td style="border:1px solid black;">지원 대상인 모든 Windows XP 에디션용 Internet Explorer 7:<br />
제어판의 <strong>프로그램 추가/제거</strong> 도구 또는 %Windir%\ie7updates\KB2183461-IE7\spuninst 폴더의 Spuninst.exe 유틸리티를 사용하십시오.</td>
</tr>
<tr class="even">
<td style="border:1px solid black;"></td>
<td style="border:1px solid black;">지원 대상인 모든 Windows XP 에디션용 Internet Explorer 8:<br />
제어판의 <strong>프로그램 추가/제거</strong> 도구 또는 %Windir%\ie8updates\KB2183461-IE8\spuninst 폴더의 Spuninst.exe 유틸리티를 사용하십시오.</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"><strong>파일 정보</strong></td>
<td style="border:1px solid black;"><a href="https://support.microsoft.com/kb/2183461">Microsoft 기술 자료 문서 2183461</a> 참조</td>
</tr>
<tr class="even">
...
-   이벤트 처리기 도메인 간 취약점(CVE-2010-1258)을 보고해 주신 [Google Inc.](https://www.google.com/)의 David Bloom  
-   초기화되지 않은 메모리 손상 취약점(CVE-2010-2556)을 보고해 주신 [VUPEN Vulnerability Research Team (영문)](https://www.vupen.com)의 Nicolas Joly  
-   초기화되지 않은 메모리 손상 취약점(CVE-2010-2557)을 보고해 주신 [VUPEN Vulnerability Research Team (영문)](https://www.vupen.com/)의 Nicolas Joly  
-   경쟁 조건 메모리 손상 취약점(CVE-2010-2558)을 보고해 주신 [VUPEN Vulnerability Research Team (영문)](https://www.vupen.com/)의 Nicolas Joly  
-   초기화되지 않은 메모리 손상 취약점(CVE-2010-2559)을 보고해 주신 Gambino ZaDarkSide  
-   HTML 레이아웃 메모리 손상 취약점(CVE-2010-2560)을 보고해 주신 [VUPEN Vulnerability Research Team (영문)](https://www.vupen.com/)의 Nicolas Joly
