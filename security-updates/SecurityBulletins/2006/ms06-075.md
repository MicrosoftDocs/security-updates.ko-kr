---
TOCTitle: 'MS06-075'
Title: 'Microsoft 보안 공지 MS06-075 - 중요'
ms:assetid: 'ms06-075'
ms:contentKeyID: 61230036
ms:mtpsurl: 'https://technet.microsoft.com/ko-KR/library/ms06-075(v=Security.10)'
---

Microsoft 보안 공지 MS06-075 - 중요
===================================

Windows의 취약점으로 인한 권한 상승 문제점(926255)
--------------------------------------------------
...
-   Microsoft Windows XP 서비스 팩 2 — [업데이트 다운로드](https://www.microsoft.com/download/details.aspx?familyid=966704b5-1a7e-4110-9694-844706a52db7&displaylang=ko)
-   Microsoft Windows Server 2003 — [업데이트 다운로드](https://www.microsoft.com/download/details.aspx?familyid=5ea314a2-d76a-46f9-853b-15ff03f8ad95&displaylang=ko)
-   Microsoft Windows Server 2003(Itanium 기반 시스템용) – [업데이트 다운로드](https://www.microsoft.com/download/details.aspx?familyid=7bceaa11-f655-4e3c-a588-5c49097e970b)
...
이 업데이트는 비공개적으로 확인된 취약점을 해결합니다. 취약점은 이 공지의 "취약점 세부 사항" 섹션에 설명되어 있습니다.
...
<tbody>
<tr class="odd">
<td style="border:1px solid black;"><strong>파일 목록 손상 취약점</strong> - <a href="https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2006-5585">CVE-2006-5585 (영문)</a></td>
...
**참고** 이 보안 업데이트의 Windows Server 2003용 버전은 Windows Server 2003 R2에도 적용됩니다.
...
**Microsoft Windows XP Home Edition 서비스 팩 1 또는 서비스 팩 1a, Windows XP Media Center Edition 2002 서비스 팩 1, Windows XP Media Center Edition 2004 서비스 팩 1, Windows XP Professional 서비스 팩 1 또는 서비스 팩 1a 및 Windows XP Tablet PC Edition 서비스 팩 1에 대한 연장된 보안 업데이트 지원은 2006년 10월 10일에 끝났습니다.**  
...
#### 파일 목록 손상 취약점 - CVE-2006-5585:
  
Microsoft Windows가 특수하게 조작된 파일 목록을 사용하여 응용 프로그램을 시작하는 방식으로 인한 [권한 상승](https://www.microsoft.com/korea/security/glossary.mspx) 취약점이 존재합니다. 이 취약점을 통해 로그온한 사용자가 시스템에 대한 전체 제어 권한을 얻을 수 있습니다.
  
#### 파일 목록 손상 취약점 - CVE-2006-5585에 대한 완화 요소:
  
-   이 취약점을 악용하기 위해서는 공격자가 유효한 로그온 자격 증명을 가지고 로컬로 로그온할 수 있어야 합니다. 익명의 사용자나 원격에서는 이 취약점을 악용할 수 없습니다.
  
#### 파일 목록 손상 취약점 - CVE-2006-5585에 대한 대안:
  
이 취약점에 대해 확인된 대안이 없습니다.
  
#### 파일 목록 손상 취약점 - CVE-2006-5585에 대한 FAQ:
...
**취약점의 원인은 무엇입니까?**  
CSRSS(Client-Server Run-time Subsystem)에서 파일 목록을 올바르지 않게 처리 및 관리하는 것이 원인입니다.
  
**CSRSS(Client Server Run-Time Subsystem)란 무엇입니까?**  
**Csrss**는 Win32 하위 시스템의 사용자 모드 일부입니다(Win32.sys는 커널 모드의 일부). Csrss는 client/server run-time subsystem의 약자로 항상 실행되어야 하는 중요한 하위 시스템입니다. Csrss는 콘솔 창과 스레드의 생성 및/또는 삭제를 담당합니다.
...
**업데이트는 어떤 기능을 수행합니까?**  
업데이트는 Client Server Run-time Subsystem이 데이터를 할당된 버퍼로 전달하기 전에 포함된 파일 목록의 유효성 검사 방식을 수정하여 취약점을 제거합니다.
  
**이 보안 공지를 게시했을 때 이 취약점이 공개되어 있었습니까?**  
아니요. 이 문제점은 일반에 취약점으로 공개되지 않았습니다.
  
**이 취약점은 Microsoft 기술 자료 문서 921337과 무슨 관련이 있습니까?**  
[Microsoft 기술 자료 문서 921337](https://support.microsoft.com/kb/921337/ko-kr)은 시스템 재부팅을 일으킬 수 있는 파일 목록 관련 문제점을 다룹니다. 이 문제점을 검토하면서 Microsoft는 이 보안 업데이트 릴리스로 해결되는 잠재적인 보안 취약점을 확인했습니다.
...
**Windowsxp-kb926255-x86-KOR /quiet**

**참고/quiet** 스위치를 사용하면 모든 메시지가 표시되지 않습니다. 마찬가지로 실패 메시지도 표시되지 않습니다. 관리자는 지원되는 방법 중 하나를 사용하여 **/quiet** 스위치를 사용할 때 설치가 제대로 되었는지 확인해야 합니다. 또한 관리자는 이 스위치를 사용할 때 KB926255.log 파일에서 모든 실패 메시지를 검토해야 합니다.

컴퓨터를 다시 시작하지 않고 보안 업데이트를 설치하려면 Windows XP 명령 프롬프트에서 다음 명령을 사용하십시오.

**Windowsxp-kb926255-x86-KOR /norestart**
...
시스템 관리자는 Spuninst.exe 유틸리티를 사용하여 이 보안 업데이트를 제거할 수도 있습니다. Spuninst.exe 유틸리티는 %Windir%\\$NTUninstallKB926255$\\Spuninst 폴더에 있습니다.
...
**Windowsserver2003-kb926255-x86-KOR /quiet**

**참고/quiet** 스위치를 사용하면 모든 메시지가 표시되지 않습니다. 마찬가지로 실패 메시지도 표시되지 않습니다. 관리자는 지원되는 방법 중 하나를 사용하여 **/quiet** 스위치를 사용할 때 설치가 제대로 되었는지 확인해야 합니다. 또한 관리자는 이 스위치를 사용할 때 KB926255.log 파일에서 모든 실패 메시지를 검토해야 합니다.

컴퓨터를 다시 시작하지 않고 보안 업데이트를 설치하려면 Windows Server 2003 명령 프롬프트에서 다음 명령을 사용하십시오.

**Windowsserver2003-kb926255-x86-KOR /norestart**
...
Windows Server 2003 Enterprise Edition(Itanium 기반 시스템용), Windows Server 2003 Datacenter Edition(Itanium 기반 시스템용), Windows Server 2003(Itanium 기반 시스템용).
...
-   **레지스트리 키 확인**
    ...
    **참고** 이 레지스트리 키에는 설치된 파일의 전체 목록이 포함되어 있지 않을 수도 있습니다. 또한 관리자나 OEM이 926255 보안 업데이트를 Windows 설치 원본 파일에 통합하거나 포함시키는 경우 이러한 레지스트리 키가 제대로 만들어지지 않을 수 있습니다.
