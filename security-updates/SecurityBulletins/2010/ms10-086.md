---
TOCTitle: 'MS10-086'
Title: 'Microsoft 보안 공지 MS10-086 - 보통'
ms:assetid: 'ms10-086'
ms:contentKeyID: 61230346
ms:mtpsurl: 'https://technet.microsoft.com/ko-KR/library/ms10-086(v=Security.10)'
---
...
게시된 날짜: 2010년 10월 13일 수요일 | 업데이트된 날짜: 2010년 11월 11일 목요일
...
이 보안 업데이트의 심각도는 지원 대상인 모든 Windows Server 2008 R2 에디션에 대해 보통입니다. 자세한 내용은 이 섹션에서 **영향을 받는 소프트웨어 및 영향을 받지 않는 소프트웨어**를 참조하십시오.

보안 업데이트는 새 공유 클러스터 디스크를 추가하거나 만들 때 Microsoft Cluster Service(MSCS) 사용자 인터페이스에서 관리 공유에 대한 사용 권한을 설정하는 방식을 변경하여 이 취약점을 해결합니다. 취약점에 대한 자세한 내용은 다음 섹션, **취약점 정보**에서 각 취약점 항목의 자주 제기되는 질문 사항(FAQ)을 참조하십시오.

**권장 사항**. 업데이트 관리 소프트웨어를 사용하여 보안 업데이트를 적용하거나 [Microsoft Update](https://go.microsoft.com/fwlink/?linkid=40747) 서비스를 통해 업데이트를 확인하여 보안 업데이트를 적용하는 방법을 고려해 보는 것이 좋습니다.
...
|-------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------|-------------|---------------------------|
| [Windows Server 2008 R2(x64 기반 시스템용)](https://www.microsoft.com/download/details.aspx?familyid=1de12fdf-b439-4020-9313-a193d47dcfb2&displaylang=ko)\* | 변조           | 보통        | 없음                      |
| [Windows Server 2008 R2(Itanium 기반 시스템용)](https://www.microsoft.com/download/details.aspx?familyid=1c607c7d-6144-4a39-beea-a31b62085047)              | 변조           | 보통        | 없음                      |
...
**이 공지가 2010년 11월 10일에 개정된 이유는 무엇입니까?**  
Microsoft는 이 업데이트가 기존 공유 클러스터 디스크의 권한을 실제로 수정한다는 사실을 표시하기 위해 이 공지를 개정했습니다. 이전 공지에서는 이 업데이트가 기존 클러스터 디스크의 권한을 수정하지 않는다고 설명했습니다. 이 변경 사항은 정보에만 해당됩니다. 이미 시스템을 성공적으로 업데이트한 고객은 추가 조치를 취할 필요가 없습니다.

**이 보안 업데이트가 기존 공유 클러스터 디스크에 대한 사용 권한을 수정합니까?**  
아니요, 이 업데이트는 기존 클러스터 디스크에 대한 사용 권한을 변경하지 않습니다. 이 업데이트가 새로 만든 클러스터 디스크에 대한 기본 사용 권한을 설정하는 방식을 수정하지만 다양한 고객 환경으로 인해 기존 사용 권한은 변경되지 않습니다. 업데이트를 적용한 후에는 관리자가 기존 클러스터 디스크 관리 공유에 대한 사용 권한을 확인하고 특정 환경에 적합한 액세스 수준을 설정해야 합니다.
...
<th style="border:1px solid black;" >새 클러스터 디스크 사용 권한 취약점(CVE-2010-3223)</th>
...
장애 조치(Failover) 클러스터 관리자 사용자 인터페이스에서 공유 클러스터 디스크에 대한 사용 권한을 처리하는 방식에 변조 취약점이 존재합니다. 이 취약점은 클러스터에 디스크를 추가할 때 장애 조치(Failover) 클러스터 관리자가 보안되지 않은 기본 사용 권한을 사용하기 때문에 존재합니다. 관리자가 공유 클러스터에 디스크를 추가하는 경우 장애 조치(Failover) 클러스터 관리자는 권한이 없는 사용자(모든 사람)에게 장애 조치(Failover) 클러스터 디스크의 관리 공유에 대한 읽기/쓰기/삭제 권한을 잠재적으로 제공하는 방식으로 공유 클러스터 디스크에 대한 사용 권한을 설정합니다.
  
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2010-3223 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-3223)을 참조하십시오.
  
#### 새 클러스터 디스크 사용 권한 취약점(CVE-2010-3223)에 대한 완화 요소
  
이 취약점에 대해 확인된 완화 요소가 없습니다.
  
#### 새 클러스터 디스크 사용 권한 취약점(CVE-2010-3223)에 대한 대안
  
대안이란 기본적인 취약점을 수정하지는 않지만 업데이트를 적용하기 전에 알려진 공격 경로를 차단하는 데 유용한 설정 또는 구성 변경을 지칭합니다. Microsoft는 다음과 같은 대안을 테스트했으며, 대안으로 인한 기능 저하 여부를 해당 항목에서 설명합니다.
  
-   클러스터 디스크 관리 공유를 만든 후 환경에 적합한 액세스 권한을 반영하도록 해당 액세스 제어 목록(ACL)을 수동으로 구성합니다. 예를 들어 모든 사람 - 모든 권한에서 관리자 - 모든 권한으로 관리 공유에 대한 사용 권한을 변경합니다.  
-   서버 관리자가 업데이트 적용 후 영향을 받는 디스크를 다시 클러스터하도록 선택할 수도 있습니다. 그러면 새 공유 클러스터 디스크에 대한 사용 권한이 제대로 설정됩니다.
  
#### 새 클러스터 디스크 사용 권한 취약점(CVE-2010-3223)에 대한 FAQ
  
**이 취약점을 악용하면 어느 작업까지 가능합니까?**    
이 취약점은 변조 취약점입니다. 이 취약점 악용에 성공한 공격자는 영향을 받는 파일 시스템의 데이터를 임의로 추가, 변경 또는 삭제하거나 읽을 수 있습니다.
  
**취약점의 원인은 무엇입니까?**    
이 취약점은 디스크 클러스터에 디스크를 추가할 때 Windows에서 기본적으로 사용 권한을 설정하는 방식으로 인해 발생합니다.
  
**관리 공유는 어떤 용도로 사용됩니까?**    
관리 공유는 Windows 운영 체제에서 만드는 기본 네트워크 공유입니다. 기본적으로 이러한 공유를 사용하면 Administrators 그룹에 속한 사용자는 누구나 시스템에 연결된 디스크의 루트 디렉터리를 관리할 수 있습니다.
  
**이 취약점의 영향을 받는 구성 요소는 무엇입니까?**    
이 취약점의 영향을 받는 구성 요소는 Microsoft Cluster Service(MSCS)입니다.
  
**공격자는 취약점을 악용하여 무엇을 할 수 있습니까?**    
이 취약점 악용에 성공한 공격자는 영향을 받는 클러스터 디스크 구성의 관리 공유에 있는 데이터를 추가, 변경 또는 삭제하거나 읽을 수 있습니다.
  
**공격자는 이러한 취약점을 어떻게 악용합니까?**    
공격자는 클러스터 디스크 관리 공유의 파일 시스템에 원격으로 액세스할 수 있습니다.
  
**취약점으로 인해 주로 공격 받을 위험이 있는 시스템은 무엇입니까?**    
장애 조치(Failover) 클러스터링을 사용하는 모든 시스템이 주로 이 취약점으로 인해 공격을 받을 위험이 있습니다. 장애 조치(Failover) 클러스터에 속하지 않는 서버는 이 취약점의 영향을 받지 않으며 업데이트가 제공되지 않습니다.
  
**업데이트는 어떤 기능을 수행합니까?**    
이 업데이트는 새 공유 클러스터 디스크를 추가할 때 Microsoft Cluster Service 사용자 인터페이스에서 관리 공유에 대한 사용 권한을 설정하는 방식을 변경하여 이 취약점을 해결합니다.
  
자세한 내용은 이전 섹션, **이 보안 업데이트와 관련된 자주 제기되는 질문 사항**의 FAQ 항목, "이 보안 업데이트가 기존 공유 클러스터 디스크에 대한 사용 권한을 수정합니까?"를 참조하십시오.
...
<td style="border:1px solid black;">Microsoft Cluster Service(MSCS) 구성이 있는 지원 대상인 모든 x64 기반 Windows Server 2008 R2 에디션:<br />
windows6.1-kb2294255-x64.msu /quiet</td>
</tr>
<tr class="even">
<td style="border:1px solid black;"></td>
<td style="border:1px solid black;">Microsoft Cluster Service(MSCS) 구성이 있는 지원 대상인 모든 Itanium 기반 Windows Server 2008 R2 에디션:<br />
...
<td style="border:1px solid black;"><a href="https://support.microsoft.com/kb/2294255">Microsoft 기술 자료 문서 2294255</a> 참조</td>
</tr>
<tr class="odd">
...
-   V1.1(2010년 11월 11일): 이 업데이트가 기존 공유 클러스터 디스크의 권한을 실제로 수정한다는 사실을 알리기 위해 업데이트 FAQ가 추가되었습니다. 이 변경 사항은 정보에만 해당됩니다. 이미 시스템을 성공적으로 업데이트한 고객은 추가 조치를 취할 필요가 없습니다.
