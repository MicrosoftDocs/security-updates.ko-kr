...
[](https://go.microsoft.com/fwlink/?linkid=251095)[MS12-038](https://go.microsoft.com/fwlink/?linkid=251095)</a>의 KB2686827, [](https://go.microsoft.com/fwlink/?linkid=242032)[MS12-025](https://go.microsoft.com/fwlink/?linkid=242032)</a>의 KB2656368 및 [](https://go.microsoft.com/fwlink/?linkid=235370)[MS12-016](https://go.microsoft.com/fwlink/?linkid=235370)</a>의 KB2633870은 KB2742595로 대체되었습니다.
</td>
</tr>
...
<tr>
<td style="border:1px solid black;">
Windows 7(32비트 시스템용)에 설치된 Microsoft .NET Framework 4  
(KB2742595)<sup>[1]</sup>
</td>
<td style="border:1px solid black;">
보통  
...
<tr>
<td style="border:1px solid black;">
Windows 7(x64 기반 시스템용)에 설치된 Microsoft .NET Framework 4  
(KB2742595)<sup>[1]</sup>
</td>
<td style="border:1px solid black;">
보통  
...
<tr>
<td style="border:1px solid black;">
Windows Server 2008 R2(x64 기반 시스템용)에 설치된 Microsoft .NET Framework 4  
(KB2742595)<sup>[1]</sup>
</td>
<td style="border:1px solid black;">
보통  
...
<tr>
<td style="border:1px solid black;">
Windows Server 2008 R2(Itanium 기반 시스템용)에 설치된 Microsoft .NET Framework 4  
(KB2742595)<sup>[1]</sup>
</td>
<td style="border:1px solid black;">
보통  
...
<tr class="alternateRow">
<td style="border:1px solid black;">
Windows Server 2008 R2(x64 기반 시스템용, Server Core 설치)의 Microsoft .NET Framework 3.5.1  
(KB2742598)
</td>
<td style="border:1px solid black;">
해당 사항 없음
...
.NET Framework의 Windows Forms에서 비관리 메모리 개체에 대한 포인터를 다루는 방식에 정보 유출 취약점이 존재합니다.

이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2013-0001](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2013-0001)(영문)을 참조하십시오.

#### System Drawing 정보 유출 취약점(CVE-2013-0001)에 대한 완화 요소

이 취약점에 대해 확인된 완화 요소가 없습니다.

#### System Drawing 정보 유출 취약점(CVE-2013-0001)에 대한 대안

이 취약점에 대한 대안을 확인하지 못했습니다.

#### System Drawing 정보 유출 취약점(CVE-2013-0001)에 대한 FAQ

이 취약점을 악용하면 어느 작업까지 가능합니까?  
이는 정보 유출 취약점입니다.

취약점의 원인은 무엇입니까?  
이 취약점은 .NET Framework에서 메모리 배열의 내용을 제대로 초기화하지 못하기 때문에 발생합니다.

공격자는 취약점을 악용하여 무엇을 할 수 있습니까?  
취약점 악용에 성공한 공격자는 CAS(코드 액세스 보안) 제한을 우회하고 비관리 메모리 위치에 저장된 데이터를 가져올 수 있습니다. 이 데이터에는 별도로 액세스할 수 없는 기밀 정보가 포함되었을 수 있습니다. 공격자가 이 취약점을 악용하더라도 코드를 실행하거나 사용자 권한을 승격시킬 수는 없습니다.

공격자는 이러한 취약점을 어떻게 악용합니까?  
이 취약점을 악용할 수 있는 공격 시나리오에는 웹 탐색 시나리오 및 CAS 제한의 Windows .NET 응용 프로그램 우회 시나리오의 두 가지가 있습니다. 아래에는 이러한 시나리오에 대한 자세한 설명이 나와 있습니다.
...
-   Windows .NET 응용 프로그램 공격 시나리오
    ...
    이 취약점은 CAS(코드 액세스 보안) 제한을 우회하고 비관리 메모리 위치에 저장된 데이터를 가져오기 위해 Windows .NET Framework 응용 프로그램에서 사용될 수도 있습니다.

Windows Forms(WinForms)란 무엇입니까?  
Windows Forms는 .NET Framework용 스마트 클라이언트 기술로, 파일 시스템에 대한 읽기/쓰기와 같은 공통 응용 프로그램 작업을 간소화하는 관리형 라이브러리 집합입니다. Visual Studio와 같은 개발 환경을 사용하는 경우, 정보를 표시하고 사용자 입력을 요청하고 네트워크를 통해 원격 컴퓨터와 통신하는 Windows Forms 스마트 클라이언트 기술을 구현할 수 있습니다. 자세한 내용은 [Windows Forms 개요](https://msdn.microsoft.com/ko-kr/library/8bxxy49h.aspx)를 참조하십시오.
...
.NET Framework에 포함된 Windows Forms 컨트롤에서 개체를 배열에 복사하기 전에 개체 수의 유효성을 검사하는 방식에 권한 상승 취약점이 존재합니다. 이 취약점을 악용한 공격자는 영향을 받는 시스템을 완전히 제어할 수 있습니다. 이렇게 되면 공격자가 프로그램을 설치할 수 있을 뿐 아니라 데이터를 보거나 변경하거나 삭제할 수 있고 모든 사용자 권한이 있는 새 계정을 만들 수도 있습니다. 시스템에 대한 사용자 권한이 적게 구성된 계정의 사용자는 관리자 권한으로 작업하는 사용자에 비해 영향을 적게 받습니다.

이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2013-0002](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2013-0002)(영문)를 참조하십시오.

#### WinForms 버퍼 오버플로 취약점(CVE-2013-0002)에 대한 완화 요소
...
#### WinForms 버퍼 오버플로 취약점(CVE-2013-0002)에 대한 대안
...
#### WinForms 버퍼 오버플로 취약점(CVE-2013-0002)에 대한 FAQ

이 취약점을 악용하면 어느 작업까지 가능합니까?   
이 취약점은 권한 상승 취약점입니다.

취약점의 원인은 무엇입니까?   
이 취약점은 .NET Framework에서 개체를 배열에 복사하기 전에 메모리 내 개체 수의 유효성을 제대로 검사하지 못하기 때문에 발생합니다.
...
업데이트는 어떤 기능을 수행합니까?   
이 업데이트는 .NET Framework가 메모리 내의 개체를 복사하는 방식을 수정하여 취약점을 해결합니다.
...
.NET Framework의 System.DirectoryServices.Protocol(S.DS.P) 네임스페이스 메서드에서 배열에 개체를 복사하기 전에 메모리 내 개체 크기의 유효성을 검사하는 방식에 권한 상승 취약점이 존재합니다. 이 취약점을 악용한 공격자는 영향을 받는 시스템을 완전히 제어할 수 있습니다. 이렇게 되면 공격자가 프로그램을 설치할 수 있을 뿐 아니라 데이터를 보거나 변경하거나 삭제할 수 있고 모든 사용자 권한이 있는 새 계정을 만들 수도 있습니다. 시스템에 대한 사용자 권한이 적게 구성된 계정의 사용자는 관리자 권한으로 작업하는 사용자에 비해 영향을 적게 받습니다.

이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2013-0003](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2013-0003)(영문)을 참조하십시오.

#### S.DS.P 버퍼 오버플로 취약점(CVE-2013-0003)에 대한 완화 요소
...
#### S.DS.P 버퍼 오버플로 취약점(CVE-2013-0003)에 대한 대안
...
-   Internet Explorer에서 XAML 브라우저 응용 프로그램 사용 안 함
    ...
    7.  설정의 XPS 문서에서 확인 또는 사용 안 함을 클릭한 다음 확인을 클릭합니다.
    8.  보안 탭에서 사용자 지정 수준을 클릭합니다.
    9.  .NET Framework 기반 구성 요소에서 Authenticode로 서명되지 않은 구성 요소 실행을 확인 또는 사용 함으로 설정하고 확인을 클릭합니다.
    10. Authenticode로 서명된 구성 요소 실행에 대해 이 단계를 반복한 다음 확인을 클릭합니다.
    11. 로컬 인트라넷, 사용자 지정 수준을 차례로 클릭합니다. 3~4단계를 반복합니다. 이러한 설정 변경을 확인하는 대화 상자가 표시되면 예를 클릭합니다.
    12. 확인을 클릭하여 Internet Explorer로 돌아갑니다.
...
#### S.DS.P 버퍼 오버플로 취약점(CVE-2013-0003)에 대한 FAQ

이 취약점을 악용하면 어느 작업까지 가능합니까?   
이 취약점은 권한 상승 취약점입니다.

취약점의 원인은 무엇입니까?   
이 취약점은 .NET Framework에서 개체를 배열에 복사하기 전에 메모리 내 개체 크기의 유효성을 제대로 검사하지 못하기 때문에 발생합니다.
...
S.DS.P (System.DirectoryServices.Protocol)란 무엇입니까?   
S.DS.P(System.DirectoryServices.Protocol)는 LDAP(Lightweight Directory Access Protocol) 프로그래밍을 관리 코드 개발자에게 불러오는 네임스페이스입니다. S.DS.P에 대한 자세한 내용은 [S.DS.P(System.DirectoryServices.Protocol)](https://msdn.microsoft.com/library/bb332056) 소개를 참조하십시오.
...
업데이트는 어떤 기능을 수행합니까?   
이 업데이트는 .NET Framework에서 메모리 내 개체를 복사하기 전에 배열 크기의 유효성 검사 방식을 수정하여 취약점을 해결합니다.
...
.NET Framework가 메모리 내 특정 개체의 사용 권한에 대한 유효성을 검사하는 방식에 권한 상승 취약점이 존재합니다. 이 취약점을 악용한 공격자는 영향을 받는 시스템을 완전히 제어할 수 있습니다. 이렇게 되면 공격자가 프로그램을 설치할 수 있을 뿐 아니라 데이터를 보거나 변경하거나 삭제할 수 있고 모든 사용자 권한이 있는 새 계정을 만들 수도 있습니다. 시스템에 대한 사용자 권한이 적게 구성된 계정의 사용자는 관리자 권한으로 작업하는 사용자에 비해 영향을 적게 받습니다.

이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2013-0004](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2013-0004)(영문)를 참조하십시오.

#### 이중 구성 취약점(CVE-2013-0004)에 대한 완화 요소
...
#### 이중 구성 취약점(CVE-2013-0004)에 대한 대안
...
#### 이중 구성 취약점(CVE-2013-0004)에 대한 FAQ

이 취약점을 악용하면 어느 작업까지 가능합니까?   
이 취약점은 권한 상승 취약점입니다.

취약점의 원인은 무엇입니까?   
이 취약점은 .NET Framework가 개체의 사용 권한에 대한 유효성을 제대로 검사하지 못하기 때문에 발생합니다.
...
업데이트는 어떤 기능을 수행합니까?   
이 업데이트는 .NET Framework가 개체의 사용 권한에 대한 유효성을 검사하는 방식을 수정하여 취약점을 해결합니다.
...
<td style="border:1px solid black;"><a href="https://support.microsoft.com/kb/2769324">Microsoft 기술 자료 문서 2769324</a> 참조</td>
</tr>
<tr class="even">
...
<tr class="odd">
<td style="border:1px solid black;">/?</td>
<td style="border:1px solid black;">지원되는 스위치에서 도움말을 표시합니다.</td>
</tr>
<tr class="even">
<td style="border:1px solid black;">/quiet</td>
<td style="border:1px solid black;">자동 모드, 사용자가 입력하지 않아도 되며 출력 결과가 표시되지 않습니다.</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;">/v</td>
<td style="border:1px solid black;">자세한 정보 표시 모드를 사용합니다.</td>
</tr>
<tr class="even">
<td style="border:1px solid black;">/log</td>
<td style="border:1px solid black;">로깅을 사용하며 지정된 위치에 로그 파일을 생성합니다.<br />
e.g. /log &lt;로그 파일의 전체 경로&gt;</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;">/passive</td>
<td style="border:1px solid black;">업데이트 설치 시 진행률 표시줄이 표시되지만 사용자 개입이 필요하지 않습니다.</td>
</tr>
<tr class="even">
<td style="border:1px solid black;">/norestart</td>
<td style="border:1px solid black;">패치를 재시작해야 하는 경우 재시작을 묻는 메시지가 표시되지 않으며 재시작되지 않습니다.</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;">/promptrestart</td>
<td style="border:1px solid black;">패치를 재시작해야 하는 경우 설치 후 재시작 메시지가 표시되며 사용자가 동의하면 재시작됩니다.</td>
</tr>
<tr class="even">
<td style="border:1px solid black;">/extract</td>
<td style="border:1px solid black;">패키지를 추출합니다.</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;">/uninstall</td>
<td style="border:1px solid black;">보안 업데이트를 제거합니다.</td>
</tr>
</tbody>
</table>
...
<br />
지원 대상인 모든 Windows Server 2003 x64 기반 에디션 및 Itanium 기반 에디션에 설치된 Microsoft .NET Framework 1.1 서비스 팩 1:<br />
HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Updates\.NETFramework\1.1\M2742597<br />
&quot;Installed&quot; = dword:1</td>
</tr>
<tr class="odd">
...
Microsoft .NET Framework 1.1 서비스 팩 1의 보안 업데이트는 다음 설치 스위치를 지원합니다.
...
Microsoft .NET Framework 2.0 서비스 팩 2, Microsoft .NET Framework 3.0 서비스 팩 2, Microsoft .NET Framework 4의 보안 업데이트는 다음 설치 스위치를 지원합니다.
...
Microsoft .NET Framework 2.0 서비스 팩 2 및 Microsoft .NET Framework 3.0 서비스 팩 2의 보안 업데이트는 다음 설치 스위치를 지원합니다.
...
Microsoft .NET Framework 1.1 서비스 팩 1, Microsoft .NET Framework 4, Microsoft .NET Framework 4.5의 보안 업데이트는 다음 설치 스위치를 지원합니다.
...
Microsoft .NET Framework 3.5.1의 보안 업데이트는 다음 설치 스위치를 지원합니다.
...
Microsoft .NET Framework 4 및 Microsoft .NET Framework 4.5의 보안 업데이트는 다음 설치 스위치를 지원합니다.
...
<tr class="even">
<td style="border:1px solid black;">배포</td>
<td style="border:1px solid black;">Windows RT의 Microsoft .NET Framework 4.5(KB2742614):<br />
Windows RT 업데이트는 <a href="https://go.microsoft.com/fwlink/?linkid=21130">Windows Update</a>를 통해 제공됩니다.</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;">다시 시작 요구 사항</td>
<td style="border:1px solid black;"></td>
...
Windows RT 업데이트는 [Windows Update](https://go.microsoft.com/fwlink/?linkid=21130)를 통해 제공됩니다.
...
-   System Drawing 정보 유출 취약점(CVE-2013-0001)을 보고해 주신 [iSIGHT Partners Labs](https://www.isightpartners.com/)(영문)의 Jon Erickson
-   WinForms 버퍼 오버플로 취약점(CVE-2013-0002)을 [Tipping Point](https://www.tippingpoint.com/)(영문)의 [Zero Day Initiative](https://www.zerodayinitiative.com/)(영문)와 협력하여 보고해 주신 Vitaliy Toropov
-   S.DS.P 버퍼 오버플로 취약점(CVE-2013-0003)을 [Tipping Point](https://www.tippingpoint.com/)(영문)의 [Zero Day Initiative](https://www.zerodayinitiative.com/)(영문)와 협력하여 보고해 주신 Vitaliy Toropov
-   이중 구성 취약점(CVE-2013-0004)을 보고해 주신 Context Information Security의 James Forshaw
...
-   V1.1(2013년 1월 15일): 요약의 알려진 문제점에 Microsoft 기술 자료 문서 2769324 링크가 추가되었으며 이 공지의 정확하지 않은 KB2742595 업데이트에 대한 레지스트리 확인 키가 수정되었습니다. 이 변경 사항은 정보에만 해당됩니다.
-   V2.0(2013년 1월 23일): 잠재적인 호환성 문제를 가질 수 있다고 알려진 특정 구성을 실행하는 Windows 7 및 Windows Server 2008 R2 시스템에 대한 보안 업데이트 KB2756920을 다시 제공하기 위해 공지를 다시 릴리스했습니다. 이 업데이트를 다시 제공 받은 고객은 이 업데이트를 재설치해야 합니다. 자세한 내용은 업데이트 FAQ를 참조하십시오.
-   V2.1(2013년 2월 13일): Windows Vista 또는 Windows Server 2008의 지원되는 에디션을 실행하는 특정 시스템에 대한 Windows Update의 다시 제공 문제를 수정하기 위한 .NET Framework 1.1 서비스 팩 1 업데이트(KB2742597)의 검색 변경 사항을 알리기 위해 공지를 수정하였습니다. 이 변경 사항은 검색에만 해당됩니다. 이미 시스템을 성공적으로 업데이트한 고객은 추가 조치를 취할 필요가 없습니다.
-   V2.2(2013년 12월 17일): Windows RT에 설치된 Microsoft .NET Framework 4.5의 제공 문제(2742614)를 수정하는 검색 변경 사항을 알리기 위해 공지를 개정했습니다. 이 변경 사항은 검색에만 해당됩니다. 업데이트 파일에 대한 변경 사항은 없습니다. 이 업데이트를 성공적으로 설치한 고객은 조치를 취할 필요가 없습니다.
