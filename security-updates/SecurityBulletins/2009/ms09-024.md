---
TOCTitle: 'MS09-024'
Title: 'Microsoft 보안 공지 MS09-024 - 긴급'
ms:assetid: 'ms09-024'
ms:contentKeyID: 61230210
ms:mtpsurl: 'https://technet.microsoft.com/ko-KR/library/ms09-024(v=Security.10)'
---


Microsoft 보안 공지 MS09-024 - 긴급
===================================

Microsoft Works 변환기의 취약점으로 인한 원격 코드 실행 문제점 (957632)
-----------------------------------------------------------------------
...
이 보안 업데이트는 비공개로 보고된 Microsoft Works 변환기의 취약점을 해결합니다. 이러한 취약점으로 인해 사용자가 특수하게 조작된 Works 파일을 열 경우 원격 코드 실행이 허용될 수 있습니다. 취약점 악용에 성공한 공격자는 로컬 사용자와 동일한 권한을 얻을 수 있습니다. 시스템에 대한 사용자 권한이 적게 구성된 계정의 사용자는 관리자 권한으로 작업하는 사용자에 비해 영향을 적게 받습니다.

이 보안 업데이트의 심각도는 지원되는 Microsoft Office Word 2000 에디션에 대해 긴급입니다. 또한 지원되는 Microsoft Office Word 2002, Microsoft Works 6–9 파일 변환기가 있는 Microsoft Office Word 2003 및 Microsoft Office Word 2007 서비스 팩 1, 그리고 Microsoft Works 8.5 및 Microsoft Works 9.0에 대해 중요입니다. 자세한 내용은 이 섹션의 **영향을 받는 소프트웨어 및 영향을 받지 않는 소프트웨어**를 참조하십시오.

보안 업데이트는 Microsoft Office에서 Works 파일을 여는 방식을 수정하여 취약점을 해결합니다. 취약점에 대한 자세한 내용은 다음 섹션, **취약점 정보**에서 각 취약점 항목의 자주 제기되는 질문 사항(FAQ)을 참조하십시오.
...
**MS09-017도 Microsoft Works 8.5 및 Microsoft Works 9의 취약점에 대해 설명하고 있습니다. MS09-017는 이 공지(MS09-024)와 어떤 관련이 있습니까?**  
Microsoft Works 8.5 (KB967043) 및 Microsoft Works 9 (KB967044)에 대한 이 공지(MS09-024)의 보안 업데이트 패키지는 [MS09-017](https://technet.microsoft.com/security/bulletin/ms09-017)에 설명되어 있는 취약점도 해결합니다. Microsoft Works 8.5 또는 Microsoft Works 9가 설치된 사용자는 이 업데이트 패키지를 한 번만 설치하면 됩니다.

**Microsoft Works 6-9 파일 변환기란 무엇입니까?**  
Microsoft Works 6-9 변환기는 Microsoft Office Word 2003이 Works 버전 6, 7, 8 및 9에서 지원하는 Works Word Processor 파일 형식의 파일을 열고, 편집하고, 저장할 수 있도록 해줍니다. 자세한 내용은 [Microsoft Works 6–9 파일 변환기](https://www.microsoft.com/download/details.aspx?familyid=bf41401e-70fa-465d-ae2e-cf44dbf05297&displaylang=en)를 참조하십시오.
...
<th style="border:1px solid black;" >
파일 변환기 버퍼 오버플로 취약점(CVE-2009-1533)
</th>
<th style="border:1px solid black;" >
전체 심각도
</th>
...
Works for Windows 문서 변환기에서 특수하게 조작된 Works 파일을 처리하는 방식에 원격 코드 실행 취약점이 존재합니다. 이러한 취약점으로 인해 사용자가 특수하게 조작된 .wps 파일을 열 경우 원격 코드 실행이 허용될 수 있습니다. 시스템에 대한 사용자 권한이 적게 구성된 계정의 사용자는 관리자 권한으로 작업하는 사용자에 비해 영향을 적게 받습니다.

이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2009-1533 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2009-1533)을 참조하십시오.

#### 파일 변환기 버퍼 오버플로 취약점(CVE-2009-1533)에 대한 완화 요소

완화란 취약점의 악용 심각도를 낮출 수 있는 설정, 일반적인 구성 또는 최선책, 기본 상태를 지칭합니다. 다음과 같은 완화 요소가 현재 상황에서 유용할 수 있습니다.

-   2007 Microsoft Office System 서비스 팩 2는 취약점의 영향을 받지 않습니다.
-   Microsoft Office Word 2003은 Works 변환기가 설치된 경우에만 영향을 받습니다. 자세한 내용은 [Microsoft Works 6–9 파일 변환기](https://www.microsoft.com/download/details.aspx?familyid=bf41401e-70fa-465d-ae2e-cf44dbf05297&displaylang=en)를 참조하십시오.
...
#### 파일 변환기 버퍼 오버플로 취약점(CVE-2009-1533)에 대한 대안

대안이란 기본적인 취약점을 수정하지는 않지만 업데이트를 적용하기 전에 알려진 공격 경로를 차단하는 데 유용한 설정 또는 구성 변경을 지칭합니다. Microsoft는 다음과 같은 대안을 테스트했으며, 대안으로 인한 기능 저하 여부를 해당 항목에서 설명합니다.

-   **Word 2000 및 Word 2002의 경우 액세스를 제한하여 Works 4.x 변환기를 사용하지 않습니다.**

    관리자는 영향을 받는 변환기에 액세스 제어 목록을 적용하여 Works 및 Office에서 더 이상 변환기를 로드하지 않도록 할 수 있습니다. 이 방법은 이 공격 경로를 사용하는 취약점 악용을 효과적으로 방지할 수 있습니다.

    **Microsoft Windows 2000, Windows XP 및 Windows Server 2003의 경우**
    ...
    **대안의 영향.** 대안을 실행하면 사용자가 더 이상 Works 4.x 문서를 열거나 저장할 수 없습니다. Works 4.x 문서를 열려고 하면 "파일이 손상되었습니다." 오류가 반환됩니다.

-   **Microsoft Works 6–9 파일 변환기가 있는**[Word 2003과](https://www.microsoft.com/download/details.aspx?familyid=bf41401e-70fa-465d-ae2e-cf44dbf05297&displaylang=en)**Word 2007의 경우 액세스를 제한하여 Works 6-9 변환기를 사용하지 않습니다.**
    ...
    **대안의 영향.** 대안을 실행하면 사용자가 더 이상 Works 6-9 문서를 열거나 저장할 수 없습니다. Works 6-9 문서를 열려고 하면 "파일이 손상되었습니다." 오류가 반환됩니다.

#### 파일 변환기 버퍼 오버플로 취약점(CVE-2009-1533)에 대한 FAQ
...
**취약점의 원인은 무엇입니까?**  
특수하게 조작된 Works 파일을 사용자가 열면 시스템 메모리가 손상되어 공격자가 임의 코드를 실행할 수 있습니다.
...
이 취약점을 악용하려면 사용자가 영향을 받는 소프트웨어 버전에서 특수하게 조작된 Works 파일을 열어야 합니다.

전자 메일을 통한 공격의 경우 공격자는 특수하게 조작된 Works 파일을 사용자에게 보내고 파일을 열도록 유도하는 방식으로 이 취약점을 악용할 수 있습니다.

웹 기반의 공격 시나리오에서 웹 사이트는 이 취약점을 악용하기 위해 사용되는 Works 파일을 포함하고 있을 수 있습니다. 공격자는 사용자가 전자 메일 메시지 또는 메신저 메시지의 링크를 클릭하여 공격자의 웹 사이트를 방문한 후에 특수하게 조작된 Works 파일을 열도록 유도하는 것이 일반적입니다.

**취약점으로 인해 주로 공격받을 위험이 있는 시스템은 무엇입니까?**  
워크스테이션 및 터미널 서버와 같이 영향을 받는 제품을 사용하는 시스템이 주로 위험합니다. 관리자가 사용자가 서버에 로그온하여 프로그램을 실행할 수 있도록 허용할 경우, 서버가 더 위험할 수 있습니다. 가장 좋은 방법은 이러한 기능을 허용하지 않는 것입니다.

**업데이트는 어떤 기능을 수행합니까?**  
업데이트는 Microsoft Office에서 Works 파일을 여는 방식을 수정하여 취약점을 해결합니다.
...
| Microsoft Office 2007 서비스 팩 1 | 예       |
...
| Office 2003 서비스 팩 3           | 예      | 예                   | 예                  | 예                         |
| Microsoft Office 2007 서비스 팩 1 | 아니요  | 아니요               | 예                  | 예                         |
...
<td style="border:1px solid black;">Office2000-kb957838-fullfile-kor /q:a</td>
</tr>
<tr class="even">
<td style="border:1px solid black;">다시 시작하지 않고 설치</td>
<td style="border:1px solid black;">Office2000-kb957838-fullfile-kor /r:n</td>
</tr>
<tr class="odd">
...
<td style="border:1px solid black;"><a href="https://support.microsoft.com/kb/957838">Microsoft 기술 자료 문서 957838</a> 참조</td>
</tr>
<tr class="even">
...
<tr class="odd">
<td style="border:1px solid black;">사용자 개입 없이 설치</td>
<td style="border:1px solid black;">OfficeXP-kb957646-fullfile-kor /q:a</td>
</tr>
<tr class="even">
<td style="border:1px solid black;">다시 시작하지 않고 설치</td>
<td style="border:1px solid black;">OfficeXP-kb957646-fullfile-kor /r:n</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;">로그 파일 업데이트</td>
<td style="border:1px solid black;">해당 사항 없음</td>
...
<tr class="odd">
<td style="border:1px solid black;"><strong>파일 정보</strong></td>
<td style="border:1px solid black;"><a href="https://support.microsoft.com/kb/957646">Microsoft 기술 자료 문서 957646</a> 참조</td>
</tr>
<tr class="even">
<td style="border:1px solid black;"><strong>레지스트리 키 확인</strong></td>
<td style="border:1px solid black;">해당 사항 없음</td>
...
#### Microsoft Works 6–9 파일 변환기가 있는 Word 2003(모든 에디션)
...
<tr class="even">
<td style="border:1px solid black;">사용자 개입 없이 설치</td>
<td style="border:1px solid black;">Microsoft Works 6–9 파일 변환기가 있는 Microsoft Office Word 2003:<br />
...
<tr class="even">
<td style="border:1px solid black;"><strong>파일 정보</strong></td>
<td style="border:1px solid black;"><a href="https://support.microsoft.com/kb/968326">Microsoft 기술 자료 문서 968326</a> 참조</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"><strong>레지스트리 키 확인</strong></td>
<td style="border:1px solid black;">해당 사항 없음</td>
...
<tr class="even">
<td style="border:1px solid black;"><strong>파일 정보</strong></td>
<td style="border:1px solid black;"><a href="https://support.microsoft.com/kb/969559">Microsoft 기술 자료 문서 969559</a> 참조</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"><strong>레지스트리 키 확인</strong></td>
<td style="border:1px solid black;">해당 사항 없음</td>
...
-   파일 변환기 버퍼 오버플로 취약점(CVE-2009-1533)을 보고해 주신 [NGS Software (영문)](https://www.ngssoftware.com/)의 Shaun Colley
