---
TOCTitle: 'MS13-006'
Title: 'Microsoft 보안 공지 MS13-006 - 중요'
ms:assetid: 'ms13-006'
ms:contentKeyID: 61230556
ms:mtpsurl: 'https://technet.microsoft.com/ko-KR/library/ms13-006(v=Security.10)'
---

Security Bulletin

Microsoft 보안 공지 MS13-006 - 중요
===================================
...
이 보안 업데이트는 Windows SSL 공급자 구성 요소가 암호화된 네트워크 패킷을 처리하는 방식을 수정하여 이 취약점을 해결합니다. 취약점에 대한 자세한 내용은 취약점 정보에서 각 취약점 항목의 자주 제기되는 질문 사항(FAQ)을 참조하십시오.
...
알려진 문제점. 이 보안 업데이트를 설치할 때 고객이 겪을 수 있는 현재까지 알려진 문제점은 [Microsoft 기술 자료 문서 2785220](http://support.microsoft.com/kb/2785220)에 나와 있습니다. 이 문서는 이러한 문제점에 대한 권장 해결 방법도 소개합니다.
...
이 보안 업데이트의 SHA1 및 SHA2 해시는 다운로드한 보안 업데이트 패키지의 진위 여부를 검증하는 데 사용할 수 있습니다. 이 업데이트와 관련된 해시 정보는 [Microsoft 기술 자료 문서 2785220](https://support.microsoft.com/kb/2785220)을 참조하십시오.
...
<span></span>
Microsoft Windows SSL/TLS(Secure Socket Layer 및 Transport Layer Security)에서 SSL 버전 3(SSLv3) 및 TLS 프로토콜을 처리하는 방식에 보안 기능 우회 취약점이 존재합니다. 이 취약점으로 인해 공격자가 SSL/TLS 세션에 특수하게 조작된 콘텐츠를 주입하는 경우 보안 기능을 우회할 수 있습니다.

이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2013-0013](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2013-0013)(영문)을 참조하십시오.

#### Microsoft SSL 버전 3 및 TLS 프로토콜 보안 기능 우회 취약점(CVE-2013-0013)에 대한 완화 요소

이 취약점에 대해 확인된 완화 요소가 없습니다.

#### Microsoft SSL 버전 3 및 TLS 프로토콜 보안 기능 우회 취약점(CVE-2013-0013)에 대한 대안

대안이란 기본적인 취약점을 수정하지는 않지만 업데이트를 적용하기 전에 알려진 공격 경로를 차단하는 데 유용한 설정 또는 구성 변경을 지칭합니다. Microsoft는 다음과 같은 대안을 테스트했으며, 대안으로 인한 기능 저하 여부를 해당 항목에서 설명합니다.

-   Internet Explorer에서 SSLv2를 사용하지 않도록 구성

    Internet Explorer에서 SSLv2를 사용하지 않도록 설정하여 이 취약점을 악용하려는 시도로부터 보호할 수 있습니다. 이렇게 설정하려면 다음 단계를 수행하십시오.

    1.  Internet Explorer에서 도구 메뉴의 인터넷 옵션을 클릭합니다.
    2.  고급 탭을 클릭합니다.
    3.  보안 섹션으로 스크롤합니다.
    4.  SSL 2.0 사용 확인란의 선택을 취소합니다.
    5.  확인을 클릭하여 Internet Explorer로 돌아갑니다.

    대안의 영향. SSLv2 제한에 따른 부작용이 생깁니다. 인터넷 또는 인트라넷의 일부 웹 사이트에서 SSLv2를 사용하여 보안 통신을 수행할 수 있습니다. SSLv2를 사용하지 않는 경우 해당 웹 사이트를 사용할 수 없습니다.
    ...
    2.  고급 탭을 클릭합니다.
    3.  보안 섹션으로 스크롤합니다.
    4.  SSL 2.0 사용 확인란을 선택합니다.
    5.  확인을 클릭하여 Internet Explorer로 돌아갑니다.

-   IIS(인터넷 정보 서비스)에서 SSLv2를 사용하지 않도록 구성

    경고 레지스트리를 잘못 수정하면 시스템 전체에 심각한 문제가 발생할 수 있으며 운영 체제를 다시 설치해야 할 수도 있습니다. Microsoft는 레지스트리를 잘못 수정하여 발생하는 문제에 대한 해결을 보증하지 않습니다. 레지스트리의 수정에 따른 모든 책임은 사용자에게 있습니다.

    IIS 웹 사이트에서 SSLv2를 사용하지 않도록 설정하여 이 취약점을 악용하는 시도로부터 보호할 수 있습니다. 이렇게 설정하려면 다음 단계를 수행하십시오.

    1.  시작, 실행을 차례로 클릭하고 regedit을 입력한 다음 확인을 클릭합니다.
    2.  레지스트리 편집기에서 다음 레지스트리 키/폴더를 찾습니다. `HKey_Local_Machine\System\CurrentControlSet\Control\SecurityProviders \SCHANNEL\Protocols\SSL 2.0`
    3.  SSL 2.0 폴더를 마우스 오른쪽 단추로 클릭하고 새로 만들기를 선택한 다음 키를 클릭합니다. 새 폴더의 이름을 Server로 지정합니다.
    4.  Server 폴더에서 편집 메뉴를 클릭하고 새로 만들기를 선택한 다음 DWORD(32비트) 값을 클릭합니다.
    5.  이름에 Enabled를 입력하고 Enter를 칩니다.
    6.  데이터 열 아래 0x00000000 (0)이 기본값으로 표시되어야 합니다. 그렇지 않은 경우 이름을 마우스 오른쪽 단추로 클릭하고 수정을 선택한 다음 값 데이터에 0을 입력합니다.
    7.  컴퓨터를 다시 시작합니다.

    대안의 영향. SSLv2를 제한하면 부작용이 생깁니다. 일부 웹 브라우저에서 SSLv2를 사용하여 보안 통신을 수행할 수 있습니다. SSLv2를 제한하면 사용자가 해당 웹 사이트를 볼 수 없습니다.

    대안 실행 취소 방법.

    1.  시작, 실행을 차례로 클릭하고 regedit을 입력한 다음 확인을 클릭합니다.
    2.  레지스트리 편집기에서 다음 레지스트리 키/폴더를 찾습니다. `HKey_Local_Machine\System\CurrentControlSet\Control\SecurityProviders\SCHANNEL\Protocols\SSL 2.0\Server`
    3.  Server 폴더를 마우스 오른쪽 단추로 클릭하고 사용하는 DWORD를 클릭한 다음 삭제를 선택합니다.
    4.  예를 선택합니다.
    5.  컴퓨터를 다시 시작합니다.

#### Microsoft SSL 버전 3 및 TLS 프로토콜 보안 기능 우회 취약점(CVE-2013-0013)에 대한 FAQ

이 취약점을 악용하면 어느 작업까지 가능합니까? 
이 취약점은 보안 기능 우회 취약점입니다. 이 취약점을 성공적으로 악용한 공격자는 SSL 버전 3 또는 TLS의 보안 기능을 우회하여 SSL 버전 2로 다운그레이드하여 연결의 보안성을 완화할 수 있습니다.

취약점의 원인은 무엇입니까? 
이 취약점은 Windows에서 SSL/TLS 세션 버전 협상을 제대로 처리하지 못할 때 발생합니다.
...
이 취약점을 성공적으로 악용한 공격자는 암호화된 통신을 모니터링하여 SSL 버전 3 또는 TLS 연결을 SSL 버전 2로, 자동으로 다운그레이드할 수 있습니다. 이로 인해 암호화가 약해질 수 있습니다.

공격자는 이러한 취약점을 어떻게 악용합니까? 
MITM(Man-in-the-Middle) 공격을 통해 공격자는 Internet Explorer와 타사 서버 또는 타사 클라이언트와 Microsoft 서버 간 SSL 버전 3/TLS 브라우징 세션에 조작된 트래픽을 주입하여 해당 연결을 SSL 버전 2로, 자동 다운그레이드할 수 있습니다.
...
SSL/TLS 암호화 트래픽을 보내고 받는 모든 시스템은 이 취약점을 이용하는 공격을 받을 위험이 가장 큽니다.

업데이트는 어떤 기능을 수행합니까? 
이 업데이트는 Windows SSL 공급자 구성 요소가 암호화된 네트워크 패킷을 처리하는 방식을 수정하여 이 취약점을 해결합니다.
...
<td style="border:1px solid black;"><a href="http://support.microsoft.com/kb/2785220">Microsoft 기술 자료 문서 2785220</a> 참조</td>
</tr>
<tr class="odd">
...
| 파일 정보             | [Microsoft 기술 자료 문서 2785220](http://support.microsoft.com/kb/2785220) 참조                                      |
...
-   Microsoft SSL 버전 3 및 TLS 프로토콜 보안 기능 우회 취약점(CVE-2013-0013)을 보고해 주신 [Kenichiro Katayama](https://twitter.com/pin_ptr)(일문)
...
-   V1.1(2013년 2월 13일): 요약의 알려진 문제점에 Microsoft 기술 자료 문서 2785220 링크가 추가되었습니다.  
-   V1.2(2013년 7월 10일): Windows Update의 다시 제공 문제를 수정하기 위해 2785220 업데이트에서 Windows Vista 패키지의 검색 변경 사항을 알리고자 공지가 개정되었습니다. 이 변경 사항은 검색에만 해당됩니다. 이미 시스템을 성공적으로 업데이트한 고객은 추가 조치를 취할 필요가 없습니다.  
-   V1.3(2013년 12월 17일): Windows RT의 제공 문제(2785220)를 수정하는 검색 변경 사항을 알리기 위해 공지를 개정했습니다. 이 변경 사항은 검색에만 해당됩니다. 업데이트 파일에 대한 변경 사항은 없습니다. 이 업데이트를 성공적으로 설치한 고객은 조치를 취할 필요가 없습니다.
