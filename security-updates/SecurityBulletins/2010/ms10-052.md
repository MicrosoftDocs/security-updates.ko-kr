---
TOCTitle: 'MS10-052'
Title: 'Microsoft 보안 공지 MS10-052 - 긴급'
ms:assetid: 'ms10-052'
ms:contentKeyID: 61230313
ms:mtpsurl: 'https://technet.microsoft.com/ko-KR/library/ms10-052(v=Security.10)'
---



Microsoft 보안 공지 MS10-052 - 긴급
===================================

Microsoft MPEG Layer-3 코덱의 취약점으로 인한 원격 코드 실행 문제점(2115168)
----------------------------------------------------------------------------
...
이 보안 업데이트는 비공개적으로 보고된 Microsoft MPEG Layer-3 오디오 코덱의 취약점을 해결합니다. 이 취약점으로 인해 사용자가 특수하게 조작된 미디어 파일을 열거나, 웹 사이트 또는 웹 콘텐츠를 제공하는 응용 프로그램으로부터 특수하게 조작된 스트리밍 콘텐츠를 받을 경우 원격 코드 실행이 허용될 수 있습니다. 취약점 악용에 성공한 공격자는 로컬 사용자와 동일한 권한을 얻을 수 있습니다. 시스템에 대한 사용자 권한이 적게 구성된 계정의 사용자는 관리자 권한으로 작업하는 사용자에 비해 영향을 적게 받습니다.

이 보안 업데이트의 등급은 지원되는 모든 Windows XP 및 Windows Server 2003 버전(지원되는 Itanium 기반 버전 제외)에 대해 긴급입니다. 자세한 내용은 이 섹션에서 **영향을 받는 소프트웨어 및 영향을 받지 않는 소프트웨어**를 참조하십시오.

이 보안 업데이트는 Microsoft MPEG Layer-3 오디오 코덱이 특수하게 조작된 미디어 파일에서 MPEG Layer-3 오디오 스트림을 처리하는 방식을 수정하여 취약점을 해결합니다. 취약점에 대한 자세한 내용은 다음 섹션, **취약점 정보**에서 각 취약점 항목의 자주 제기되는 질문 사항(FAQ)을 참조하십시오.
...
|-------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------|-------------|---------------------------|
| [Windows XP 서비스 팩 3](https://www.microsoft.com/download/details.aspx?familyid=08159149-17de-4640-8818-cb7bd4811531&displaylang=ko)                      | 원격 코드 실행 | 긴급        | 없음                      |
| [Windows XP Professional x64 Edition 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=b7f28d7a-6b27-4059-865b-5fd55edb6299)            | 원격 코드 실행 | 긴급        | 없음                      |
| [Windows Server 2003 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=9c2110ec-7e6c-4e73-9785-0a8196095ea0&displaylang=ko)             | 원격 코드 실행 | 긴급        | 없음                      |
| [Windows Server 2003 x64 Edition 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=fdfad4ca-37c4-4ac5-bebc-a5ad61299503&displaylang=ko) | 원격 코드 실행 | 긴급        | 없음                      |
...
<th style="border:1px solid black;" >MPEG Layer-3 오디오 디코더 버퍼 오버플로 취약점(CVE-2010-1882)</th>
...
MPEG Layer-3 오디오 디코더 버퍼 오버플로 취약점(CVE-2010-1882)  
--------------------------------------------------------------
  

Microsoft DirectShow MP3 필터에서 지원되는 형식의 파일을 처리하는 방식에 원격 코드 실행 취약점이 존재합니다. 이 취약점으로 인해 사용자가 특수하게 조작된 오디오 파일을 열 경우 코드 실행이 허용될 수 있습니다. 사용자가 관리자 권한으로 로그온한 경우, 이 취약점을 악용한 공격자는 영향을 받는 시스템을 완전히 제어할 수 있습니다. 이렇게 되면 공격자가 프로그램을 설치할 수 있을 뿐 아니라 데이터를 보거나 변경하거나 삭제할 수 있고 모든 사용자 권한이 있는 새 계정을 만들 수도 있습니다. 시스템에 대한 사용자 권한이 적게 구성된 계정의 사용자는 관리자 권한으로 작업하는 사용자에 비해 영향을 적게 받습니다.
  
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2010-1882 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-1882)를 참조하십시오.
  
#### MPEG Layer-3 오디오 디코더 버퍼 오버플로 취약점(CVE-2010-1882)에 대한 완화 요소
...
#### MPEG Layer-3 오디오 디코더 버퍼 오버플로 취약점(CVE-2010-1882)에 대한 대안
  
대안이란 기본적인 취약점을 수정하지는 않지만 업데이트를 적용하기 전에 알려진 공격 경로를 차단하는 데 유용한 설정 또는 구성 변경을 지칭합니다. Microsoft는 다음과 같은 대안을 테스트했으며, 대안으로 인한 기능 저하 여부를 해당 항목에서 설명합니다.
  
-   **l3codecx.ax 등록 취소**
  
    Windows XP 및 Windows Server 2003 시스템의 명령 프롬프트에서 다음 명령을 실행합니다.
...
-   **DirectShow에서 MPEG Layer-3 구문 분석 사용 안 함**
  
    아래 지정된 레지스트리 키를 제거하면 DirectShow를 사용하여 MPEG Layer-3 콘텐츠를 재생하는 응용 프로그램에서 MPEG Layer-3 구문 분석을 사용할 수 없게 됩니다.
    ...
    **대화형 방법 사용**
    ...
    4.  레지스트리 파일 내보내기 대화 상자에 **MP3\_Parser\_Backup.reg**를 입력하고 **저장**을 클릭합니다.  
    ...
    3.  **파일** 메뉴에서 **내보내기**를 클릭합니다.  
    4.  레지스트리 파일 내보내기 대화 상자에 **MP3\_Parser\_Backup1.reg**를 입력하고 **저장**을 클릭합니다.  
        **참고** 이렇게 하면 이 레지스트리 키의 백업이 기본적으로 내 문서 폴더에 만들어집니다.  
    5.  키보드의 **Delete** 키를 눌러 레지스트리 키를 삭제합니다. 키 삭제 확인 대화 상자에 레지스트리 키를 삭제할지 묻는 메시지가 표시되면 **예**를 클릭합니다  
    ...
    9.  레지스트리 파일 내보내기 대화 상자에 **MP3\_Parser\_Backup 2.reg**를 입력하고 **저장**을 클릭합니다.  
        **참고** 이렇게 하면 이 레지스트리 키의 백업이 기본적으로 내 문서 폴더에 만들어집니다.  
    ...
    **관리되는 배포 스크립트 사용**
    ...
    2.  다음 내용을 .REG 확장명의 파일(예: Disable\_MP3\_Parser.reg)로 저장합니다.  
        **32비트 Windows 시스템**:  
    ...
    **대화형 방법 사용**
    ...
    3.  레지스트리 파일 가져오기 대화 상자에서 **MP3\_Parser\_Backup.reg**를 선택하고 **열기**를 클릭합니다.
  
    64비트 Windows 시스템:
  
    1.  **시작**, **실행**을 차례로 클릭하고 **열기** 상자에 **Regedit**를 입력한 다음 **확인**을 클릭합니다.  
    2.  **파일** 메뉴를 클릭하고 **가져오기**를 선택합니다.  
    3.  레지스트리 파일 가져오기 대화 상자에서 **MP3\_Parser\_Backup1.reg**를 선택하고 **열기**를 클릭합니다.  
    4.  레지스트리 파일 가져오기 대화 상자에서 **MP3\_Parser\_Backup2.reg**를 선택하고 **열기**를 클릭합니다.
...
#### MPEG Layer-3 오디오 디코더 버퍼 오버플로 취약점(CVE-2010-1882)에 대한 FAQ
...
**취약점의 원인은 무엇입니까?**    
Microsoft MPEG Layer-3 오디오 코덱이 MPEG Layer-3 오디오 스트림을 포함하는 특수하게 조작된 미디어 파일을 올바르게 처리하지 못하기 때문입니다.
  
**Microsoft MPEG Layer-3 오디오 코덱이란 무엇입니까?**    
취약한 MPEG Layer-3 오디오 코덱은 Microsoft DirectShow용 MPEG Layer-3 오디오 코덱(l3codecx.ax)입니다. 이 MPEG Layer-3 오디오 코덱은 Windows Media 및 Windows 운영 체제의 일부로 제공됩니다.
...
**업데이트는 어떤 기능을 수행합니까?**    
이 업데이트는 Microsoft MPEG Layer-3 오디오 코덱이 특수하게 조작된 미디어 파일에서 MPEG Layer-3 오디오 스트림을 처리하는 방식을 수정하여 문제점을 해결합니다.
...
<td style="border:1px solid black;">제어판의 <strong>프로그램 추가/제거</strong> 도구 또는 %Windir%\$NTUninstallKB2115168$\Spuninst 폴더의 Spuninst.exe 유틸리티를 사용하십시오.</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"><strong>파일 정보</strong></td>
<td style="border:1px solid black;"><a href="https://support.microsoft.com/kb/2115168">Microsoft 기술 자료 문서 2115168</a> 참조</td>
</tr>
<tr class="even">
...
-   MPEG Layer-3 오디오 디코더 버퍼 오버플로 취약점(CVE-2010-1882)을 [TippingPoint (영문)](https://www.tippingpoint.com/)의 [Zero Day Initiative (영문)](https://www.zerodayinitiative.com/)와 협력하여 보고해 주신 n.runs AG의 Moritz Jodeit
