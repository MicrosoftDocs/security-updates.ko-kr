---
TOCTitle: 'MS08-034'
Title: 'Microsoft 보안 공지 MS08-034 - 중요'
ms:assetid: 'ms08-034'
ms:contentKeyID: 61230142
ms:mtpsurl: 'https://technet.microsoft.com/ko-KR/library/ms08-034(v=Security.10)'
---
...
이 보안 업데이트의 심각도는 지원되는 모든 Microsoft Windows 2000 Server 및 Microsoft Server 2003 에디션에 대해 중요입니다. 자세한 내용은 이 섹션에서 **영향을 받는 소프트웨어 및 영향을 받지 않는 소프트웨어** 하위 섹션을 참조하십시오.

업데이트는 WINS에서 처리되는 네트워크 패킷의 데이터 구조에 대한 유효성 검사를 수행하여 취약점을 제거합니다. 취약점에 대한 자세한 내용은 **취약점 정보**에서 각 취약점 항목의 자주 제기되는 질문 사항(FAQ)을 참조하십시오.
...
|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------|-------------|---------------------------------------------------------------------|
| [Microsoft Windows 2000 Server 서비스 팩 4](https://www.microsoft.com/download/details.aspx?familyid=aa8aa79f-c2cc-440c-9e5c-089143e6f814&displaylang=ko)                                         | 권한 상승      | 중요        | [MS04-045](https://technet.microsoft.com/security/bulletin/ms04-045) |
| [Windows Server 2003 서비스 팩 1 및 Windows Server 2003 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=08fc90d5-23aa-4327-8aef-16bc5170769d&displaylang=ko)                | 권한 상승      | 중요        | 없음                                                                |
| [Windows Server 2003 x64 Edition](https://www.microsoft.com/download/details.aspx?familyid=71675ae8-d60a-4834-b358-2d8e761e62fc&displaylang=ko)                                                   | 권한 상승      | 중요        | [MS04-045](https://technet.microsoft.com/security/bulletin/ms04-045) |
| [Windows Server 2003 x64 Edition 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=71675ae8-d60a-4834-b358-2d8e761e62fc&displaylang=ko)                                       | 권한 상승      | 중요        | 없음                                                                |
| [Windows Server 2003 SP1(Itanium 기반 시스템용) 및 Windows Server 2003 SP2(Itanium 기반 시스템용)](https://www.microsoft.com/download/details.aspx?familyid=87affdc9-d9fe-413c-af30-f3d3b671ec72) | 권한 상승      | 중요        | 없음                                                                |
...
파일 정보 세부 사항은 [Microsoft 기술 자료 문서 948745](https://support.microsoft.com/kb/948745)에 나와 있습니다.

**취약점의 설명은 원격 코드 실행과 유사하게 들립니다. 이 취약점이 권한 상승 취약점인 이유는 무엇입니까?**  
이 취약점을 악용하기 위해 공격자는 영향을 받는 WINS 서버에 유효한 자격 증명으로 로그온하여 악성 SDP 패킷을 조작하는 응용 프로그램을 실행한 다음 해당 패킷을 로컬에서 실행되는 WINS 서비스에 전송해야 합니다. WINS 서비스는 원격 위치에서 보낸 SDP 패킷을 수락하지 않으며, 로컬 호스트에서 보낸 패킷만 수락합니다. 이 취약점을 악용하려면 공격자가 유효한 로그온 자격 증명을 가지고 로컬로 로그온할 수 있어야 합니다. 익명의 사용자에 의해서나 원격으로는 이 취약점을 악용할 수 없습니다.
...
<th style="border:1px solid black;" >메모리 덮어쓰기 취약점(CVE-2008-1451)</th>
...
메모리 덮어쓰기 취약점(CVE-2008-1451)  
-------------------------------------
  

특수하게 조작된 WINS 네트워크 패킷 내의 데이터 구조에 대한 유효성 검사를 WINS에서 충분히 수행하지 않기 때문에 발생하는 권한 상승 취약점이 WINS(Windows Internet Name Service)에 존재합니다. 취약점을 통해 로컬 공격자가 권한을 상승시키는 코드를 실행할 수 있습니다. 이 취약점을 악용한 공격자는 영향을 받는 시스템을 완전히 제어할 수 있습니다. 이렇게 되면 공격자가 프로그램을 설치할 수 있을 뿐 아니라 날짜를 보거나 변경하거나 삭제할 수 있고 새 계정을 만들 수도 있습니다. 이 취약점을 악용하려면 공격자가 유효한 로그온 자격 증명을 가지고 로컬로 로그온할 수 있어야 합니다. 익명의 사용자에 의해서나 원격으로는 이 취약점을 악용할 수 없습니다.
  
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2008-1541 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2008-1451)을 참조하십시오.
  
#### 메모리 덮어쓰기 취약점(CVE-2008-1451)에 대한 완화 요소
  
완화란 취약점의 악용 심각도를 낮출 수 있는 설정, 일반적인 구성 또는 최선책, 기본 상태를 지칭합니다.
  
-   이 취약점에 대한 완화 요소를 확인하지 못했습니다.
  
#### 메모리 덮어쓰기 취약점(CVE-2008-1451)에 대한 대안
  
대안이란 기본적인 취약점을 수정하지는 않지만 업데이트를 적용하기 전에 알려진 공격 경로를 차단하는 데 유용한 설정 또는 구성 변경을 지칭합니다. Microsoft는 다음과 같은 대안을 테스트했으며, 대안으로 인한 기능 저하 여부를 해당 항목에서 설명합니다.
  
-   이 취약점에 대한 대안을 확인하지 못했습니다.
  
#### 메모리 덮어쓰기 취약점(CVE-2008-1451)에 대한 FAQ
  
**이 취약점을 악용하면 어느 작업까지 가능합니까?**  
WINS에 권한 상승 취약점이 존재합니다. 이 취약점 악용에 성공한 공격자는 상승된 권한을 사용하여 코드를 실행할 수 있습니다. 그런 다음 공격자는 영향을 받는 시스템을 완전히 제어할 수 있습니다. 자세한 내용은 [WINS (영문)](https://technet2.microsoft.com/windowsserver/en/library/ea41036e-5cbf-427b-a34f-9e4d1d1519581033.mspx?mfr=true) Technet 문서를 참조하십시오.
  
**취약점의 원인은 무엇입니까?**  
WINS에서 특수하게 조작된 WINS 네트워크 패킷 내의 데이터 구조에 대해 정확한 유효성 검사를 수행하지 않기 때문입니다.
  
**WINS란 무엇입니까?**  
WINS는 NetBIOS 이름을 TCP/IP 네트워크에서 주소로 변환하는 서비스입니다. NetBIOS 및 NetBIOS 이름이 TCP/IP 이외의 네트워크 프로토콜에서 사용될 수도 있지만, WINS(Windows Internet Naming Service)는 특히 NetBT(NetBIOS over TCP/IP)를 지원하도록 설계되었습니다. 사용자가 리소스에 액세스하는 데 NetBIOS 이름이 필요한 모든 환경에서는 WINS가 필수적입니다. 그러한 네트워크에서 WINS를 사용하지 않으면, 원격 네트워크 리소스에 연결하거나 파일 및 인쇄 공유 연결을 구성하기 위해 각 시스템에 해당 LMHOSTS 파일을 포함해야만 합니다.
...
이 취약점을 악용하려면 로컬 공격자가 영향을 받는 WINS 서비스에 특수하게 조작된 패킷을 전송해야 합니다.
  
**취약점으로 인해 주로 공격받을 위험이 있는 시스템은 무엇입니까?**  
Windows WINS 서버가 이 취약점으로 인한 위험에 노출됩니다.
  
**업데이트는 어떤 기능을 수행합니까?**  
업데이트는 WINS에서 처리되는 네트워크 패킷의 데이터 구조에 대한 유효성 검사를 수행하여 취약점을 제거합니다.
...
<td style="border:1px solid black;">Microsoft Windows 2000 Server 서비스 팩 4:<br />
Windows2000-kb948745-x86-kor /quiet</td>
</tr>
<tr class="even">
<td style="border:1px solid black;">다시 시작하지 않고 설치</td>
<td style="border:1px solid black;">Microsoft Windows 2000 Server 서비스 팩 4:<br />
Windows2000-kb948745-x86-kor /norestart</td>
</tr>
<tr class="odd">
...
<td style="border:1px solid black;">Microsoft Windows 2000 Server 서비스 팩 4:<br />
제어판의 프로그램 추가/제거 도구 또는 %Windir%\$NTUninstallKB948745$\Spuninst 폴더의 Spuninst.exe 유틸리티를 사용하십시오.</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"><strong>파일 정보</strong></td>
<td style="border:1px solid black;"><a href="https://support.microsoft.com/kb/948745">Microsoft 기술 자료 문서 948745</a>를 참조하십시오.</td>
</tr>
<tr class="even">
<td style="border:1px solid black;"><strong>레지스트리 키 확인</strong></td>
<td style="border:1px solid black;">Microsoft Windows 2000 Server 서비스 팩 4: 
...
Windowsserver2003-kb948745-x86-kor /quiet</td>
</tr>
<tr class="even">
<td style="border:1px solid black;"></td>
<td style="border:1px solid black;">지원 대상인 모든 Windows Server 2003 x64 기반 에디션:<br />
Windowsserver2003.WindowsXP-KB948745-x64-kor /quiet</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"></td>
...
<td style="border:1px solid black;">다시 시작하지 않고 설치</td>
<td style="border:1px solid black;">지원 대상인 모든 Windows Server 2003 32비트 에디션:<br />
Windowsserver2003-kb948745-x86-kor /norestart</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"></td>
<td style="border:1px solid black;">지원 대상인 모든 Windows Server 2003 x64 기반 에디션:<br />
Windowsserver2003.WindowsXP-KB948745-x64-kor /norestart</td>
</tr>
<tr class="even">
<td style="border:1px solid black;"></td>
<td style="border:1px solid black;">지원 대상인 모든 Windows Server 2003 Itanium 기반 에디션:<br />
...
<tr class="even">
<td style="border:1px solid black;"><strong>제거 정보</strong></td>
<td style="border:1px solid black;">제어판의 <strong>프로그램 추가/제거</strong> 도구 또는 %Windir%\$NTUninstallKB948745$\Spuninst 폴더의 Spuninst.exe 유틸리티를 사용하십시오.</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"><strong>파일 정보</strong></td>
<td style="border:1px solid black;"><a href="https://support.microsoft.com/kb/948745">Microsoft 기술 자료 문서 948745</a>를 참조하십시오.</td>
...
-   V1.1 (2008년 6월 12일): 해당 취약점이 원격 코드 실행 취약점이 아닌 이유를 설명하기 위해 업데이트 FAQ에 항목이 추가되었습니다. 또한 시스템에서 WINS를 사용하도록 설정하지 않은 경우 업데이트가 제공되지 않는 이유를 설명하기 위해 업데이트 FAQ에 항목이 추가되었습니다.
