---
TOCTitle: 'MS12-072'
Title: 'Microsoft 보안 공지 MS12-072 - 긴급'
ms:assetid: 'ms12-072'
ms:contentKeyID: 61230539
ms:mtpsurl: 'https://technet.microsoft.com/ko-KR/library/ms12-072(v=Security.10)'
---

Microsoft 보안 공지 MS12-072 - 긴급
===================================
...
이 보안 업데이트의 심각도는 지원 대상인 모든 Windows XP, Windows Server 2003, Windows Vista, Windows Server 2008(Windows Server 2008 Itanium 기반 시스템용 제외), Windows 7, Windows Server 2008 R2(Windows Server 2008 R2 Itanium 기반 시스템용 제외), Windows 8(Windows RT 제외) 및 Windows Server 2012 에디션에 대해 긴급입니다. Windows Server 2008 Itanium 기반 시스템용, Windows Server 2008 R2 Itanium 기반 시스템용 및 Windows RT는 이 취약점의 영향을 받지 않습니다. 자세한 내용은 이 섹션에서 영향을 받는 소프트웨어 및 영향을 받지 않는 소프트웨어를 참조하십시오.

이 보안 업데이트는 Microsoft Windows가 특수하게 조작된 서류 가방을 처리하는 방식을 수정하여 취약점을 해결합니다. 취약점에 대한 자세한 내용은 다음 섹션, 취약점 정보에서 각 취약점 항목의 자주 제기되는 질문 사항(FAQ)을 참조하십시오.
...
현재 Windows Server 2008을 사용하고 있습니다. 보안 업데이트 KB2727528가 제공되지 않는 이유는 무엇입니까?  
KB2727528 업데이트는 선택적인 [데스크톱 경험 기능](http://technet.microsoft.com/library/cc772567(v=ws.10))이 설치되고 활성화된 경우, 지원되는 Windows Server 2008 에디션을 실행하는 시스템에만 적용됩니다. 데스크톱 경험 기능을 설치하고 사용하는 Windows Server 2008 고객은 KB2727528로 해결되는 취약한 구성 요소를 갖게 되므로 이 보안 업데이트를 설치해야 합니다. 데스크톱 경험 기능을 설치하지 않았거나 설치만 하고 이 기능을 사용하지 않는 고객은 취약한 구성 요소가 없기 때문에 보안 업데이트 패키지 KB2727528가 제공되지 않습니다.

현재 Windows Server 2008 R2를 사용하고 있습니다. 보안 업데이트 KB2727528가 제공되지 않는 이유는 무엇입니까?  
KB2727528 업데이트는 선택적인 [데스크톱 경험 기능](http://technet.microsoft.com/library/cc772567)이 설치되고 활성화된 경우, 지원되는 Windows Server 2008 R2 에디션을 실행하는 시스템에만 적용됩니다. 데스크톱 경험 기능을 설치하고 사용하는 Windows Server 2008 R2 고객은 KB2727528로 해결되는 취약한 구성 요소를 갖게 되므로 이 보안 업데이트를 설치해야 합니다. 데스크톱 경험 기능을 설치하지 않았거나 설치만 하고 이 기능을 사용하지 않는 고객은 취약한 구성 요소가 없기 때문에 보안 업데이트 패키지 KB2727528가 제공되지 않습니다.

파일 정보 세부 사항은 어디에 있습니까?   
파일 정보 세부 사항의 위치에 대해서는 보안 업데이트 배포 섹션에서 참조 표를 참조하십시오.

보안 업데이트의 해시는 어디에 있습니까?   
이 보안 업데이트의 SHA1 및 SHA2 해시는 다운로드한 보안 업데이트 패키지의 진위 여부를 검증하는 데 사용할 수 있습니다. 이 업데이트와 관련된 해시 정보는 [Microsoft 기술 자료 문서 2727528](https://support.microsoft.com/kb/2727528)을 참조하십시오.

Windows 8 Release Preview 또는 Windows Server 2012 출시 후보 버전은 이 권고에서 해결된 취약점의 영향을 받습니까?   
예. Windows 8 Release Preview 및 Windows Server 2012 출시 후보 버전에서 KB2727528 업데이트를 사용할 수 있습니다. Windows 8 Release Preview와 Windows Server 2012 출시 후보 버전을 보유한 고객은 시스템에 이 업데이트를 적용하는 것이 좋습니다. 이 업데이트는 [Windows Update](http://go.microsoft.com/fwlink/?linkid=21130)에서만 구할 수 있습니다.
...
Windows의 서류 가방 기능에 원격 코드 실행 취약점이 존재합니다. 공격자는 특수하게 조작된 서류 가방을 열도록 사용자를 유도하여 이 취약점을 악용할 수 있습니다. 이 취약점 악용에 성공한 공격자는 현재 사용자의 보안 컨텍스트에서 임의 코드를 실행할 수 있습니다.

이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2012-1527](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2012-1527)(영문)을 참조하십시오.

#### Windows 서류 가방 정수 언더플로 취약점(CVE-2012-1527)에 대한 완화 요소
...
#### Windows 서류 가방 정수 언더플로 취약점(CVE-2012-1527)에 대한 대안

대안이란 기본적인 취약점을 수정하지는 않지만 업데이트를 적용하기 전에 알려진 공격 경로를 차단하는 데 유용한 설정 또는 구성 변경을 지칭합니다. Microsoft는 다음과 같은 대안을 테스트했으며, 대안으로 인한 기능 저하 여부를 해당 항목에서 설명합니다.

-   서류 가방 네임스페이스 셸 확장 제거

    경고 레지스트리 편집기를 잘못 사용하면 운영 체제를 다시 설치해야 할 만큼 심각한 문제가 발생할 수 있습니다. Microsoft는 레지스트리 편집기를 잘못 사용하여 발생하는 문제에 대한 해결 여부를 보장하지 않습니다. 레지스트리 편집기의 사용에 따른 모든 책임은 사용자에게 있습니다.

    1.  상승된 권한의 명령 프롬프트에서 다음 명령을 수행하여 레지스트리 키의 백업 사본을 만듭니다.
    2.  Regedit.exe /e briefcase_workaround_rollback.reg HKEY_CLASSES_ROOT\CLSID\{85BBD920-42A0-1069-A2E4-08002B30309D}
    3.  다음과 같은 내용으로 Disable\_briefcase.reg 텍스트 파일을 만듭니다.
    4.  Windows Registry Editor Version 5.00[-HKEY_CLASSES_ROOT\CLSID\{85BBD920-42A0-1069-A2E4-08002B30309D}]
    5.  상승된 권한의 명령 프롬프트에서 다음 명령으로 Disable\_briefcase.reg를 실행합니다.
    6.  Regedit.exe /s Disable_briefcase.reg

    대안의 영향. 서류 가방 네임스페이스 셸 확장을 제거하면 서류 가방 동기화 메커니즘을 더 이상 사용할 수 없습니다.

    대안 실행 취소 방법.

    상승된 권한의 명령 프롬프트에서 다음 명령을 수행하여 원래 상태로 복원합니다.

    `Regedit.exe /s briefcase_workaround_rollback.reg`

#### Windows 서류 가방 정수 언더플로 취약점(CVE-2012-1527)에 대한 FAQ

이 취약점을 악용하면 어느 작업까지 가능합니까?   
이 취약점은 현재 사용자 컨텍스트에 적용되는 원격 코드 실행 취약점입니다.

취약점의 원인은 무엇입니까?   
이 취약점은 Windows가 특수하게 조작된 서류 가방을 제대로 처리하지 못할 때 발생합니다.

서류 가방이란 무엇입니까?   
서류 가방은 두 폴더의 콘텐츠를 동기화하는 Windows의 기능입니다. 서류 가방을 통해 사용자는 USB 썸 드라이브와 같은 장치와 컴퓨터 간에 폴더를 동기화할 수 있습니다. 자세한 내용은 [서류 가방을 사용한 동기화](http://windows.microsoft.com/en-us/windows7/sync-using-briefcase)를 참조하십시오.
...
공격자는 네트워크 공유 위치에 특수하게 조작된 서류 가방을 호스팅하여 사용자가 Windows 탐색기를 사용하여 이 위치를 탐색하도록 유인할 수 있습니다.

취약점으로 인해 위험에 노출되는 시스템은 주로 무엇입니까?   
사용자가 신뢰할 수 없는 네트워크 위치를 탐색할 수 있는 워크스테이션이 여기에 해당합니다. 관리자가 사용자가 서버에 로그온하여 프로그램을 실행할 수 있도록 허용한 경우에도 서버가 위험할 수 있습니다. 가장 좋은 방법은 이러한 기능을 허용하지 않는 것입니다.

업데이트는 어떤 기능을 수행합니까?   
이 업데이트는 Microsoft Windows가 특수하게 조작된 서류 가방을 처리하는 방식을 수정하여 취약점을 해결합니다.
...
Windows의 서류 가방 기능에 원격 코드 실행 취약점이 존재합니다. 공격자는 특수하게 조작된 Windows 서류 가방을 열도록 사용자를 유도하여 이 취약점을 악용할 수 있습니다. 이 취약점 악용에 성공한 공격자는 현재 사용자의 보안 컨텍스트에서 임의 코드를 실행할 수 있습니다.

이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2012-1528](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2012-1528)(영문)을 참조하십시오.

#### Windows 서류 가방 정수 오버플로 취약점(CVE-2012-1528)에 대한 완화 요소
...
#### Windows 서류 가방 정수 오버플로 취약점(CVE-2012-1528)에 대한 대안
...
-   서류 가방 네임스페이스 셸 확장 제거
    ...
    대안의 영향. 서류 가방 네임스페이스 셸 확장을 제거하면 서류 가방 동기화 메커니즘의 작동이 중지됩니다.
...
#### Windows 서류 가방 정수 오버플로 취약점(CVE-2012-1528)에 대한 FAQ
...
<td style="border:1px solid black;">제어판의 프로그램 추가/제거 항목 또는 %Windir%\$NTUninstallKB2727528$\Spuninst 폴더의 Spuninst.exe 유틸리티를 사용하십시오.</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;">파일 정보</td>
<td style="border:1px solid black;"><a href="http://support.microsoft.com/kb/2727528">Microsoft 기술 자료 문서 2727528</a> 참조</td>
</tr>
<tr class="even">
...
-   Windows 서류 가방 정수 언더플로 취약점(CVE-2012-1527)을 [VeriSign iDefense Labs](http://labs.idefense.com/)와 협력하여 보고해 주신 Tal Zeltzer  
-   Windows 서류 가방 정수 오버플로 취약점(CVE-2012-1528)을 [VeriSign iDefense Labs](http://labs.idefense.com/)와 협력하여 보고해 주신 Tal Zeltzer
...
-   V1.1(2012년 11월 15일): 선택적인 데스크톱 경험 기능이 설치되고 활성화된 경우에만, 지원되는 Windows Server 2008 및 Windows Server 2008 R2 에디션을 실행하는 시스템에서 이 업데이트가 적용됨에 대한 설명이 추가되었습니다. 자세한 내용은 업데이트 FAQ를 참조하십시오.
