...
<tbody>
<tr class="odd">
<td style="border:1px solid black;"><a href="https://www.microsoft.com/download/details.aspx?familyid=7b983156-9e9f-4d29-9e9b-2369747e3b62&amp;displaylang=ko">Microsoft Exchange Server 2007 서비스 팩 2(x64 기반 시스템용)</a><br />
(KB2407132)</td>
...
예. 이 업데이트는 Exchange Server 2007 서비스 팩 2에 대한 업데이트 롤업 5 형식으로 발표되며, 이에 따라 이전 업데이트 롤업에서 제공하는 모든 내용이 포함됩니다. 이 보안 업데이트 이외의 기타 변경 사항은 업데이트 롤업 5에 포함되어 있지 않습니다. 이전 업데이트 롤업의 내용에 대한 자세한 내용은 [Microsoft 기술 자료 문서 981383](https://support.microsoft.com/kb/981383)을 참조하십시오.
  
**Exchange Server 2007 기반 서버에 대한 업데이트 롤업을 특정 순서대로 설치해야 합니까?**    
TechNet 문서 [Exchange 2007용 최신 서비스 팩 또는 업데이트 롤업을 설치하는 방법](https://technet.microsoft.com/ko-kr/library/ee221147(exchg.80).aspx)을 참조하십시오.
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2010-3937 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-3937)을 참조하십시오.
...
Exchange Server 저장소는 여러 유형의 정보를 하나의 인프라에서 관리하기 위한 단일 리포지토리를 제공하는 저장 플랫폼입니다. Exchange 저장소에는 서로 상호 작용하는 여러 논리적 구성 요소가 있습니다. 이러한 구성 요소는 단일 서버에 있거나 여러 서버에 걸쳐 분산될 수 있습니다. 자세한 내용은 TechNet 문서 [Exchange 2007 저장소 이해](https://technet.microsoft.com/ko-kr/library/bb331958(exchg.80).aspx)를 참조하십시오.
...
아니요. 사서함 서버 역할이 있는 Exchange 서버만 이 문제의 영향을 받습니다. 서버 역할은 메시징 환경에서 특정 기능을 수행하는 데 필요한 기능 및 구성 요소를 논리적으로 그룹화하는 단위입니다. Exchange 서버는 단일 컴퓨터에 공존하는 여러 서버 역할을 가지도록 구성하거나 지정된 컴퓨터에 서버 역할을 배포할 수 있습니다. 이 문제는 사서함 서버에 영향을 미치므로 이 역할이 배포된 서버만 영향을 받습니다. 하지만 이 업데이트는 시스템에 구성된 역할에 상관 없이 영향을 받는 모든 Microsoft Exchange 서버에 제공됩니다. 서버 역할에 대한 자세한 내용은 TechNet 문서 [서버 역할 배포](https://technet.microsoft.com/ko-kr/library/aa997610(exchg.80).aspx)를 참조하십시오.
  
**Exchange 사서함 서버 역할이란 무엇입니까?**    
사서함 서버 역할은 사용자의 사서함을 보관하는 사서함 데이터베이스를 호스트합니다. 사용자 사서함, 공용 폴더 또는 이 두 가지 모두를 호스트하려는 경우 사서함 서버 역할이 필요합니다. Exchange Server 2007의 사서함 서버 역할은 이전 버전 Exchange의 사서함 기능보다 Active Directory 디렉터리 서비스와 더욱 잘 통합됩니다. 이 향상된 통합으로 배포 및 운영 작업이 더욱 쉬워집니다. 또한 사서함 서버 역할은 더욱 풍부한 캘린더 기능, 리소스 관리 및 오프라인 주소록 다운로드를 제공하여 정보 작업자 경험을 향상시킵니다. 자세한 내용은 TechNet 문서 [사서함 서버 역할: 개요](https://technet.microsoft.com/ko-kr/library/bb123698(exchg.80).aspx)를 참조하십시오.
...
이 문제는 Exchange 사서함 서버에 대한 RPC 연결에 영향을 미칩니다. 이러한 연결을 위해 RPC를 사용하도록 구성된 모든 TCP 또는 UDP 포트가 영향을 받을 수 있습니다. 정적 포트 목록 및 해당 환경에서 사용될 수 있는 동적 포트를 확인하려면 TechNet 문서 [혼합 환경에서 Exchange 2007에 사용되는 포트 이해](https://technet.microsoft.com/ko-kr/library/dd789693(exchg.80).aspx)를 참조하십시오.
...
<td style="border:1px solid black;"><a href="https://support.microsoft.com/kb/2407132">Microsoft 기술 자료 문서 2407132</a> 참조</td>
</tr>
<tr class="even">
...
-   Exchange Server 무한 루프 취약점(CVE-2010-3937)을 [TippingPoint (영문)](https://www.tippingpoint.com/)의 [Zero Day Initiative (영문)](https://www.zerodayinitiative.com/)와 협력하여 보고해 주신 Oleksandr Mirosh
