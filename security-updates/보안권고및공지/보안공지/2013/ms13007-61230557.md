---
TOCTitle: 'MS13-007'
Title: 'Microsoft 보안 공지 MS13-007 - 중요'
ms:assetid: 'ms13-007'
ms:contentKeyID: 61230557
ms:mtpsurl: 'https://technet.microsoft.com/ko-KR/library/ms13-007(v=Security.10)'
---

Security Bulletin

Microsoft 보안 공지 MS13-007 - 중요
===================================

Open Data 프로토콜의 취약점으로 인한 서비스 거부 (2769327)
----------------------------------------------------------

게시된 날짜: 2013년 1월 8일 화요일 | 업데이트된 날짜: 2013년 4월 4일 목요일
...
이 보안 업데이트의 심각도는 Microsoft .NET Framework 3.5, Microsoft .NET Framework 3.5 서비스 팩 1, Microsoft .NET Framework 3.5.1, Microsoft .NET Framework 4에 대해 중요입니다. Microsoft Windows Server 2012에 설치된 관리 OData IIS 확장에 대해서도 중요입니다. 자세한 내용은 이 섹션의 영향을 받는 소프트웨어 및 영향을 받지 않는 소프트웨어를 참조하십시오.

이 보안 업데이트는 WCF 교체 기능을 기본적으로 사용하지 않도록 설정하여 취약점을 해결합니다. 취약점에 대한 자세한 내용은 취약점 정보에서 각 취약점 항목의 자주 제기되는 질문 사항(FAQ)을 참조하십시오.
...
알려진 문제점. 이 보안 업데이트를 설치할 때 고객이 겪을 수 있는 현재까지 알려진 문제점은 [Microsoft 기술 자료 문서 2769327](http://support.microsoft.com/kb/2769327)에 나와 있습니다. 이 문서는 이러한 문제점에 대한 권장 해결 방법도 소개합니다.
...
이 보안 업데이트의 SHA1 및 SHA2 해시는 다운로드한 보안 업데이트 패키지의 진위 여부를 검증하는 데 사용할 수 있습니다. 이 업데이트와 관련된 해시 정보는 [Microsoft 기술 자료 문서 2769327](http://support.microsoft.com/kb/2769327)을 참조하십시오.
...
<tr class="alternateRow">
<td style="border:1px solid black;">
Windows XP 서비스 팩 3에 설치된 Microsoft .NET Framework 4
</td>
<td style="border:1px solid black;">
중요   
서비스 거부
...
<tr>
<td style="border:1px solid black;">
Windows XP Professional x64 Edition 서비스 팩 2에 설치된 Microsoft .NET Framework 4
</td>
<td style="border:1px solid black;">
중요   
서비스 거부
...
<tr class="alternateRow">
<td style="border:1px solid black;">
Windows Server 2012에 설치된 관리 OData IIS 확장
</td>
<td style="border:1px solid black;">
중요   
서비스 거부
...
<tr>
<td style="border:1px solid black;">
Windows Server 2012에 설치된 관리 OData IIS 확장(Server Core 설치)
</td>
<td style="border:1px solid black;">
중요   
서비스 거부
...
<span></span>
서비스 거부 취약점이 OData 사양에 존재하여 서비스가 거부될 수 있습니다. 취약점으로 인해 서버 또는 서비스가 응답하지 않고 다시 시작될 수 있습니다.

이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2013-0005](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2013-0005)(영문)를 참조하십시오.

#### 교체 서비스 거부 취약점(CVE-2013-0005)에 대한 완화 요소
...
-   관리 OData IIS 확장은 기본적으로 사용되지 않으므로 Windows Server 2012를 기본 구성으로 실행하는 시스템은 이 취약점으로 인한 위험에 노출되지 않습니다. 관리 OData IIS 확장을 사용하는 Windows Server 2012 시스템만 이 문제에 취약합니다.

#### 교체 서비스 거부 취약점(CVE-2013-0005)에 대한 대안

대안이란 기본적인 취약점을 수정하지는 않지만 업데이트를 적용하기 전에 알려진 공격 경로를 차단하는 데 유용한 설정 또는 구성 변경을 지칭합니다. Microsoft는 다음과 같은 대안을 테스트했으며, 대안으로 인한 기능 저하 여부를 해당 항목에서 설명합니다.

-   방화벽에서 OData 웹 응용 프로그램 포트 차단

    OData 포트는 일반적으로 TCP 포트 80 및 443에서 호스팅되지만 기타 비표준 포트에서도 호스팅될 수 있습니다. 사용 환경을 평가하고 기업 방화벽의 OData 서비스가 호스팅되는 모든 포트에 대해 인바운드 및 아웃바운드 연결을 차단하면 시스템에 이 취약점을 악용하려는 시도로부터 보호할 수 있습니다. 다른 포트를 사용하는 공격을 방지하려면 인바운드 원치 않는 인터넷 통신을 모두 차단하는 것이 좋습니다. 포트에 대한 자세한 내용은 TechNet 문서, [TCP 및 UDP 포트 할당](http://go.microsoft.com/fwlink/?linkid=21312)을 참조하십시오.

-   클라이언트용 인증이 IIS에 연결하도록 요청

    IIS의 인증을 사용하도록 설정하면 인증되지 않은 공격자가 이 취약점을 악용하지 못하게 할 수 있습니다. 사용 중인 환경에서 인증을 수행하는 방법에 대한 자세한 내용은 사용 중인 IIS 버전에 대한 TechNet 문서를 참조하십시오.
    IIS 7인 경우 [IIS 7에서 인증 구성](http://technet.microsoft.com/ko-kr/library/cc733010(v=ws.10).aspx)

    IIS 6.0인 경우 [IIS 6.0의 Windows 통합 인증](http://technet.microsoft.com/ko-kr/library/cc738016(v=ws.10).aspx)

#### 교체 서비스 거부 취약점(CVE-2013-0005)에 대한 FAQ

이 취약점을 악용하면 어느 작업까지 가능합니까? 
이 취약점은 서비스 거부 취약점입니다.

취약점의 원인은 무엇입니까? 
이 취약점은 WCF 교체 기능이 특수하게 조작된 값을 잘못 삭제할 때 발생하여 데이터 복제 및 시스템 리소스 부족이 야기될 수 있습니다.

OData란 무엇입니까? 
OData(Open Data) 프로토콜은 데이터의 쿼리 및 업데이트용 프로토콜로, 오늘날 응용 프로그램에 존재하는 사일로로부터 데이터를 잠금 해제하며 데이터가 영향을 받지 않도록 하는 방식을 제공합니다. OData는 [HTTP](http://www.w3.org/protocols/), [Atom Publishing Protocol](http://www.ietf.org/rfc/rfc4287.txt)(AtomPub), [JSON](http://json.org/) 등 웹 기술을 통해 적용 및 구현함으로써 이러한 방식을 제공하므로 다양한 응용 프로그램, 서비스, 저장소의 정보에 액세스할 수 있습니다. 이 프로토콜은 지난 몇 년간 다양한 제품에서 AtomPub 클라이언트 및 서버를 구현한 경험을 토대로 개발되었습니다. OData는 관계형 데이터베이스, 파일 시스템, 콘텐츠 관리 시스템, 일반 웹 사이트 등 다양한 출처의 정보를 노출 및 활용하는데 사용됩니다. OData 사양에 대한 자세한 내용은 [Open Data 프로토콜 웹 사이트](http://www.odata.org/)를 참조하십시오.

관 리 OData IIS 확장 웹 서비스란 무엇입니까?  
관리 OData IIS 확장은 IIS에서 실행되는 OData 기반 웹 서비스를 통해 Windows PowerShell cmdlet을 간편하게 노출하기 위한 프레임워크입니다. 자세한 내용은 [관리 OData IIS 확장 웹 서비스 생성](http://msdn.microsoft.com/ko-kr/library/windows/desktop/hh880865(v=vs.85).aspx)(영문)을 참조하십시오.
...
공격자는 이 취약점을 악용하여 서비스 거부 공격을 발생시키고 .NET WCF 서비스를 사용하는 사이트의 가용성을 방해할 수 있습니다.

공격자는 이러한 취약점을 어떻게 악용합니까? 
인증되지 않은 공격자는 소수의 특수하게 조작된 HTTP 요청을 영향을 받는 사이트에 전송하여 서비스 거부 상황을 발생시킬 수 있습니다.

취약점으로 인해 주로 공격 받을 위험이 있는 시스템은 무엇입니까? 
.NET Framework 및 WCF 서비스가 설치되고 인터넷에 접속된 시스템이 이 취약점으로 인해 공격을 받을 위험이 있습니다. IIS 및 관리 OData IIS 확장을 실행하는 Windows Server 2012 또한 이 취약점으로 인해 공격을 받을 위험이 있습니다.

업데이트는 어떤 기능을 수행합니까? 
이 업데이트는 WCF 교체 기능을 기본적으로 사용하지 않도록 설정하여 취약점을 해결합니다.
...
<td style="border:1px solid black;"><a href="http://support.microsoft.com/kb/2736416">Microsoft 기술 자료 문서 2736416</a> 참조<br />
<a href="http://support.microsoft.com/kb/2736428">Microsoft 기술 자료 문서 2736428</a> 참조</td>
</tr>
<tr class="odd">
...
<tr class="odd">
<td style="border:1px solid black;">파일 정보</td>
<td style="border:1px solid black;"><a href="http://support.microsoft.com/kb/2736418">Microsoft 기술 자료 문서 2736418</a> 참조<br />
<a href="http://support.microsoft.com/kb/2736422">Microsoft 기술 자료 문서 2736422</a> 참조<br />
<a href="http://support.microsoft.com/kb/2736428">Microsoft 기술 자료 문서 2736428</a> 참조</td>
</tr>
<tr class="even">
<td style="border:1px solid black;">레지스트리 키 확인</td>
...
Microsoft .NET Framework 4의 보안 업데이트는 다음 설치 스위치를 지원합니다.
...
<tr class="even">
<td style="border:1px solid black;">파일 정보</td>
<td style="border:1px solid black;"><a href="http://support.microsoft.com/kb/2736693">Microsoft 기술 자료 문서 2736693</a> 참조</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;">레지스트리 키 확인</td>
...
<tr class="even">
<td style="border:1px solid black;"></td>
<td style="border:1px solid black;">Windows Server 2012에 설치된 관리 OData IIS 확장:<br />
...
<tr class="even">
<td style="border:1px solid black;">파일 정보</td>
<td style="border:1px solid black;"><a href="http://support.microsoft.com/kb/2736693">Microsoft 기술 자료 문서 2736693</a> 참조<br />
<a href="http://support.microsoft.com/kb/2753596">Microsoft 기술 자료 문서 2753596</a> 참조</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;">레지스트리 키 확인</td>
<td style="border:1px solid black;">Microsoft .NET Framework 3.5:<br />
...
-   V1.1(2013년 4월 4일): Windows Server 2012를 실행하는 시스템에 대한 CVE-2013-0005에 완화 요소가 추가되었습니다.
