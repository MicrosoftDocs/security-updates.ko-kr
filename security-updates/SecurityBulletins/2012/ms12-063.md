...
이 보안 업데이트는 [Microsoft 보안 권고 2757760](https://technet.microsoft.com/security/advisory/2757760)에서 처음 설명한 취약점도 해결합니다.

권장 사항. 대다수의 고객은 자동 업데이트를 사용하고 있기 때문에 따로 조치를 취할 필요가 없습니다. 이 보안 업데이트는 자동으로 다운로드되고 설치됩니다. 자동 업데이트를 사용하고 있지 않은 고객은 수동으로 업데이트를 확인하고 이 업데이트를 설치해야 합니다. 자동 업데이트의 특정 구성 옵션에 대한 자세한 내용은 [Microsoft 기술 자료 문서 294871](https://support.microsoft.com/kb/294871)을 참조하십시오.
...
<td style="border:1px solid black;">
[MS12-052](https://go.microsoft.com/fwlink/?linkid=255327)의 KB2722913은 KB2744842로 대체되었습니다.
</td>
</tr>
...
[Microsoft 보안 권고 2757760](https://technet.microsoft.com/security/advisory/2757760)의 Prevent Memory Corruption via ExecCommand in Internet Explorer"(Internet Explorer의 ExecCommand를 통한 메모리 손상 방지) Microsoft Fix it 솔루션을 구현한 고객은 이 업데이트를 적용하기 전에 Microsoft Fix it 솔루션을 실행 취소할 필요가 없습니다.

파일 정보 세부 사항은 어디에 있습니까?  
파일 정보 세부 사항의 위치에 대해서는 보안 업데이트 배포 섹션에서 참조 표를 참조하십시오.

보안 ***업데이트***의 해시는 ***어디에*****있습니까?  
이 보안 업데이트의 SHA1 및 SHA2 해시는 다운로드한 보안 업데이트 패키지의 진위 여부를 검증하는 데 사용할 수 있습니다. 이 업데이트와 관련된 해시 정보는 [Microsoft 기술 자료 문서 2744842](https://support.microsoft.com/kb/2744842)를 참조하십시오.

Server Core 설치가 어떻게 이 공지에서 설명한 취약점의 영향을 받습니까?  
Server Core 설치 옵션을 사용하여 설치한 경우 이 업데이트에서 해결하는 취약점은 영향을 받지 않는 소프트웨어 표에 명시된대로, 지원 대상인 Windows Server 2008, Windows Server 2008 R2 또는 Windows Server 2012 에디션에 영향을 주지 않습니다. 이 설치 옵션에 대한 자세한 내용은 TechNet 문서, [Server Core 설치 관리: 개요](https://technet.microsoft.com/library/ee441255)(영문), [Server Core 설치 서비스](https://technet.microsoft.com/library/ff698994)(영문) 및 [Server Core 및 Full Server Integration 개요](https://technet.microsoft.com/library/hh831758)(영문)를 참조하십시오.
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2012-1529](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2012-1529)를 참조하십시오.
...
-   기본적으로 Windows Server 2003, Windows Server 2008 및 Windows Server 2008 R2의 Internet Explorer는 [보안 강화 구성](https://technet.microsoft.com/library/dd883248)으로 알려진 제한된 모드로 실행됩니다. 이 모드에서는 이 취약점이 완화됩니다. Internet Explorer 보안 강화 구성에 대한 자세한 내용은 이 취약점에 대한 FAQ 항목을 참조하십시오.
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2012-2546](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2012-2546)(영문)을 참조하십시오.
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2012-2548](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2012-2548)(영문)을 참조하십시오.
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2012-2557](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2012-2557)(영문)을 참조하십시오.
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2012-4969](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2012-4969)(영문)를 참조하십시오.
...
-   Microsoft Fix it 솔루션 "Prevent Memory Corruption via ExecCommand in Internet Explorer"(Internet Explorer의 ExecCommand를 통한 메모리 손상 방지)를 적용하여 이 문제의 취약점 방지
    ...
    이 대안을 사용하거나 사용하지 않도록 설정하려면 [Microsoft 기술 자료 문서 2744842](https://support.microsoft.com/kb/2744842)를 참조하여 Microsoft Fix it 자동화 솔루션을 사용하십시오.
...
-   EMET(Enhanced Mitigation Experience Toolkit) 배포
    ...
    현재 EMET는 제한적 지원과 함께 영어로만 제공되고 있습니다. 자세한 내용은 [Microsoft 기술 자료 문서 2458544](https://support.microsoft.com/kb/2458544)를 참조하십시오.
    ...
    참고: 그룹 정책에 대한 자세한 내용은 [그룹 정책 컬렉션](https://go.microsoft.com/fwlink/?linkid=215719)(영문)을 참조하십시오.
...
ASLR(Address Space Layout Randomization)이 구현되는 시스템은 일반적으로 예측 가능한 기능 진입점을 메모리에서 임의의 가상 위치로 재배치합니다. Windows ASLR은 시스템 DLL 또는 실행 파일을 메모리에 있는 256개의 임의 위치 중 하나로 재지정합니다. 따라서 하드코드된 주소를 사용하는 공격자는 256번의 시도 중에 하나를 "올바르게 추측"할 수 있습니다. ASLR에 대한 자세한 내용은 TechNet 매거진 문서, [Windows Vista 커널 속으로: 3부](https://technet.microsoft.com/en-us/magazine/2007.04.vistakernel.aspx?pr=blog)를 참조하십시오.

업데이트는 어떤 기능을 수행합니까?  
이 업데이트는 Internet Explorer가 메모리에서 개체를 처리하는 방식을 수정하여 취약점을 해결합니다.

이 보안 공지를 게시했을 때 이 취약점이 공개되어 있었습니까?  
예. 이 취약점은 공개되었습니다. 이 취약점에는 CVE(Common Vulnerability and Exposure) 번호 [CVE-2012-4969](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2012-4969)(영문)가 지정되었습니다. 이 취약점은 [Microsoft 보안 권고 2757760](https://technet.microsoft.com/security/advisory/2757760)에서 처음 설명되었습니다.
...
<td style="border:1px solid black;"><a href="https://support.microsoft.com/kb/2744842">Microsoft 기술 자료 문서 2744842</a> 참조</td>
</tr>
<tr class="even">
...
-   OnMove 해제 후 사용 취약점(CVE-2012-1529)을 [VeriSign iDefense Labs](https://labs.idefense.com/)(영문)와 협력하여 보고해 주신 익명 연구자  
-   이벤트 수신기 해제 후 사용 취약점(CVE-2012-2546)을 보고해 주신 [Rosario Valotta](https://sites.google.com/site/tentacoloviola)(영문)  
-   레이아웃 해제 후 사용 취약점(CVE-2012-2548)을 [TippingPoint](https://www.hpenterprisesecurity.com/products/hp-tippingpoint-network-security/)(영문)의 [Zero Day Initiative](https://www.zerodayinitiative.com/)(영문)와 협력하여 보고해 주신 [Harmony Security](https://www.harmonysecurity.com/)(영문)의 Stephen Fewer  
-   [TippingPoint](https://www.hpenterprisesecurity.com/products/hp-tippingpoint-network-security/)(영문)의 [Zero Day Initiative](https://www.zerodayinitiative.com/)(영문)와 협력하여 cloneNode 해제 후 사용 취약점(CVE-2012-2557)을 보고해 주신 익명 연구자  
-   [TippingPoint](https://www.hpenterprisesecurity.com/products/hp-tippingpoint-network-security/)(영문)의 [Zero Day Initiative](https://www.zerodayinitiative.com/)(영문)와 협력하여 execCommand 해제 후 사용 취약점(CVE-2012-4969)을 보고해 주신 익명 연구자  
-   execCommand 해제 후 사용 취약점(CVE-2012-4969)에 대해 협력해 주신 [Mitre](https://www.mitre.org/)(영문)
