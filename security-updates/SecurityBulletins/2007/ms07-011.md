---
TOCTitle: 'MS07-011'
Title: 'Microsoft 보안 공지 MS07-011 - 중요'
ms:assetid: 'ms07-011'
ms:contentKeyID: 61230050
ms:mtpsurl: 'https://technet.microsoft.com/ko-KR/library/ms07-011(v=Security.10)'
---


Microsoft 보안 공지 MS07-011 - 중요
===================================
...
-   Microsoft Windows 2000 서비스 팩 4 - [업데이트 다운로드](https://www.microsoft.com/download/details.aspx?familyid=7b1a81d5-1072-49d9-a24a-0e2630f62d8c&displaylang=ko)
-   Microsoft Windows XP 서비스 팩 2 - [업데이트 다운로드](https://www.microsoft.com/download/details.aspx?familyid=e9b84661-25e3-4d38-95b1-8d3e7af565aa&displaylang=ko)
-   Microsoft Windows XP Professional x64 Edition - [업데이트 다운로드](https://www.microsoft.com/download/details.aspx?familyid=57c1b19f-3242-457c-bedf-d35a8efe525c)
-   Microsoft Windows Server 2003 및 Microsoft Windows Server 2003 서비스 팩 1 - [업데이트 다운로드](https://www.microsoft.com/download/details.aspx?familyid=eaed6f59-801e-45d7-9518-469d0de13cad&displaylang=ko)
-   Microsoft Windows Server 2003(Itanium 기반 시스템용) 및 Microsoft Windows Server 2003 SP1(Itanium 기반 시스템용) - [업데이트 다운로드](https://www.microsoft.com/download/details.aspx?familyid=cd1b18ae-bc8d-4d73-847f-4fa7ca672c88)
-   Microsoft Windows Server 2003 x64 Edition - [업데이트 다운로드](https://www.microsoft.com/download/details.aspx?familyid=11f4f8f6-b8ce-4a5f-b7ed-8389ccc56473)
...
이 취약점을 악용한 공격자는 영향을 받는 시스템을 완전히 제어할 수 있습니다. 이렇게 되면 공격자가 프로그램을 설치할 수 있을 뿐 아니라 데이터를 보거나 변경하거나 삭제할 수 있고 모든 사용자 권한이 있는 새 계정을 만들 수도 있습니다. 시스템에 대한 사용자 권한이 적게 구성된 계정의 사용자는 관리자 권한으로 작업하는 사용자에 비해 영향을 적게 받습니다. 이 취약점을 악용하려면 많은 사용자 조작이 필요합니다.

가능한 빨리 이 업데이트를 설치할 것을 권장합니다.

**심각도 및 취약점:**

| 취약점                                                                                                                  | 취약점으로 인한 영향 | Windows 2000 서비스 팩 4 | Windows XP 서비스 팩 2 | Windows Server 2003 및 Windows Server 2003 서비스 팩 1 |
|-------------------------------------------------------------------------------------------------------------------------|----------------------|--------------------------|------------------------|--------------------------------------------------------|
| OLE 대화 상자 메모리 손상 취약점 - [CVE-2007-0026 (영문)](https://cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2007-0026): | 원격 코드 실행       | 중요                     | 중요                   | 중요                                                   |
...
**이 보안 업데이트로 MS07-012 및 MS07-013에 공지된 취약점이 제거됩니까?**  
...
**이 보안 업데이트를 MS07-012 및 MS07-013보다 먼저 배포해야 합니까?**  
...
| Microsoft Windows Server 2003(Itanium 기반 시스템용) 및 Microsoft Windows Server 2003 SP1(Itanium 기반 시스템용) | 아니요     | 예       |
| Microsoft Windows Server 2003 x64                                                                                | 아니요     | 예       |
...
#### OLE 대화 상자 메모리 손상 취약점 - CVE-2007-0026:

Microsoft Windows에 제공된 OLE 대화 상자 구성 요소에 원격 코드 실행 취약점이 존재합니다. 사용자가 (RTF) 파일 또는 서식 있는 텍스트 전자 메일 메시지의 조작된 포함 OLE 개체를 조작하려고 할 때 공격자가 이 취약점을 악용할 수 있습니다.

#### OLE 대화 상자 메모리 손상 취약점 - CVE-2007-0026에 대한 완화 요소:
...
#### OLE 대화 상자 메모리 손상 취약점 - CVE-2007-0026에 대한 대안:
...
**업데이트는 어떤 기능을 수행합니까?**  
업데이트는 Windows OLE 대화 상자 구성 요소가 RTF 파일 내의 OLE 스트림을 구문 분석하는 방법을 수정함으로써 취약점을 제거합니다.
...
**Windows2000-kb926436-x86-kor /quiet**

**참고** **/quiet** 스위치를 사용하면 모든 메시지가 표시되지 않습니다. 마찬가지로 실패 메시지도 표시되지 않습니다. 관리자는 지원되는 방법 중 하나를 사용하여 **/quiet** 스위치를 사용할 때 설치가 제대로 되었는지 확인해야 합니다. 또한 관리자는 이 스위치를 사용할 때 KB926436.log 파일에서 모든 실패 메시지를 검토해야 합니다.

컴퓨터를 다시 시작하지 않고 보안 업데이트를 설치하려면 Windows 2000 서비스 팩 4 명령 프롬프트에서 다음 명령을 사용하십시오.

**Windows2000-kb926436-x86-kor /norestart**
...
시스템 관리자는 Spuninst.exe 유틸리티를 사용하여 이 보안 업데이트를 제거할 수도 있습니다. Spuninst.exe 유틸리티는 %Windir%\\$NTUninstallKB926436$\\Spuninst 폴더에 있습니다.
...
-   **레지스트리 키 확인**
    ...
    **참고** 이 레지스트리 키에는 설치된 파일의 전체 목록이 포함되어 있지 않을 수도 있습니다. 또한 관리자나 OEM이 926436 보안 업데이트를 Windows 설치 원본 파일에 통합하거나 포함시키는 경우 이러한 레지스트리 키가 제대로 만들어지지 않을 수 있습니다.
...
**Windowsxp-kb926436-x86-kor /quiet**

**참고** **/quiet** 스위치를 사용하면 모든 메시지가 표시되지 않습니다. 마찬가지로 실패 메시지도 표시되지 않습니다. 관리자는 지원되는 방법 중 하나를 사용하여 **/quiet** 스위치를 사용할 때 설치가 제대로 되었는지 확인해야 합니다. 또한 관리자는 이 스위치를 사용할 때 KB926436.log 파일에서 모든 실패 메시지를 검토해야 합니다.

컴퓨터를 다시 시작하지 않고 보안 업데이트를 설치하려면 Windows XP 명령 프롬프트에서 다음 명령을 사용하십시오.

**Windowsxp-kb926436-x86-kor /norestart**
...
-   **레지스트리 키 확인**
    ...
    **참고** 이러한 레지스트리 키에는 설치된 파일의 전체 목록이 포함되어 있지 않을 수도 있습니다. 또한 관리자나 OEM이 926436 보안 업데이트를 Windows 설치 원본 파일에 통합하거나 포함시키는 경우 이러한 레지스트리 키가 제대로 만들어지지 않을 수 있습니다.
...
**Windowsserver2003-kb926436-x86-kor /quiet**

**참고** **/quiet** 스위치를 사용하면 모든 메시지가 표시되지 않습니다. 마찬가지로 실패 메시지도 표시되지 않습니다. 관리자는 지원되는 방법 중 하나를 사용하여 **/quiet** 스위치를 사용할 때 설치가 제대로 되었는지 확인해야 합니다. 또한 관리자는 이 스위치를 사용할 때 KB926436.log 파일에서 모든 실패 메시지를 검토해야 합니다.

컴퓨터를 다시 시작하지 않고 보안 업데이트를 설치하려면 Windows Server 2003 명령 프롬프트에서 다음 명령을 사용하십시오.

**Windowsserver2003-kb926436-x86-kor /norestart**
...
-   OLE 대화 상자 메모리 손상 취약점(CVE-2007-0026)을 보고해 주신 [Immunity, Inc. (영문)](https://www.immunityinc.com/)의 Kostya Kortchinsky 및 [EADS Common Research Center (영문)](https://www.eads.net/)의 Fabrice Desclaux
