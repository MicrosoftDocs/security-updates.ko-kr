---
TOCTitle: 'MS11-058'
Title: 'Microsoft 보안 공지 MS11-058 - 긴급'
ms:assetid: 'ms11-058'
ms:contentKeyID: 61230425
ms:mtpsurl: 'https://technet.microsoft.com/ko-KR/library/ms11-058(v=Security.10)'
---



Microsoft 보안 공지 MS11-058 - 긴급
===================================

DNS 서버의 취약점으로 인한 원격 코드 실행 문제점 (2562485)
----------------------------------------------------------

게시된 날짜: 2011년 8월 9일 화요일 | 업데이트된 날짜: 2011년 10월 26일 수요일
...
이 보안 업데이트의 심각도는 Windows Server 2008의 32비트 및 x64 기반 에디션과 Windows Server 2008 R2의 x64 기반 에디션에 대해 긴급이며, Windows Server 2003의 지원되는 모든 에디션에 대해 중요입니다. 자세한 내용은 이 절의 하위 절 영향을 받는 소프트웨어 및 영향을 받지 않는 소프트웨어를 참조하십시오.

보안 업데이트는 DNS 서버가 메모리에서 NAPTR 쿼리를 처리하는 방법과 사용 전에 메모리에서 개체를 초기화하는 방법을 수정하여 취약점을 해결합니다. 취약점에 대한 자세한 내용은 다음 섹션, 취약점 정보에서 각 취약점 항목의 자주 제기되는 질문 사항(FAQ)을 참조하십시오.
...
|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------|-------------|---------------------------------------------------------------------------|
| [Windows Server 2003 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=c2d4aa38-9241-465d-8d65-aba73e936d0f&displaylang=ko)                                                                      | 서비스 거부    | 중요        | [MS09-008](https://technet.microsoft.com/ko-kr/security/bulletin/ms09-008) |
| [Windows Server 2003 x64 Edition 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=2db4098a-f4f9-4211-b55d-5d0df1409c43&displaylang=ko)                                                          | 서비스 거부    | 중요        | [MS09-008](https://technet.microsoft.com/ko-kr/security/bulletin/ms09-008) |
| [Windows Server 2003 SP2(Itanium 기반 시스템용)](https://www.microsoft.com/download/details.aspx?familyid=1934acfa-5637-4ae9-9e9a-fc7e58930b7a&displaylang=ko)                                                       | 서비스 거부    | 중요        | [MS09-008](https://technet.microsoft.com/ko-kr/security/bulletin/ms09-008) |
| [Windows Server 2008(32비트 시스템용) 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=ac2d5122-6f9b-46f5-9840-77aa7ff84308&displaylang=ko)\*                                                   | 원격 코드 실행 | 긴급        | 없음                                                                      |
| [Windows Server 2008(x64 기반 시스템용) 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=5f605f6f-3854-403d-878b-637626aef78f&displaylang=ko)\*                                                 | 원격 코드 실행 | 긴급        | 없음                                                                      |
| [Windows Server 2008 R2(x64 기반 시스템용) 및 Windows Server 2008 R2(x64 기반 시스템용) 서비스 팩 1](https://www.microsoft.com/download/details.aspx?familyid=6a86e2cf-4e7d-4012-88c3-6957a3842dd3&displaylang=ko)\* | 원격 코드 실행 | 긴급        | 없음                                                                      |
...
이 공지가 2011년 10월 25일에 개정된 이유는 무엇입니까?  
Microsoft는 검색 로직에 대한 변경 사항을 알리고 지원 대상인 모든 Windows Server 2003 에디션에 대한 공지 교체 정보를 수정하기 위해 이 공지를 개정하였습니다. 보안 업데이트 파일에 대한 변경 사항은 없습니다. 이미 시스템을 성공적으로 업데이트한 고객은 추가 조치를 취할 필요가 없습니다.
...
다음의 심각도는 취약점의 잠재적인 최대 영향을 가정합니다. 이 보안 공지 발표 후 30일 이내의 심각도 및 보안 영향과 관련된 취약점 악용 가능성에 대해서는 [8월 공지 요약](https://technet.microsoft.com/ko-kr/security/bulletin/ms11-aug)의 악용 가능성 인덱스를 참조하십시오. 자세한 내용은 [Microsoft 악용 가능성 인덱스](https://technet.microsoft.com/ko-kr/security/cc998259.aspx)를 참조하십시오.
...
<th style="border:1px solid black;" >DNS NAPTR 쿼리 취약점(CVE-2011-1966)</th>
<th style="border:1px solid black;" >DNS 초기화되지 않은 메모리 손상 취약점(CVE-2011-1970)</th>
...
Windows DNS 서버가 메모리에서 특수하게 조작된 NAPTR 쿼리 문자열을 잘못 처리할 경우에 원격 코드 실행 취약점이 존재합니다. 이 취약점 악용에 성공한 공격자는 시스템의 컨텍스트에서 임의 코드를 실행할 수 있습니다. 이렇게 되면 공격자가 프로그램을 설치할 수 있을 뿐 아니라 데이터를 보거나 변경하거나 삭제할 수 있고 모든 사용자 권한이 있는 새 계정을 만들 수도 있습니다.
  
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2011-1966(영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-1966)을 참조하십시오.
  
#### DNS NAPTR 쿼리 취약점(CVE-2011-1966)에 대한 완화 요소
  
이 취약점에 대해 확인된 완화 요소가 없습니다.
  
#### DNS NAPTR 쿼리 취약점(CVE-2011-1966)에 대한 대안
  
대안이란 기본적인 취약점을 수정하지는 않지만 업데이트를 적용하기 전에 알려진 공격 경로를 차단하는 데 유용한 설정 또는 구성 변경을 지칭합니다. Microsoft는 다음과 같은 대안을 테스트했으며, 대안으로 인한 기능 저하 여부를 해당 항목에서 설명합니다.
  
-   사용하지 않는 경우 DNS 서비스 비활성화
  
    대화형 방법:
  
    1.  시작, 실행을 차례로 클릭하고 regedit을 입력한 다음 확인을 클릭합니다.  
    2.  HKEY\_LOCAL\_MACHINE, System, CurrentControlSet, Services를 차례로 확장합니다.  
    3.  DNS를 클릭합니다.  
    4.  파일 메뉴를 클릭한 후 내보내기를 선택합니다.  
    5.  레지스트리 파일 내보내기 대화 상자에 DNS\_configuration\_backup.reg를 입력한 다음 저장을 클릭합니다.  
    6.  Start 값을 두 번 클릭한 다음 값 데이터 필드를 4로 변경합니다.  
    7.  확인을 클릭합니다.  
    8.  관리자로 실행하여 명령 프롬프트에서 다음 명령을 실행합니다.  
        `scstop DNS`
  
    관리되는 배포 스크립트 사용:
  
    1.  다음 명령을 사용하여 관리되는 배포 스크립트에서 레지스트리 키를 백업합니다.  
        `regedit/e DNS_configuration_backup.reg HKEY_LOCAL_MACHINE\System\CurrentControlSet\Services\DNS`  
    2.  다음 텍스트를 .REG 확장명의 파일(예: "Disable\_service\_DNSame.reg")로 저장합니다.  
        `Windows Registry Editor Version 5.00`  
        `[HKEY_LOCAL_MACHINE\System\CurrentControlSet\Services\DNS]`  
        `"Start"=dword:00000004`  
    3.  다음 명령을 사용하여 위의 2단계에서 만든 레지스트리 스크립트를 대상 컴퓨터에서 실행합니다.  
    ...
    1.  시작, 실행을 차례로 클릭하고 regedit을 입력한 다음 확인을 클릭합니다.  
    2.  파일 메뉴를 클릭한 후 가져오기를 선택합니다.  
    3.  레지스트리 파일 가져오기 창에서 DNS\_configuration\_backup.reg를 선택한 다음 열기를 클릭합니다.
  
    관리되는 배포 스크립트 사용:
  
    관리자로 실행하여 명령 프롬프트에서 다음 명령을 실행하여 레지스트리를 원래 상태로 복원합니다.  
    `Regedit/s DNS_configuration_backup.reg`
  
#### DNS NAPTR 쿼리 취약점(CVE-2011-1966)에 대한 FAQ
  
이 취약점을 악용하면 어느 작업까지 가능합니까?    
이 취약점은 원격 코드 실행 취약점입니다.
  
취약점의 원인은 무엇입니까?    
이 취약점은 권한이 부여된 DNS 서버에서 반복해서 레코드를 요청하는 동안 권한이 부여되지 않은 DNS 서버가 메모리에서 특수하게 조작된 NAPTR 레코드를 적절하게 처리하지 못할 경우에 발생합니다.
  
이 취약점의 영향을 받는 구성 요소는 무엇입니까?    
Dns.exe가 이 취약점의 영향을 받는 구성 요소입니다.
  
공격자는 취약점을 악용하여 무엇을 할 수 있습니까?    
이 취약점 악용에 성공한 공격자는 시스템의 컨텍스트에서 임의 코드를 실행할 수 있습니다. 이렇게 되면 공격자가 프로그램을 설치할 수 있을 뿐 아니라 데이터를 보거나 변경하거나 삭제할 수 있고 모든 사용자 권한이 있는 새 계정을 만들 수도 있습니다.
  
공격자는 이러한 취약점을 어떻게 악용합니까?    
인증되지 않은 원격 공격자가 도메인을 등록하고 NAPTR DNS 리소스 레코드를 만든 후 특수하게 조작된 NAPTR 쿼리를 대상 DNS 서버로 보내는 방법으로 이 취약점을 악용할 수 있습니다.
  
취약점으로 인해 주로 공격받을 위험이 있는 시스템은 무엇입니까?    
DNS 역할을 실행하는 서버가 주로 위험합니다. 이 역할이 활성화되지 않은 서버는 위험하지 않습니다.
  
NAPTR DNS 리소스 레코드란 무엇입니까?    
NAPTR(Naming Authority Pointe) DNS RR(리소스 레코드)을 사용하면 도메인 이름 구문에 있지 않은 URI 등 다양한 리소스 이름에 대한 서비스를 찾는 데 DNS를 사용할 수 있습니다. NAPTR에 대한 자세한 내용은 [RFC-2915(영문)](https://www.ietf.org/rfc/rfc2915.txt)를 참조하십시오.
  
DNS 리소스 레코드란 무엇입니까?    
DNS RR(리소스 레코드)은 도메인 안에서 특정 리소스를 식별합니다. 자세한 내용은 TechNet 문서 [How DNS Works(DNS 작동 방식)](https://technet.microsoft.com/ko-kr/library/dd197446(ws.10).aspx)을 참조하십시오.
  
URI란 무엇입니까?    
URI(Uniform Resource Identifier)는 인터넷에 있거나 네트워크를 통해서 접근할 수 있는 리소스를 조작하거나 식별하는 데 사용되는 문자열입니다. 웹 사이트 등의 리소스를 참조하는 URL이 URI의 대표적인 예입니다. URI에 대한 자세한 내용은 [RFC 2396(영문)](https://www.ietf.org/rfc/rfc2915.txt)을 참조하십시오.
  
업데이트는 어떤 기능을 수행합니까?    
업데이트는 DNS 서버가 메모리에서 NAPTR 쿼리를 처리하는 방법을 수정하여 취약점을 해결합니다.
...
DNS 서버가 초기화되지 않은 메모리의 개체를 적절하게 처리하지 못할 경우에 서비스 거부 취약점이 존재합니다. 이 취약점을 악용한 공격자는 대상 시스템에서 DNS 서버 서비스가 응답을 중지하게 할 수 있습니다.
  
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2011-1970(영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-1970)을 참조하십시오.
  
#### DNS 초기화되지 않은 메모리 손상 취약점(CVE-2011-1970)에 대한 완화 요소
  
이 취약점에 대해 확인된 완화 요소가 없습니다.
  
#### DNS 초기화되지 않은 메모리 손상 취약점(CVE-2011-1970)에 대한 대안
  
이 취약점에 대한 대안을 확인하지 못했습니다.
  
#### DNS 초기화되지 않은 메모리 손상 취약점(CVE-2011-1970)에 대한 FAQ
  
이 취약점을 악용하면 어느 작업까지 가능합니까?    
이 취약점은 서비스 거부 취약점입니다.
  
취약점의 원인은 무엇입니까?    
이 취약점은 DNS 서버가 존재하지 않는 도메인의 리소스 레코드를 찾는 동안 메모리에서 초기화되지 않은 개체를 적절하게 처리하지 못할 경우에 발생합니다.
  
이 취약점의 영향을 받는 구성 요소는 무엇입니까?    
Dns.exe가 이 취약점의 영향을 받는 구성 요소입니다.
  
공격자는 취약점을 악용하여 무엇을 할 수 있습니까?    
이 취약점을 악용한 공격자는 대상 시스템에서 DNS 서버 서비스가 응답을 중지하게 할 수 있습니다.
  
공격자는 이러한 취약점을 어떻게 악용합니까?    
인증되지 않은 원격 공격자는 존재하지 않는 도메인의 리소스 레코드에 대해 특수하게 조작된 DNS 쿼리를 대상 DNS 서버에 보내 이 취약점을 악용할 수 있습니다.
...
업데이트는 어떤 기능을 수행합니까?    
업데이트는 DNS 서버가 메모리에서 사용 전 메모리의 개체를 초기화하는 방법을 수정하여 취약점을 해결합니다.
...
<td style="border:1px solid black;">제어판의 프로그램 추가/제거 도구 또는 %Windir%\$NTUninstallKB2562485$\Spuninst 폴더의 Spuninst.exe 유틸리티를 사용하십시오.</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;">파일 정보</td>
<td style="border:1px solid black;"><a href="https://support.microsoft.com/kb/2562485">Microsoft 기술 자료 문서 2562485</a> 참조</td>
</tr>
<tr class="even">
...
-   DNS NAPTR 쿼리 취약점(CVE-2011-1966)을 보고해 주신 Grischa Zengel(Zengel Medizintechnik GmbH)
...
-   V1.1(2011년 9월 21일): 공지가 이 업데이트로 대체됨에 따라 영향을 받는 모든 Windows Server 2003 에디션의 영향을 받는 소프트웨어 표에서 MS11-046을 제거하도록 수정되었습니다. 이 변경 사항은 검색에만 해당됩니다. 보안 업데이트 파일에 대한 변경 사항은 없습니다. 이미 시스템을 성공적으로 업데이트한 고객은 추가 조치를 취할 필요가 없습니다.  
-   V1.2 (2011년 10월 26일): 검색 로직에 대한 변경 사항을 알리고 일부 영향을 받는 구성에 대한 공지 교체 정보가 수정되었습니다. 보안 업데이트 파일에 대한 변경 사항은 없습니다. 자세한 내용은 업데이트 FAQ를 참조하십시오.
