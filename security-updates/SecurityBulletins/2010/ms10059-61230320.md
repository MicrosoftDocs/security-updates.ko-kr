...
|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------|-------------|---------------------------|
| [Windows Vista 서비스 팩 1 및 Windows Vista 서비스 팩 2](http://www.microsoft.com/downloads/details.aspx?familyid=dfb31aa2-7457-4581-9e28-7984a360edf4&displaylang=ko)                                         | 권한 상승      | 중요        | 없음                      |
| [Windows Vista x64 Edition 서비스 팩 1 및 Windows Vista x64 Edition 서비스 팩 2](http://www.microsoft.com/downloads/details.aspx?familyid=110f932f-d13c-4486-a295-e6068d5d8d7a&displaylang=ko)                 | 권한 상승      | 중요        | 없음                      |
| [Windows Server 2008(32비트 시스템용) 및 Windows Server 2008(32비트 시스템용) 서비스 팩 2](http://www.microsoft.com/downloads/details.aspx?familyid=4c9b3e60-e166-40c9-8938-3cba0a399c47&displaylang=ko)\*     | 권한 상승      | 중요        | 없음                      |
| [Windows Server 2008(x64 기반 시스템용) 및 Windows Server 2008(x64 기반 시스템용) 서비스 팩 2](http://www.microsoft.com/downloads/details.aspx?familyid=fa84e547-2190-402f-9467-2450deeff565&displaylang=ko)\* | 권한 상승      | 중요        | 없음                      |
| [Windows Server 2008(Itanium 기반 시스템용) 및 Windows Server 2008(Itanium 기반 시스템용) 서비스 팩 2](http://www.microsoft.com/downloads/details.aspx?familyid=84f89dca-108c-4956-9aa2-866e17a872fc)          | 권한 상승      | 중요        | 없음                      |
| [Windows 7(32비트 시스템용)](http://www.microsoft.com/downloads/details.aspx?familyid=ce6233f3-2ee5-4329-908d-ba9b28ecc553&displaylang=ko)                                                                     | 권한 상승      | 중요        | 없음                      |
| [Windows 7(x64 기반 시스템용)](http://www.microsoft.com/downloads/details.aspx?familyid=146270fa-cd6f-440a-aa3e-e93af0bff447&displaylang=ko)                                                                   | 권한 상승      | 중요        | 없음                      |
| [Windows Server 2008 R2(x64 기반 시스템용)](http://www.microsoft.com/downloads/details.aspx?familyid=333fb6e4-f867-4dcb-beb3-2d88e428ca2e)\*                                                                   | 권한 상승      | 중요        | 없음                      |
| [Windows Server 2008 R2(Itanium 기반 시스템용)](http://www.microsoft.com/downloads/details.aspx?familyid=62034ecb-a6bd-46c5-a03d-9642880bc2d6)                                                                 | 권한 상승      | 중요        | 없음                      |
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2010-2554 (영문)](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-2554)를 참조하십시오.
...
액세스 제어 목록(ACL)은 개체에 적용되는 보안 정보 목록입니다. 개체는 파일, 프로세스, 이벤트 또는 모든 엔터티 등 보안 설명자가 있는 모든 항목일 수 있습니다. ACL의 항목을 액세스 제어 항목(ACE)이라고 합니다. 이러한 액세스 제어 목록에는 임의 액세스 제어 목록과 시스템 액세스 제어 목록이 있습니다. ACL에서 각 ACE는 해당하는 신뢰 대상에 대한 허용, 거부 또는 감사 액세스 권한을 식별합니다. 자세한 내용은 MSDN 문서, [액세스 제어 목록 (영문)](http://msdn.microsoft.com/en-us/library/aa374872(vs.85).aspx)을 참조하십시오.
  
**명명된 파이프란 무엇입니까?**    
명명된 파이프는 파이프 서버와 하나 이상의 파이프 클라이언트 간 통신을 위한 명명된 단방향 또는 양방향 채널입니다. 명명된 파이프의 모든 인스턴스는 동일한 파이프 이름을 공유하지만, 각 인스턴스에는 고유의 버퍼와 핸들이 있으며 클라이언트/서버 통신을 위해 별도의 도관을 제공합니다. 인스턴스를 사용하면 여러 파이프 클라이언트가 동시에 같은 명명된 파이프를 사용할 수 있습니다. 모든 프로세스는 보안 점검이라고 명명된 파이프에 액세스할 수 있으며, 이를 통해 명명된 파이프를 관련되었거나 관련되지 않은 프로세스 간의 간편한 형태의 통신으로 만들 수 있습니다. 자세한 내용은 MSDN 문서, [명명된 파이프 (영문)](http://msdn.microsoft.com/en-us/library/aa365590(vs.85).aspx)를 참조하십시오.
...
예. 이 취약점은 공개되었습니다. 이 취약점에는 CVE(Common Vulnerability and Exposure) 번호 [CVE-2010-2554 (영문)](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-2554)가 지정되었습니다.
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2010-2555 (영문)](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-2555)를 참조하십시오.
...
<td style="border:1px solid black;"><a href="http://support.microsoft.com/kb/982799">Microsoft 기술 자료 문서 982799</a> 참조</td>
</tr>
<tr class="odd">
...
-   추적 레지스트리 키 ACL 취약점(CVE-2010-2554)을 보고해 주신 [Argeniss (영문)](http://www.argeniss.com/)의 Cesar Cerrudo  
-   추적 메모리 손상 취약점(CVE-2010-2555)을 보고해 주신 [Argeniss](http://www.argeniss.com/)
