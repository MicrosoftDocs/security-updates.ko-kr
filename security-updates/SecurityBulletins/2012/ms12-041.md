---
TOCTitle: 'MS12-041'
Title: 'Microsoft 보안 공지 MS12-041 - 중요'
ms:assetid: 'ms12-041'
ms:contentKeyID: 61230508
ms:mtpsurl: 'https://technet.microsoft.com/ko-KR/library/ms12-041(v=Security.10)'
---

Microsoft 보안 공지 MS12-041 - 중요
===================================
...
이 보안 업데이트는 Windows 커널 모드 드라이버가 사용자 모드에서 전달된 입력의 유효성을 검사하는 방식과 TrueType 글꼴 로딩을 처리하는 방식을 수정하고 스레드 생성 메커니즘에 추가 런타임 유효성 검사를 도입하여 취약점을 해결합니다. 취약점에 대한 자세한 내용은 다음 섹션, 취약점 정보에서 각 취약점 항목의 자주 제기되는 질문 사항(FAQ)을 참조하십시오.
...
<td style="border:1px solid black;">
[MS12-018](https://go.microsoft.com/fwlink/?linkid=235364)의 KB2641653은 KB2709162로 대체되었습니다.
</td>
</tr>
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2012-1864(영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2012-1864)를 참조하십시오.

#### 문자열 Atom 클래스 이름 처리 취약점(CVE-2012-1864)에 대한 완화 요소

완화란 취약점의 악용 심각도를 낮출 수 있는 설정, 일반적인 구성 또는 최선책, 기본 상태를 지칭합니다. 다음과 같은 완화 요소가 현재 상황에서 유용할 수 있습니다.

-   이 취약점을 악용하려면 공격자가 유효한 로그온 자격 증명을 가지고 로컬로 로그온할 수 있어야 합니다.

#### 문자열 Atom 클래스 이름 처리 취약점(CVE-2012-1864)에 대한 대안

이 취약점에 대한 대안을 확인하지 못했습니다.

#### 문자열 Atom 클래스 이름 처리 취약점(CVE-2012-1864)에 대한 FAQ
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2012-1865(영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2012-1865)를 참조하십시오.

#### 문자열 Atom 클래스 이름 처리 취약점(CVE-2012-1865)에 대한 완화 요소

완화란 취약점의 악용 심각도를 낮출 수 있는 설정, 일반적인 구성 또는 최선책, 기본 상태를 지칭합니다. 다음과 같은 완화 요소가 현재 상황에서 유용할 수 있습니다.

-   이 취약점을 악용하려면 공격자가 유효한 로그온 자격 증명을 가지고 로컬로 로그온할 수 있어야 합니다.

#### 문자열 Atom 클래스 이름 처리 취약점(CVE-2012-1865)에 대한 대안

이 취약점에 대한 대안을 확인하지 못했습니다.

#### 문자열 Atom 클래스 이름 처리 취약점(CVE-2012-1865)에 대한 FAQ
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2012-1866(영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2012-1866)을 참조하십시오.

#### 클립보드 형식 Atom 이름 처리 취약점(CVE-2012-1866)에 대한 완화 요소

완화란 취약점의 악용 심각도를 낮출 수 있는 설정, 일반적인 구성 또는 최선책, 기본 상태를 지칭합니다. 다음과 같은 완화 요소가 현재 상황에서 유용할 수 있습니다.

-   이 취약점을 악용하려면 공격자가 유효한 로그온 자격 증명을 가지고 로컬로 로그온할 수 있어야 합니다.

#### 클립보드 형식 Atom 이름 처리 취약점(CVE-2012-1866)에 대한 대안

이 취약점에 대한 대안을 확인하지 못했습니다.

#### 클립보드 형식 Atom 이름 처리 취약점(CVE-2012-1866)에 대한 FAQ
...
Windows 커널 모드 드라이버가 글꼴을 처리할 때 메모리를 제대로 할당하지 못하기 때문에 권한 상승 취약점이 존재합니다. 이 취약점 악용에 성공한 공격자는 커널 모드에서 임의 코드를 실행할 수 있습니다. 이렇게 되면 공격자가 프로그램을 설치할 수 있을 뿐 아니라 데이터를 보거나 변경하거나 삭제할 수 있고 전체 관리자 권한이 있는 새 계정을 만들 수도 있습니다.

이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2012-1867(영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2012-1867)을 참조하십시오.

#### 글꼴 리소스 Refcount 정수 오버플로 취약점(CVE-2012-1867)에 대한 완화 요소

완화란 취약점의 악용 심각도를 낮출 수 있는 설정, 일반적인 구성 또는 최선책, 기본 상태를 지칭합니다. 다음과 같은 완화 요소가 현재 상황에서 유용할 수 있습니다.

-   이 취약점을 악용하려면 공격자가 유효한 로그온 자격 증명을 가지고 로컬로 로그온할 수 있어야 합니다.

#### 글꼴 리소스 Refcount 정수 오버플로 취약점(CVE-2012-1867)에 대한 대안

이 취약점에 대한 대안을 확인하지 못했습니다.

#### 글꼴 리소스 Refcount 정수 오버플로 취약점(CVE-2012-1867)에 대한 FAQ

이 취약점을 악용하면 어느 작업까지 가능합니까?  
이 취약점은 권한 상승 취약점입니다.

취약점의 원인은 무엇입니까?  
이 취약점은 Windows 커널 모드 드라이버가 TrueType 글꼴 로딩을 제대로 처리하지 못할 때 발생합니다.
...
업데이트는 어떤 기능을 수행합니까?  
업데이트는 Windows 커널 모드 드라이버가 TrueType 글꼴 로딩을 처리하는 방식을 수정하여 이 취약점을 해결합니다.
...
Windows 커널에는 커널이 특정 스레드 생성 시도를 처리하는 방식으로 인해 권한 상승 취약점이 존재합니다. 이 취약점 악용에 성공한 공격자는 커널 모드에서 임의 코드를 실행할 수 있습니다. 이렇게 되면 공격자가 프로그램을 설치할 수 있을 뿐 아니라 데이터를 보거나 변경하거나 삭제할 수 있고 전체 관리자 권한이 있는 새 계정을 만들 수도 있습니다.

이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2012-1868(영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2012-1868)을 참조하십시오.

#### Win32k.sys 경쟁 조건 취약점(CVE-2012-1868)에 대한 완화 요소

완화란 취약점의 악용 심각도를 낮출 수 있는 설정, 일반적인 구성 또는 최선책, 기본 상태를 지칭합니다. 다음과 같은 완화 요소가 현재 상황에서 유용할 수 있습니다.

-   이 취약점을 악용하려면 공격자가 유효한 로그온 자격 증명을 가지고 로컬로 로그온할 수 있어야 합니다.

#### Win32k.sys 경쟁 조건 취약점(CVE-2012-1868)에 대한 대안

이 취약점에 대한 대안을 확인하지 못했습니다.

#### Win32k.sys 경쟁 조건 취약점(CVE-2012-1868)에 대한 FAQ
...
<td style="border:1px solid black;">제어판의 프로그램 추가/제거 항목 또는 %Windir%\$NTUninstallKB2709162$\Spuninst 폴더의 Spuninst.exe 유틸리티를 사용하십시오.</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;">파일 정보</td>
<td style="border:1px solid black;"><a href="https://support.microsoft.com/kb/2709162">Microsoft 기술 자료 문서 2709162</a> 참조</td>
</tr>
<tr class="even">
...
-   문자열 Atom 클래스 이름 처리 취약점(CVE-2012-1864)을 보고해 주신 [Azimuth Security(영문)](https://www.azimuthsecurity.com/)의 Tarjei Mandt  
-   Win32k.sys 경쟁 조건 취약점(CVE-2012-1865)을 보고해 주신 [Azimuth Security](https://www.azimuthsecurity.com/)(영문)의 Tarjei Mandt  
-   클립보드 형식 Atom 이름 처리 취약점(CVE-2012-1866)을 보고해 주신 [Azimuth Security](https://www.azimuthsecurity.com/)(영문)의 Tarjei Mandt  
-   글꼴 리소스 Refcount 정수 오버플로 취약점(CVE-2012-1867)을 보고해 주신 [Google Inc(영문).](https://www.google.com)의 [Mateusz "j00ru" Jurczyk](https://j00ru.vexillium.org)(영문)
