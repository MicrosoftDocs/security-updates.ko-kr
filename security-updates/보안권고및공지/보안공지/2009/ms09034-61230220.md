...
이 보안 업데이트는 취약한 Microsoft ATL(액티브 템플릿 라이브러리) 버전을 사용하여 개발된 구성 요소와 컨트롤의 취약점에 대해 설명하는 Microsoft 보안 공지 [MS09-035](http://technet.microsoft.com/security/bulletin/ms09-035)와 함께 릴리스되었습니다. 심층적인 보안 대응책으로써, 이 Internet Explorer 보안 업데이트는 [Microsoft 보안 권고(973882)](http://technet.microsoft.com/security/advisory/973882) 및 Microsoft 보안 공지 [MS09-035](http://technet.microsoft.com/security/bulletin/ms09-035)에 설명되어 있는 취약한 버전의 ATL로 개발된 구성 요소와 컨트롤을 이용하는 Internet Explorer의 알려진 공격 방법을 완화할 뿐만 아니라
...
**문제점.** 이 보안 업데이트를 설치할 때 고객이 겪을 수 있는 현재까지 알려진 문제점은 [Microsoft 기술 자료 문서 972260](http://support.microsoft.com/kb/972260)에 나와 있습니다. 이 문서는 이러한 문제점에 대한 권장 해결 방법도 소개합니다.
...
예. Microsoft 보안 공지 [MS09-035](http://technet.microsoft.com/security/bulletin/ms09-035)에서는 ATL(액티브 템플릿 라이브러리)의 취약한 공개 버전을 사용하여 개발된 구성 요소와 컨트롤의 취약점에 대해 설명합니다. 심층적인 보안 대응책으로써, 이 보안 업데이트(MS09-034)는 [Microsoft 보안 권고(973882)](http://technet.microsoft.com/security/advisory/973882) 및 MS09-035에 설명되어 있는 ATL 버전으로 개발된 구성 요소와 컨트롤을 이용하는 Internet Explorer의 알려진 공격 방법을 완화합니다.

**이 보안 공지가 Microsoft 보안 권고 973882와 관계가 있습니까?**  
예. [Microsoft 보안 권고(973882)](http://technet.microsoft.com/security/advisory/973882)에서는 ATL(액티브 템플릿 라이브러리)의 공개 버전과 비공개 버전의 취약점에 대해 설명합니다. 심층적인 보안 대응책으로써, 이 보안 업데이트(MS09-034)는 Microsoft 보안 권고(973882) 및 [MS09-035](http://technet.microsoft.com/security/bulletin/ms09-035)에 설명되어 있는 ATL 버전으로 개발된 구성 요소와 컨트롤을 이용하는 Internet Explorer의 알려진 공격 방법을 완화합니다.

**Microsoft ATL(액티브 템플릿 라이브러리)의 보안 취약점과 관련 업데이트에 대한 자세한 정보는 어디에서 찾을 수 있습니까?**  
ATL(액티브 템플릿 라이브러리) 보안 취약점에 대한 자세한 내용은 [Microsoft 보안 권고(973882)](http://technet.microsoft.com/security/advisory/973882)를 참조하십시오.

ATL에 대한 업데이트를 다운로드하려면 Microsoft 보안 공지 [MS09-035](http://technet.microsoft.com/security/bulletin/ms09-035)를 참조하십시오.

**MS09-035 업데이트를 설치한 경우에도 여전히 이 업데이트를 설치해야 합니까?**  
예. [MS09-035](http://technet.microsoft.com/security/bulletin/ms09-035)는 Microsoft Visual Studio에서 ATL(액티브 템플릿 라이브러리)을 사용하는 개발자를 위해 특별히 개발된 것입니다. ATL로 작성된 구성 요소와 컨트롤을 재배포하는 개발자는 MS09-035로 제공된 업데이트를 설치하여, 자신이 개발하는 구성 요소와 컨트롤이 해당 보안 공지에 설명되어 있는 취약점을 포함하지 않도록 해야 합니다.
...
`HKEY_LOCAL_MACHINE (또는 HKEY_CURRENT_USER)SOFTWAREMicrosoftInternet ExplorerMainFeatureControlFEATURE_RESTRICT_OBJECT_DATA_ATTRIBUTE* = 0x00000001`

다음과 같은 새 ActiveX 호환성 플래그 DWORD 값을 사용하면 개별 컨트롤을 이 완화 조치로부터 제외할 수 있습니다.

`HKEY_LOCAL_MACHINESOFTWAREMicrosoftInternet ExplorerActiveX Compatibility{ActiveX 컨트롤의 CLSID}Compatibility Flags = 0x02000000`
...
파일 정보 세부 사항은 [Microsoft 기술 자료 문서 972260](http://support.microsoft.com/kb/972260)에 나와 있습니다.
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2009-1917 (영문)](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2009-1917)을 참조하십시오.
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2009-1918 (영문)](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2009-1918)을 참조하십시오.
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2009-1919 (영문)](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2009-1919)를 참조하십시오.
...
\*SMS 2.0 및 SMS 2003(SUSFP 포함)은 Internet Explorer 7 및 Internet Explorer 8을 제외한 영향을 받는 모든 Internet Explorer 버전을 지원합니다. 자세한 내용은 [Microsoft 기술 자료 문서 924178](http://support.microsoft.com/kb/924178)을 참조하십시오.
...
<td style="border:1px solid black;"><a href="http://support.microsoft.com/kb/972260">Microsoft 기술 자료 문서 972260</a> 참조</td>
</tr>
<tr class="even">
...
-   메모리 손상 취약점(CVE-2009-1917)을 보고해 주신 [VeriSign iDefense Labs (영문)](http://labs.idefense.com/)의 Peter Vreugdenhil  
-   HTML 개체 메모리 손상 취약점(CVE-2009-1918)을 [TippingPoint (영문)](http://www.tippingpoint.com/) 및 [Zero Day Initiative (영문)](http://www.zerodayinitiative.com/)와 협력하여 보고해 주신 [team509 (영문)](http://www.team509.com/)의 Wushi 및 Ling  
-   초기화되지 않은 메모리 손상 취약점(CVE-2009-1919)을 [TippingPoint (영문)](http://www.tippingpoint.com/) 및 [Zero Day Initiative (영문)](http://www.zerodayinitiative.com/)와 협력하여 보고해 주신 Peter Vreugdenhil
