---
TOCTitle: 'MS11-033'
Title: 'Microsoft 보안 공지 MS11-033 - 중요'
ms:assetid: 'ms11-033'
ms:contentKeyID: 61230400
ms:mtpsurl: 'https://technet.microsoft.com/ko-KR/library/ms11-033(v=Security.10)'
---



Microsoft 보안 공지 MS11-033 - 중요
===================================

WordPad 텍스트 변환기의 취약점으로 인한 원격 코드 실행 문제점(2485663)
----------------------------------------------------------------------
...
이 보안 업데이트는 WordPad 텍스트 변환기가 특수하게 조작된 파일을 처리하는 방식을 변경하여 이 취약점을 해결합니다. 취약점에 대한 자세한 내용은 다음 섹션, **취약점 정보**에서 각 취약점 항목의 자주 제기되는 질문 사항(FAQ)을 참조하십시오.
...
|-------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------|-------------|---------------------------------------------------------------------|
| [Windows XP 서비스 팩 3](https://www.microsoft.com/download/details.aspx?familyid=6753ca98-feb4-4c7f-9969-9294038a2bbb&displaylang=ko)                      | 원격 코드 실행 | 중요        | [MS10-067](http://technet.microsoft.com/security/bulletin/ms10-067) |
| [Windows XP Professional x64 Edition 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=3c94bc96-99ea-44a1-9052-e69de5e21f81)            | 원격 코드 실행 | 중요        | [MS10-067](http://technet.microsoft.com/security/bulletin/ms10-067) |
| [Windows Server 2003 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=9fbfc742-6c74-49a2-b3cc-e1d5d8c84b77&displaylang=ko)             | 원격 코드 실행 | 중요        | [MS10-067](http://technet.microsoft.com/security/bulletin/ms10-067) |
| [Windows Server 2003 x64 Edition 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=897b97b0-1bab-4b1b-b417-950fab0d4a65&displaylang=ko) | 원격 코드 실행 | 중요        | [MS10-067](http://technet.microsoft.com/security/bulletin/ms10-067) |
| [Windows Server 2003 with SP2(Itanium 기반 시스템용)](https://www.microsoft.com/download/details.aspx?familyid=ede38974-4e57-4ea1-8731-b91e96534693)        | 원격 코드 실행 | 중요        | [MS10-067](http://technet.microsoft.com/security/bulletin/ms10-067) |
...
<th style="border:1px solid black;" >WordPad 변환기 구문 분석 취약점(CVE-2011-0028)</th>
...
WordPad 변환기 구문 분석 취약점(CVE-2011-0028)  
----------------------------------------------
  

Microsoft WordPad가 특수하게 조작된 Word 문서를 구문 분석하는 방식에 원격 코드 실행 취약점이 존재합니다. 이 취약점으로 인해 사용자가 잘못된 형식의 구조를 포함하는 특수하게 조작된 Word 파일을 열면 원격 코드 실행이 허용될 수 있습니다. 이렇게 되면 공격자가 프로그램을 설치할 수 있을 뿐 아니라 데이터를 보거나 변경하거나 삭제할 수 있고 모든 사용자 권한이 있는 새 계정을 만들 수도 있습니다.
  
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2011-0028 (영문)](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-0028)을 참조하십시오.
  
#### WordPad 변환기 구문 분석 취약점(CVE-2011-0028)에 대한 완화 요소
...
-   Microsoft Word가 설치된 시스템은 일반적으로 WordPad 대신 Microsoft Office로 Word 문서를 엽니다.
  
#### WordPad 변환기 구문 분석 취약점(CVE-2011-0028)에 대한 대안
  
대안이란 기본적인 취약점을 수정하지는 않지만 업데이트를 적용하기 전에 알려진 공격 경로를 차단하는 데 유용한 설정 또는 구성 변경을 지칭합니다. Microsoft는 다음과 같은 대안을 테스트했으며, 대안으로 인한 기능 저하 여부를 해당 항목에서 설명합니다.
  
-   변환기 파일에 대한 액세스를 제한하여 WordPad Word 97 텍스트 변환기를 사용하지 않도록 설정합니다.
  
    관리자는 영향을 받는 변환기에 액세스 제어 목록을 적용하여 WordPad에서 더 이상 변환기를 로드하지 않도록 할 수 있습니다. 이 방법은 이 공격 경로를 사용하는 문제점 악용을 효과적으로 방지할 수 있습니다.
  
    **경고** 이 보안 업데이트를 설치하기 전에 이 대안 실행을 취소하십시오.
  
    액세스 목록을 적용하려면 관리자 권한으로 로그온하고 명령 프롬프트에서 다음 명령을 실행하십시오.
  
    **참고** 이러한 명령 일부는 오류 메시지를 발생할 수 있습니다. 사용 권한을 변경하려는 파일이 존재하지 않을 경우 이러한 오류 메시지가 발생할 수 있습니다.
    ...
    **대안의 영향.** 대안을 실행한 후 WordPad에서 Word 문서를 열면 WordPad에 서식이 지정된 텍스트가 아닌 이진 데이터가 표시됩니다.
  
    **대안 실행 취소 방법.** 관리자 권한으로 로그온한 후 명령 프롬프트에서 다음 명령을 실행합니다.
  
    **참고** 운영 체제의 버전, 운영 체제 업그레이드 및 서비스 팩의 시스템 기록에 따라 아래 파일 중 일부가 존재하지 않을 수 있습니다.
...
#### WordPad 변환기 구문 분석 취약점(CVE-2011-0028)에 대한 FAQ
...
**취약점의 원인은 무엇입니까?**      
이 취약점은 WordPad 텍스트 변환기가 Word 문서의 특정 필드를 잘못 구문 분석할 때 발생합니다.
  
**WordPad 텍스트 변환기란 무엇입니까?**      
WordPad는 Microsoft Windows 운영 체제의 기본 구성 요소입니다. WordPad의 텍스트 변환기를 사용하면 Microsoft Word를 설치하지 않은 사용자도 Microsoft Windows Write(.wri) 및 Microsoft Word(.doc) 파일 형식의 문서를 열 수 있습니다.
...
웹을 통한 공격의 경우 공격자는 이 취약점 악용 시도에 사용되는 Word 문서를 포함한 웹 사이트를 호스팅해야 합니다. 또한 사용자가 제공한 콘텐츠를 허용하거나 호스팅하는 공격 당한 웹 사이트에는 이 취약점을 악용할 수 있는 특수하게 조작된 콘텐츠가 포함되어 있을 수 있습니다. 공격자는 강제로 사용자가 특수하게 조작된 웹 사이트를 방문하도록 만들 수 없습니다. 대신, 공격자는 사용자가 공격자의 사이트로 연결되는 링크를 눌러 공격자의 웹 사이트를 방문하도록 유인해야 합니다. 링크를 클릭한 후에 사용자가 문서를 열 것인지 묻는 대화 상자에서 직접 수락해야 합니다.
  
**취약점으로 인해 주로 공격 받을 위험이 있는 시스템은 무엇입니까?**      
워크스테이션 또는 터미널 서버와 같이 WordPad를 사용하여 신뢰할 수 없는 출처의 Word 문서를 여는 영향을 받는 시스템이 주로 위험합니다. 관리자가 사용자가 서버에 로그온하여 프로그램을 실행할 수 있도록 허용할 경우, 서버가 더 위험할 수 있습니다. 가장 좋은 방법은 이러한 기능을 허용하지 않는 것입니다. Microsoft Word가 설치된 시스템은 일반적으로 WordPad 대신 Microsoft Office로 Word 문서를 엽니다.
  
**업데이트는 어떤 기능을 수행합니까?**      
이 업데이트는 WordPad 텍스트 변환기가 특수하게 조작된 Word 97 파일을 처리하는 방식을 수정하여 이러한 취약점을 해결합니다.
...
<td style="border:1px solid black;">제어판의 <strong>프로그램 추가/제거</strong> 도구 또는 %Windir%\$NTUninstallKB2485663$\Spuninst 폴더의 Spuninst.exe 유틸리티를 사용하십시오.</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"><strong>파일 정보</strong></td>
<td style="border:1px solid black;"><a href="http://support.microsoft.com/kb/2485663">Microsoft 기술 자료 문서 2485663</a> 참조</td>
</tr>
<tr class="even">
...
-   WordPad 변환기 구문 분석 취약점(CVE-2011-0028)을 보고해 주신 [Secunia (영문)](http://secunia.com/)의 Carsten Eiram
