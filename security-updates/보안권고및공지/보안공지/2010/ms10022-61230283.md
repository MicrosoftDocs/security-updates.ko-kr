...
이 보안 업데이트는 [Microsoft 보안 권고 981169](http://technet.microsoft.com/security/advisory/981169)에서 처음 설명한 취약점을 해결합니다.
...
**문제점.** 이 보안 업데이트를 설치할 때 고객이 겪을 수 있는 현재까지 알려진 문제점은 [Microsoft 기술 자료 문서 981169](http://support.microsoft.com/kb/981169)에 나와 있습니다. 이 문서는 이러한 문제점에 대한 권장 해결 방법도 소개합니다.
...
<td style="border:1px solid black;">
[VBScript 5.6](http://www.microsoft.com/downloads/details.aspx?familyid=aa8ff157-a7b3-4787-80c9-5bc453f0f1c9&displaylang=ko)  
(KB981350)
</td>
<td style="border:1px solid black;">
원격 코드 실행
...
시스템의 VBScript 버전은 시스템에 설치되어 있는 Internet Explorer, Windows 운영 체제 또는 Windows 스크립트 호스트에 포함된 최고 버전으로 결정되며 최신 버전의 Microsoft Internet Explorer, Windows 서비스 팩 또는 Windows 스크립트 호스트가 설치되어 있는 경우 변경할 수 있습니다. 자세한 내용은 MSDN 문서, [VBScript 버전 정보 (영문)](http://msdn.microsoft.com/en-us/library/4y5y7bh5(vs.85).aspx)를 참조하십시오.
...
예, 보안 업데이트 KB981350은 시스템에 적용되며 사용 중인 VBScript 버전을 VBScript 5.6으로 업그레이드합니다. 이 업데이트에 대한 자세한 내용은 [Microsoft 기술 자료 문서 981350](http://support.microsoft.com/kb/981350)을 참조하십시오.
...
**Windows** **Vista, Windows Server 2008, Windows 7 또는 Windows Server 2008 R2를 사용하고 있습니다. 이 업데이트가 제공되는 이유는 무엇입니까?**  
...
[Microsoft 보안 권고 981169](http://technet.microsoft.com/security/advisory/981169)에 제공된 대안을 적용한 경우 이 업데이트를 설치하기 전에 해당 대안을 실행 취소할 필요가 없습니다. 대안을 실행 취소하려는 경우 업데이트를 설치한 이후에 취소하는 것이 안전합니다. 이 권고에서는 이 공지에 권장된 대안을 사용합니다. 대안을 적용된 상태로 둘 경우에 발생하는 결과에 대한 자세한 내용은 해당 **대안의 영향** 정보를 검토하십시오.
...
<span></span>
VBScript가 Internet Explorer를 사용할 때 Windows 도움말 파일과 상호 작용하는 방식에 원격 코드 실행 취약점이 존재합니다. 악의적인 웹 사이트에 특수하게 조작된 대화 상자가 표시되고 사용자가 F1 키를 누른 경우 Windows 도움말 시스템이 공격자가 제공한 Windows 도움말 파일로 시작될 수 있습니다. 사용자가 관리자 권한으로 로그온한 경우, 이 취약점을 악용한 공격자는 영향을 받는 시스템을 완전히 제어할 수 있습니다. 이렇게 되면 공격자가 프로그램을 설치할 수 있을 뿐 아니라 데이터를 보거나 변경하거나 삭제할 수 있고 모든 사용자 권한이 있는 새 계정을 만들 수도 있습니다. 시스템에 대한 사용자 권한이 적게 구성된 계정의 사용자는 관리자 권한으로 작업하는 사용자에 비해 영향을 적게 받습니다. Windows Server 2003을 실행하는 시스템에서는 이 문제를 완화하는 데 도움이 되는 Internet Explorer [보안 강화 구성 (영문)](http://msdn.microsoft.com/en-us/library/ms537180(vs.85).aspx)이 기본적으로 사용됩니다.

이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2010-0483](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-0483)을 참조하십시오.
...
-   기본적으로 Windows Server 2003의 Internet Explorer는 [보안 강화 구성](http://go.microsoft.com/fwlink/?linkid=92039)으로 알려진 제한된 모드로 실행됩니다. 이 모드는 인터넷 영역 보안 수준을 높음으로 설정합니다. Internet Explorer의 신뢰할 수 있는 사이트 영역에 추가되지 않는 웹 사이트에 대한 완화 요소입니다.
...
**VBScript란 무엇입니까?**  
VBScript(Visual Basic 스크립트)는 웹 사이트를 보다 유연하게 만들거나 보다 원활한 상호 작용을 위해 종종 사용되는 인터프리터 방식의 개체 기반 스크립트 언어입니다. VBScript 스크립트는 ASP(Active Server Page), Internet Explorer 또는 Windows 스크립트 호스트와 같은 호스트나 인터프리터가 있어야만 실행될 수 있습니다. 자세한 내용은 MSDN 문서, [VBScript 기본 사항](http://msdn.microsoft.com/en-us/library/0ad0dkea(vs.85).aspx)을 참조하십시오.

**안전하지 않은 파일 형식이란 무엇입니까?**  
Microsoft는 "안전하지 않은 파일 형식" 개념을 사용하여 공격자가 이러한 파일에 포함할 수 있는 실행 가능 콘텐츠로부터 사용자를 보다 잘 보호하는 데 도움을 줍니다. 자세한 내용은 백서 Understanding Executable Content in Microsoft Products(Microsoft 제품의 실행 가능 콘텐츠 이해) 및 [Microsoft 기술 자료 문서 291369](http://support.microsoft.com/kb/291369)를 참조하십시오.
...
기본적으로 Windows Server 2003 및 Windows Server 2008의 Internet Explorer는 [보안 강화 구성](http://go.microsoft.com/fwlink/?linkid=92039)으로 알려진 제한된 모드로 실행되어 이 취약점을 완화합니다. 보안 강화 구성은 미리 구성된 Internet Explorer 설정 그룹으로, 사용자나 관리자가 서버에서 특수하게 조작된 웹 콘텐츠를 다운로드하고 실행할 가능성을 줄여 줍니다. Internet Explorer의 신뢰할 수 있는 사이트 영역에 추가되지 않는 웹 사이트에 대한 완화 요소입니다. [Internet Explorer 보안 강화 구성 관리](http://go.microsoft.com/fwlink/?linkid=92055)도 참조하십시오.
...
**이 취약점이 Windows** **Vista, Windows Server 2008, Windows 7 및 Windows Server 2008 R2에서 악용되지 않는 이유는 무엇입니까?**  
...
예. 이 취약점은 공개되었습니다. 이 취약점에는 CVE(Common Vulnerability and Exposure) 번호 [CVE-2010-0483](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-0483)이 지정되었습니다. 이 취약점은 [Microsoft 보안 권고 981169](http://technet.microsoft.com/security/advisory/981169)에서 설명되었습니다.
...
<td style="border:1px solid black;">VBScript 5.1 및 VBScript 5.6의 경우 <a href="http://support.microsoft.com/kb/981350">Microsoft 기술 자료 문서 981350</a>을 참조하십시오.</td>
</tr>
<tr class="even">
<td style="border:1px solid black;"></td>
<td style="border:1px solid black;">VBScript 5.7의 경우 <a href="http://support.microsoft.com/kb/981349">Microsoft 기술 자료 문서 981349</a>를 참조하십시오.</td>
</tr>
<tr class="odd">
...
<tr class="odd">
<td style="border:1px solid black;"><strong>파일 정보</strong></td>
<td style="border:1px solid black;">VBScript 5.6의 경우 <a href="http://support.microsoft.com/kb/981350">Microsoft 기술 자료 문서 981350</a>를 참조하십시오.</td>
</tr>
<tr class="even">
<td style="border:1px solid black;"></td>
<td style="border:1px solid black;">VBScript 5.7의 경우 <a href="http://support.microsoft.com/kb/981349">Microsoft 기술 자료 문서 981349</a>를 참조하십시오.</td>
...
<tr class="odd">
<td style="border:1px solid black;"></td>
<td style="border:1px solid black;">VBScript 5.8의 경우 <a href="http://support.microsoft.com/kb/981332">Microsoft 기술 자료 문서 981332</a>를 참조하십시오.</td>
</tr>
<tr class="even">
<td style="border:1px solid black;"><strong>레지스트리 키 확인</strong></td>
<td style="border:1px solid black;">Windows XP 서비스 팩 2의 VBScript 5.6:<br />
