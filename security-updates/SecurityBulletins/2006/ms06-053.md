---
TOCTitle: 'MS06-053'
Title: 'Microsoft 보안 공지 MS06-053 - 보통'
ms:assetid: 'ms06-053'
ms:contentKeyID: 61230014
ms:mtpsurl: 'https://technet.microsoft.com/ko-KR/library/ms06-053(v=Security.10)'
---

Microsoft 보안 공지 MS06-053 - 보통
===================================

인덱싱 서비스 취약점으로 인한 사이트 간 스크립팅 문제점 (920685)
----------------------------------------------------------------

게시된 날짜: 2006년 9월 13일 수요일 | 업데이트된 날짜: 2006년 9월 14일 목요일
...
**보안 업데이트 대체:** 이 공지는 이전의 보안 업데이트를 대체합니다. 이 공지의 자주 제기되는 질문 사항(FAQ)을 참조하십시오.
...
-   Microsoft Windows 2000 서비스 팩 4 - [업데이트 다운로드](https://www.microsoft.com/download/details.aspx?familyid=778294ae-c5e3-4f17-b0e4-308e46e00105&displaylang=ko)
-   Microsoft Windows XP 서비스 팩 1 및 Microsoft Windows XP 서비스 팩 2 - [업데이트 다운로드](https://www.microsoft.com/download/details.aspx?familyid=2731c0bf-6034-4c16-bb57-66e70a31a3d6&displaylang=ko)
-   Microsoft Windows XP Professional x64 Edition - [업데이트 다운로드](https://www.microsoft.com/download/details.aspx?familyid=3f604b2a-1383-4a45-b25b-c468deefbfc1)
-   Microsoft Windows Server 2003 및 Microsoft Windows Server 2003 서비스 팩 1 - [업데이트 다운로드](https://www.microsoft.com/download/details.aspx?familyid=0182e8e7-9755-46cc-a393-c1e95fd508b2&displaylang=ko)
-   Microsoft Windows Server 2003(Itanium 기반 시스템용) 및 Microsoft Windows Server 2003 SP1(Itanium 기반 시스템용) - [업데이트 다운로드](https://www.microsoft.com/download/details.aspx?familyid=e3e4a66c-ca9d-453b-8875-fb57528117ac)
-   Microsoft Windows Server 2003 x64 Edition - [업데이트 다운로드](https://www.microsoft.com/download/details.aspx?familyid=acf35f34-0d26-4b79-b81f-1111a784a66d)
...
공격자는 이러한 취약점을 악용하여 정보에 대한 승인되지 않은 액세스 권한을 얻을 수 있습니다. 이 취약점으로 인해 공격자가 직접 코드를 실행하여 해당 사용자 권한을 상승시킬 수는 없지만 영향을 받는 시스템의 손상을 악화시키는 데 사용할 수 있는 정보를 빼낼 수 있습니다.
...
|--------------------------------------------------------------------------------------------------------------------------|----------------------|--------------------------|------------------------|------------------------|---------------------|---------------------------------|
| Microsoft 인덱싱 서비스 취약점 - [CVE-2006-0032 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2006-0032) | 정보 유출            | 보통                     | 보통                   | 보통                   | 낮음                | 낮음                            |
...
이 보안 업데이트는 이전의 보안 업데이트를 대체합니다. 보안 공지 번호와 영향을 받는 응용 프로그램은 다음 표를 참조하십시오.

| 공지 번호                                                               | Windows 2000 서비스 팩 4 | Windows XP 서비스 팩 1 | Windows XP 서비스 팩 2 | Windows Server 2003 | Windows Server 2003 서비스 팩 1 |
|-------------------------------------------------------------------------|--------------------------|------------------------|------------------------|---------------------|---------------------------------|
| [**MS05-003**](https://technet.microsoft.com/security/bulletin/ms05-003) | 대체되지 않음            | 대체됨                 | 해당 사항 없음         | 대체됨              | 해당 사항 없음                  |
...
#### Microsoft 인덱싱 서비스 취약점 - CVE-2006-0032:

인덱싱 서비스의 쿼리 유효성 확인 방식에 정보 유출 취약점이 존재합니다. 이 취약점을 악용하여 공격자가 사용자 대신 클라이언트 쪽 스크립트를 실행할 수 있습니다. 스크립트는 콘텐츠를 스푸핑하거나 정보를 유출하고 영향을 받는 웹 사이트에서 사용자가 수행하는 어떠한 동작도 대신 수행할 수 있습니다.

#### Microsoft 인덱싱 서비스 취약점 - CVE-2006-0032에 대한 완화 요소:

-   기본적으로 IIS(인터넷 정보 서비스)는 Windows XP 또는 Windows Server 2003에 설치되지 않습니다.
-   Windows Server 2003에서는 인덱싱 서비스가 기본적으로 사용되지 않습니다.
-   Windows Server 2003의 경우에는 인덱싱 서비스를 설치하더라도 기본적으로 IIS를 통해 액세스할 수 없습니다. IIS를 인덱싱 서비스용 웹 기반 인터페이스로 활성화하려면 수동으로 설정해야 합니다. 인덱싱 서비스는 기본적으로 로컬 및 원격 파일 시스템 쿼리 수행에만 사용됩니다.

#### Microsoft 인덱싱 서비스 취약점 - CVE-2006-0032에 대한 대안:
...
-   **서버 역할의 시스템에서 인터넷 탐색 안 함**
    웹 서버 또는 기타 서버 역할의 시스템에서 인터넷 탐색을 하지 않습니다.
-   **Internet Explorer에서 페이지 인코딩 자동 감지 사용 안 함**
    Internet Explorer에서 페이지 인코딩 자동 감지를 사용하지 않도록 설정하면 이 취약점을 악용하려는 시도로부터 영향을 받는 시스템을 보호할 수 있습니다.

    구성 요소 및 서비스를 구성하려면 다음을 수행합니다.

    1.  Internet Explorer를 시작합니다.
    2.  **보기**, **인코딩**을 차례로 클릭한 다음 **자동 선택** 옆에 확인 표시가 있으면 **자동 선택**을 클릭하여 이 설정의 선택을 취소합니다.

    **대안의 영향:** 기본 문자 인코딩을 지정하지 않은 웹 페이지에 사용된 인코딩을 Internet Explorer에서 적절히 감지하지 못해 인코딩된 문자가 올바르게 표시되지 않을 수 있습니다. 그러나 사용자가 적절한 인코딩 유형을 수동으로 선택하여 페이지를 올바르게 표시할 수는 있습니다.

-   **IIS 5.0을 사용하는 Windows 2000에서 URLScan 사용**
    [URLScan (영문)](https://www.microsoft.com/technet/security/tools/urlscan.mspx)을 설치하면 이 취약점을 악용하려는 시도로부터 영향을 받는 시스템을 보호할 수 있습니다.

    1.  URLScan을 기본 구성으로 설치합니다. URLScan 2.5는 영향을 받는 Index Server 확장명인 .IDA, .IDQ 및 .HTW 파일에 대한 요청을 차단합니다.
    2.  IISAdmin 및 WWW 게시 서비스를 다시 시작하여 변경 내용을 적용합니다.

    **대안의 영향:** URLScan의 기본 구성은 Index Server 파일 형식(.IDA, .IDQ, .HTW)에 대한 요청을 차단하는 방식으로 이러한 파일 형식을 사용하는 웹 기반 검색을 차단합니다.

-   **인터넷 정보 서비스(IIS) 5.0이 설치된 Windows 2000의 IIS에서 Index Server ISAPI 확장 스크립트 매핑 제거**
    IIS에서 Index Server ISAPI 확장 스크립트 매핑을 제거하면 이 취약점을 악용하려는 시도로부터 영향을 받는 시스템을 보호할 수 있습니다.

    1.  **시작**을 클릭한 다음 **제어판** 을 클릭합니다.또는 **설정**을 가리킨 다음 **제어판**을 클릭합니다.
    2.  **관리 도구**를 두 번 클릭합니다.
    3.  **인터넷 정보 서비스**를 두 번 클릭합니다.
    4.  웹 서버를 마우스 오른쪽 단추로 클릭하고 **속성**을 선택합니다.
    5.  **마스터 속성**을 선택하고 **WWW 서비스**를 선택한 다음 **편집**을 클릭합니다.
    6.  **WWW 서비스 마스터 속성**에서 **홈 디렉터리**를 선택하고 **구성**을 클릭합니다.
    7.  **응용 프로그램 구성**에서 .HTW, .IDA 및 .IDQ 확장명을 강조 표시하고 **제거**를 클릭합니다.

    **대안의 영향:** URLScan의 기본 구성은 Index Server 파일 형식(.IDA, .IDQ, .HTW)에 대한 요청을 차단하는 방식으로 이러한 파일 형식을 사용하는 웹 기반 검색을 차단합니다.

-   **인덱싱 서비스 제거**
    **인덱싱 서비스가 더 이상 필요하지 않은 경우 아래 절차에 따라 제거할 수 있습니다.**
    ...
    4.  인덱싱 서비스 확인란의 선택을 취소하여 인덱싱 서비스를 제거합니다.
    5.  화면 상의 지시에 따라 Windows 구성 요소 마법사를 완료합니다.

    **대안의 영향:** 이 서비스를 제거할 경우 모든 검색 기능은 폴더 계층 구조를 탐색하여 요청된 문자열에 대해 각 파일을 검색하는 방식으로 제공되며 검색 응답 속도가 느려집니다. MMC 인덱싱 서비스 스냅인을 사용하여 새 카탈로그를 만드는 경우 해당 카탈로그는 이 서비스를 시작할 때까지 오프라인 상태로 유지됩니다.

-   **IIS 6.0이 설치된 Windows 2003의 IIS에서 인덱싱 서비스 확장 사용 안 함**
    **인덱싱 서비스 확장이 더 이상 필요하지 않은 경우 아래 절차에 따라 사용하지 않도록 설정할 수 있습니다.**

    구성 요소 및 서비스를 구성하려면 다음을 수행합니다.

    1.  **관리 도구**를 두 번 클릭합니다.
    2.  **인터넷 정보 서비스**를 두 번 클릭합니다.
    3.  **웹 기반 확장**을 클릭합니다.
    4.  **인덱싱 서비스**를 클릭합니다.
    5.  **금지**를 클릭합니다.

    **대안의 영향:** 이 서비스 확장을 제거할 경우 모든 검색 기능은 폴더 계층 구조를 탐색하여 요청된 문자열에 대해 각 파일을 검색하는 방식으로 제공되며 검색 응답 속도가 느려집니다. MMC 인덱싱 서비스 스냅인을 사용하여 새 카탈로그를 만드는 경우 해당 카탈로그는 이 서비스를 시작할 때까지 오프라인 상태로 유지됩니다.

#### Microsoft 인덱싱 서비스 취약점 - CVE-2006-0032에 대한 FAQ:

**이 취약점을 악용하면 어느 작업까지 가능합니까?**  
인덱싱 서비스의 쿼리 유효성 확인 방식에 정보 유출 취약점이 존재합니다. 이 취약점을 악용하여 공격자가 사용자 대신 클라이언트 쪽 스크립트를 실행할 수 있습니다. 스크립트는 콘텐츠를 스푸핑하거나 정보를 유출하고 영향을 받는 웹 사이트에서 사용자가 수행하는 어떠한 동작도 대신 수행할 수 있습니다.

**취약점의 원인은 무엇입니까?**  
인덱싱 서비스에서 쿼리 매개 변수에 대한 유효성을 적절히 확인하지 못해 사이트 간 스크립팅이 발생할 수 있습니다.

**인덱싱 서비스란 무엇입니까?**  
인덱싱 서비스는 영향을 받는 운영 체제의 기본 서비스입니다. 전에는 Index Server로 알려져 있는 이 기능은 원래 인터넷 정보 서비스(IIS) 웹 서버의 콘텐츠를 색인화하는 것이었습니다. 이제 인덱싱 서비스는 파일 시스템 및 가상 웹 서버 모두에 대한 콘텐츠 및 속성을 인덱싱된 카탈로그로 작성합니다.인덱싱 서비스는 응용 프로그램 및 스크립트에 제공되어 파일 시스템 및 웹 서버에 있는 정보를 효율적으로 관리, 쿼리 및 인덱싱할 수 있는 수단입니다. 인덱싱 서비스는 또한 카탈로그의 정보에 효율적으로 액세스할 수 있는 쿼리 방식을 제공합니다. 인덱싱된 정보는 Microsoft가 제공하는 필터와 사용자 지정된 필터(옵션)로 파일 시스템 및 웹 서버를 필터링한 결과입니다.

**사이트 간 스크립팅이란 무엇입니까?**  
사이트 간 스크립팅(XSS)은 일종의 보안 취약점으로서 공격자가 웹 사이트의 사용자 세션에 코드를 "주입"할 수 있도록 합니다. HTML 페이지를 동적으로 만드는 웹 서버가 공격에 포함됩니다. 브라우저로 다시 전송되는 동적 페이지에 이러한 서버에서 브라우저 입력을 포함하는 경우 동적 페이지에 콘텐츠를 포함하도록 서버가 조작될 수 있습니다. 이를 통해 악성 스크립트가 실행될 수 있습니다. 웹 브라우저는 "신뢰할 수 있는" 사이트의 해당 기본 가정과 쿠키 사용을 통해 이 문제를 지속시켜 자주 사용하는 웹 사이트에 영구적인 상태를 유지할 수 있습니다. 이 공격은 웹 사이트 콘텐츠를 수정하지 않습니다. 대신 신뢰할 수 있는 서버와 연결된 보안 컨텍스트의 브라우저에서 실행 가능한 악성 스크립트를 삽입합니다.
...
취약점 악용에 성공한 공격자는 악성 스크립트를 실행할 수 있습니다. 이 악성 스크립트는 클라이언트에 있는 사용자의 보안 컨텍스트에서 실행됩니다. 이 스크립트는 웹 사이트에서 승인하는 모든 작업을 사용자 컴퓨터에서 수행할 수 있습니다. 사용자의 웹 세션을 모니터링하고 제3자에게 정보를 전달하고 사용자 컴퓨터에서 다른 코드를 실행하고 쿠키를 읽거나 쓰는 작업이 가능합니다.
...
IIS를 통한 인덱싱 서비스 액세스를 허용한 시스템은 인터넷 기반 공격을 받을 수 있는 취약점에 노출됩니다. 인덱싱 서비스가 활성화되어 있지 않은 경우 시스템은 이 문제에 취약하지 않게 됩니다.

공격자는 클라이언트 시스템을 통해 사용자 대신 클라이언트 쪽 스크립트를 실행할 수 있습니다. 스크립트는 콘텐츠를 스푸핑하거나 정보를 유출하고 영향을 받는 웹 사이트에서 사용자가 수행하는 어떠한 동작도 대신 수행할 수 있습니다.

**업데이트는 어떤 기능을 합니까?**  
이 업데이트는 인덱싱 서비스에서 쿼리 매개 변수 유효성 확인 방식을 수정하여 취약점을 제거합니다.
...
**Windows2000-KB920685-x86-KOR /quiet**

**참고** **/quiet** 스위치를 사용하면 모든 메시지가 표시되지 않습니다. 마찬가지로 실패 메시지도 표시되지 않습니다. 관리자는 지원되는 방법 중 하나를 사용하여 **/quiet** 스위치를 사용할 때 설치가 제대로 되었는지 확인해야 합니다. 또한 관리자는 이 스위치를 사용할 때 KB920685.log 파일에서 모든 실패 메시지를 검토해야 합니다.

컴퓨터를 다시 시작하지 않고 보안 업데이트를 설치하려면 Windows 2000 서비스 팩 4 명령 프롬프트에서 다음 명령을 사용하십시오.

**Windows2000-KB920685-x86-KOR /norestart**
...
시스템 관리자는 Spuninst.exe 유틸리티를 사용하여 이 보안 업데이트를 제거할 수도 있습니다. Spuninst.exe 유틸리티는 %Windir%\\$NTUninstallKB920685$\\Spuninst 폴더에 있습니다.
...
| 파일 이름 | 버전          | 날짜       | 시간      | 크기      |
|-----------|---------------|------------|-----------|-----------|
| Query.dll | 5.0.2195.7100 | 2006-06-27 | 오후 2:01 | 1,427,728 |
...
-   **레지스트리 키 확인**
    ...
    **참고** 이 레지스트리 키에는 설치된 파일의 전체 목록이 포함되어 있지 않을 수도 있습니다. 또한 관리자나 OEM이 920685 보안 업데이트를 Windows 설치 원본 파일에 통합하거나 포함시키는 경우 이러한 레지스트리 키가 제대로 만들어지지 않을 수 있습니다.
...
**WindowsXP-KB920685-x86-KOR /quiet**

**참고** **/quiet** 스위치를 사용하면 모든 메시지가 표시되지 않습니다. 마찬가지로 실패 메시지도 표시되지 않습니다. 관리자는 지원되는 방법 중 하나를 사용하여 **/quiet** 스위치를 사용할 때 설치가 제대로 되었는지 확인해야 합니다. 또한 관리자는 이 스위치를 사용할 때 KB920685.log 파일에서 모든 실패 메시지를 검토해야 합니다.

컴퓨터를 다시 시작하지 않고 보안 업데이트를 설치하려면 Windows XP 명령 프롬프트에서 다음 명령을 사용하십시오.

**WindowsXP-KB920685-x86-KOR /norestart**
...
|-----------|---------------|------------|-----------|-----------|--------|
| Ciodm.dll | 5.1.2600.1860 | 2006-06-22 | 오후 2:19 | 64,512    | SP1QFE |
| Query.dll | 5.1.2600.1860 | 2006-06-22 | 오후 2:19 | 1,340,928 | SP1QFE |
| Ciodm.dll | 5.1.2600.2935 | 2006-06-22 | 오후 2:06 | 69,120    | SP2GDR |
| Query.dll | 5.1.2600.2935 | 2006-06-22 | 오후 2:06 | 1,426,432 | SP2GDR |
| Ciodm.dll | 5.1.2600.2935 | 2006-06-22 | 오후 2:22 | 69,120    | SP2QFE |
| Query.dll | 5.1.2600.2935 | 2006-06-22 | 오후 2:22 | 1,426,432 | SP2QFE |
...
-   **레지스트리 키 확인**
    ...
    **참고** 이러한 레지스트리 키에는 설치된 파일의 전체 목록이 포함되어 있지 않을 수도 있습니다. 또한 관리자나 OEM이 920685 보안 업데이트를 Windows 설치 원본 파일에 통합하거나 포함시키는 경우 이러한 레지스트리 키가 제대로 만들어지지 않을 수 있습니다.
...
**WindowsServer2003-KB920685-x86-KOR /quiet**

**참고** **/quiet** 스위치를 사용하면 모든 메시지가 표시되지 않습니다. 마찬가지로 실패 메시지도 표시되지 않습니다. 관리자는 지원되는 방법 중 하나를 사용하여 **/quiet** 스위치를 사용할 때 설치가 제대로 되었는지 확인해야 합니다. 또한 관리자는 이 스위치를 사용할 때 KB920685.log 파일에서 모든 실패 메시지를 검토해야 합니다.

컴퓨터를 다시 시작하지 않고 보안 업데이트를 설치하려면 Windows Server 2003 명령 프롬프트에서 다음 명령을 사용하십시오.

**WindowsServer2003-KB920685-x86-KOR /norestart**
...
| 파일 이름 | 버전          | 날짜       | 시간       | 크기      | 폴더   |
|-----------|---------------|------------|------------|-----------|--------|
| Ciodm.dll | 5.2.3790.552  | 2006-06-28 | 오전 12:32 | 66,048    | RTMGDR |
| Query.dll | 5.2.3790.552  | 2006-06-28 | 오전 12:32 | 1,350,656 | RTMGDR |
| Ciodm.dll | 5.2.3790.552  | 2006-06-28 | 오전 12:35 | 66,048    | RTMQFE |
| Query.dll | 5.2.3790.552  | 2006-06-28 | 오전 12:35 | 1,351,168 | RTMQFE |
| Ciodm.dll | 5.2.3790.2734 | 2006-06-28 | 오전 12:59 | 70,144    | SP1GDR |
| Query.dll | 5.2.3790.2734 | 2006-06-28 | 오전 12:59 | 1,423,360 | SP1GDR |
| Ciodm.dll | 5.2.3790.2734 | 2006-06-28 | 오전 1:02  | 70,144    | SP1QFE |
| Query.dll | 5.2.3790.2734 | 2006-06-28 | 오전 1:02  | 1,423,360 | SP1QFE |
...
-   Microsoft 인덱싱 서비스 취약점 - [CVE-2006-0032 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2006-0032)를 보고해 주신 Eiji James Yoshida
...
-   V1.1 (2006년 9월 14일): Office XP 서비스 팩 2에 대한 MS05-003과 관련하여 공지의 ‘이 버전으로 어떤 업데이트가 대체됩니까?’ 항목이 업데이트되었습니다.
