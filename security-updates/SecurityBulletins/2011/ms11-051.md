---
TOCTitle: 'MS11-051'
Title: 'Microsoft 보안 공지 MS11-051 - 중요'
ms:assetid: 'ms11-051'
ms:contentKeyID: 61230418
ms:mtpsurl: 'https://technet.microsoft.com/ko-KR/library/ms11-051(v=Security.10)'
---



Microsoft 보안 공지 MS11-051 - 중요
===================================

Active Directory 자격 증명 서비스 웹 등록의 취약점으로 인한 권한 상승 문제점(2518295)
-------------------------------------------------------------------------------------

게시된 날짜: 2011년 6월 15일 수요일 | 업데이트된 날짜: 2011년 6월 16일 목요일
...
이 보안 업데이트의 심각도는 Itanium을 제외한 지원 대상인 모든 Windows Server 2003, Windows Server 2008 및 Windows Server 2008 R2 에디션에 대해 중요입니다. 자세한 내용은 이 섹션에서 **영향을 받는 소프트웨어 및 영향을 받지 않는 소프트웨어**를 참조하십시오.

이 보안 업데이트는 Active Directory 자격 증명 서비스 웹 등록 사이트에서 입력 매개 변수의 유효성을 검사하는 방식을 수정하여 취약점을 해결합니다. 취약점에 대한 자세한 내용은 다음 섹션, **취약점 정보**에서 각 취약점 항목의 자주 제기되는 질문 사항(FAQ)을 참조하십시오.
...
**문제점.** 이 보안 업데이트를 설치할 때 고객이 겪을 수 있는 현재까지 알려진 문제점은 [Microsoft 기술 자료 문서 2518295](https://support.microsoft.com/kb/2518295)에 나와 있습니다. 이 문서는 이러한 문제점에 대한 권장 해결 방법도 소개합니다.
...
|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------|-------------|---------------------------|
| [Windows Server 2003 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=ef90d6c1-ea7f-4c32-9c90-0303e04c7436)                                                                        | 권한 상승      | 중요        | 없음                      |
| [Windows Server 2003 x64 Edition 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=62944095-33d6-4131-be32-a79d9ec4d4a9)                                                            | 권한 상승      | 중요        | 없음                      |
| [Windows Server 2008(32비트 시스템용) 및 Windows Server 2008(32비트 시스템용) 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=46ade106-e0cb-4c71-8230-793a15062823)\*\*           | 권한 상승      | 중요        | 없음                      |
| [Windows Server 2008(x64 기반 시스템용) 및 Windows Server 2008(x64 기반 시스템용) 서비스 팩 2](https://www.microsoft.com/download/details.aspx?familyid=6141a1c5-ecaf-4553-9d27-dd6e5c4a13fd)\*\*       | 권한 상승      | 중요        | 없음                      |
| [Windows Server 2008 R2(x64 기반 시스템용) 및 Windows Server 2008 R2(x64 기반 시스템용) 서비스 팩 1](https://www.microsoft.com/download/details.aspx?familyid=1da04414-6210-43ea-8e0a-cf21cf144076)\*\* | 권한 상승      | 중요        | 없음                      |
...
**내 웹 등록 ASP 페이지를 사용자 정의하였습니다. Microsoft 기술 자료 문서 2518295에 나열된 알려진 문제점을 고려해야 합니까?**    
예. 웹 등록 ASP 페이지를 편집한 고객은 [Microsoft 기술 자료 문서 2518295](https://support.microsoft.com/kb/2518295)에서 시스템에 영향을 줄 수 있는 알려진 문제점에 대한 중요한 정보를 검토하는 것이 좋습니다.
...
<th style="border:1px solid black;" >Active Directory 자격 증명 서비스 취약점(CVE-2011-1264)</th>
...
공격자가 사용자의 Internet Explorer 인스턴스에 클라이언트쪽 스크립트를 삽입할 수 있는 변형된 XSS 취약점이 Active Directory 자격 증명 서비스 웹 등록에 존재합니다. 스크립트는 콘텐츠를 스푸핑하거나 정보를 유출하고 사이트에서 대상 사용자 대신 어떠한 동작도 수행할 수 있습니다.
  
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2011-1264 (영문)](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-1264)를 참조하십시오.
  
#### Active Directory 자격 증명 서비스 취약점(CVE-2011-1264)에 대한 완화 요소
...
#### Active Directory 자격 증명 서비스 취약점(CVE-2011-1264)에 대한 대안
...
#### Active Directory 자격 증명 서비스 취약점(CVE-2011-1264)에 대한 FAQ
  
**이 취약점을 악용하면 어느 작업까지 가능합니까?**      
이 취약점은 권한 상승을 허용할 수 있는 변형된 XSS 취약점입니다.
  
**취약점의 원인은 무엇입니까?**      
Active Directory 자격 증명 서비스 웹 등록 사이트에서 요청 매개 변수의 잘못된 입력 유효성 검사로 인해 취약점이 발생합니다.
...
영향을 받는 구성 요소는 Active Directory 자격 증명 서비스 웹 등록입니다.
...
Active Directory 자격 증명 서비스 웹 등록을 실행하는 서버가 주로 위험합니다. 이 특정 역할을 실행하지 않는 서버는 위험하지 않습니다.
  
**업데이트는 어떤 기능을 수행합니까?**      
이 업데이트는 Active Directory 자격 증명 서비스 웹 등록 사이트에서 입력 매개 변수의 유효성을 검사하는 방식을 수정하여 취약점을 해결합니다.
...
<td style="border:1px solid black;">제어판의 <strong>프로그램 추가/제거</strong> 도구 또는 %Windir%\$NTUninstallKB2518295$\Spuninst 폴더의 Spuninst.exe 유틸리티를 사용하십시오.</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"><strong>파일 정보</strong></td>
<td style="border:1px solid black;"><a href="https://support.microsoft.com/kb/2518295">Microsoft 기술 자료 문서 2518295</a> 참조</td>
</tr>
<tr class="even">
...
-   Active Directory 자격 증명 서비스 취약점(CVE-2011-1264)을 보고해 주신 [Emaze Networks (영문)](https://www.emaze.net/)의 Ruggero Strabla와 [Saipem Security Team (영문)](https://www.saipem.com/)
...
-   V1.1(2011년 6월 16일): XSS 필터 완화 요소에 대한 설명이 추가되었습니다.
