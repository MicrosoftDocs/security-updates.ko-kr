...
**문제점.** 이 보안 업데이트를 설치할 때 고객이 겪을 수 있는 현재까지 알려진 문제점은 [Microsoft 기술 자료 문서 981832](http://support.microsoft.com/kb/981832)에 나와 있습니다. 이 문서는 이러한 문제점에 대한 권장 해결 방법도 소개합니다.
...
[Microsoft Windows 2000 서비스 팩 4](http://www.microsoft.com/downloads/details.aspx?familyid=88a0e872-01de-495b-8eec-d105a970daa7&displaylang=ko)  
(KB976323)
</td>
<td style="border:1px solid black;">
서비스 거부
...
Microsoft Exchange 2007이 릴리스됨과 함께 Microsoft Exchange는 새로운 서비스 모델로 전환됩니다. Microsoft Exchange 서비스 모델에 대한 자세한 내용은 TechNet 문서, [Exchange 2007 서비스](http://go.microsoft.com/fwlink/?linkid=89577)를 참조하십시오. 새로운 Microsoft Exchange 서비스 모델과 관련한 질문은 [Microsoft 도움말 및 지원](http://support.microsoft.com/)으로 문의하십시오.
...
예. 이 공지의 취약점 정보 섹션에 나열된 변경 사항 외에도, 이 업데이트에는 OWA(Outlook Web Access)를 사용할 때 중국어 문자 집합 문제를 해결하는 Microsoft Exchange 2000 Server 서비스 팩 3에 대한 심층 보안 변경 사항이 포함되어 있습니다. 이 문제는 Exchange 2000 시스템에만 영향을 미칩니다. Microsoft Exchange 2000 Server 서비스 팩 3용 업데이트를 수행하기 전에 Exchange 2000 Server 서비스 팩 3(SP3) 이후 업데이트 롤업 패치가 필요합니다. 이 보안 공지에서 제공하는 심층 보안 업데이트를 설치하기 전에 Exchange 2000(KB870540)용 업데이트 롤업을 설치해야 합니다. 이 업데이트는 업데이트 롤업이 설치되어 있는지 확인합니다. 업데이트 롤업이 설치되어 있지 않은 경우 다운로드 웹 사이트로 안내를 받게 됩니다. 자세한 내용은 [Microsoft 기술 자료 문서 870540](http://support.microsoft.com/kb/870540)을 참조하십시오. 필수 업데이트를 다운로드하려면 [Exchange 2000용 업데이트 롤업(KB870540)](http://www.microsoft.com/downloads/details.aspx?familyid=363a57a4-8bed-4bbb-bbe4-abc11ab04611)을 참조하십시오.
...
TechNet 문서 [Exchange 2007용 최신 서비스 팩 또는 업데이트 롤업을 설치하는 방법 (영문)](http://technet.microsoft.com/en-us/library/ee221147(exchg.80).aspx)을 참조하십시오.
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2010-0024 (영문)](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-0024)를 참조하십시오.
...
**SMTP 서비스란 무엇입니까?**  
SMTP(Simple Mail Transfer Protocol) 서비스는 전자 메일을 전송하며, 전자 메일 서비스 또는 IIS(인터넷 정보 서비스)의 일부로 설치되어 있습니다. SMTP는 전자 메일이 전송된 후 인터넷을 통해 대상 서버로 전달되는 방식을 제어합니다. SMTP 서비스는 서버 간에 전자 메일을 보내고 받습니다. 자세한 내용은 TechNet 문서 [SMTP(Simple Mail Transfer Protocol) 서비스 (영문)](http://technet.microsoft.com/en-us/library/cc759281(ws.10).aspx)를 참조하십시오.

**MX 리소스 레코드란 무엇입니까?**  
MX(메일 교환) 리소스 레코드는 DNS 도메인 이름에 대한 메일 교환 서버를 지정합니다. 메일 교환 서버는 DNS 도메인 이름에 대한 메일을 처리하고 전달하는 호스트입니다. 메일을 처리하는 것은 메일을 주소로 전달하거나 다른 유형의 메일 전송으로 전달하는 것을 의미합니다. 메일을 전달하는 것은 메일을 최종 대상 서버로 전송하거나, SMTP(Simple Mail Transfer Protocol)를 사용하여 최종 대상과 가까운 다른 메일 교환 서버로 전송하거나, 지정된 기간 동안 큐에 대기시키는 것을 의미합니다. MX 리소스 레코드에 대한 자세한 내용은 TechNet 문서 [리소스 레코드 형식(영문)](http://technet.microsoft.com/en-us/library/cc958958.aspx)을 참조하십시오.
...
예. 이 취약점은 공개되었습니다. 이 취약점에는 CVE(Common Vulnerability and Exposure) 번호 [CVE-2010-0024 (영문)](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-0024)가 지정되었습니다.
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2010-0025 (영문)](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-0025)를 참조하십시오.
...
**SSL이란 무엇입니까?**  
SSL(Secure Sockets Layer)은 TLS(Transport Layer Security) 프로토콜의 전신으로, TLS와 동일한 기능을 수행하며 공개 및 비밀 키 기술의 조합을 사용하는 보안 네트워크 통신을 지원합니다. 자세한 내용은 TechNet 문서 [TLS/SSL 작동 방식(영문)](http://technet.microsoft.com/en-us/library/cc783349(ws.10).aspx)을 참조하십시오.

**SChannel 보안 패키지란 무엇입니까?**  
Secure Channel(Schannel) 보안 패키지는 인터넷 표준 인증 프로토콜인 SSL(Secure Sockets Layer) 및 TLS(Transport Layer Security)를 구현하는 SSP(보안 지원 공급자)입니다. 이러한 구성 요소는 웹 탐색과 같은 여러 일반 인터넷과 네트워크 응용 프로그램을 지원하는 보안 통신을 구현하는 데 사용됩니다. SChannel은 클라이언트와 서버 간에 보안 통신을 지원하기 위한 인증 서비스를 제공하는 보안 패키지의 일부입니다. 자세한 내용은 MSDN 문서, [Secure Channel(영문)](http://msdn.microsoft.com/en-us/library/aa380123(vs.85).aspx)을 참조하십시오.
...
| **파일 정보**             | [Microsoft 기술 자료 문서 976323](http://support.microsoft.com/kb/976323) 참조                                                   |
...
<td style="border:1px solid black;"><a href="http://support.microsoft.com/kb/976323">Microsoft 기술 자료 문서 976323</a> 참조</td>
</tr>
<tr class="even">
...
<tr class="odd">
<td style="border:1px solid black;"><strong>파일 정보</strong></td>
<td style="border:1px solid black;"><a href="http://support.microsoft.com/kb/976703">Microsoft 기술 자료 문서 976703</a> 참조</td>
</tr>
<tr class="even">
<td style="border:1px solid black;"><strong>레지스트리 키 확인</strong></td>
...
<tr class="odd">
<td style="border:1px solid black;"><strong>파일 정보</strong></td>
<td style="border:1px solid black;"><a href="http://support.microsoft.com/kb/976702">Microsoft 기술 자료 문서 976702</a> 참조</td>
</tr>
<tr class="even">
<td style="border:1px solid black;"><strong>레지스트리 키 확인</strong></td>
...
<td style="border:1px solid black;"><strong>파일 정보</strong></td>
<td style="border:1px solid black;">Microsoft Exchange Server 2007 서비스 팩 1(x64 기반 시스템용):<br />
<a href="http://support.microsoft.com/kb/981407">Microsoft 기술 자료 문서 981407</a> 참조</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"></td>
<td style="border:1px solid black;">Microsoft Exchange Server 2007 서비스 팩 2(x64 기반 시스템용):<br />
<a href="http://support.microsoft.com/kb/981383">Microsoft 기술 자료 문서 981383</a> 참조</td>
</tr>
<tr class="even">
<td style="border:1px solid black;"><strong>레지스트리 키 확인</strong></td>
<td style="border:1px solid black;">Microsoft Exchange Server 2007 서비스 팩 1(x64 기반 시스템용):<br />
...
<tr class="odd">
<td style="border:1px solid black;"><strong>파일 정보</strong></td>
<td style="border:1px solid black;"><a href="http://support.microsoft.com/kb/981401">Microsoft 기술 자료 문서 981401</a> 참조</td>
</tr>
<tr class="even">
<td style="border:1px solid black;"><strong>레지스트리 키 확인</strong></td>
<td style="border:1px solid black;">Microsoft Exchange Server 2010(x64 기반 시스템용):<br />
