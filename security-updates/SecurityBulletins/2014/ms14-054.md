...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2014-4074](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2014-4074)(영문)를 참조하십시오.
...
작업 스케줄러는 선택한 컴퓨터에서 일상적인 작업의 자동화를 가능하게 하는 Windows 서비스입니다. 작업 스케줄러는 사용자가 작업 시작을 위해 선택한 기준(트리거라고 함)을 모니터링하고 기준이 충족되면 작업을 실행하여 이 기능을 수행합니다. 자세한 내용은 MSDN 라이브러리 항목 [작업 스케줄러](https://msdn.microsoft.com/ko-kr/library/aa383614(vs.85).aspx)(영문)를 참조하십시오.
