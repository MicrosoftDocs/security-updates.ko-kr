---
TOCTitle: 'MS06-077'
Title: 'Microsoft 보안 공지 MS06-077 - 중요'
ms:assetid: 'ms06-077'
ms:contentKeyID: 61230038
ms:mtpsurl: 'https://technet.microsoft.com/ko-KR/library/ms06-077(v=Security.10)'
---

Microsoft 보안 공지 MS06-077 - 중요
===================================

원격 설치 서비스의 취약점으로 인한 원격 코드 실행 문제점(926121)
----------------------------------------------------------------

게시된 날짜: 2006년 12월 12일 화요일 | 업데이트된 날짜: 2007년 3월 21일 수요일
...
**주의 사항:** 이 보안 업데이트를 설치할 때 고객이 겪을 수 있는 현재까지 알려진 문제점은 [Microsoft 기술 자료 문서 926121](https://support.microsoft.com/kb/926121)에 나와 있습니다. 이 문서는 이러한 문제점에 대한 권장 해결 방법도 소개합니다. 자세한 내용은 [Microsoft 기술 자료 문서 926121](https://support.microsoft.com/kb/926121)을 참조하십시오.

**테스트된 소프트웨어 및 보안 업데이트 다운로드 위치:**

**영향을 받는 소프트웨어:**

-   Microsoft Windows 2000 서비스 팩 4 - [업데이트 다운로드](https://www.microsoft.com/download/details.aspx?familyid=0ed62db9-4534-4f27-a49e-020c7a7d69e0&displaylang=ko)
...
<tbody>
<tr class="odd">
<td style="border:1px solid black;">RIS 쓰기 가능한 경로 취약점 - <a href="https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2006-5584">CVE-2006-5584 (영문)</a></td>
...
#### RIS 쓰기 가능한 경로 취약점 - CVE-2006-5584:

원격 설치 서비스를 사용하면 서버의 TFTP 서비스를 통해 기본적으로 익명의 사용자가 기존 운영 체제 파일을 덮어 쓰거나 특수하게 조작한 파일을 업로드할 수 있습니다. 이렇게 되면 공격자는 RIS 서버에서 제공되는 설치된 운영 체제를 손상시킬 수 있습니다.

#### RIS 쓰기 가능한 경로 취약점 - CVE-2006-5584에 대한 완화 요소:

-   공격자가 특수하게 조작한 파일이나 프로그램을 배치하려면 TFTP 서비스를 통해 원격 설치 서버에 연결해야 합니다.
-   Windows 2000 서비스 팩 4에는 기본적으로 원격 설치 서비스가 설치되지 않습니다.
...
#### RIS 쓰기 가능한 경로 취약점 - CVE-2006-5584에 대한 대안:
...
-   **TFTP 서비스를 읽기 전용으로 구성합니다.**

    레지스트리 값을 설정하여 TFTP 서비스를 읽기 전용으로 구성할 수 있습니다.

    **참고** 레지스트리 편집기를 잘못 사용하면 시스템 전체에 심각한 문제가 발생할 수 있으며 운영 체제를 다시 설치해야 할 수도 있습니다. Microsoft는 레지스트리 편집기를 잘못 사용하여 발생하는 문제에 대한 해결을 보증하지 않습니다. 레지스트리 편집기의 사용에 따른 모든 책임은 사용자에게 있습니다. 레지스트리를 편집하는 방법에 대한 자세한 내용은 레지스트리 편집기(Regedit.exe)의 "키와 값 변경" 도움말 항목 또는 regedt.exe의 "레지스트리의 정보 추가와 삭제" 및 "레지스트리 정보 편집" 도움말 항목을 참조하십시오.

    **참고** 레지스트리를 편집하기 전에 백업해 두는 것이 좋습니다.

    RIS 서버에서 무단 액세스를 허용하지 않으려면 Regedt.exe를 사용하여 다음 내용을 레지스트리에 추가합니다.

    1.  **시작**, **실행**을 차례로 클릭하고 "regedt"(따옴표 없이)를 입력한 다음 **확인**을 클릭합니다.
    2.  레지스트리 편집기에서 다음 레지스트리 키를 찾습니다.
    3.  HKEY\_LOCAL\_MACHINE\\SYSTEM\\CurrentControlSet\\Services\\TFTPD\\Parameters
    4.  REG\_SZ 값 **Masters**를 추가합니다. 값을 **0**으로 설정합니다. 이 값은 쓰기 가능한 TFTPD 액세스를 비활성화합니다.
    5.  변경 사항을 적용하려면 TFTP 서비스를 다시 시작해야 합니다.
    6.  TP 서비스:
    7.  **시작**, **실행**을 차례로 클릭하고 "cmd"(따옴표 없이)를 입력한 다음 **확인**을 클릭합니다.
    8.  cmd 창에 “Net stop TFTPD”(따옴표 없이)를 입력하고 **Enter**를 누릅니다.

-   **방화벽에서 포트 69(UDP)를 차단합니다.**

    이 포트는 영향을 받는 구성 요소와 연결을 시작하는 데 사용됩니다. 방화벽에서 UDP 포트 69를 차단하면 이 취약점을 악용한 방화벽 뒤의 시스템에 대한 공격을 막을 수 있습니다. 기타 포트를 악용할 수 있는 공격을 방지하려면 들어오는 방향의 원치 않는 인터넷 통신을 모두 차단하는 것이 좋습니다. 이 포트에 대한 자세한 내용은 다음 [웹 사이트 (영문)](https://go.microsoft.com/fwlink/?linkid=21312)를 참조하십시오.

-   **필요 없는 경우 TFTP 서비스 중지**

    1.  **시작**, **실행**을 차례로 클릭하고 "cmd"(따옴표 없이)를 입력한 다음 **확인**을 클릭합니다.
    2.  cmd 창에 “Net stop TFTPD”(따옴표 없이)를 입력하고 **Enter**를 누릅니다.

    **대안의 영향:** TFTP 서비스를 비활성화하면 사용자가 TFTP 디렉터리의 콘텐츠에 업로드하거나 보거나 쓸 수 없습니다.
...
#### RIS 쓰기 가능한 경로 취약점 - CVE-2006-5584에 대한 FAQ:
...
**취약점의 원인은 무엇입니까?**  
이 취약점은 RIS TFTP 서비스를 통해 구축되어 호스팅되는 운영 체제의 파일 구조에 익명 액세스를 허용할 경우 발생합니다.

**RIS란 무엇입니까?**  
원격 설치 서비스(RIS)는 네트워크를 통해 Windows 설치를 시작할 수 있는 PXE(Pre-boot Execution Environment) 기반 배포 기술입니다.

**누가 취약점을 악용할 수 있습니까?**  
영향을 받는 RIS 시스템에 특수하게 조작한 실행 파일이나 파일을 제공할 수 있는 익명의 사용자 누구나 조작된 운영 체제 빌드를 사용하여 구축된 시스템에서 나중에 이 취약점을 악용할 수 있습니다.

**취약점으로 인해 노출되는 시스템은 주로 무엇입니까?**  
RIS가 설치된 모든 Windows 2000 Server 서비스 팩 4 시스템이 주로 이 취약점으로 인한 위험에 노출됩니다.

**인터넷을 통해서도 취약점을 악용할 수 있습니까?**  
예. 공격자는 인터넷을 통해 이 취약점을 악용할 수 있으나 우선 RIS 서비스에 액세스할 수 있는 네트워크 권한을 가지고 있어야 합니다. 방화벽 구성 모범 사례와 표준 기본 방화벽 구성을 이용하면 인터넷을 이용한 공격으로부터 네트워크를 보호할 수 있습니다.

**업데이트는 어떤 기능을 수행합니까?**  
업데이트는 익명의 TFTP 사용자가 RIS에서 호스팅하는 운영 체제 빌드의 파일 구조에 쓰지 못하도록 만들어 취약점을 제거합니다. 업데이트는 공지의 대안 섹션에 나와 있는 레지스트리 키를 추가합니다.
...
**참고/quiet** 스위치를 사용하면 모든 메시지가 표시되지 않습니다. 마찬가지로 실패 메시지도 표시되지 않습니다. 관리자는 지원되는 방법 중 하나를 사용하여 **/quiet** 스위치를 사용할 때 설치가 제대로 되었는지 확인해야 합니다. 또한 관리자는 이 스위치를 사용할 때 KB**926121**.log 파일에서 모든 실패 메시지를 검토해야 합니다.
...
시스템 관리자는 Spuninst.exe 유틸리티를 사용하여 이 보안 업데이트를 제거할 수도 있습니다. Spuninst.exe 유틸리티는 %Windir%\\$NTUninstallKB926121$\\Spuninst 폴더에 있습니다.
...
-   **Microsoft Baseline Security Analyzer**
    ...
    MBSA(Microsoft Baseline Security Analyzer) 도구를 사용하면 영향을 받는 시스템에 보안 업데이트가 적용되었는지 확인할 수 있습니다. 관리자는 MBSA를 사용하여 로컬 및 원격 시스템에서 누락된 보안 업데이트와 일반적인 보안 구성 오류를 검색할 수 있습니다. MBSA에 대한 자세한 내용은 \\lang1053 Microsoft Baseline Security Analyzer 를 참조하십시오.
...
-   RIS 쓰기 가능한 경로 TFTP 취약점(CVE-2006-5584)을 보고해 주신 Nicolas Ruff
...
-   V1.1 (2007년 3월 21일): 공지가 다음과 같이 개정되었습니다. 정확한 레지스트리 키 값을 반영하여 대안 섹션이 업데이트되었습니다. 정확한 레지스트리 키 값 유형은 이전에 나열된 REG\_DWORD가 아니라, REG\_SZ입니다.
