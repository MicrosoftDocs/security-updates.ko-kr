...
Windows 7 서비스 팩 1 베타는 이 공지에 설명된 취약점의 영향을 받습니다. 이러한 베타 릴리스를 실행하는 고객은 이 업데이트를 다운로드하여 시스템에 적용하는 것이 좋습니다. 보안 업데이트는 [Microsoft Update](http://go.microsoft.com/fwlink/?linkid=40747) 및 [Windows Update](http://go.microsoft.com/fwlink/?linkid=21130)를 통해 제공됩니다. [Microsoft 다운로드 센터](http://go.microsoft.com/fwlink/?linkid=21129)에서 이 보안 업데이트를 다운로드할 수도 있습니다.
...
이 취약점을 CVE(Common Vulnerabilities and Exposures) 목록의 표준 항목으로 보려면 [CVE-2010-3225 (영문)](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-3225)를 참조하십시오.
...
**RTSP란 무엇입니까?**    
RTSP(Real Time Streaming Protocol)는 실시간 속성이 있는 데이터의 전달을 제어하기 위한 응용 프로그램 수준 프로토콜입니다. RTSP가 제공하는 확장할 수 있는 프레임워크를 통해 오디오, 비디오 등의 실시간 데이터를 요청 시 제어된 방식으로 전달할 수 있습니다. 데이터 원본에는 라이브 데이터 피드와 저장된 클립이 모두 포함될 수 있습니다. 이 프로토콜은 여러 데이터 전달 세션을 제어하는 데 사용되며 이 프로토콜을 통해 UDP, 멀티캐스트 UDP, TCP 등의 전달 채널과 RTP(RFC 1889)에 기반한 전달 메커니즘을 선택할 수 있습니다. RTSP는 [RFC 2326 (영문)](http://www.ietf.org/rfc/rfc2326.txt)에서 정의합니다.
...
**네트워크 서비스 계정이란 무엇입니까?**    
네트워크 서비스 계정은 기본 제공되는 계정으로, 리소스와 개체에 대해 Users 그룹의 구성원보다 많은 액세스 권한을 가집니다. 네트워크 서비스 계정으로 실행되는 서비스는 컴퓨터 계정의 자격 증명을 사용하여 네트워크 리소스에 액세스합니다. 계정의 실제 이름은 NT AUTHORITY\\NETWORK SERVICE입니다. 자세한 내용은 MSDN 문서, [NetworkService 계정 (영문)](http://msdn.microsoft.com/en-us/library/ms684272(vs.85).aspx)을 참조하십시오.
...
<td style="border:1px solid black;"><a href="http://support.microsoft.com/kb/2281679">Microsoft 기술 자료 문서 2281679</a> 참조</td>
</tr>
<tr class="odd">
...
-   RTSP 해제 후 사용 취약점(CVE-2010-3225)을 [TippingPoint (영문)](http://www.tippingpoint.com/)의 [Zero Day Initiative (영문)](http://www.zerodayinitiative.com/)와 협력하여 보고해 주신 Oleksandr Mirosh
